...
class Driving(ICAN):
    ...
    def __init__(self, ip=IP, group=GROUP):
        ...
        self.__spin = None
        self.__max_value = 80
        # Keep testing it until you get solid motor driver.
        self.__spin_offset = 10          # set it properly with tests
        self.__straight_offset = 30      # set it properly with tests
        self.__angle_offset = 50         # set it properly with tests
        self.stop()

    def __calculate_omni_wheel(self):
        if self.__spin:
            offset = np.sign(self.__throttle) * self.__spin_offset

            if self.__spin == self.SPIN_RIGHT:
                for i in range(3):
                    self.__wheel_vec[i] = self.THROTTLE_0POINT - round(
                        0.01 * (self.__max_value - self.__spin_offset) * self.throttle + offset
                    )
            ...
            if angle_condition:
                offset = np.sign(self.__throttle) * self.__straight_offset
                weight = (self.__throttle + offset) * 0.7077  # 1.15 * 80 / 130 
            else:
                offset = np.sign(self.__throttle) * self.__angle_offset
                weight = (self.__throttle + offset) * 0.5333  # 80 / 150
    ...
    def __calculate_steering(self):
        ...
        self.__wheel_vec[2] = Driving.THROTTLE_0POINT - F_long
    ...
    def steering(self, steering):
        ...
        if (self.__spin is not None) or ((self.__angle % 180) != 0):
            print("[INFO] Hint: Steering can only be set during straight-forward movement.")

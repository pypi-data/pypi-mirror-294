...
def test_baseline_correction(test_signal):
    ...
    assert len(clean_signal) == len(test_signal), "Baseline correction output size mismatch."
    assert not np.allclose(clean_signal, test_signal), "Baseline correction did not modify the signal."
...
def test_median_filter_removal(test_signal):
    ...
    assert np.allclose(clean_signal_no_padding, expected_signal_no_padding), "Median filter removal failed."
...
def test_adaptive_filtering(test_signal):
    ...
    assert len(clean_signal) == len(test_signal), "Adaptive filtering output size mismatch."
    assert np.all(np.abs(clean_signal - reference_signal) < np.abs(test_signal - reference_signal)), "Adaptive filtering did not work properly."
...
def test_notch_filter(test_signal):
    ...
    assert not np.allclose(clean_signal, test_signal), "Notch filter did not remove interference."
...
def test_ica_artifact_removal(test_signal):
    ...
    window_size = 3
    step_size = 1
    clean_signal = ar.ica_artifact_removal(num_components=1, window_size=window_size, 
                                         step_size=step_size, batch_size=200)

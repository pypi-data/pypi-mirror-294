...
from vitalDSP.transforms.vital_transformation import VitalTransformation  # Assuming VitalTransformation is in vitalDSP.transformation
...
def sample_signal():
    ...
    signal = np.sin(2 * np.pi * 5 * t) + 0.5 * np.random.randn(1000)  # Sine wave with noise
    return signal
...
def test_compute_fusion(sample_signal):
    ...
    assert len(fusion_result) <= len(sample_signal)  # Fusion result should not exceed signal length
...
def test_apply_transformations(transformer):
    ...
    options = {
        ...
        'bandpass_filter': {'lowcut': 1.0, 'highcut': 20.0, 'filter_order': 5, 'filter_type': 'cheby1'},
        ...
        'advanced_filtering': {'filter_type': 'optimization_based_filtering', 'target_signal': transformer.signal, 'loss_type': 'mse', 'learning_rate': 0.01}
    }
    method_order = ['artifact_removal', 'bandpass_filter', 'detrending', 'normalization', 'smoothing', 'enhancement', 'advanced_filtering']
    transformed_signal = transformer.apply_transformations(options, method_order)
    assert len(transformed_signal) == len(transformer.signal)
    ...
    options = {
        ...
        'bandpass_filter': {'lowcut': 0.2, 'highcut': 40.0, 'filter_order': 3, 'filter_type': 'ellip'},
        'detrending': {'detrend_type': 'linear'},
        'normalization': {'normalization_range': (0, 1)},
        'smoothing': {'smoothing_method': 'moving_average', 'window_size': 10, 'iterations': 2},
        'enhancement': {'enhance_method': 'square'},
        'advanced_filtering': {'filter_type': 'gradient_descent_filter', 'target_signal': transformer.signal, 'learning_rate': 0.001, 'iterations': 100}
    }
    method_order = ['artifact_removal', 'bandpass_filter', 'detrending', 'normalization', 'smoothing', 'enhancement', 'advanced_filtering']
    transformed_signal = transformer.apply_transformations(options, method_order)
    assert len(transformed_signal) == len(transformer.signal)
...
def test_apply_artifact_removal(transformer):
    ...
    transformer.apply_artifact_removal(method='median_filter_removal', options={'kernel_size': 5})
    assert transformer.signal is not None

    with pytest.raises(ValueError, match="Reference signal is required for adaptive filtering"):
...
def test_apply_smoothing(transformer):
    ...
    transformer.apply_smoothing(options={'smoothing_method': 'gaussian', 'sigma': 1.0, 'iterations': 2})
    assert transformer.signal is not None
...
def test_apply_advanced_filtering(transformer):
    ...
    transformer.apply_advanced_filtering(options={'filter_type': 'optimization_based_filtering', 'target_signal': transformer.signal})
    assert transformer.signal is not None

    # Pass the Kalman filter as a bound method in the ensemble filtering options
    transformer.apply_advanced_filtering(options={
        'filter_type': 'ensemble_filtering',
        'filters': [advanced_filtering_instance.kalman_filter],  # Pass instance method
        'method': 'mean'
    })
    assert transformer.signal is not None

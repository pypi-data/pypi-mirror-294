from _typeshed import Incomplete
from virtualitics_sdk.flow.step import Step as Step

logger: Incomplete

class Flow:
    '''A Flow organizes an App\'s workflow in the Virtualitics AI Platform. A Flow can contain many :class:`~virtualitics_sdk.flow.step.Step`s that can
    get inputs, run computation, and show results. 

    :param name: The name of the App.
    :param description: The App description
    :param image_path: The base64 encoding of an image or a public link to an image, defaults to None
    :param is_shareable: Whether a given App is shareable, defaults to False
    :param sort_key: An optional value to sort the App by, this controls the sort order of Apps on the home page

    **EXAMPLE:**

       .. code-block:: python

           # Imports 
           from virtualitics_sdk.flow.flow import Flow
           . . .
           # Example usage
           tile_image_link = "https://example-image.jpeg"
           example_flow = Flow(name="Example Flow", 
                                     description="Simple example for using Flow class",
                                     image_path=tile_image_link)
    '''
    id: Incomplete
    title: Incomplete
    name: Incomplete
    description: Incomplete
    last_run_step: Incomplete
    step_map: Incomplete
    steps: Incomplete
    is_locked: bool
    is_complete: bool
    image: Incomplete
    is_shareable: Incomplete
    sort_key: Incomplete
    def __init__(self, name: str, description: str, image_path: str | None = None, is_shareable: bool = False, sort_key: str | None = None) -> None: ...
    def chain(self, ordered_steps: list[Step], lock: bool = True):
        """Add all steps to the App's workflow with one function. Locks Apps by default. 

        :param ordered_steps: Ordered list of Steps to be added.
        """
    def get_data(self, is_self_service: bool = False, app_id: str | None = None): ...

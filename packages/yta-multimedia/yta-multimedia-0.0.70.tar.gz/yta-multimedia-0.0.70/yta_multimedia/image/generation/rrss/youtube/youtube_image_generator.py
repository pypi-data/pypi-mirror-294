...
class YoutubeImageGenerator:
    def generate_comment(self, author: str, avatar_url: str, time: str, message: str, likes_number: int, output_filename: Union[str, None] = None):
        """
    ...
    def generate_thumbnail(self, title: str = None, image_url: str = None, channel_name: str = None, channel_image_url: str = None, duration_in_seconds: int = None, output_filename: Union[str, None] = None):
        if not title:
            # TODO: Fake it
        ...
        scrapper = ChromeScrapper(gui = True, disable_popups_and_cookies = False)
        ...
        filename = get_resource('https://drive.google.com/file/d/1rcowE61X8c832ynh0xOt60TH1rJfcJ6z/view?usp=drive_link', 'base_thumbnail.png')
        scrapper.set_file_input(upload_input, filename)

        button = scrapper.find_element_by_text_waiting('button', 'Revisa tu miniatura')
        scrapper.scroll_to_element(button)
        scrapper.wait(1)
        button.click()
        # TODO: Change this type of waiting to <body> class is 'modal-open'
        scrapper.wait(1)

        thumbnail_container_element = scrapper.find_element_by_class('div', 'yt_main_box')
        ...
        # li[1] is views li[2] is time
        scrapper.set_element_inner_text(listed_items[1], '2.5M visitas')
        scrapper.set_element_inner_text(listed_items[2], 'hace 2 semanas')

        style = 'width: 500px; padding: 10px;'
        scrapper.set_element_style(thumbnail_container_element, style)

        if not output_filename:
            output_filename = create_tmp_filename('tmp_youtube_thumbnail.png')

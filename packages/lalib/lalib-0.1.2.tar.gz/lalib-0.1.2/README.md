...
All contributions become open-source themselves,
...
`poetry` also creates and manages a
    ...
    and pins their exact installation versions
    in the [poetry.lock](poetry.lock) file.
...
This includes running the test suite for the main Python version
...
- We can *not* start with **v0.1.0**
  because we already used this when learning to use PyPI

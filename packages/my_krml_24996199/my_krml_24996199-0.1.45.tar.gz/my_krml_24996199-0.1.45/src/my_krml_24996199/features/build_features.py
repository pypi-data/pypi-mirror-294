...
def build_features(input_filepath, output_filepath, is_train=True, selected_features_path=None):
    ...
    if is_train:
        ...
        all_features = np.array(list(numeric_features) + list(categorical_features))
        print(all_features)
        matching_columns_mask = np.isin(all_features, selected_feature_names)
        if matching_columns_mask.sum() != X_processed.shape[1]:
            raise ValueError("Mismatch between selected features and test set features.")
        X_selected = X_processed[:, matching_columns_mask]
        df_processed = pd.DataFrame(X_selected, columns=selected_feature_names)

...
class Scheduler:
    ...
    async def schedule(self, func, args, repeat: int = 1, interval: int = 6, duration: int = 5):
        logger.info(
        ...
        if repeat <= 0:
            ...
            raise Exception("interval is only allowed between 1 and 2629746 sec (1 month)")
        elif interval < duration:
            raise Exception("interval should be larger than duraion")
        else:
            # run once
            await func(args)
            if repeat == 1:

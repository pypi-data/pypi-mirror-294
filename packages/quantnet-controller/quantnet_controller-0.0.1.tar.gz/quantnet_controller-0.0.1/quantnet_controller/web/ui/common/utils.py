...
def set_cookies(token, cookie_accounts, attribs, ui_account=None):
    ...
    if token:
        ...
        setcookie("rucio-auth-token-created-at", value=time(), path="/")
...
def __to_js(var, value):
    ...
    return f'<script type="text/javascript">var {var} = "{value}";</script>'
...
def get_token():
    ...
    try:
        token = authentication.get_auth_token_x509(account, dn, "webui", ctx.env.get("REMOTE_ADDR")).token
        return token
    except Exception:
        return False
...
def check_token(rendered_tpl):
    ...
    if not dn.startswith("/"):
        dn = f'/{"/".join(dn.split(",")[::-1])}'

    msg = f"Your certificate ({dn}) is not mapped to any rucio account."
    msg += '<br><br><font color="red">First, please make sure it is correctly registered in '
    msg += '<a href="https://voms2.cern.ch:8443/voms/atlas">VOMS</a> and be patient until it '
    msg += 'has been fully propagated through the system.</font>'
    msg += '<br><br>Then, if it is still not working please contact'
    msg += '<a href="mailto:atlas-adc-ddm-support@cern.ch">DDM Support</a>.'
    ...
    if ui_account:
        ...
        if ui_account not in accounts:
            return render.problem(
                f"The rucio account ({ui_account}) you selected is not mapped to your certificate ({dn})." +
                "Please select another account or none at all to automatically use your default account."
            )

        cookie_accounts = accounts
        if (validate_token is None) or (validate_token["account"] != ui_account):
            try:
                token = authentication.get_auth_token_x509(ui_account, dn, "webui", ctx.env.get("REMOTE_ADDR")).token
            except Exception:
                return render.problem(msg)
        ...
        if validate_token is None:
            ...
            try:
                token = authentication.get_auth_token_x509(def_account, dn, "webui", ctx.env.get("REMOTE_ADDR")).token
            except Exception:
                return render.problem(msg)
    ...
    if validate_token and not data:
        ...
        try:
            token = authentication.get_auth_token_user_pass(
                def_account, data.username, data.password.encode("ascii"), "webui", ctx.env.get("REMOTE_ADDR")
            ).token
    ...
    if method == "GET":
        ...
        try:
            token = authentication.get_auth_token_saml(
                def_account, saml_nameid, "webui", ctx.env.get("REMOTE_ADDR")
            ).token

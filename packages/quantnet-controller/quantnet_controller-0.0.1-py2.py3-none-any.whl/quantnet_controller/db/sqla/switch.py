...
def add_switch(id, system_settings=None, channels=None, *, session: "Session"):
    ...
    # type_ = NodeType.SWITCH
    try:
        new_node = models.Switch(id=id, status=NodeStatus.ACTIVE)
    except Exception as e:
        raise e
    # sys_setting = models.SystemSetting(id=generate_uuid(),type='type',name='name',control_if='control_if',
        # queue='queue', mode='mode', threads=5, max_workers=10, qnode_id=id)
    if system_settings:
...
# @stream_session
# def list_switchs(filter_=None, *, session: "Session"):
...
#     if filter_ is None:
#         filter_ = {}
#     query = session.query(models.Switch.id, models.Switch.Switch_type,
#                           models.Switch.name).filter_by(status=NodeStatus.ACTIVE)
...
#
#     for id, switch_type, name in query.order_by(models.Switch.id).yield_per(25):
#         yield {'id': id, 'type': switch_type, 'name': name}

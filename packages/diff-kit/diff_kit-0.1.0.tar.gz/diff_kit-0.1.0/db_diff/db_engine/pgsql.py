...
import asyncpg
from db_diff.db_engine.abc import DbEngine
from db_diff.db_engine.exception import handle_db_exception
...
class AsyncPgEngine(DbEngine):
    ...
    async def execute(self, sql, values=None):
        query_sql = sql
        async with self.pool.acquire() as conn:
            res = await conn.fetch(sql, *values) if values else await conn.fetch(sql)
            return res


if __name__ == '__main__':
    host = "gp-uf6su7pn5np37b51xo-c-master.gpdb.rds.aliyuncs.com"
    port = 5432
    user = "adbadmin"
    password = "qicY30WxyJ123"
    db = "qa"


    async def main():
        engine = AsyncPgEngine(host, port, user, password, db)
        await engine.create_pool()
        values = (1714124727000730434, 9)
        res = await engine.execute(
            f"select * from bond_price.mix_trade_flow where id=$21 and price_source=$2", values)
        print(res)
        for i in res:
            print(i['id'])
        # res = await engine.get_table_columns("bond_price.mix_trade_flow")
        # print(res)

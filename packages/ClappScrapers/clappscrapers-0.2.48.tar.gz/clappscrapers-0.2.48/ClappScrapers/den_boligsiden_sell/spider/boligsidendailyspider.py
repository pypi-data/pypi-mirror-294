...
class BoligsidendailyspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield scrapy.Request(url = 'https://www.boligsiden.dk/tilsalg/villa,ejerlejlighed,raekkehus,andelsbolig,landejendom,villalejlighed?timeOnMarketMax=0&page=1',callback=self.parse_properties)
    ...
    def parse_properties(self,response):
        ...
        for script_text in response.css("script::text").getall():
            ...
            if 'props' in script_text:
                ...
                properties_data = "".join(script_text)

                properties_data = json.loads(properties_data)

                properties = []

                for item in properties_data['props']['pageProps']['searchResults']['on_market']:
                    href = item['address']['_links']['self']['href']
                    if href.startswith('/addresses/'):
                ...
                next_page_url = f'https://www.boligsiden.dk/tilsalg/villa,ejerlejlighed,raekkehus,andelsbolig,landejendom,villalejlighed?timeOnMarketMax=0&page={next_page}'

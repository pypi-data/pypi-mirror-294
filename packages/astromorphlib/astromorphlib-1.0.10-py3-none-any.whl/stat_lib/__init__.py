...
def computing_sky(gal, band, size_image_phy, imag,
    ...
    if sky_method == 'masking_sources':
        ...
        if version <= '1.5.0':
            ...
            print(Fore.BLUE + '\n\nMean, Median and STD of sky background after masking the sources')
            print('------------------------------------------')
            print('mean_sky = {:.4f}'.format(mean_sky))
            print('median_sky = {:.4f}'.format(median_sky))
            print('std_sky = {:.4f}'.format(std_sky))
        else:
            sky_method ='none'
            print(Fore.RED + '\nThe photutils version is newer than 1.5.0, then')
            print(Fore.RED + 'the method "masking_sources" cannot be used!')

    print(('\n\nEstimating the 2D sky background by using a box' +
...
def plot_stat(morph, gal, band, imag, median_sky, area, t, zp,
    ...
    if w is None:
        ...
        print ("\nThe galaxy center coordinates were update!\n")

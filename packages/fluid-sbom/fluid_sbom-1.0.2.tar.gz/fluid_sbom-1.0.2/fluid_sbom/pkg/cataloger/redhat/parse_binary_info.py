...
def hdrblob_init(data: bytes) -> HdrBlob:
    try:
        ...
        for _ in range(idle_length):
            ...
            tag, type_, offset_entry, count = struct.unpack(
                "<iIiI", data[offset : offset + ENTRY_INFO_SIZE]
            )
            entry = EntryInfo(
                tag=tag, type_=type_, offset=offset_entry, count=count
            )
...
def hdrblob_verify_region(blob: HdrBlob, data: bytes) -> None:
    ...
    try:
        ...
        tag, type_, offset, count = struct.unpack("<iIii", trailer_data[0])
        trailer = EntryInfo(tag=tag, type_=type_, offset=offset, count=count)

...
class MavenSearchDocResponse:  # pylint: disable=too-many-instance-attributes
    id_: str
    group: str
    artifact: str
    version: str
...
@dataclass(frozen=True)
class AuthorsLicenses:
    authors: list[str]
    licenses: list[str]


@dataclass(frozen=True)
class MavenPackageInfo:  # pylint: disable=too-many-instance-attributes
    group_id: str
    artifact_id: str
...
def _get_all_package(
    ...
    if response:
        ...
        release_date = metadata_xml.find("versioning/lastUpdated")
        authors_licenses = (

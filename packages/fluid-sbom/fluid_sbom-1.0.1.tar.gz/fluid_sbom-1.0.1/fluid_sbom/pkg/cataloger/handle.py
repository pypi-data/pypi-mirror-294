...
def handle_parser() -> Callable[[Observable[str]], Observable[Request]]:
    def _apply_parsers(source: Observable[str]) -> Observable[Request]:
        return source.pipe(
            ops.flat_map(
                lambda item: reactivex.merge(  # type: ignore
                    (on_next_python(reactivex.just(item))),
                    (on_next_db_file(reactivex.just(item))),
                    (on_next_java(reactivex.just(item))),
                    (on_next_javascript(reactivex.just(item))),
                    (on_next_redhat(reactivex.just(item))),
                    (on_next_dotnet(reactivex.just(item))),
                    (on_next_rust(reactivex.just(item))),
                    (on_next_ruby(reactivex.just(item))),
                    (on_next_elixir(reactivex.just(item))),
                    (on_next_php(reactivex.just(item))),
                    (on_next_swift(reactivex.just(item))),
                    (on_next_dart(reactivex.just(item))),
                )
            ),
        )

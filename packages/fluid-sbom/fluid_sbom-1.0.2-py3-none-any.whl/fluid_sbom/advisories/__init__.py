...
def version_match_constraint(version: Version, constraint: str) -> bool:
    ...
    result = any(
        _compare_single_constraint(
            version, constraint.strip(), version.raw_scheme
        )
...
def _compare_single_constraint(
    version: Version, constraint: str, scheme: type[ISchemeFactory]
) -> bool:
    ...
    result = operators[operator](version, other)
    return result


def _get_version_scheme_by_namespace(
    namespace: str,
) -> type[ISchemeFactory] | None:
    namespace_type = namespace.split(":")[1]
    scheme: type[ISchemeFactory] | None = None
    if namespace_type == "language":
...
def get_namespace_sufix(package: Package) -> str | None:
    if (
        package.p_url
        and (
        ...
        return f'distro:{distro.replace("-", ":")}'

    if package.language != Language.UNKNOWN_LANGUAGE:
        return f"language:{package.language.value}"  # type: ignore
...
def _get_matching_advisories(package: Package) -> list[Advisory]:
    ...
    if namespace_sufix := get_namespace_sufix(package):
        distro = f"%:{namespace_sufix}"
        cursor.execute(
            """
            SELECT
                vuln.cpes,

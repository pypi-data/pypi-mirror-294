...
TItem = str | int | None
RItem = list[TItem | list[TItem] | None]


def _cmp(a_item: TItem | RItem, b_item: TItem | RItem) -> int:
...
class Version:
    def __init__(
        self, version_string: str, scheme: type[ISchemeFactory]
    ) -> None:
        self.raw_scheme = scheme
        self.scheme = scheme()
        self.raw = version_string
        self.parts = self.scheme.parse(version_string)
    ...
    def _compare(
        self,
        other: Version | str,
        method: Callable[[TItem | RItem, TItem | RItem], bool],
    ) -> bool:
        _other: Version | str | None = None
        if not isinstance(other, Version):
            try:
                _other = Version(str(other), scheme=self.raw_scheme)
        ...
        return method(self.parts, _other.parts)
    ...
    def __lt__(self, other: Version | str) -> bool:
        with suppress(NotImplementedError):
            raw: str = other.raw if isinstance(other, Version) else other
            return self.scheme.compare(raw) < 0

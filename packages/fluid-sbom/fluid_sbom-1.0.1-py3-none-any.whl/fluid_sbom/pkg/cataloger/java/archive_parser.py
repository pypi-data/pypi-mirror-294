...
class ArchiveParser:
    ...
    def parse(self) -> tuple[list[Package] | None, list[Relationship] | None]:
        ...
        for pkg in packages:
            ...
            if isinstance(metadata, JavaArchive):
                pkg.p_url = package_url(pkg.name, pkg.version, metadata)
        return packages, []
    ...
    def discover_main_package(self) -> Package | None:
        ...
        if len(manifest_matches) > 1:
            logging.error("found multiple manifests in the jar")
        ...
        try:
            return Package(
                ...
                metadata=JavaArchive(
                    ...
                    manifest=manifest,
                    archive_digests=get_digests_from_archive(
                        self.archive_path
                    ),
                ),
            )
        ...
        for (
            ...
            if license_matches and self.archive_path:
                ...
                for license_match in license_matches:
                    license_contents = contents[license_match]

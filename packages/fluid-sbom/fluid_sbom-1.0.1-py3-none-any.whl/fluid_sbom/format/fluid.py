...
def validate_pkgs(sbom_pkgs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, pkg in enumerate(sbom_pkgs):
        try:
            json.dumps(pkg, indent=4, cls=EnumEncoder)
        except TypeError:
...
def format_fluid_sbom(  # pylint:disable=too-many-locals
    ...
    sbom_pkgs = [
        {
            ...
            "advisories": [
                to_dict(advisory) for advisory in package.advisories or []
            ],
        }
    ...
    try:
        LOGGER.info("Validating Fluid JSON format")
        ...
        LOGGER.exception(
            ...
            extra={
                "extra": {
                    ...
                    "path_in_file": "".join(
                        [f"[{repr(item)}]" for item in ex.path]
                    ),
                },
            },
        )

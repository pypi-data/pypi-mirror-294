...
class BaseOpenVINOBackend(BaseBackend):
    ...
    def send_to_batch(self, input_data: OV_INPUT_TYPE) -> Future:
        ...
        with self._get_free_request_lock:
            ...
            def on_result(request, userdata):
                try:
                    outputs = self.exec_net.requests[request].outputs
                    userdata.set_result(outputs)
                except Exception as e:
                    userdata.set_exception(e)
                finally:
                    with self._num_ongoing_requests_lock:
                        self._num_ongoing_requests -= 1

            self.exec_net.requests[request_id].set_completion_callback(on_result, future)

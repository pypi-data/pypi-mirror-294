...
async def test_memory_operations():
    ...
    seq_of_acts = {
        "goal": "Submit a comment to existing issue",
        "acts": [
            ["user", "Asked to add a comment to the github issue"],
            ["assistant", "I will run `hg issue comment ...`, do you want to continue?"],
            ["user", "yes"],
            ["call", "github", "gh issue comment 255 --body \"whooaa you ask too much\""],
        ],
        "outcome": "THUMBS_UP",
    }

    m0 = await chat_client.mem_add(
        base_url,
        "seq-of-acts",
        "Submit a comment to existing issue",
        "project1",
        json.dumps(seq_of_acts)
        )

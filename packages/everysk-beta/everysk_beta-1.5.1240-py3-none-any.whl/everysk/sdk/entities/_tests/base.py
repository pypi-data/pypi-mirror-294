...
class TestBaseEntity(TestCase):
    ...
    def test_script_query(self):
        expected_data = compress({'class_name': 'Script', 'method_name': '_fetch', 'self_obj': {'_klass': 'Portfolio', '__class_path__': 'everysk.sdk.entities.script.Script'}, 'params': {'user_input': 'port_12456', 'variant': 'select', 'workspace': None}}, protocol='gzip', serialize='json')
        ...
        expected_data = compress({'class_name': 'Script', 'method_name': '_fetch', 'self_obj': Portfolio.script.to_dict(add_class_path=True), 'params': {'user_input': 'port_12456', 'variant': 'select', 'workspace': None}}, protocol='gzip', serialize='json')
        mock_post.assert_called_with(url=self.api_url, headers=self.headers, verify=settings.HTTP_DEFAULT_SSL_VERIFY, timeout=settings.EVERYSK_SDK_HTTP_DEFAULT_TIMEOUT, data=expected_data)
        self.assertIsInstance(result, Portfolio)

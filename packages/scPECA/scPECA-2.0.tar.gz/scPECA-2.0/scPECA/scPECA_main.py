...
from scPECA.preprocessing import CPM, FPKM, RNA_process_mode_1, RNA_process_mode_2, RNA_process_mode_3, RNA_process_mode_h5ad, ATAC_process_mode_1, ATAC_process_mode_2, ATAC_process_mode_3
...
import pybedtools
import subprocess
from scPECA.utils import cluster_and_plot, _tf_layer, _top_tf_layer_plot
...
class scPECAclass:
    def __init__(self, path, sample_name, genome, pkg_path):
        ...
        os.chdir(self.path)
    def RNA_process(self, mode, **kwargs):
        ...
        if mode == 1:
            ...
            RNA_process_mode_h5ad(self.sample_name, self.genome, self.pkg_path, **kwargs)
        else:
            print("Unknown mode")
    ...
    def co_module(self, celltype, num_clusters, method = 'kMeans'):
        cluster_and_plot('./{}'.format(celltype), num_clusters, method)

    def tf_layer(self, celltype):
        _tf_layer('./{}'.format(celltype))
    
    def top_tf_layer_plot(self, celltype):
        _top_tf_layer_plot('./{}'.format(celltype))

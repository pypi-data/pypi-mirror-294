...
def RNA_process_mode_2(sample_name, genome, pkg_path, NORMALIZE = 'CPM'):
    """
    ...
    if NORMALIZE == 'FPKM':
      ...
      scrna_df = pd.read_csv('{}_scRNA.csv'.format(sample_name),index_col=0)
      scrna_df = scrna_df.sum(axis=1).to_frame()
      scrna_CPM = CPM(scrna_df)
      scrna_CPM = scrna_CPM[scrna_CPM.iloc[:, 0] > 0.1]
      scrna_CPM.to_csv('{}_PSExp.txt'.format(sample_name), sep='\t', header=False, index=True)

def RNA_process_mode_3(sample_name, genome, pkg_path, NORMALIZE = 'CPM'):
    """
    scRNA-seq count with meta
    :return: pseudo bulk RNA FPKM for different cell types
    """
    scrna_df = pd.read_csv('{}_scRNA.csv'.format(sample_name), index_col=0)
    meta = pd.read_csv('{}_scRNA_meta.csv'.format(sample_name), index_col=0, header=None)
    meta.columns = ['celltype']
    celltype_name = meta['celltype'].value_counts().reset_index()
    if NORMALIZE == 'FPKM':
        trans_df = pd.read_csv(os.path.join(pkg_path, 'Data', '{}_gene_length.txt'.format(genome)), sep='\t', header=None)
        for i in range(celltype_name.shape[0]):
            barcode_index = meta.loc[meta['celltype'] == celltype_name.iloc[i,0]].index
            ...
            scrna_FPKM = FPKM(merged_df)
            scrna_FPKM = scrna_FPKM[scrna_FPKM.iloc[:, 1] > 0.1]
            scrna_FPKM.to_csv('{}_PSExp.txt'.format(str(celltype_name.iloc[i,0])), sep='\t', header=False, index=False)
    else:
        for i in range(celltype_name.shape[0]):
            barcode_index = meta.loc[meta['celltype'] == celltype_name.iloc[i,0]].index
            scrna_df2 = scrna_df[barcode_index]
            scrna_df2 = scrna_df2.sum(axis=1).to_frame()
            scrna_CPM = CPM(scrna_df2)
            scrna_CPM = scrna_CPM[scrna_CPM.iloc[:, 0] > 0.1]
            scrna_CPM.to_csv('{}_PSExp.txt'.format(str(celltype_name.iloc[i,0])), sep='\t', header=False, index=True)

def RNA_process_mode_h5ad(sample_name, genome, pkg_path, obs_cluster, NORMALIZE = 'CPM'):

    adata = anndata.read_h5ad('{}.h5ad'.format(sample_name))
    scrna_df = pd.DataFrame(adata.X.T, index=adata.var_names, columns=adata.obs_names)
    meta = pd.DataFrame(adata.obs['{}'.format(obs_cluster)].values, index=adata.obs_names, columns=['celltype'])
    celltype_name = meta['celltype'].value_counts().reset_index()
    if NORMALIZE == 'FPKM':
        trans_df = pd.read_csv(os.path.join(pkg_path, 'Data', '{}_gene_length.txt'.format(genome)), sep='\t', header=None)
        for i in range(celltype_name.shape[0]):
...
def ATAC_process_mode_3(sample_name):
    ...
    """
    scatac_df = pd.read_csv('{}_scATAC.csv'.format(sample_name), index_col=0)
    meta = pd.read_csv('{}_scATAC_meta.csv'.format(sample_name), index_col=0, header=None)
    meta.columns = ['celltype']
    celltype_name = meta['celltype'].value_counts().reset_index()
    for i in range(celltype_name.shape[0]):
        barcode_index = meta.loc[meta['celltype'] == celltype_name.iloc[i,0]].index
        ...
        scatac_CPM.to_csv('{}_PSOpn.txt'.format(str(celltype_name.iloc[i,0])), sep='\t', header=False)

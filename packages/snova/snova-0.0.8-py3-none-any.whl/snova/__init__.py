...
class SnSdk:
    ...
    async def _gather(self, gen):
        ...
        if output:
            ...
            return self.refresh()
        raise Exception("Error occured with API endpoint. Connection timeout reached.")
        
    def refresh(self):
        self = SnSdk(model=self.model,
                     messages = self.messages,
                     remember = self.remember,
                     limit = self.limit,
                     system = self.system.get("content", None))
        return self.chat(message=None)
    ...
    def chat(self, 
        ...
        else: return self.loop.run_until_complete(self._gather(self._stream_chat(self.template, remember)))

    ...
    for dest_node, vms_to_migrate in migration_vms.items():
        if vms_to_migrate:
            for vm in vms_to_migrate:
                check_vm_migrate(proxmox_api=proxmox_api, dest_node=dest_node, vm=vm)
            migrate_vms(
                api=proxmox_api,
                dest_node=dest_node,
...
def reboot_cluster(ctx: typer.Context):
    proxmox_api: Proxmox = ctx.obj['proxmox_api']
    nodes = ctx.obj['nodes']
    parallel_migrations = ctx.obj['parallel_migrations']
    vms_all = proxmox_api.vm.list()
    ...
    for vm in vms_running:
        try:
            check_vm_migrate(proxmox_api=proxmox_api, vm=vm, dest_node=node_migration_map[vm['node']])
        except PVECLIError as err:
    ...
    for node_data in nodes:
        ...
        migration_failed = migrate_vms(
            api=proxmox_api, dest_node=tmp_node, vmid_list=node_running_vms, parallel_migrations=parallel_migrations
        )
        if migration_failed:
        ...
        migration_failed = migrate_vms(
            api=proxmox_api, dest_node=node, vmid_list=node_running_vms, parallel_migrations=parallel_migrations
        )
        if migration_failed:
            with tempfile.NamedTemporaryFile(mode='wt', delete=False, prefix='pve-cli') as tmpfile:
                json.dump(vms_all, tmpfile, indent=2)
...
def migrate_vms(api: Proxmox, dest_node: str, vmid_list: list[int], parallel_migrations: int = 4) -> bool:
    ...
    with Progress(spinner_col, text_col) as progress:
        while vm_list_working_copy:
            while len(running_tasks_list) < parallel_migrations and vm_list_working_copy:
                ...
                vm = api.vm.get(vm_id)
                upid = api.vm.migrate(vm['node'], vm['vmid'], dest_node)

        ...
        while True:
            ...
            if choice == 'p' and pagination.get('has_prev', False):
                ...
                if current_mode == mode_file:
                    ...
                    extract_response = requests.get(extract_url, headers=headers, json={'file_metadata_ids': id_list})
                    ...
                    extract_response = requests.get(extract_url, headers=headers, json={'archive_item_folder_md5s': ids})
                    if not Utilities.handle_response(extract_response):
                        return
                    content_disposition = extract_response.headers.get('Content-Disposition')
                    if content_disposition:

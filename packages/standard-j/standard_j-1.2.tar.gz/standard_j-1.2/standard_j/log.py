...
__do_terminal_output: bool = True
...
def enable_console_output() -> None:
    ...
    """
    global __do_terminal_output
    __do_terminal_output = True
...
def disable_console_output() -> None:
    ...
    """
    global __do_terminal_output
    __do_terminal_output = False


def __print(status, current_time, msg, file):
    color = ""
    end = ""
    if file == sys.stdout:
        end = AsciiFormatting.END
        if status is LogStatusType.INFO:
            color = AsciiFormatting.OK_CYAN
        if status is LogStatusType.SUCCESS:
            color = AsciiFormatting.OK_GREEN
        if status is LogStatusType.FAILURE:
            color = AsciiFormatting.FAIL
        if status is LogStatusType.WARNING:
            color = AsciiFormatting.WARNING
        if status is LogStatusType.ERROR:
            color = AsciiFormatting.FAIL + AsciiFormatting.BOLD
    print(color, __output_format.format(status=status.name, time=current_time), msg, end, sep="", file=file, flush=True)
...
def put(*values: object, sep: str = " ", status: LogStatusType = LogStatusType.INFO,
    ...
    for f in files if files is not None else __output_files:
        if isinstance(f, str):
            f = open(f, "a")
        __print(status, current_time, f"{sep.join(str(val) for val in values)}", f)
        f.close()
    if __do_terminal_output:
        __print(status, current_time, f"{sep.join(str(val) for val in values)}", sys.stdout)

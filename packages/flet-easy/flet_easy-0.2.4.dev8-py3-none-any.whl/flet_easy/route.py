...
class FletEasyX:
    ...
    def __view_pop(self, e):
        if len(self.__data.history_routes) > 1:
            ...
            raise Exception("Can't go back, there is no route history")
    ...
    def __process_route(
        self, custom_params: Dict[str, Callable[[], bool]], path: str, route: str
    ):
    ...
    def _go(self, route: str, use_route_change: bool = False, use_reload: bool = False):
        ...
        for page in self.__pages:
            route_math, route_check = self.__process_route(
                page.custom_params, route, page.route
            )
            if route_check:
                pg_404 = False
                try:
        ...
        if pg_404:
            if self.__page_404 is not None:
                ...
                page = Pagesy(
                    self.__view_404.route, self.__view_404.view, "FletEasy-404"
                )

...
def vcode_siblings(vcode):
    """
    Finds the sibling tiles of a given vcode at the same zoom level,
    based on the child tiles generated at the next zoom level.

    Args:
        vcode (str): The tile code in the format 'zXxYyZ', where X, Y, and Z are integers.

    Returns:
        list: A list of vcodes representing the sibling tiles.
    """
    # Extract z, x, y from the vcode
    match = re.match(r'z(\d+)x(\d+)y(\d+)', vcode)
    if not match:
    ...
    # Find siblings by determining which of the four child tiles are adjacent to the given tile
    siblings = []
    for child in children:
        match_child = re.match(r'z(\d+)x(\d+)y(\d+)', child)
        if not match_child:
            continue
        
        z_child = int(match_child.group(1))
        x_child = int(match_child.group(2))
        y_child = int(match_child.group(3))

        # Determine the coordinates of siblings by adjusting the child coordinates
        sibling_coords = [
            (x_child - 1, y_child),       # Left
            (x_child + 1, y_child),       # Right
            (x_child, y_child - 1),       # Top
            (x_child, y_child + 1)        # Bottom
        ]

        for (x_sibling, y_sibling) in sibling_coords:
            if x_sibling != x_child or y_sibling != y_child:
                sibling_vcode = f"z{z}x{x_sibling}y{y_sibling}"
                if sibling_vcode not in siblings:
                    siblings.append(sibling_vcode)

    # Filter out the original tile from the list of siblings
    siblings = [s for s in siblings if s != vcode]

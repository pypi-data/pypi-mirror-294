...
@LastEditTime: 2024-09-06 14:44:13
                ...
                if len(order_data) > 0:
                    ...
                    for order in reward_order_list:
                        try:
                            ...
                            #判断是否已经加过奖励
                            if order["order_id"] not in pay_order_no_list and int(order["order_status"]) in reward_order_status_list and order["after_sale_info"]["refund_status"] != '3': # 订单状态;1 待确认/待支付-订单创建完毕;105-已支付; 2-备货中； 101-部分发货; 3-已发货（全部发货）;4- 已取消;5 已完成（已收货）;
                                asset_object_name = ""
                                asset_object_id = ""
                                now_price_gear_dict = None
                                for price_gear_dict in price_gear_dict_list:
                                ...
                                third_pay_order.sub_pay_order_no = order['order_id']

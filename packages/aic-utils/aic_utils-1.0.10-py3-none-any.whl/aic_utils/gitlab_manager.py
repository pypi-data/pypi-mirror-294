...
class GitLabManager:
    def __init__(self, aic_instance, gitlab_token=None, gitlab_namespace='pin/pin-analytics/pin-fusion-2.0/pipelines', 
        ...
        self.gitlab_token = gitlab_token or self.read_gitlab_token()
        self.headers = {
    ...
    def push_new_file(self, repo_path, file_name, file_content):
        """Pushes a new file to the specified repository in GitLab."""
        project_url = f"{self.gitlab_base_url}/projects/{repo_path.replace('/', '%2F')}/repository/files/{file_name}"
        data = {
            'branch': 'main',
            'content': file_content,
            'commit_message': f"Add {file_name} - {datetime.now().strftime('%Y-%m-%d')}"
        }

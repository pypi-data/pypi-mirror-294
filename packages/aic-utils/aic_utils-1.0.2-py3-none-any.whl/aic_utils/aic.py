...
class AIC():
            ...
            for qa_pipeline in AIC_qa.pipeline_configs:
                ...
                if prod_name == qa_name:
                    if prod_to_qa:
                        ...
                        qa_pipeline['jobConfig']['$id'] = original_id  # Update the $id in QA jobConfig to match the source $id
                        # Set QA variable to True for QA
                        qa_pipeline['jobConfig'] = AIC.update_qa_variable(qa_pipeline['jobConfig'], True)
                        ...
                        prod_pipeline['jobConfig']['$id'] = original_id  # Update the $id in Production jobConfig to match the source $id
                        # Set QA variable to False for production
                        prod_pipeline['jobConfig'] = AIC.update_qa_variable(prod_pipeline['jobConfig'], False)
    ...
    @staticmethod
    def update_qa_variable(job_config, is_qa):
        """
        Updates the 'QA' variable in the job configuration to the correct value.

        Args:
            job_config (dict): The job configuration dictionary.
            is_qa (bool): True if the configuration is for QA, False for production.

        Returns:
            dict: Updated job configuration with the correct 'QA' variable value.
        """
        # Find the 'QA' variable and update its value
        for variable in job_config.get('variables', []):
            if variable['id'] == 'QA':
                variable['value'] = 'True' if is_qa else 'False'
                break
        else:
            # If 'QA' variable doesn't exist, add it
            job_config.setdefault('variables', []).append({
                'id': 'QA',
                'type': 'string',
                'value': 'True' if is_qa else 'False'
            })

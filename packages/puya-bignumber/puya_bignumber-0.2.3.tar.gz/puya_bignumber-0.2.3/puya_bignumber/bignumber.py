...
def multiply(x: Bytes, y: Bytes) -> Bytes:
    ...
    # print(p_1.length, p_4.length, second_half)
...
def _calc_mod_barrett_reduce(a: Bytes, mod: Bytes, precomputed_factor: Bytes) -> Bytes:
    ...
    print("ISSUE", precomputed_factor.length, a.length)
...
def modexp_barrett_reduce(
    ...
    result: Bytes = itob(1)
    # Last bit is 0
    base = mod_barrett_reduce(base, mod, precomputed_factor)
    for bit_i in reversed(urange(exp.length * 8)):
        bit: UInt64 = getbit(exp, bit_i)
        if bit == 1:
            result = mod_barrett_reduce(multiply(result, base), mod, precomputed_factor)
        base = mod_barrett_reduce(multiply(base, base), mod, precomputed_factor)
    return result

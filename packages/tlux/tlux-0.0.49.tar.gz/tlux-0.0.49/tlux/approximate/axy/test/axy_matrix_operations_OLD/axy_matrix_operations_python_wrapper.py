...
! TODO: Move allocation to be allowed as input.'''
    ...
    def radialize(self, x, shift, vecs, inverse=None, order=None, maxbound=None, max_to_flatten=None, max_to_square=None, svd_steps=None, svd_update=None, apply=None):
...
!   APPLY -- Optional input with default TRUE, apply the radialization to data matrix X.'''
        ...
        # Setting up "svd_update"
        svd_update_present = ctypes.c_bool(True)
        if (svd_update is None):
            svd_update_present = ctypes.c_bool(False)
            svd_update = ctypes.c_int()
        else:
            svd_update = ctypes.c_int(svd_update)
        if (type(svd_update) is not ctypes.c_int): svd_update = ctypes.c_int(svd_update)
        ...
        # Call C-accessible Fortran wrapper.
        clib.c_radialize(ctypes.byref(x_dim_1), ctypes.byref(x_dim_2), ctypes.c_void_p(x.ctypes.data), ctypes.byref(shift_dim_1), ctypes.c_void_p(shift.ctypes.data), ctypes.byref(vecs_dim_1), ctypes.byref(vecs_dim_2), ctypes.c_void_p(vecs.ctypes.data), ctypes.byref(inverse_present), ctypes.byref(inverse_dim_1), ctypes.byref(inverse_dim_2), ctypes.c_void_p(inverse.ctypes.data), ctypes.byref(order_present), ctypes.byref(order_dim_1), ctypes.c_void_p(order.ctypes.data), ctypes.byref(maxbound_present), ctypes.byref(maxbound), ctypes.byref(max_to_flatten_present), ctypes.byref(max_to_flatten), ctypes.byref(max_to_square_present), ctypes.byref(max_to_square), ctypes.byref(svd_steps_present), ctypes.byref(svd_steps), ctypes.byref(svd_update_present), ctypes.byref(svd_update), ctypes.byref(apply_present), ctypes.byref(apply))

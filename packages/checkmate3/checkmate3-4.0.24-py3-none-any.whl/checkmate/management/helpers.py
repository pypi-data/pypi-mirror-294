...
# Add missing imports
from blitzdb import SqlBackend as Backend
from checkmate.models import Project as ProjectClass
...
def get_backend(project_path, project_config, settings, echo=False, initialize_db=True):
    ...
    backend = Backend(engine, table_postfix="", ondelete="CASCADE", create_schema=initialize_db)
...
def get_project(project_path, project_config, settings, backend):
    try:
        ...
        raise Exception(f"Error retrieving or creating project: {str(e)}")

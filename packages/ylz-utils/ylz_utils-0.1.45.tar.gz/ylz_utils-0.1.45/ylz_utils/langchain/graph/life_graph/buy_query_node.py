...
class BuyQueryNode(Node):
    ...
    def __call__(self,state:State):
        ...
        schema_description = """
           ...
           (:Product).properties: <<name:购物的名称[String]>>
           (:Place).properties: <<name:地点名称[String]>>
           [s:buy].properties: <<sdt:起始时间,edt:截止时间,place:购物地点[String],value:购物的数量[Double],unit:购物数量的单位[String],buy:购物所花费的金额[Double],duration:购物所花费的时间[String]>>
           [at].properties:<<sdt:起始时间,edt:截止时间>>
           (:Person)-[:buy{{sdt,edt,place,value,unit,buy,duration}}]->(:Product) 某人购买某些物品花费多少钱和多少时间

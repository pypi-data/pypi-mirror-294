...
from Industrial_time_series_analysis.Diagnose.diagnose_utils.mcfmaae_util.MCFMAAE import utils, models
...
def load_dataset(data_dir='./data/mcfmaae_data', dataset='SMD', less=False, percentage=0.2):
    ...
    if less:
        loader[0] = utils.cut_array(0.2, loader[0])
    # 加载训练数据
...
def test(model, optimizer, scheduler, test_loader=None, labels=None):
    model.eval()

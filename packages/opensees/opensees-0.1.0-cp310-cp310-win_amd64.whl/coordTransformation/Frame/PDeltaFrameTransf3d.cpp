...
PDeltaFrameTransf3d::PDeltaFrameTransf3d(int tag, const Vector &vecInLocXZPlane,
    ...
    : FrameTransform3d(tag, CRDTR_TAG_PDeltaFrameTransf3d), nodeIPtr(0), nodeJPtr(0),
      nodeIOffset(0), nodeJOffset(0), L(0), ul17(0), ul28(0),
      nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
  for (int i = 0; i < 2; i++)
...
PDeltaFrameTransf3d::PDeltaFrameTransf3d()
    : FrameTransform3d(0, CRDTR_TAG_PDeltaFrameTransf3d), nodeIPtr(0), nodeJPtr(0),
      nodeIOffset(0), nodeJOffset(0), L(0), ul17(0), ul28(0),
      nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
  for (int i = 0; i < 3; i++)
...
int
PDeltaFrameTransf3d::commitState()
{
  return 0;
}

int
PDeltaFrameTransf3d::revertToLastCommit()
{
  return 0;
}

int
PDeltaFrameTransf3d::revertToStart()
{
  return 0;
}

int
PDeltaFrameTransf3d::initialize(Node *nodeIPointer, Node *nodeJPointer)
{
  int error;
  ...
  if ((!nodeIPtr) || (!nodeJPtr)) {
    opserr << "\nPDeltaFrameTransf3d::initialize";
...
void
PDeltaFrameTransf3d::compTransfMatrixLocalGlobal(Matrix &Tlg)
{
...
PDeltaFrameTransf3d::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
  ...
  if (ynorm == 0) {
    opserr << "\nPDeltaFrameTransf3d::getLocalAxes";
...
double
PDeltaFrameTransf3d::getInitialLength()
{
  return L;
}

double
PDeltaFrameTransf3d::getDeformedLength()
{
  return L;
}

const Vector &
PDeltaFrameTransf3d::getBasicTrialDisp()
{
...
const Vector &
PDeltaFrameTransf3d::getBasicIncrDisp()
{
  // determine global displacements
...
const Vector &
PDeltaFrameTransf3d::getBasicIncrDeltaDisp()
{
  // determine global displacements
...
const Vector &
PDeltaFrameTransf3d::getBasicTrialVel()
{
...
const Vector &
PDeltaFrameTransf3d::getBasicTrialAccel()
{
...
const Vector &
PDeltaFrameTransf3d::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
...
PDeltaFrameTransf3d::pushResponse(VectorND<12>&pl)
{
  ...
  double NoverL;
  NoverL = ul17 * N * oneOverL;
  pl[1] += NoverL;
  pl[7] -= NoverL;
  NoverL = ul28 * N * oneOverL;
  pl[2] += NoverL;
  pl[8] -= NoverL;
...
VectorND<12>
PDeltaFrameTransf3d::pushConstant(const VectorND<12>&pl) const
{
...
const Matrix &
PDeltaFrameTransf3d::getGlobalStiffMatrix(const Matrix &KB, const Vector &pb)
{
...
MatrixND<12,12>
PDeltaFrameTransf3d::pushConstant(const MatrixND<12,12>& kl)
{
...
const Matrix &
PDeltaFrameTransf3d::getInitialGlobalStiffMatrix(const Matrix &KB)
{
  static double kb[6][6];     // Basic stiffness
  static MatrixND<12,12> kl;  // Local stiffness
  static double tmp[12][12];  // Temporary storage
...
FrameTransform3d *
PDeltaFrameTransf3d::getCopy()
{
  // create a new instance of PDeltaFrameTransf3d
  ...
  theCopy = new PDeltaFrameTransf3d(this->getTag(), xz, offsetI, offsetJ);
...
int
PDeltaFrameTransf3d::sendSelf(int cTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "PDeltaFrameTransf3d::sendSelf - failed to send Vector\n";
    return res;
  }
...
int
PDeltaFrameTransf3d::recvSelf(int cTag, Channel &theChannel,
                            FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "PDeltaFrameTransf3d::recvSelf - failed to receive Vector\n";
    return res;
  }
...
const Matrix &
PDeltaFrameTransf3d::getGlobalMatrixFromLocal(const Matrix &ml)
{
  Matrix3D Rm;
  for (int i=0; i<3; i++)
    for (int j=0; j<3; j++)
...
const Vector &
PDeltaFrameTransf3d::getPointGlobalCoordFromLocal(const Vector &xl)
{
  static Vector xg(3);
...
const Vector &
PDeltaFrameTransf3d::getPointGlobalDisplFromBasic(double xi, const Vector &uxb)
{
  // determine global displacements
  const Vector &disp1 = nodeIPtr->getTrialDisp();
  const Vector &disp2 = nodeJPtr->getTrialDisp();
...
const Vector &
PDeltaFrameTransf3d::getPointLocalDisplFromBasic(double xi, const Vector &uxb)
{
  // determine global displacements
  const Vector &disp1 = nodeIPtr->getTrialDisp();
  const Vector &disp2 = nodeJPtr->getTrialDisp();

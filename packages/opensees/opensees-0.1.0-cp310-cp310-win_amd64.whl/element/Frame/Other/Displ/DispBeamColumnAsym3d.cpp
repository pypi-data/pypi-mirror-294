...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumnAsym3d)
{
  ...
  Element *theEle =
      new DispBeamColumnAsym3d(iData[0], iData[1], iData[2], secTags.Size(), sections,
...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumnAsym3dTcl)
{
  ...
  if (theSection == 0) {
    opserr << "WARNING section with tag " << secTag << "not found for element " << eleTag
           << endln;
    return 0;
  }
  ...
  while (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (strcmp(sData, "-cMass") == 0) {
      ...
      if (strcmp(sData2, "Lobatto") == 0) {
        ...
        numData = 4;
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
          opserr << "WARNING invalid input, want: -integration RegularizedLobatto $lpI "
                    "$lpJ $zetaI $zetaJ \n";
          return 0;
        }
        BeamIntegration *otherBeamInt = 0;
        otherBeamInt                  = new LobattoBeamIntegration();
        beamIntegr = new RegularizedHingeIntegration(*otherBeamInt, dData[0], dData[1],
                                                     dData[2], dData[3]);
        if (otherBeamInt != 0) {
          delete otherBeamInt;
        }
  ...
  Element *theElement =
      new DispBeamColumnAsym3d(eleTag, nodeI, nodeJ, numIntgrPts, sections, *beamIntegr,
                               *theTransf, dData2[0], dData2[1], massDens, cmass);

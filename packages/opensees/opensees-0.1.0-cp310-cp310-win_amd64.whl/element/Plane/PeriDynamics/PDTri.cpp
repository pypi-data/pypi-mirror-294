...
#define ELE_TAG_PDTri 0

double PDTri::matrixData[36];
Matrix PDTri::K(matrixData, 6, 6);
Vector PDTri::P(6);
double PDTri::shp[3][3];
double PDTri::pts[1][2];
double PDTri::wts[1];


PDTri::PDTri(int tag, int nd1, int nd2, int nd3, NDMaterial& m, const char* type, double t,
             double p, double r, double b1, double b2)
 : Element(tag, ELE_TAG_PDTri),
   theMaterial(0),
   ...
   Ki(0)
{
  ...
  if (strcmp(type, "PlaneStrain") != 0 && strcmp(type, "PlaneStress") != 0 &&
    ...
    opserr << "PDTri::PDTri -- improper material type: " << type << "for PDTri\n";
    exit(-1);
  }
  ...
  if (theMaterial == 0) {
    opserr << "PDTri::PDTri - failed allocate material model pointer\n";
    exit(-1);
  }
  ...
  for (i = 0; i < numgp; i++) {
    ...
    // Check allocation
    if (theMaterial[i] == 0) {
      opserr << "PDTri::PDTri -- failed to get a copy of material model\n";
      exit(-1);
    }
  }
...
int
PDTri::getNumExternalNodes() const
{
  return numnodes;
}

const ID&
PDTri::getExternalNodes()
{
...
void
PDTri::setDomain(Domain* theDomain)
{
  ...
  if (theNodes[0] == 0 || theNodes[1] == 0 || theNodes[2] == 0) {
    //opserr << "FATAL ERROR PDTri (tag: %d), node not found in domain",
    //this->getTag());
    return;
  }
  ...
  if (dofNd1 != 2 && dofNd1 != 3) {
    opserr << "WARNING PDTri::setDomain() element " << this->getTag()
           << " does not have 2 or 3 DOF at node " << theNodes[0]->getTag() << "\n";
    return;
  }
  if (dofNd2 != 2 && dofNd2 != 3) {
    opserr << "WARNING PDTri::setDomain() element " << this->getTag()
           << " does not have 2 or 3 DOF at node " << theNodes[1]->getTag() << "\n";
    return;
  }
  if (dofNd3 != 2 && dofNd3 != 3) {
    opserr << "WARNING PDTri::setDomain() element " << this->getTag()
           << " does not have 2 or 3 DOF at node " << theNodes[2]->getTag() << "\n";
    return;
  }
...
PDTri::commitState()
{
  ...
  if ((retVal = this->Element::commitState()) != 0) {
    opserr << "PDTri::commitState () - failed in base class";
  }
...
int
PDTri::addLoad(ElementalLoad* theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_SelfWeight) {
    ...
    opserr << "PDTri::addLoad - load type unknown for ele with tag: " << this->getTag() << "\n";
    return -1;
  }
...
int
PDTri::addInertiaLoadToUnbalance(const Vector& accel)
{
  int i;
  static double rhoi[1]; //numgp
  double sum = 0.0;
  ...
  if (2 != Raccel1.Size() || 2 != Raccel2.Size() || 2 != Raccel3.Size()) {
    opserr << "PDTri::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector&
PDTri::getResistingForceIncInertia()
{
  int i;
  static double rhoi[1]; //numgp
  double sum = 0.0;
...
int
PDTri::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  // PDTri packs its data into a Vector and sends this to theChannel
  ...
  if (res < 0) {
    opserr << "WARNING PDTri::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }

  // Now PDTri sends the ids of its materials
  int matDbTag;
  int count = 0;
  ...
  count += 2 * numgp;
  idData(count) = connectedExternalNodes(0);
  count += 1;
  idData(count) = connectedExternalNodes(1);
  count += 1;
  idData(count) = connectedExternalNodes(2);

  res += theChannel.sendID(dataTag, commitTag, idData);
  if (res < 0) {
    opserr << "WARNING PDTri::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }

  // Finally, PDTri asks its material objects to send themselves
  for (i = 0; i < numgp; i++) {
    res += theMaterial[i]->sendSelf(commitTag, theChannel);
    if (res < 0) {
      opserr << "WARNING PDTri::sendSelf() - " << this->getTag()
...
int
PDTri::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;

  int dataTag = this->getDbTag();

  // PDTri creates a Vector, receives the Vector and then sets the
  // internal data with the data in the Vector
  static Vector data(10);
  res += theChannel.recvVector(dataTag, commitTag, data);
  if (res < 0) {
    opserr << "WARNING PDTri::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  static ID idData(2 * numgp + numnodes + 1);
  // PDTri now receives the tags of its four external nodes
  res += theChannel.recvID(dataTag, commitTag, idData);
  if (res < 0) {
    opserr << "WARNING PDTri::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }

  int count = 2 * numgp;

  connectedExternalNodes(0) = idData(count);
  count += 1;
  connectedExternalNodes(1) = idData(count);
  count += 1;
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new NDMaterial*[numgp];
    if (theMaterial == 0) {
      opserr << "PDTri::recvSelf() - Could not allocate NDMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < numgp; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "PDTri::recvSelf() - Broker could not create NDMaterial of class type "
               << matClassTag << "\n";
        return -1;
      }
...
void
PDTri::Print(OPS_Stream& s, int flag)
{
  if (flag == OPS_PRINT_CURRENTSTATE) {
    s << "\nPDTri, element id:  " << this->getTag() << "\n";
...
Response*
PDTri::setResponse(const char** argv, int argc, OPS_Stream& output)
{
  Response* theResponse = 0;

  output.tag("ElementOutput");
  output.attr("eleType", "PDTri");
...
int
PDTri::getResponse(int responseID, Information& eleInfo)
{
  ...
  if (responseID == 1) {
    ...
    double We[numnodes][numgp] = {{1.0}, {1.0}, {1.0}};
...
int
PDTri::setParameter(const char** argv, int argc, Parameter& param)
{
  if (argc < 1)
    return -1;
...
int
PDTri::updateParameter(int parameterID, Information& info)
{
  switch (parameterID) {
  case -1: return -1;
...
double
PDTri::shapeFunction(double xi, double eta)
{

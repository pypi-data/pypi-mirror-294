...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumn3dWithSensitivity)
{
...
DispBeamColumn3dWithSensitivity::DispBeamColumn3dWithSensitivity(
    ...
    : Element(tag, ELE_TAG_DispBeamColumn3dWithSensitivity), numSections(numSec),
      theSections(0), crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(12), q(6),
      rho(r)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumn3dWithSensitivity::DispBeamColumn3dWithSensitivity - "
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumn3d::DispBeamColumn3d - failed to copy beam "
              "integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn3dWithSensitivity::DispBeamColumn3dWithSensitivity - "
              "failed to copy coordinate transformation\n";
    exit(-1);
  }
...
DispBeamColumn3dWithSensitivity::DispBeamColumn3dWithSensitivity()
    : Element(0, ELE_TAG_DispBeamColumn3dWithSensitivity), numSections(0), theSections(0),
...
DispBeamColumn3dWithSensitivity::commitState()
{
  ...
  if ((retVal = this->Element::commitState()) != 0) {
    opserr << "DispBeamColumn3dWithSensitivity::commitState () - failed in "
              "base class";
  }
...
DispBeamColumn3dWithSensitivity::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam3dUniformLoad) {
    ...
    opserr << "DispBeamColumn2d::addLoad() -- load type unknown for element "
              "with tag: "
           << this->getTag() << endln;
    return -1;
  }
...
DispBeamColumn3dWithSensitivity::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
    opserr << "DispBeamColumn3dWithSensitivity::addInertiaLoadToUnbalance "
...
DispBeamColumn3dWithSensitivity::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
    opserr << "DispBeamColumn3dWithSensitivity::sendSelf() - failed to send ID "
              "data\n";
    return -1;
  }
  if (idData(6) == 1) {
  ...
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
    opserr << "DispBeamColumn3dWithSensitivity::sendSelf() - failed to send "
              "crdTranf\n";
    return -1;
  }
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn3dWithSensitivity::sendSelf() - section " << j
                                          ...
                                          FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
    opserr << "DispBeamColumn3dWithSensitivity::recvSelf() - failed to recv ID "
              "data\n";
    return -1;
  }
  ...
  if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "DispBeamColumn3dWithSensitivity::sendSelf() - failed to recv "
              "crdTranf\n";
    return -3;
  }
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumn3dWithSensitivity::recvSelf() - out of memory "
                "creating sections array of size"
             << idData(3) << endln;
      exit(-1);
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumn3dWithSensitivity::recvSelf() - Broker could "
                  "not create Section of class type"
               << sectClassTag << endln;
        exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn3dWithSensitivity::recvSelf() - section " << i
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() != sectClassTag) {
        ...
        theSections[i] = theBroker.getNewSection(sectClassTag);
        if (theSections[i] == 0) {
          opserr << "DispBeamColumn3dWithSensitivity::recvSelf() - Broker "
                    "could not create Section of class type"
                 << sectClassTag << endln;
          exit(-1);
        }
      }
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
DispBeamColumn3dWithSensitivity::setParameter(const char **argv, int argc,
                                              Parameter &param)
{
  if (argc < 1)
    return -1;
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_MY:
        ...
        qsens(3) += (xi6 - 4.0) * sensi;
        qsens(4) += (xi6 - 2.0) * sensi;
        break;
      case SECTION_RESPONSE_T:
        //				q(5) += si;
        qsens(5) += sensi;

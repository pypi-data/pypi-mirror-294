...
RemoveRecorder::RemoveRecorder(int nodeID, ID &eleIDs, ID &secIDs,
    ...
    : Recorder(RECORDER_TAGS_RemoveRecorder), nodeTag(nodeID),
      ...
      eleResponses(0)
{
  ...
  for (int j = 0; j < numEles; j++) {
    ...
    if (theEle == NULL) {
      ...
      if (eleResponses[j] == 0) {
        opserr << "WARNING :: getRemCriteria - not a response quantity of "
                  "element\n";
      } else {
        if (fileNameinf != 0) {
  ...
  if (secondaryEleTags[0] != 0) {
    for (int k = 0; k < secondaryTags.Size(); k++) {
      ...
      if (theEle == NULL) {
        opserr << "WARNING RemoveRecorder::RemoveRecorder() - no element with "
                  "tag: "
      ...
      if (remFlag == 0) {
          ...
          for (int m = 0; m < RemoveRecorder::numRemEles; m++) {
            if (eleTags[j] == RemoveRecorder::remEleList[m]) {
              ...
              opserr << " element " << eleTags[j]
                     << " already removed from node " << nodeTag << " "
                     << numConEles << " elements left" << endln;
#endif
            }
          }
        }
        ...
        if (numConEles == 0) {
          ...
          if (theNode != 0)
            ...
            //                    numRemNodes ++;
#ifdef MMTDEBUG
          opserr << "Node removal # " << numRemNodes << " of node # "
                 << remNodeList[numRemNodes - 1] << endln;
...
#endif
      int eleCount =
          0; // a counter to see if all elements of this specific recorder were removed
      for (int j = 0; j < numEles; j++) {
        ...
        if (remFlag == 0) {
          ...
          if (theEle != 0) {
            ...
            for (int i = 0; i < numSecs; i++) {
              // changed
              int secFlag = this->checkEleRemoval(theEle, eleResponses[j],
                                                  secTags[i], criteria);
              if (secFlag == -1)
                return -1;
              eleFlag += secFlag;
...
int RemoveRecorder::playback(int commitTag) { return 0; }

int RemoveRecorder::restart(void)
{
  theFile.close();
  theFile.open(fileName, ios::out);
  return 0;
                                ...
                                int &theComponent, const Vector &criteria)
{
  ...
  for (int k = 0; k < numRules; k++) {
    ...
    default:
      ...
      opserr << "RemoveRecorder::checkRemoval() - Failed to identify removal "
                "criterion "
             << criteria[2 * k] << endln;
      return -1;
      break;
...
//                cnt++;
#ifdef MMTDEBUG
        opserr << "identified SP_Constraint " << theSP->getTag()
               << " acting on node " << spNode << endln;
#endif
  ...
  if (echoTimeFlag == true)
    opserr << "Node " << theNode->getTag()
           << " removed, Time/Load Factor = " << timeStamp << endln;

  return 0;
}

int RemoveRecorder::elimSecondaries(double timeStamp)
{
#ifdef MMTDEBUG
  ...
  for (int i = 0; i < secondaryEleTags.Size(); i++) {
    ...
    if (remFlag == 0) {
      ...
      for (int k = 0; k < theEle->getNumExternalNodes(); k++) {
        ...
        for (int m = 0; m < RemoveRecorder::numRemNodes; m++) {
          ...
          if (secondaryNodes[k] == RemoveRecorder::remNodeList[m]) {
            nodeRemFlag = 1;
#ifdef MMTDEBUG
            opserr << " node " << secondaryNodes[k] << " already removed "
                   << endln;
#endif
          }
        }
      ...
      opserr << "Element removal # " << numRemEles << " of element # "
             << remEleList[numRemEles - 1] << endln;
#endif
    }
  }
...
int RemoveRecorder::updateNodalMasses(int theEleTag, double theEleMass)
{
  ...
  if (theEleMass != 0) {
    ...
    for (int k = 0; k < theEle->getNumExternalNodes(); k++) {
      ...
      if (theDomain->setMass(theNodalMass, theNode->getTag()) != 0) {
        opserr << "Remove Recorder::WARNING failed to set mass at node "
      ...
      if (this->gAcc != 0) {
        ...
        opserr << "eleWeight " << theEleWeight << " NodalForces "
               << theNodalForces(0) << " " << theNodalForces.Size() << endln;
        int nodeLoadTag = 987654 + theEleTag * 30 +
                          k; // big number + many element nodes + current node
        NodalLoad *theNodalLoad = new NodalLoad(nodeLoadTag, theNode->getTag(),
                                                theNodalForces, isLoadConst);
        if (theNodalLoad == 0) {
          opserr << "RemoveRecorder::WARNING ran out of memory while updating "
                    "loads on node  "
                 << theNode->getTag();
        }
        if (theDomain->addNodalLoad(theNodalLoad, gPat) == false) {
          opserr << "RemoveRecorder::WARNING could not add updated nodal load "
                    "to domain\n";
          delete theNodalLoad;
        }
      }

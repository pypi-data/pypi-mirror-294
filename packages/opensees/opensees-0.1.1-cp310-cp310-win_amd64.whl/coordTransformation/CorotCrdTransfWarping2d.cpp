      ...
      nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
  ...
  if (rigJntOffsetI.Size() != 2) {
    opserr << "CorotCrdTransfWarping2d::CorotCrdTransfWarping2d:  Invalid "
...
CorotCrdTransfWarping2d::compElemtLengthAndOrient(void)
{
  ...
  if (nodeOffsets == true)
    dx = (nodeJPtr->getCrds() + nodeJOffset) -
         (nodeIPtr->getCrds() + nodeIOffset);
  else
...
int
CorotCrdTransfWarping2d::compElemtLengthAndOrientWRTLocalSystem(
    const Vector &ul)
{
...
CorotCrdTransfWarping2d::getBasicTrialAccel(void)
{
  ...
  ab(1) =
      al(2) - (Lx * Lydotdot - Ly * Lxdotdot) / pow(Ln, 2) +
...
#if 0 // This assumes member loads are in basic system
...
CorotCrdTransfWarping2d::getCopy2d(void)
{
  ...
  CorotCrdTransfWarping2d *theCopy =
      new CorotCrdTransfWarping2d(this->getTag(), nodeIOffset, nodeJOffset);
                                  ...
                                  FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvVector(this->getTag(), cTag, data) < 0) {
    opserr << " CorotCrdTransfWarping2d::recvSelf() - data could not be "
              "received\n";
    return -1;
  }
...
{
  static Vector xg(5);
  opserr << " CorotCrdTransfWarping2d::getPointGlobalCoordFromLocal: not "
            "implemented yet";
...
CorotCrdTransfWarping2d::getPointGlobalDisplFromBasic(double xi,
                                                      const Vector &uxb)
{
  opserr << " CorotCrdTransfWarping2d::getPointGlobalDisplFromBasic: not "
            "implemented yet";
...
CorotCrdTransfWarping2d::getPointLocalDisplFromBasic(double xi,
                                                     const Vector &uxb)
{
  opserr << " CorotCrdTransfWarping2d::getPointLocalDisplFromBasic: not "
            "implemented yet";
...
const Vector &
CorotCrdTransfWarping2d::getGlobalResistingForceShapeSensitivity(
    const Vector &q, const Vector &p0, int gradNumber)
{

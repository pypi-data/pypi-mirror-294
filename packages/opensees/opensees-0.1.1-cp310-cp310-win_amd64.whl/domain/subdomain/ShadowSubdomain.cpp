...
ShadowSubdomain::ShadowSubdomain(int tag, MachineBroker &theMachineBroker,
...
ShadowSubdomain::ShadowSubdomain(int tag, Channel &the_Channel,
                                 FEM_ObjectBroker &theObjectBroker)
    : Shadow(the_Channel, theObjectBroker), Subdomain(tag), msgData(4),
      theElements(0, 128), theNodes(0, 128), theExternalNodes(0, 128),
      theLoadCases(0, 128), theShadowSPs(0), theShadowMPs(0), theShadowLPs(0),
      numDOF(0), numElements(0), numNodes(0), numExternalNodes(0), numSPs(0),
      numMPs(0), buildRemote(false), gotRemoteData(false), theFEele(0),
...
#if 0 // cmp - was a multiline comment
int
...
int ShadowSubdomain::getRemoteData(void)
{
...
bool ShadowSubdomain::addSP_Constraint(SP_Constraint *theSP, int loadPattern)
{
...
bool ShadowSubdomain::addElementalLoad(ElementalLoad *theLoad, int loadPattern)
{
...
int ShadowSubdomain::getNumElements(void) const { return numElements; }

int ShadowSubdomain::getNumLoadPatterns(void) const { return numLoadPatterns; }

int ShadowSubdomain::getNumNodes(void) const { return numNodes; }

int ShadowSubdomain::getNumSPs(void) const { return numSPs; }

int ShadowSubdomain::getNumMPs(void) const { return numMPs; }
...
int ShadowSubdomain::record(bool fromAnalysis)
{
  DomainDecompositionAnalysis *theDDA = this->getDDAnalysis();
  if (theDDA != 0 &&
      (fromAnalysis == false || theDDA->doesIndependentAnalysis() != true)) {
...
void ShadowSubdomain::setDomainDecompAnalysis(
...
void ShadowSubdomain::Print(OPS_Stream &s, ID *nodeTags, ID *eleTags, int flag)
{
...
const Vector *ShadowSubdomain::getElementResponse(int tag, const char **argv,
                                                  int argc)
{
  if (theElements.getLocation(tag) < 0)
    return NULL;

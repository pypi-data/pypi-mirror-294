...
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumn3dThermal)
{
  ...
  Element *theEle = new DispBeamColumn3dThermal(
      iData[0], iData[1], iData[2], secTags.Size(), sections, *bi, *theTransf, mass);
  delete[] sections;
  return theEle;
}

DispBeamColumn3dThermal::DispBeamColumn3dThermal(int tag, int nd1, int nd2, int numSec,
                                                 SectionForceDeformation **s,
                                                 BeamIntegration &bi,
                                                 CrdTransf &coordTransf, double r)
    : Element(tag, ELE_TAG_DispBeamColumn3dThermal), numSections(numSec), theSections(0),
      ...
      parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumn3dThermal::DispBeamColumn3dThermal - failed to "
              "allocate section model pointer\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn3dThermal::DispBeamColumn3dThermal - failed to "
              "copy coordinate transformation\n";
    exit(-1);
  }
...
DispBeamColumn3dThermal::DispBeamColumn3dThermal()
    : Element(0, ELE_TAG_DispBeamColumn3dThermal), numSections(0), theSections(0),
...
DispBeamColumn3dThermal::update(void)
{
...
#ifdef _DEBUG
    if (this->getTag() == 601 || this->getTag() == 602) {
      opserr << "disp3d Tag: " << this->getTag() << " SECTION " << i
             << "  deformation: " << e << endln;
    }
#endif
  ...
  if (err != 0) {
    opserr << "DispBeamColumn3dThermal::update() - failed "
...
DispBeamColumn3dThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
      ...
      if (NodalT0->Size() == 9) {
        ...
        for (int m = 0; m < 5; m++) {
          (*dataMixV)(2 * m) =
              ...
              xi[i] *
                  ((*NodalT1)(m) -
                   (*NodalT0)(m)); ////5 temps through y, Linear temperature interpolation
          (*dataMixV)(3 * m + 10) =
              (*NodalT0)(m + 5) +
              xi[i] * ((*NodalT1)(m + 5) -
                       (*NodalT0)(m + 5)); ///////5 temps through Z in bottom flange
          (*dataMixV)(3 * m + 11) =
              (*NodalT0)(m + 10) +
              xi[i] * ((*NodalT1)(m + 10) -
                       (*NodalT0)(m + 10));        /////5 temps through Z in top flange
...
#ifdef _BDEBUG
      if (this->getTag() == 1) {
        opserr << "Tag: " << this->getTag() << "Beam3D::dataMixv " << endln << dataMixV
               << endln;
      }
#endif
  ...
  //----------------------------------------------
    ...
    for (int i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() == SEC_TAG_FiberSection3dThermal)
        SectionThermalElong[i] =
            ((FiberSection3dThermal *)theSections[i])->getThermalElong()(0);
    ...
    opserr << "DispBeamColumn3dThermal::addLoad() -- load type unknown for "
              "element with tag: "
           << this->getTag() << endln;
    return -1;
  }
...
DispBeamColumn3dThermal::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
    opserr << "DispBeamColumn3dThermal::addInertiaLoadToUnbalance matrix and "
...
DispBeamColumn3dThermal::getResistingForce()
{
  ...
  if (this->getTag() == 601 || this->getTag() == 602) {
    ...
    opserr << " ResidThermal  " << residThermal[0] << " , " << residThermal[1] << " ,"
           << residThermal[2] << endln;
  }
...
DispBeamColumn3dThermal::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (idData(6) == 1) {
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
      opserr << "DispBeamColumn3dThermal::sendSelf() - failed to send double "
                "data\n";
      return -1;
    }
  }
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn3dThermal::sendSelf() - section " << j
                                  ...
                                  FEM_ObjectBroker &theBroker)
{
  ...
  if (idData(6) == 1) {
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
      opserr << "DispBeamColumn3dThermal::sendSelf() - failed to recv double "
                "data\n";
      return -1;
    }
    alphaM = dData(0);
  ...
  if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
    ...
    if (beamInt == 0) {
      opserr << "DispBeamColumn3dThermal::recvSelf() - failed to obtain the "
                "beam integration object with classTag"
  ...
  if (beamInt->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "DispBeamColumn3dThermal::sendSelf() - failed to recv beam "
              "integration\n";
    return -3;
  }
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumn3dThermal::recvSelf() - out of memory creating "
                "sections array of size"
             << idData(3) << endln;
      exit(-1);
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumn3dThermal::recvSelf() - Broker could not "
                  "create Section of class type"
               << sectClassTag << endln;
        exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn3dThermal::recvSelf() - section " << i

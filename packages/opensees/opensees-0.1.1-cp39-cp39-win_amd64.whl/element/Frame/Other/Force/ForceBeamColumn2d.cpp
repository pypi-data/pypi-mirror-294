...
{
  // TODO: Will remove once we clean up the corotational 2d transformation -- MHS
  crdTransf->update();
...
ForceBeamColumn2d::update()
{
  ...
  while (converged == false && numSubdivide <= maxSubdivisions) {
    ...
    for (int l=Newton; l < BreakAlgorithms; l++) {
      ...
      if (initialFlag != 2) {
        ...
        for (int j=0; j <numIters; j++) {
          ...
          //} else if (l == 2) { // TODO - This seems to have been the wrong flag
            } else if (l == Initial) {
          ...
          if (fabs(dW) < tol) {
            ...
            if (j == (numIters-1) && (l == BreakAlgorithms-1)) {
              dvTrial /= factor;
              numSubdivide++;
            }
          }
...
{  
  ...
  for (int k=0; k<numSections; k++)
     for (int i=0; i<sections[k]->getOrder(); i++)

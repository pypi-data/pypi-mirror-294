...
ShellANDeS::ShellANDeS()
    : Element(0, ELE_TAG_ShellANDeS), connectedExternalNodes(3), K(18, 18),
      M(18, 18), P(18), Q(18), bf(3), is_stiffness_calculated(false),
      is_mass_calculated(false), thickness(0), xl1(3), xl2(3), xl3(3), x0(3),
      T_lg(3, 3), rho(0), E_planestress(3, 3), initialized_disps(false)
{
  // zero node pointers
  for (int i = 0; i < 3; i++) {
    theNodes[i] = 0;
...
ShellANDeS::ShellANDeS(int element_number, int node_numb_1, int node_numb_2,
                       int node_numb_3, double t, double E_, double nu_,
                       double rho_)
    : Element(element_number, ELE_TAG_ShellANDeS), connectedExternalNodes(3),
      K(18, 18), M(18, 18), P(18), Q(18), bf(3), is_stiffness_calculated(false),
      is_mass_calculated(false), thickness(t), xl1(3), xl2(3), xl3(3), x0(3),
      T_lg(3, 3), rho(rho_), E_planestress(3, 3), initialized_disps(false)
{
...
ShellANDeS::ShellANDeS(int element_number, int node_numb_1, int node_numb_2,
                       int node_numb_3, double t, double E11, double E22,
                       double E33, double E12, double E13, double E23,
                       double n1_, double n2_, double n3_, double rho_)
    : Element(element_number, ELE_TAG_ShellANDeS), connectedExternalNodes(3),
      K(18, 18), M(18, 18), P(18), Q(18), bf(3), is_stiffness_calculated(false),
      is_mass_calculated(false), thickness(t), xl1(3), xl2(3), xl3(3), x0(3),
      T_lg(3, 3), rho(rho_), E_planestress(3, 3), initialized_disps(false)
...
int ShellANDeS::getNumExternalNodes() const { return 3; }

const ID &ShellANDeS::getExternalNodes() { return connectedExternalNodes; }

Node **ShellANDeS::getNodePtrs(void) { return theNodes; }

int ShellANDeS::getNumDOF() { return 18; }
...
void ShellANDeS::setDomain(Domain *theDomain)
{
  if (theDomain == nullptr) {
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0 || theNodes[2] == 0) {
      opserr << "FATAL ERROR ShellANDeS (tag: " << this->getTag()
...
int ShellANDeS::commitState() { return 0; }

int ShellANDeS::revertToLastCommit() { return 0; }

int ShellANDeS::revertToStart() { return 0; }

int ShellANDeS::update(void) { return 0; }
...
const Matrix &ShellANDeS::getInitialStiff() { return getTangentStiff(); }
...
void ShellANDeS::zeroLoad() { Q.Zero(); }
...
int ShellANDeS::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  //    }
    ...
    opserr << "ShellANDeS::addLoad() - addLoad " << this->getTag()
...
int ShellANDeS::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (6 != Raccel1.Size() || 6 != Raccel2.Size() || 6 != Raccel3.Size()) {
    ...
    opserr << "ShellANDeS::addInertiaLoadToUnbalance matrix and vector sizes "
              "are incompatable\n";
    return -1;
  }
...
int ShellANDeS::sendAndCheckVector(int dataTag, int commitTag, Vector &v,
                                   Channel &channel, const std::string &name)
{
  int res = channel.sendVector(dataTag, commitTag, v);
  if (res < 0) {
    opserr << "WARNING LysmerTriangle::sendAndCheckVector() - "
           << this->getTag() << " failed to send " << name.c_str() << endln;
    return -1;
  }
  return res;
}

int ShellANDeS::recvAndCheckVector(int dataTag, int commitTag, Vector &v,
                                   Channel &channel, const std::string &name)
{
  int res = channel.recvVector(dataTag, commitTag, v);
  if (res < 0) {
    opserr << "WARNING LysmerTriangle::recvAndCheckVector() - "
           << this->getTag() << " failed to receive " << name.c_str() << endln;
    return -1;
  }
  return res;
}

int ShellANDeS::sendAndCheckMatrix(int dataTag, int commitTag, Matrix &v,
                                   Channel &channel, const std::string &name)
{
  int res = channel.sendMatrix(dataTag, commitTag, v);
  if (res < 0) {
    opserr << "WARNING LysmerTriangle::sendAndCheckMatrix() - "
           << this->getTag() << " failed to send " << name.c_str() << endln;
    return -1;
  }
  return res;
...
int ShellANDeS::recvAndCheckMatrix(int dataTag, int commitTag, Matrix &v,
                                   Channel &channel, const std::string &name)
{
  int res = channel.recvMatrix(dataTag, commitTag, v);
  if (res < 0) {
    opserr << "WARNING LysmerTriangle::recvAndCheckMatrix() - "
           << this->getTag() << " failed to receive " << name.c_str() << endln;
    return -1;
  }
  return res;
...
int ShellANDeS::sendAndCheckID(int dataTag, int commitTag, ID &v,
                               Channel &channel, const std::string &name)
{
  int res = channel.sendID(dataTag, commitTag, v);
  if (res < 0) {
    opserr << "WARNING LysmerTriangle::sendAndCheckID() - " << this->getTag()
           << " failed to send " << name.c_str() << endln;
    return -1;
  }
  return res;
}

int ShellANDeS::recvAndCheckID(int dataTag, int commitTag, ID &v,
                               Channel &channel, const std::string &name)
{
  int res = channel.recvID(dataTag, commitTag, v);
  if (res < 0) {
    opserr << " WARNING LysmerTriangle::recvAndCheckID() - " << this->getTag()
           << " failed to receive " << name.c_str() << endln;
    return -1;
  }
  return res;
}
...
int ShellANDeS::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  sendAndCheckMatrix(commitTag, dataTag, E_planestress, theChannel,
                     "E_planestress");

  return 0;
}

int ShellANDeS::recvSelf(int commitTag, Channel &theChannel,
                         FEM_ObjectBroker &theBroker)
{
  ...
  recvAndCheckMatrix(commitTag, dataTag, E_planestress, theChannel,
                     "E_planestress");
...
Response *ShellANDeS::setResponse(const char **argv, int argc,
                                  OPS_Stream &theHandler)
{
  ...
  } else if (strcmp(argv[0], "moments") == 0 ||
...
double ShellANDeS::getArea() const { return Area; }

bool ShellANDeS::gotMass() const { return is_mass_calculated; }
...
const Vector &ShellANDeS::get_bending_moment_field()
{
  ...
  EL.Zero();
  EL.addMatrixProduct(thickness * thickness * thickness / 12 / Area,
                      E_planestress, L, 1.0);
...
const Matrix &ShellANDeS::getMembraneMass()
{
  ...
  Ml(2, 2) = -45 * alpha * x12 * (-alpha * x12 / 45 - alpha * x13 / 90) / 2 -
             45 * alpha * x13 * (-alpha * x12 / 90 - alpha * x13 / 45) / 2 +
             45 * alpha * y12 * (alpha * y12 / 45 + alpha * y13 / 90) / 2 +
             45 * alpha * y13 * (alpha * y12 / 90 + alpha * y13 / 45) / 2;
  Ml(2, 3) = 3 * alpha * y12 + 3 * alpha * y13 / 2;
  Ml(2, 4) = -3 * alpha * x12 - 3 * alpha * x13 / 2;
  Ml(2, 5) = 45 * alpha * x12 * (-alpha * x12 / 45 - alpha * x13 / 90) / 2 -
             45 * alpha * x23 * (-alpha * x12 / 90 - alpha * x13 / 90) / 2 -
             45 * alpha * y12 * (alpha * y12 / 45 + alpha * y13 / 90) / 2 +
             45 * alpha * y23 * (alpha * y12 / 90 + alpha * y13 / 90) / 2;
  Ml(2, 6) = 3 * alpha * y12 / 2 + 3 * alpha * y13;
  Ml(2, 7) = -3 * alpha * x12 / 2 - 3 * alpha * x13;
  Ml(2, 8) = 45 * alpha * x13 * (-alpha * x12 / 90 - alpha * x13 / 45) / 2 +
             45 * alpha * x23 * (-alpha * x12 / 90 - alpha * x13 / 90) / 2 -
             45 * alpha * y13 * (alpha * y12 / 90 + alpha * y13 / 45) / 2 -
             45 * alpha * y23 * (alpha * y12 / 90 + alpha * y13 / 90) / 2;
  Ml(3, 0) = 15;
  ...
  Ml(5, 2) = -45 * alpha * x12 * (alpha * x12 / 45 - alpha * x23 / 90) / 2 -
             45 * alpha * x13 * (alpha * x12 / 90 - alpha * x23 / 90) / 2 +
             45 * alpha * y12 * (-alpha * y12 / 45 + alpha * y23 / 90) / 2 +
             45 * alpha * y13 * (-alpha * y12 / 90 + alpha * y23 / 90) / 2;
  Ml(5, 3) = -3 * alpha * y12 + 3 * alpha * y23;
  Ml(5, 4) = 3 * alpha * x12 - 3 * alpha * x23;
  Ml(5, 5) = 45 * alpha * x12 * (alpha * x12 / 45 - alpha * x23 / 90) / 2 -
             45 * alpha * x23 * (alpha * x12 / 90 - alpha * x23 / 45) / 2 -
             45 * alpha * y12 * (-alpha * y12 / 45 + alpha * y23 / 90) / 2 +
             45 * alpha * y23 * (-alpha * y12 / 90 + alpha * y23 / 45) / 2;
  Ml(5, 6) = -3 * alpha * y12 / 2 + 3 * alpha * y23;
  Ml(5, 7) = 3 * alpha * x12 / 2 - 3 * alpha * x23;
  Ml(5, 8) = 45 * alpha * x13 * (alpha * x12 / 90 - alpha * x23 / 90) / 2 +
             45 * alpha * x23 * (alpha * x12 / 90 - alpha * x23 / 45) / 2 -
             45 * alpha * y13 * (-alpha * y12 / 90 + alpha * y23 / 90) / 2 -
             45 * alpha * y23 * (-alpha * y12 / 90 + alpha * y23 / 45) / 2;
  Ml(6, 0) = 15;
  ...
  Ml(8, 2) = -45 * alpha * x12 * (alpha * x13 / 90 + alpha * x23 / 90) / 2 -
             45 * alpha * x13 * (alpha * x13 / 45 + alpha * x23 / 90) / 2 +
             45 * alpha * y12 * (-alpha * y13 / 90 - alpha * y23 / 90) / 2 +
             45 * alpha * y13 * (-alpha * y13 / 45 - alpha * y23 / 90) / 2;
  Ml(8, 3) = -3 * alpha * y13 / 2 - 3 * alpha * y23;
  Ml(8, 4) = 3 * alpha * x13 / 2 + 3 * alpha * x23;
  Ml(8, 5) = 45 * alpha * x12 * (alpha * x13 / 90 + alpha * x23 / 90) / 2 -
             45 * alpha * x23 * (alpha * x13 / 90 + alpha * x23 / 45) / 2 -
             45 * alpha * y12 * (-alpha * y13 / 90 - alpha * y23 / 90) / 2 +
             45 * alpha * y23 * (-alpha * y13 / 90 - alpha * y23 / 45) / 2;
  Ml(8, 6) = -3 * alpha * y13 - 3 * alpha * y23;
  Ml(8, 7) = 3 * alpha * x13 + 3 * alpha * x23;
  Ml(8, 8) = 45 * alpha * x13 * (alpha * x13 / 45 + alpha * x23 / 90) / 2 +
             45 * alpha * x23 * (alpha * x13 / 90 + alpha * x23 / 45) / 2 -
             45 * alpha * y13 * (-alpha * y13 / 45 - alpha * y23 / 90) / 2 -
             45 * alpha * y23 * (-alpha * y13 / 90 - alpha * y23 / 45) / 2;
...
const Matrix &ShellANDeS::getBendingMass()
{
  ...
  Gp(1, 0) = (-x_1 - x_2 + 2 * x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(1, 1) = (2 * x_1 - x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(1, 2) = (-x_1 + 2 * x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(1, 3) =
      2 * (-x_1 - x_2 + 2 * x_3) /
  ...
  Gp(2, 0) = (-y_1 - y_2 + 2 * y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(2, 1) = (2 * y_1 - y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(2, 2) = (-y_1 + 2 * y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(2, 3) =
      2 * (-y_1 - y_2 + 2 * y_3) /
      (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 + x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  ...
  Gp(4, 0) = (-x_1 - x_2 + 2 * x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(4, 1) = (2 * x_1 - x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(4, 2) = (-x_1 + 2 * x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(4, 3) =
      2 * (x_1 + x_2 - 2 * x_3) /
      (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 + x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  ...
  Gp(5, 0) = (-y_1 - y_2 + 2 * y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(5, 1) = (2 * y_1 - y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(5, 2) = (-y_1 + 2 * y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(5, 3) =
      2 * (y_1 + y_2 - 2 * y_3) /
      (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 + x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  ...
  Gp(7, 0) = (-x_1 - x_2 + 2 * x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(7, 1) = (2 * x_1 - x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(7, 2) = (-x_1 + 2 * x_2 - x_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(7, 3) = 0;
  Gp(7, 4) =
      2 * (-2 * x_1 + x_2 + x_3) /
  ...
  Gp(8, 0) = (-y_1 - y_2 + 2 * y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(8, 1) = (2 * y_1 - y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                      x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(8, 2) = (-y_1 + 2 * y_2 - y_3) / (x_1 * y_2 - x_1 * y_3 - x_2 * y_1 +
                                       x_2 * y_3 + x_3 * y_1 - x_3 * y_2);
  Gp(8, 3) = 0;
  Gp(8, 4) =
      2 * (-2 * y_1 + y_2 + y_3) /
...
Matrix ShellANDeS::getBendingBasicStiffness()
{
  ...
  L(1, 5) = (c12 * c12 * y12 + c23 * c23 * y23) /
            2.0; //- Izmena 11-05-2002 [[ Google translate: changed 11-05-2002]]
  L(2, 5) = s12 * s12 * x21 + s23 * s23 * x32;
  //--------------------------------------------------
  L(0, 6) = -c31 * s31 + c23 * s23;
  L(1, 6) = -c23 * s23 + c31 * s31;
  ...
  L(2, 7) = c23 * c23 * y32 +
            c31 * c31 * y13; //# odnosno permutovan je index y31 u y13
  ...
  Kb.addMatrixTripleProduct(
      1.0, L, E_planestress * thickness * thickness * thickness / 12 / Area,
      1.0);
...
Matrix ShellANDeS::getBendingHighOrderStiffness()
{
  ...
  D.addMatrixTripleProduct(
      1.0, T, E_planestress * thickness * thickness * thickness / 12, 1.0);
  ...
  Kh(1, 4) =
      (r12 * y21 + r23 * y13) * y32 + r11 * pow(y21, 2) + r13 * y13 * y21;
  ...
  Kh(1, 8) =
      (r12 * x32 + r13 * x13) * y12 +
      (r23 * x32 + r33 * x13) *
          y31; // Ovde je bila greska !!! 6 - 04 - 2002 [[ Google translate: There was a mistake! 6 - 04-2002 ]]
  ...
  Kh(2, 5) =
      (r12 * x21 + r23 * x13) * x32 + r11 * pow(x21, 2) + r13 * x13 * x21;
  ...
  Kh(4, 7) =
      r22 * pow(y32, 2) + (r12 * y21 + r23 * y13) * y32 + r13 * y13 * y21;
  ...
  Kh(5, 8) =
      r22 * pow(x32, 2) + (r12 * x21 + r23 * x13) * x32 + r13 * x13 * x21;
...
Matrix ShellANDeS::getMembraneHighOrderStiffness()
{
  ...
  Enat.Zero();
  Enat.addMatrixTripleProduct(1.0, T_e, E_planestress,
                              1.0); // (T_e)^T*E_planestress*T_e
  ...
  double a4 =
      Enat(0, 0) * Q4(0, 0) + Enat(0, 1) * Q4(1, 0) + Enat(0, 2) * Q4(2, 0);
  double b4 =
      Enat(1, 0) * Q4(0, 0) + Enat(1, 1) * Q4(1, 0) + Enat(1, 2) * Q4(2, 0);
  double c4 =
      Enat(2, 0) * Q4(0, 0) + Enat(2, 1) * Q4(1, 0) + Enat(2, 2) * Q4(2, 0);
  double a5 =
      Enat(0, 0) * Q5(0, 0) + Enat(0, 1) * Q5(1, 0) + Enat(0, 2) * Q5(2, 0);
  double b5 =
      Enat(1, 0) * Q5(0, 0) + Enat(1, 1) * Q5(1, 0) + Enat(1, 2) * Q5(2, 0);
  double c5 =
      Enat(2, 0) * Q5(0, 0) + Enat(2, 1) * Q5(1, 0) + Enat(2, 2) * Q5(2, 0);
  double a6 =
      Enat(0, 0) * Q6(0, 0) + Enat(0, 1) * Q6(1, 0) + Enat(0, 2) * Q6(2, 0);
  double b6 =
      Enat(1, 0) * Q6(0, 0) + Enat(1, 1) * Q6(1, 0) + Enat(1, 2) * Q6(2, 0);
  double c6 =
      Enat(2, 0) * Q6(0, 0) + Enat(2, 1) * Q6(1, 0) + Enat(2, 2) * Q6(2, 0);

  double S11 = (Q4(0, 0) * a4 + Q4(1, 0) * b4 + Q4(2, 0) * c4 + Q5(0, 0) * a5 +
                Q5(1, 0) * b5 + Q5(2, 0) * c5 + Q6(0, 0) * a6 + Q6(1, 0) * b6 +
                Q6(2, 0) * c6) *
               sfac;
  double S12 = (Q4(0, 1) * a4 + Q4(1, 1) * b4 + Q4(2, 1) * c4 + Q5(0, 1) * a5 +
                Q5(1, 1) * b5 + Q5(2, 1) * c5 + Q6(0, 1) * a6 + Q6(1, 1) * b6 +
                Q6(2, 1) * c6) *
               sfac;
  double S13 = (Q4(0, 2) * a4 + Q4(1, 2) * b4 + Q4(2, 2) * c4 + Q5(0, 2) * a5 +
                Q5(1, 2) * b5 + Q5(2, 2) * c5 + Q6(0, 2) * a6 + Q6(1, 2) * b6 +
                Q6(2, 2) * c6) *
               sfac;
  ...
  double S22 = (Q4(0, 1) * a4 + Q4(1, 1) * b4 + Q4(2, 1) * c4 + Q5(0, 1) * a5 +
                Q5(1, 1) * b5 + Q5(2, 1) * c5 + Q6(0, 1) * a6 + Q6(1, 1) * b6 +
                Q6(2, 1) * c6) *
               sfac;
  double S23 = (Q4(0, 2) * a4 + Q4(1, 2) * b4 + Q4(2, 2) * c4 + Q5(0, 2) * a5 +
                Q5(1, 2) * b5 + Q5(2, 2) * c5 + Q6(0, 2) * a6 + Q6(1, 2) * b6 +
                Q6(2, 2) * c6) *
               sfac;
  ...
  double S33 = (Q4(0, 2) * a4 + Q4(1, 2) * b4 + Q4(2, 2) * c4 + Q5(0, 2) * a5 +
                Q5(1, 2) * b5 + Q5(2, 2) * c5 + Q6(0, 2) * a6 + Q6(1, 2) * b6 +
                Q6(2, 2) * c6) *
               sfac;
...
void ShellANDeS::calculate_E_planestress_and_beta0()
{
  ...
  double Edet = E11 * E22 * E33 + 2 * E12 * E13 * E23 - E11 * pow(E23, 2) -
                E22 * pow(E13, 2) - E33 * pow(E12, 2);

  double E11C11 =
      (-5 * E11 * pow(E12, 2) - 6 * pow(E12, 3) - 3 * E11 * pow(E13, 2) +
       14 * E12 * pow(E13, 2) + 5 * pow(E11, 2) * E22 + 6 * E11 * E12 * E22 -
       5 * pow(E12, 2) * E22 - 75 * pow(E13, 2) * E22 + 5 * E11 * pow(E22, 2) -
       14 * E11 * E13 * E23 + 92 * E12 * E13 * E23 - 14 * E13 * E22 * E23 -
       75 * E11 * pow(E23, 2) + 14 * E12 * pow(E23, 2) - 3 * E22 * pow(E23, 2) +
       (3 * pow(E11, 2) + 82 * E11 * E22 + 3 * pow(E22, 2) -
        4 * (6 * pow(E12, 2) + 5 * pow(E13, 2) - 6 * E13 * E23 +
             5 * pow(E23, 2))) *
           E33 +
       4 * (5 * E11 - 6 * E12 + 5 * E22) * pow(E33, 2)) /
      (128 * Edet);

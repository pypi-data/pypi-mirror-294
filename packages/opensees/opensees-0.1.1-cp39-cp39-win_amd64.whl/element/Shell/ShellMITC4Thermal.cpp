...
ShellMITC4Thermal::ShellMITC4Thermal()
    : Element(0, ELE_TAG_ShellMITC4Thermal), connectedExternalNodes(4), load(0),
      Ki(0)
{
  for (int i = 0; i < 4; i++)
...
//*********************************************************************
//full constructor
ShellMITC4Thermal::ShellMITC4Thermal(int tag, int node1, int node2, int node3,
                                     int node4,
                                     SectionForceDeformation &theMaterial)
    : Element(tag, ELE_TAG_ShellMITC4Thermal), connectedExternalNodes(4),
      load(0), Ki(0)
{
  int i;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (materialPointers[i] == 0) {
      opserr << "ShellMITC4Thermal::constructor - failed to get a material of "
                "type: ShellSection\n";
    } //end if
...
void ShellMITC4Thermal::setDomain(Domain *theDomain)
{
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (nodeDisp.Size() != 6) {
      opserr << "ShellMITC4Thermal::setDomain - node "
             << connectedExternalNodes(i);
...
int ShellMITC4Thermal::getNumExternalNodes() const { return 4; }
...
Node **ShellMITC4Thermal::getNodePtrs(void) { return nodePointers; }

//return number of dofs
int ShellMITC4Thermal::getNumDOF() { return 24; }
...
Response *ShellMITC4Thermal::setResponse(const char **argv, int argc,
                                         OPS_Stream &output)
{
  ...
  if (strcmp(argv[0], "force") == 0 || strcmp(argv[0], "forces") == 0 ||
    ...
    if (argc < 2) {
      opserr
          << "ShellMITC4Thermal::setResponse() - need to specify more data\n";
      return 0;
    }
...
int ShellMITC4Thermal::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
#ifdef _SDEBUG
  if (this->getTag() == 1)
    opserr << "ShellMITC4Thermal: " << this->getTag()
           << "Tangent stiffness: " << endln << stiff << endln;
...
#ifdef _SDEBUG
  opserr << "ShellMITC4Thermal: " << this->getTag()
         << "InitialTangent stiffness: " << endln << stiff << endln;
#endif
...
int ShellMITC4Thermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
 ...
 */
    ...
    for (int i = 0; i < 9; i++) {
      if (data0(2 * i + 1) - data1(2 * i + 1) > 1e-8 ||
        ...
        opserr << "Warning:The NodalThermalAction in ShellMITC4Thermal "
  ...
  //end of thermalActionWrapper
    ...
    opserr << "ShellMITC4Thermal::ShellMITC4Thermal -- load type unknown for "
              "element with tag: "
...
int ShellMITC4Thermal::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
  static Vector r(24);
...
void ShellMITC4Thermal::formInertiaTerms(int tangFlag)
{
...
int ShellMITC4Thermal::formResidAndTangent(int tang_flag)
{
  //
  ...
  for (i = 0; i < ngauss; i++) {
    ...
    if (this->getTag() == 1 && i == 2)
      opserr << "Shellthermal " << this->getTag() << " stress " << stress
             << endln;
#endif
    //drilling "stress"
...
void ShellMITC4Thermal::computeBasis()
{
...
void ShellMITC4Thermal::updateBasis()
{
  //could compute derivatives \frac{ \partial {\bf x} }{ \partial L_1 }
  //                     and  \frac{ \partial {\bf x} }{ \partial L_2 }
  //and use those as basis vectors but this is easier
...
double *ShellMITC4Thermal::computeBdrill(int node, const double shp[3][4])
{
...
const Matrix &ShellMITC4Thermal::assembleB(const Matrix &Bmembrane,
                                           const Matrix &Bbend,
                                           const Matrix &Bshear)
{
...
const Matrix &ShellMITC4Thermal::computeBmembrane(int node,
...
const Matrix &ShellMITC4Thermal::computeBbend(int node, const double shp[3][4])
{
...
void ShellMITC4Thermal::shape2d(double ss, double tt, const double x[2][4],
...
double ShellMITC4Thermal::shapefn2d(double ss, double tt, int i)

{
  ...
  case 4:
    ...
    break;
  default:
    opserr << "ShellMITC4Thermal::shapefn2d received an invalid i: " << i
           << endln;
  }
  return shpVal;
}
...
int ShellMITC4Thermal::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellMITC4Thermal::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
...
int ShellMITC4Thermal::recvSelf(int commitTag, Channel &theChannel,
                                FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellMITC4Thermal::recvSelf() - " << this->getTag()
  ...
  if (materialPointers[0] == 0) {
    for (int i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
        opserr << "ShellMITC4Thermal::recvSelf() - Broker could not create "
      ...
      if (res < 0) {
        opserr << "ShellMITC4Thermal::recvSelf() - material " << i
...
int ShellMITC4Thermal::displaySelf(Renderer &theViewer, int displayMode,

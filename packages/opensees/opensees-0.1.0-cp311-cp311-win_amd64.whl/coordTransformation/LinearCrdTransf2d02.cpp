...
// LinearCrdTransf2d02 class. LinearCrdTransf2d02 is a linear
      ...
      nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
  ...
  if (rigJntOffset1.Size() != 2) {
    ...
    opserr << "LinearCrdTransf2d02::LinearCrdTransf2d02:  Invalid rigid joint "
...
int
LinearCrdTransf2d02::initialize(Node *nodeIPointer, Node *nodeJPointer)
{
  int error;
  ...
  if ((!nodeIPtr) || (!nodeJPtr)) {
    opserr << "\nLinearCrdTransf2d02::initialize";
...
int
LinearCrdTransf2d02::computeElemtLengthAndOrient()
{
  // element projection
  static Vector dx(2);
...
//}
  ...
  if (L == 0.0) {
    opserr << "\nLinearCrdTransf2d02::computeElemtLengthAndOrien: 0 length\n";
    return -2;
  }
  ...
  oneOverL = 1.0 / L;
  cosTheta = oneOverL*dx(0);
  sinTheta = oneOverL*dx(1);
  sl       = sinTheta * oneOverL;
  cl       = cosTheta * oneOverL;
...
void
LinearCrdTransf2d02::compTransfMatrixLocalGlobal(Matrix &Tlg)
{
...
inline const Vector &
LinearCrdTransf2d02::makeBasic(const double ug[6], Vector& ub)
{
//static Vector ub(3);
...
LinearCrdTransf2d02::getBasicTrialAccel(void)
{
  ...
  static Vector ab(3);
  return makeBasic(ag, ab);
}

const Vector &
LinearCrdTransf2d02::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
...
LinearCrdTransf2d02::getGlobalStiffMatrix(const Matrix &kb, const Vector &pb)
{
  ...
  // offset at I
  double t02 = -this->t02;
  double t22 = oneOverL * this->t12;
  double t12 = oneOverL * this->t12 + 1.0;
  // offset at J
  double t05 =  this->t35;
  double t15 = -oneOverL * this->t45;
  double t25 = -oneOverL * this->t45 + 1.0;

  tmp[0][0] = -cosTheta * kb00 - sl * (kb01 + kb02);
  tmp[0][1] = -sinTheta * kb00 + cl * (kb01 + kb02);
  tmp[0][2] =  t02 * kb00 + t12 * kb01 + t22 * kb02;
  tmp[0][3] = -tmp[0][0];
  tmp[0][4] = -tmp[0][1];
  tmp[0][5] =  t05 * kb00 + t15 * kb01 + t25 * kb02;

  tmp[1][0] = -cosTheta * kb10 - sl * (kb11 + kb12);
  tmp[1][1] = -sinTheta * kb10 + cl * (kb11 + kb12);
  tmp[1][2] =  t02 * kb10 + t12 * kb11 + t22 * kb12;
  tmp[1][3] = -tmp[1][0];
  tmp[1][4] = -tmp[1][1];
  tmp[1][5] =  t05 * kb10 + t15 * kb11 + t25 * kb12;

  tmp[2][0] = -cosTheta * kb20 - sl * (kb21 + kb22);
  tmp[2][1] = -sinTheta * kb20 + cl * (kb21 + kb22);
  tmp[2][2] =  t02 * kb20 + t12 * kb21 + t22 * kb22;
  tmp[2][3] = -tmp[2][0];
  tmp[2][4] = -tmp[2][1];
  tmp[2][5] =  t05 * kb20 + t15 * kb21 + t25 * kb22;
...
const Matrix &
LinearCrdTransf2d02::getInitialGlobalStiffMatrix(const Matrix &kb)
{
...
int
LinearCrdTransf2d02::sendSelf(int cTag, Channel &theChannel)
{
  int res = 0;
...
//}
  ...
  if (res < 0) {
    opserr << "LinearCrdTransf2d02::sendSelf - failed to send Vector\n";
    return res;
  }
...
int
LinearCrdTransf2d02::recvSelf(int cTag, Channel &theChannel,
                            FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "LinearCrdTransf2d02::recvSelf - failed to receive Vector\n";
    return res;
  }
...
const Vector &
LinearCrdTransf2d02::getPointGlobalCoordFromLocal(const Vector &xl)
{
  static Vector xg(2);
...
const Vector &
LinearCrdTransf2d02::getPointGlobalDisplFromBasic(double xi, const Vector &uxb)
{
  // determine global displacements
  const Vector &disp1 = nodeIPtr->getTrialDisp();
  const Vector &disp2 = nodeJPtr->getTrialDisp();
...
const Vector &
LinearCrdTransf2d02::getPointLocalDisplFromBasic(double xi, const Vector &uxb)
{
  // determine global displacements
  const Vector &disp1 = nodeIPtr->getTrialDisp();
  const Vector &disp2 = nodeJPtr->getTrialDisp();
...
void
LinearCrdTransf2d02::Print(OPS_Stream &s, int flag)
{
...
LinearCrdTransf2d02::getBasicDisplSensitivity(int gradNumber)
{
  ...
  dudh(1) = -sinTheta * dUdh(0) + cosTheta * dUdh(1) - dsinThetadh * U(0) +
            dcosThetadh * U(1);
  dudh(2) = dUdh(2);
  dudh(3) = cosTheta * dUdh(3) + sinTheta * dUdh(4) + dcosThetadh * U(3) +
            dsinThetadh * U(4);
  dudh(4) = -sinTheta * dUdh(3) + cosTheta * dUdh(4) - dsinThetadh * U(3) +
            dcosThetadh * U(4);
  dudh(5) = dUdh(5);
...
int
LinearCrdTransf2d02::getLocalAxes(Vector &xAxis, Vector &yAxis, Vector &zAxis)
{
  xAxis(0) = cosTheta;
  xAxis(1) = sinTheta;
  xAxis(2) = 0;

...
// $Source:
// /usr/local/cvs/OpenSees/SRC/element/PFEMElement/TclPFEMCommands.cpp,v $
                             ...
                             TCL_Char ** const argv, Domain *theDomain)
{
  ...
  if (strcmp(argv[1], "discretize") == 0) {
    ...
    if (strcmp(argv[2], "PSLG") == 0) {
      if (argc < 6) {
        ...
        opserr
            << "-fix {constrValues} -mass {massValues} -holes {x1 y1 ...} >\n";
        return TCL_ERROR;
      }
      int startnodetag;
      ...
      int res =
          theMesher2D.discretize(startnodetag, points, segments, holes, maxarea,
                                 ndf, fix, vel, mass, theDomain, endnodetag);
      ...
      int endnodetag = startnodetag;
      int res = theMesher2D.discretize(startnodetag, points, ndf, fix, vel,
                                       mass, theDomain, endnodetag);
      ...
      if (argc < 12) {
        opserr << "WARNING: wrong num of args -- ";
        opserr << "PFEM2D discretize Rectangle startnodetag x1 y1 hx hy angle "
                  "nx ny ndf ";
      ...
      int endnodetag = startnodetag;
      int res = theMesher2D.discretize(startnodetag, x1, y1, hx, hy, angle, nx,
                                       ny, ndf, fix, vel, mass, boundary,
      ...
      int endnodetag = startnodetag;
      int res =
          theMesher2D.discretize(startnodetag, x1, y1, h, angle, num, ndf, fix,
                                 vel, mass, boundary, theDomain, endnodetag);
      ...
      if (argc < 13) {
        opserr << "WARNING: wrong num of args -- ";
        opserr << "PFEM2D discretize triangle startnodetag x1 y1 x2 y2 x3 y3 "
                  "n1 n2 ndf ";
        opserr << "<-fix {constrValues} -mass {massValues} -vel {vx vy} ";
        opserr << "-boundary {boundaryValues}>\n";
        return TCL_ERROR;
      ...
      int res =
          theMesher2D.discretize(startnode, x1, y1, x2, y2, x3, y3, n1, n2, ndf,
                                 fix, vel, mass, boundary, theDomain, endnode);
      if (res < 0) {
        opserr << "WARNING: failed to discretize\n";
        return TCL_ERROR;
      }
      ...
      int res = theMesher2D.discretize(startnode, xc, yc, r1, r2, nc, nr, ndf,
                                       fix, vel, mass, boundary, angle,
                                       theDomain, endnode);
      if (res < 0) {
        opserr << "WARNING: failed to discretize\n";
        return TCL_ERROR;
      ...
      ID nodelist;
      int res = theMesher2D.discretize(startnode, x, y, hcol, hbeam, ndf, spans,
                                       heights, fix, colvel, beamvel, colmass,
                                       beammass, roofmass, theDomain, nodelist);
      if (res < 0) {
        opserr << "WARNING: failed to discretize\n";
        return TCL_ERROR;
      }
    ...
    if (argc < 4) {
      ...
      opserr << "-PFEMElement2DCompressible {starteletag rho mu b1 b2 <thk "
                "kappa>} ";
    ...
    if (eletype >= 1 && eletype <= 7 && eletype != 2) {
      ...
      res = theMesher2D.doTriangulation(startele, alpha, nodes, addnodes,
                                        theDomain, rho, mu, b1, b2, thk, kappa,
                                        eletype, endele);
      ...
      if (params.Size() >= 4) {
        res = theMesher2D.doTriangulation(startele, alpha, nodes, addnodes,
                                          theDomain, thk, type.c_str(), matTag,
                                          p, rho, b1, b2, endele);
      }
      ...
      res =
          theMesher2D.doTriangulation(alpha, nodes, addnodes, theDomain, eles);
      if (res < 0) {
        opserr << "WARNING: failed to do triangulation -- ";
        opserr << " -- PFEM2D doTriangulation\n";
        return TCL_ERROR;
  ...
  } else if (strcmp(argv[1], "updateNode") == 0 ||
             strcmp(argv[1], "updatenode") == 0) {
    if (argc < 6) {
      opserr << "Warning: wrong num of args -- ";
  ...
  } else if (strcmp(argv[1], "region") == 0 || strcmp(argv[1], "Region") == 0) {
    ...
    if (strcmp(argv[2], "PSLG") == 0) {
      ...
      bool series = false;
      int action = 1; // append
      theMesher2D.setNodes(regionnodes, regTag, series, action, theDomain);
    ...
    } else if (strcmp(argv[2], "mesh") == 0 || strcmp(argv[2], "Mesh") == 0) {
      if (argc < 5) {
        opserr << "WARNING: wrong num of args -- ";
      ...
      if (eletype >= 1 && eletype != 2 && eletype <= 7) {
        ...
        res = theMesher2D.doTriangulation(startele, alpha, nodes, addnodes,
                                          theDomain, rho, mu, b1, b2, thk,
                                          kappa, eletype, endele);
      ...
      if (strcmp(argv[3], "add") == 0 || strcmp(argv[3], "Add") == 0) {
        ...
        action = 2;
        series = true;
      } else if (strcmp(argv[3], "set") == 0 || strcmp(argv[3], "Set") == 0) {
        action = 0;
        series = true;
                             ...
                             TCL_Char ** const argv, Domain *theDomain)
{
  ...
  if (argc < 2) {
    opserr
        << "WARNING: at least 2 arguments -- PFEM3D subcommands: discretize, ";
  ...
  if (strcmp(argv[1], "discretize") == 0) {
    ...
    if (strcmp(argv[2], "PLC") == 0) {
      ...
      for (int i = 0; i < (int)states.size(); ++i) {
        ...
        if (state == 1) { // points
          ...
          for (int loc = l0; loc < l1; loc++) {
            ...
            if (count == 3) {
              points.push_back(
                  PFEMMesher3D::Point(coord[0], coord[1], coord[2]));
              count = 0;
            }
          }
      ...
      int endnode = startnode;
      int res = theMesher3D.discretize(startnode, points, facets, holes, maxvol,
                                       ndf, fix, mass, theDomain, endnode);
    ...
    std::vector<int> eles;
    if (pfem) {
      res = theMesher3D.doTriangulation(starteletag, nodes, alpha, volthresh,
                                        addnodes, theDomain, pfemparams[0],
                                        pfemparams[1], pfemparams[2],
                                        pfemparams[3], pfemparams[4]);
    } else {
      res = theMesher3D.doTriangulation(nodes, alpha, volthresh, addnodes,
                                        theDomain, eles);
      if (res < 0) {
        opserr << "WARNING: failed to do triangulation -- ";
        opserr << " -- PFEM3D doTriangulation\n";
    ...
    theMesher3D.setBoundary(bound[0], bound[1], bound[2], bound[3], bound[4],
                            bound[5]);

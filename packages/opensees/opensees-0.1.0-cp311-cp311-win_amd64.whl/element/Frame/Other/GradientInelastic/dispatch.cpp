                                               ...
                                               TCL_Char ** const argv)
{
  ...
  if (ndm == 2) {
    ...
    if ((argc - eleArgStart) < 10) {
      ...
      opserr << "Want: gradientInelasticBeamColumn eleTag? iNode? jNode? "
                ...
                "<-iter maxIter? minTol? maxTol?> <-corControl auto/maxEpsInc? "
                "maxPhiInc?>\n";
      return TCL_ERROR;
    }

    // get the id and end nodes
    int iNode, jNode, endSecTag1, intSecTag, endSecTag2, numIntegrPts,
        transfTag;
    ...
    theElement = new GradientInelasticBeamColumn2d(
        tag, iNode, jNode, numIntegrPts, &endSection1, &intSection,
        &endSection2, secLR1, secLR2, *beamIntegr, *theTransf2d, lc, minTol,
    ...
    // now create the NonlocalBeamColumn
    theElement = new GradientInelasticBeamColumn3d(
        tag, iNode, jNode, numIntegrPts, &endSection1, &intSection,
  ...
  // if get here we have sucessfully created the NonlocalBeamColumn and added it
  // to the domain
  return TCL_OK;
}

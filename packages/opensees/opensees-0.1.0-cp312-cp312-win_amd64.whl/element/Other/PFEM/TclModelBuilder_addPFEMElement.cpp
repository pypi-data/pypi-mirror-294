#include "TclBasicBuilder_addPFEMElement.h"
                                 ...
                                 TclBasicBuilder *theBuilder)
{
  ...
  if (strcmp(argv[loc], "FIC") == 0 || strcmp(argv[loc], "fic") == 0 ||
    ...
    type = 2;
  } else if (strcmp(argv[loc], "mini") == 0 || strcmp(argv[loc], "Mini") == 0) {
    type = 3;
  } else {
  ...
  if (type == 0) {
    ...
    // ele = new
    // PFEMElement2DCompressible(tag,nd1,nd2,nd3,rho,mu,b1,b2,thickness,kappa);
  } else if (type == 2) {
    // ele = new
    // PFEMElement2DBubble(tag,nd1,nd2,nd3,rho,mu,b1,b2,thickness,kappa);
  } else if (type == 3) {
    // ele = new
    // PFEMElement2Dmini(tag,nd1,nd2,nd3,rho,mu,b1,b2,thickness,kappa,lumped,checkJ);
  }
  if (ele == 0) {
                                 ...
                                 TclBasicBuilder *theBuilder)
{
  ...
  if (argc < 13) {
    ...
    opserr
        << "tag nd1 nd2 nd3 nd4 type rho mu b1 b2 b3 <kappa lumped checkJ>\n";
    return TCL_ERROR;
  }
  ...
  if (type == "pressuregradient" || type == "PressureGradient") {
    ...
    opserr << "element PFEMElement3D type " << type.c_str()
           << " is not known\n";
    return TCL_ERROR;
  }

...
int ShellDKGQ::getNumExternalNodes() const { return 4; }

//return connected external nodes
const ID &ShellDKGQ::getExternalNodes() { return connectedExternalNodes; }

Node **ShellDKGQ::getNodePtrs(void) { return nodePointers; }

//return number of dofs
int ShellDKGQ::getNumDOF() { return 24; }
...
Response *ShellDKGQ::setResponse(const char **argv, int argc,
                                 OPS_Stream &output)
{
...
const Matrix &ShellDKGQ::getInitialStiff()
{
  ...
  // Tmat(6,6):  local-global coordinates matrix
  // Pmat(6,6):  from (u1 u2 theta3 w theta1 theta2) to (u1 u2 w theta1 theta2 theta3)
  // J0(2,2):    Jacobian at center
  // J0inv(2,2): inverse of Jacobian at center
  ...
  static Matrix BJtran(ndf, nstress);  // BJ Transposed
...
int ShellDKGQ::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "ShellDKGQ::addLoad - load type unknown for ele with tag: "
         << this->getTag() << endln;
  return -1;
}

int ShellDKGQ::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
...
void ShellDKGQ::formResidAndTangent(int tang_flag)
{
  //
  ...
  static Matrix BJPT(
      nstress, ndf); //BJP * Tmmat, from global coordinates to local coordinates
...
const Matrix &ShellDKGQ::assembleB(const Matrix &Bmembrane, const Matrix &Bbend,
                                   const Matrix &Bshear)
{
  static Matrix B(8, 6);
...
const Matrix &ShellDKGQ::computeBmembrane(int node, const double shp[3][4],
...
const Matrix &ShellDKGQ::computeBbend(int node, const double shpBend[6][12])
{
...
void ShellDKGQ::shape2d(double ss, double tt, const double x[2][4],
...

{
  ...
  for (i = 0; i < 4; i++) {
    ...
    shp[1][i] =
        t[i] * (0.5 + s[i] * ss); // derivative to isoparametric coordinates 1
  }                               // end for i
...
void ShellDKGQ::shapeBend(double ss, double tt, const double x[2][4],
...
int ShellDKGQ::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellDKGQ::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
...
int ShellDKGQ::recvSelf(int commitTag, Channel &theChannel,
                        FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellDKGQ::recvSelf() - " << this->getTag()
  ...
  if (materialPointers[0] == 0) {
    for (i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
        opserr << "ShellDKGQ::recvSelf() - Broker could not create NDMaterial "
                  "of class type"
               << matClassTag << endln;
        ;
        return -1;
      ...
      if (res < 0) {
        opserr << "ShellDKGQ::recvSelf() - material " << i
  ...
  // Number of materials is the same, receive materials into current space
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (materialPointers[i]->getClassTag() != matClassTag) {
        ...
        materialPointers[i] = theBroker.getNewSection(matClassTag);
        if (materialPointers[i] == 0) {
          opserr << "ShellDKGQ::recvSelf() - Broker could not create "

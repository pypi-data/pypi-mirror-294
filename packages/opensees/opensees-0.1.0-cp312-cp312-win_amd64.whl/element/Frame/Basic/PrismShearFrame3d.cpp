...
//         tag, iNode, jNode 
//         E, G, A, Jx, Iy, Iz, Avy, Avz
//
// Adapted from ElasticTimoshenkoBeam3d.cpp
//
// TODO:
//    - add P-delta
//    - add Warp DOF
//    - move P-Delta to coordinate transform
//
//
...
ElasticTimoshenkoBeam3d::ElasticTimoshenkoBeam3d(int tag, int Nd1, int Nd2, double e, double g,
                                                 double a, double jx, double iy, double iz,
                                                 double avy, double avz, CrdTransf& coordTransf,
                                                 double r, int cm)
...
int
ElasticTimoshenkoBeam3d::getNumExternalNodes() const
{
  return 2;
}
...
const ID&
ElasticTimoshenkoBeam3d::getExternalNodes()
{
...
Node**
ElasticTimoshenkoBeam3d::getNodePtrs()
{
  return theNodes;
}
...
int
ElasticTimoshenkoBeam3d::getNumDOF()
{
  return 12;
}
...
void
ElasticTimoshenkoBeam3d::setDomain(Domain* theDomain)
{
  ...
  if (!theNodes[0] || !theNodes[1]) {
    if (!theNodes[0]) {
      opserr << "WARNING ElasticTimoshenkoBeam3d::setDomain() - Nd1: " << connectedExternalNodes(0)
             << " does not exist in the model for";
    } else {
      opserr << "WARNING ElasticTimoshenkoBeam3d::setDomain() - Nd2: " << connectedExternalNodes(1)
             << " does not exist in the model for";
    }
  ...
  if (dofNd1 != 6) {
    opserr << "ElasticTimoshenkoBeam3d::setDomain() - node 1: " << connectedExternalNodes(0)
           << " has incorrect number of DOF (not 6).\n";
    return;
  }
  if (dofNd2 != 6) {
    opserr << "ElasticTimoshenkoBeam3d::setDomain() - node 2: " << connectedExternalNodes(1)
           << " has incorrect number of DOF (not 6).\n";
    return;
  }
...
int
ElasticTimoshenkoBeam3d::commitState()
{
  int errCode = 0;
...
int
ElasticTimoshenkoBeam3d::revertToLastCommit()
{
  return 0;
}
...
int
ElasticTimoshenkoBeam3d::revertToStart()
{
  return 0;
}
...
const Matrix&
ElasticTimoshenkoBeam3d::getInitialStiff()
{
  return Ki;
}
...
const Matrix&
ElasticTimoshenkoBeam3d::getMass()
{
  return M;
}
...
void
ElasticTimoshenkoBeam3d::zeroLoad()
{
  theLoad.Zero();
  ql0.Zero();
...
int
ElasticTimoshenkoBeam3d::addLoad(ElementalLoad* theLoad, double loadFactor)
{
  int type;
...
int
ElasticTimoshenkoBeam3d::addInertiaLoadToUnbalance(const Vector& accel)
{
...
const Vector&
ElasticTimoshenkoBeam3d::getResistingForce()
{
  // zero the residual
  theVector.Zero();
...
const Vector&
ElasticTimoshenkoBeam3d::getResistingForceIncInertia()
{
...
void
ElasticTimoshenkoBeam3d::Print(OPS_Stream& s, int flag)
{
  ...
  if (flag == OPS_PRINT_CURRENTSTATE) {
    ...
    s << "  E: " << E << "  G: " << G << "\n";
    s << "  A: " << A << "  Jx: " << Jx << "  Iy: " << Iy;
    s << "  Iz: " << Iz << "  Avy: " << Avy << "  Avz: " << Avz << "\n";
    s << "  coordTransf: " << theCoordTransf->getClassType() << "\n";
    s << "  rho: " << density << "  cMass: " << cMass << "\n";
    // determine resisting forces in global system
...
Response*
ElasticTimoshenkoBeam3d::setResponse(const char** argv, int argc, OPS_Stream& output)
{
...
int
ElasticTimoshenkoBeam3d::getResponse(int responseID, Information& info)
{
  switch (responseID) {
...
int
ElasticTimoshenkoBeam3d::setParameter(const char** argv, int argc, Parameter& param)
{
  if (argc < 1)
    return -1;
...
int
ElasticTimoshenkoBeam3d::updateParameter(int parameterID, Information& info)
{
  switch (parameterID) {
  case -1: return -1;
  ...
  case  4: Jx  = info.theDouble; return 0;
  case  5: Iy  = info.theDouble; return 0;
  case  6: Iz  = info.theDouble; return 0;
  case  7: Avy = info.theDouble; return 0;
  case  8: Avz = info.theDouble; return 0;
  default: return -1;
  }
}
...
int
ElasticTimoshenkoBeam3d::sendSelf(int commitTag, Channel& sChannel)
{
  int res = 0;
...
int
ElasticTimoshenkoBeam3d::recvSelf(int commitTag, Channel& rChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;

...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumnNL3d)
{
  if (OPS_GetNumRemainingInputArgs() < 5) {
    opserr << "insufficient arguments:eleTag,iNode,jNode,transfTag,integrationTag <-mass "
              "mass> <-cmass>\n";
    return 0;
  }
  ...
  BeamIntegrationRule *theRule =
      G3_getSafeBuilder(rt)->getTypedObject<BeamIntegrationRule>(iData[4]);
  if (theRule == 0) {
  ...
  Element *theEle = new DispBeamColumnNL3d(iData[0], iData[1], iData[2], secTags.Size(),
...
DispBeamColumnNL3d::DispBeamColumnNL3d(int tag, int nd1, int nd2, int numSec,
                                       SectionForceDeformation **s, BeamIntegration &bi,
                                       CrdTransf &coordTransf, double r)
    : Element(tag, ELE_TAG_DispBeamColumnNL3d), numSections(numSec), theSections(0),
      crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(12), q(6), rho(r),
      parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumnNL3d::DispBeamColumnNL3d - failed to allocate section model "
              "pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumnNL3d::DispBeamColumnNL3d -- failed to get a copy of "
                "section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr
        << "DispBeamColumnNL3d::DispBeamColumnNL3d - failed to copy beam integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumnNL3d::DispBeamColumnNL3d - failed to copy coordinate "
              "transformation\n";
    exit(-1);
  }
...
DispBeamColumnNL3d::DispBeamColumnNL3d()
    : Element(0, ELE_TAG_DispBeamColumnNL3d), numSections(0), theSections(0),
      crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(12), q(6), rho(0.0),
      parameterID(0)
{
  q0[0] = 0.0;
  q0[1] = 0.0;
...
DispBeamColumnNL3d::setDomain(Domain *theDomain)
{
  ...
  if (theNodes[0] == 0 || theNodes[1] == 0) {
    opserr << "WARNING DispBeamColumnNL3d (tag: %d), node not found in domain"
           << this->getTag() << endln;
    ;
    return;
  }
...
DispBeamColumnNL3d::update()
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double dz = dNv1*v(1) + dNv2*v(2);
    double dy = dNv1*v(3) + dNv2*v(4);

    for (int j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL*v(0) + 0.5*dz*dz + 0.5*dy*dy;
        break;
      case SECTION_RESPONSE_MZ:
        e(j) = ddNv1*v(1) + ddNv2*v(2);
        break;
      case SECTION_RESPONSE_MY:
        e(j) = ddNv1*v(3) + ddNv2*v(4);
        break;
...
DispBeamColumnNL3d::getBasicStiff(Matrix &kb, int initial)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double dNv1     = 3 * xi1 * xi1 - 4 * xi1 + 1;
    double dNv2     = 3 * xi1 * xi1 - 2 * xi1;
    double dz = dNv1 * v(1) + dNv2 * v(2);
    double dy = dNv1 * v(3) + dNv2 * v(4);
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_P:
        tmp = s(j) * wt[i] * L;
        kb(1, 1) += tmp * dNv1 * dNv1;
        kb(2, 1) += tmp * dNv2 * dNv1;
        kb(1, 2) += tmp * dNv1 * dNv2;
        kb(2, 2) += tmp * dNv2 * dNv2;

        kb(3, 3) += tmp * dNv1 * dNv1;
        kb(4, 3) += tmp * dNv2 * dNv1;
        kb(3, 4) += tmp * dNv1 * dNv2;
        kb(4, 4) += tmp * dNv2 * dNv2;
        break;
      }
    }
    ...
    kb.addMatrixTransposeProduct(1.0, B, kC, dz * wt[i]);
    kC.addMatrixProduct(0.0, ks, Cy, 1.0);
    kb.addMatrixTransposeProduct(1.0, B, kC, dy * wt[i]);
    ...
    for (int j = 0; j < order; j++) {
      if (code(j) == SECTION_RESPONSE_P) {
        ...
        kb.addMatrixTransposeProduct(1.0, Cz1, ksB, dz * wt[i]);
        ksB.addMatrixProduct(0.0, ks1, Cz, 1.0);
        kb.addMatrixTransposeProduct(1.0, Cz1, ksB, dz * dz * wt[i] * L);
        ksB.addMatrixProduct(0.0, ks1, Cz, 1.0);
        kb.addMatrixTransposeProduct(1.0, Cy1, ksB, dz * dy * wt[i] * L);

        ksB.addMatrixProduct(0.0, ks1, B, 1.0);
        kb.addMatrixTransposeProduct(1.0, Cy1, ksB, dy * wt[i]);
        ksB.addMatrixProduct(0.0, ks1, Cy, 1.0);
        kb.addMatrixTransposeProduct(1.0, Cy1, ksB, dy * dy * wt[i] * L);
        ksB.addMatrixProduct(0.0, ks1, Cy, 1.0);
        kb.addMatrixTransposeProduct(1.0, Cz1, ksB, dy * dz * wt[i] * L);
      }
    }
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_MZ:
        ...
        for (int jj = 0; jj < order; jj++) {
          switch (code(jj)) {
          case SECTION_RESPONSE_P:
            q(1) += dNv1*dz * s(jj) * wt[i] * L;
            q(2) += dNv2*dz * s(jj) * wt[i] * L;
            break;
          }
        }
        break;
      case SECTION_RESPONSE_MY:
        ...
        for (int jj = 0; jj < order; jj++) {
          switch (code(jj)) {
          case SECTION_RESPONSE_P:
            q(3) += dNv1*dy * s(jj) * wt[i] * L;
            q(4) += dNv2*dy * s(jj) * wt[i] * L;
            break;
          }
        }
        break;
...
DispBeamColumnNL3d::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
    opserr << "DispBeamColumnNL3d::addInertiaLoadToUnbalance matrix and vector sizes are "
              "incompatible\n";
    return -1;
  }
...
DispBeamColumnNL3d::getResistingForce()
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double dNv1     = 3 * zeta * zeta - 4 * zeta + 1;
    double dNv2     = 3 * zeta * zeta - 2 * zeta;
    double dz = dNv1 * v(1) + dNv2 * v(2);
    double dy = dNv1 * v(3) + dNv2 * v(4);
...
DispBeamColumnNL3d::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumnNL3d::sendSelf() - section " << j
                             ...
                             FEM_ObjectBroker &theBroker)
{
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
    ...
    if (crdTransf == 0) {
      opserr << "DispBeamColumnNL3d::recvSelf() - failed to obtain a CrdTrans object "
                "with classTag "
  ...
  if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
    ...
    if (beamInt == 0) {
      opserr << "DispBeamColumnNL3d::recvSelf() - failed to obtain the beam integration "
                "object with classTag"
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumnNL3d::recvSelf() - out of memory creating sections array "
                "of size "
             << idData(3) << endln;
      return -1;
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumnNL3d::recvSelf() - Broker could not create Section of "
                  "class type "
               << sectClassTag << endln;
        exit(-1);
      }
      ...
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumnNL3d::recvSelf() - section " << i
...
Response *
DispBeamColumnNL3d::setResponse(const char **argv, int argc, OPS_Stream &output)
{
                                           ...
                                           Information &eleInfo)
{
  ...
  // dsdh
    ...
    double theta =
        (3 * zeta * zeta - 4 * zeta + 1) * v(1) + (3 * zeta * zeta - 2 * zeta) * v(2);
    double dthetadh = (3 * zeta * zeta - 4 * zeta + 1) * dvdh(1) +
                      (3 * zeta * zeta - 2 * zeta) * dvdh(2);

    int j;
    for (j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        dedh(j) = oneOverL * dvdh(0) + theta * dthetadh;
        break;
      case SECTION_RESPONSE_MZ:
...
DispBeamColumnNL3d::getResistingForceSensitivity(int gradNumber)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double dNv1  = 3 * xi1 * xi1 - 4 * xi1 + 1;
    double dNv2  = 3 * xi1 * xi1 - 2 * xi1;
    double theta = dNv1 * v(1) + dNv2 * v(2);
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_P:
        dqdh(0) += sensi;                    // 1
        dqdh(1) += dNv1 * theta * sensi * L; // 2
        dqdh(2) += dNv2 * theta * sensi * L; // 2
        break;
      case SECTION_RESPONSE_MZ:
...
DispBeamColumnNL3d::commitSensitivity(int gradNumber, int numGrads)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    for (int j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL * dvdh(0) + d1oLdh * v(0) + theta * (dNv1 * dvdh(1) + dNv2 * dvdh(2));
        break;
      case SECTION_RESPONSE_MZ:

...
DispBeamColumnNL2d::setDomain(Domain *theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
      opserr << "WARNING Element (tag: %d), node not found in domain" << this->getTag() << endln;;
      return;
    }

    assert(theNodes[0]->getNumberDOF() == 3);
    assert(theNodes[1]->getNumberDOF() == 3);
...
DispBeamColumnNL2d::update(void)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double theta = (3*xi1*xi1-4*xi1+1)*v(1) + (3*xi1*xi1-2*xi1)*v(2);

    for (int j = 0; j < order; j++) {
      switch(code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL*v(0) + 0.5*theta*theta;
        break;
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_MZ:
        ...
        for (int jj = 0; jj < order; jj++) {
          switch(code(jj)) {
          case SECTION_RESPONSE_P:
            q(1) += (3*xi1*xi1-4*xi1+1)*theta*s(jj)*wt[i]*L;
            q(2) += (3*xi1*xi1-2*xi1)*theta*s(jj)*wt[i]*L;
            break;
          }
        }
        break;
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_MZ:
        ...
        for (int jj = 0; jj < order; jj++) {
          switch(code(jj)) {
          case SECTION_RESPONSE_P:
            q(1) += dNv1*theta*s(jj)*wt[i]*L;
            q(2) += dNv2*theta*s(jj)*wt[i]*L;
            break;
          }
        }
        break;
                                           ...
                                           Information &eleInfo)
{
  ...
  // dsdh
    ...
    double theta = (3*xi1*xi1-4*xi1+1)*v(1) + (3*xi1*xi1-2*xi1)*v(2);
    double dthetadh = (3*xi1*xi1-4*xi1+1)*dvdh(1) + (3*xi1*xi1-2*xi1)*dvdh(2);

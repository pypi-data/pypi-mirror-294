...
//*********************************************************************
//full constructor
ShellNLDKGT::ShellNLDKGT(int tag, int node1, int node2, int node3,
                         SectionForceDeformation &theMaterial)
    : Element(tag, ELE_TAG_ShellNLDKGT), connectedExternalNodes(3),
      CstrainGauss(32), TstrainGauss(32), load(0), Ki(0), nodePointers(), xl(),
      g1(), g2(), g3()
{
  int i;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (materialPointers[i] == 0) {
      opserr << "ShellNLDKGT::constructor - failed to get a material of type: "
                "ShellSection\n";
    }
...
void ShellNLDKGT::setDomain(Domain *theDomain)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (nodePointers[i] == 0) {
      opserr << "ShellNLDKGT::setDomain - no node "
...
Response *ShellNLDKGT::setResponse(const char **argv, int argc,
                                   OPS_Stream &output)
{
...
int ShellNLDKGT::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "ShellNLDKGT::addLoad - load type unknown for ele with tag: "
         << this->getTag() << endln;
  return -1;
}

int ShellNLDKGT::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
  static Vector r(18);
...
//************************************************
//compute BG matrix
const Matrix &ShellNLDKGT::computeBG(int node, const double shpBend[6][9])
{
...
void ShellNLDKGT::shapeDrill(double ss, double tt, double qq,
                            ...
                            double shpBend[6][9])
{
  ...
  for (int i = 0; i < 9; i++) {
    shpBend[2][i] =
        temp[0][i] * y31 / 2 / area + temp[1][i] * y12 / 2 / area; //Hx,x
    shpBend[3][i] =
        temp[0][i] * (-x31) / 2 / area + temp[1][i] * (-x12) / 2 / area; //Hx,y
    shpBend[4][i] =
        temp[2][i] * y31 / 2 / area + temp[3][i] * y12 / 2 / area; //Hy,x
    shpBend[5][i] =
        temp[2][i] * (-x31) / 2 / area + temp[3][i] * (-x12) / 2 / area; //Hy,y
  }
...
int ShellNLDKGT::recvSelf(int commitTag, Channel &theChannel,
                          FEM_ObjectBroker &theBroker)
{
  ...
  if (materialPointers[0] == nullptr) {
    for (i = 0; i < 4; i++) {
      ...
      if (res < 0) {
        opserr << "ShellNLDKGT::recvSelf() - material " << i

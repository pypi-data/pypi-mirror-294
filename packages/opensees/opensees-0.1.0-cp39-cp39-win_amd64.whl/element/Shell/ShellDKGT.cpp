...
Node **ShellDKGT::getNodePtrs(void) { return nodePointers; }

//return number of dofs
int ShellDKGT::getNumDOF() { return 18; }
...
int ShellDKGT::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
int ShellDKGT::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  opserr << "ShellDKGT::addLoad - load type unknown for ele with tag: "
         << this->getTag() << endln;
  return -1;
}

int ShellDKGT::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
...
void ShellDKGT::formInertiaTerms(int tangFlag)
...
void ShellDKGT::formResidAndTangent(int tang_flag)
{
  //
...
int ShellDKGT::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellDKGT::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
                        ...
                        FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellDKGT::recvSelf() - " << this->getTag()
  ...
  if (materialPointers[0] == 0) {
    for (int i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
        opserr << "ShellDKGT::recvSelf() - Broker could not create NDMaterial "
                  "of class type"
               << matClassTag << endln;
        ;
        return -1;
      ...
      if (res < 0) {
        opserr << "ShellDKGT::recvSelf() - material " << i
...

{
  ...
  area = 0.5 * (x[0][0] * x[1][1] + x[0][1] * x[1][2] + x[0][2] * x[1][0] -
                x[0][0] * x[1][2] - x[0][1] * x[1][0] - x[0][2] * x[1][1]);
...
void ShellDKGT::shapeDrill(double ss, double tt, double qq,
...
const Matrix &ShellDKGT::assembleB(const Matrix &Bmembrane, const Matrix &Bbend,
                                   const Matrix &Bshear)
{
  static Matrix B(8, 6);
...
const Matrix &ShellDKGT::computeBmembrane(int node, const double shp[3][3],
...
const Matrix &ShellDKGT::computeBbend(int node, const double shpBend[6][9])
{
...
void ShellDKGT::shapeBend(double ss, double tt, double qq, const double x[2][3],
                          double sx[2][2], double shpBend[6][9])
{
  ...
  a4 = -x23 / L23;
  b4 = 0.75 * x23 * y23 / L23;
  c4 = 0.25 * (x23 * x23 - 2.0 * y23 * y23) / L23;
  d4 = -y23 / L23;
  e4 = 0.25 * (y23 * y23 - 2.0 * x23 * x23) / L23;

  a5 = -x31 / L31;
  b5 = 0.75 * x31 * y31 / L31;
  c5 = 0.25 * (x31 * x31 - 2.0 * y31 * y31) / L31;
  d5 = -y31 / L31;
  e5 = 0.25 * (y31 * y31 - 2.0 * x31 * x31) / L31;

  a6 = -x12 / L12;
  b6 = 0.75 * x12 * y12 / L12;
  c6 = 0.25 * (x12 * x12 - 2.0 * y12 * y12) / L12;
  d6 = -y12 / L12;
  e6 = 0.25 * (y12 * y12 - 2.0 * x12 * x12) / L12;
  ...
  temp[0][2] =
      -4.0 + 6.0 * (l[1] + l[2]) + r6 * (1.0 - 2.0 * l[1]) - l[2] * (r5 + r6);
  ...
  temp[1][2] =
      -4.0 + 6.0*(l[1] + l[2]) + r5*(1.0 - 2.0 * l[2]) - l[1]*(r5 + r6);
  temp[1][3] =  l[1]*(p4 + p6);
  temp[1][4] =  l[1]*(q4 - q6);
  temp[1][5] = -l[1]*(r6 - r4);
  ...
  for (int i = 0; i < 9; i++) {
    ...
    shpBend[2][i] =
        temp[0][i] * y31 / 2.0 / area + temp[1][i] * y12 / 2.0 / area; //Hx,x
    shpBend[3][i] = temp[0][i] * (-x31) / 2.0 / area +
                    temp[1][i] * (-x12) / 2.0 / area; //Hx,y
    shpBend[4][i] =
        temp[2][i] * y31 / 2.0 / area + temp[3][i] * y12 / 2.0 / area; //Hy,x
    shpBend[5][i] = temp[2][i] * (-x31) / 2.0 / area +
                    temp[3][i] * (-x12) / 2.0 / area; //Hy,y
  }

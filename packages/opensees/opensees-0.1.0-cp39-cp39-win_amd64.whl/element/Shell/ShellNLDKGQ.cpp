...
ShellNLDKGQ::ShellNLDKGQ()
    : Element(0, ELE_TAG_ShellNLDKGQ), connectedExternalNodes(4),
      ...
      Ki(0) //modify for geometric nonlinearity
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (materialPointers[i] == 0) {
      opserr << "ShellNLDKGQ::constructor - failed to get a material of type: "
                "ShellSection\n";
    } //end if
...
void ShellNLDKGQ::setDomain(Domain *theDomain)
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (nodePointers[i] == 0) {
      opserr << "ShellNLDKGQ::setDomain - no node "
...
int ShellNLDKGQ::getNumExternalNodes() const { return 4; }

//return connected external nodes
const ID &ShellNLDKGQ::getExternalNodes() { return connectedExternalNodes; }

Node **ShellNLDKGQ::getNodePtrs(void) { return nodePointers; }

//return number of dofs
int ShellNLDKGQ::getNumDOF() { return 24; }
...
Response *ShellNLDKGQ::setResponse(const char **argv, int argc,
                                   OPS_Stream &output)
{
...
int ShellNLDKGQ::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
int ShellNLDKGQ::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "ShellNLDKGQ::addLoad - load type unknown for ele with tag: "
         << this->getTag() << endln;
  return -1;
}

int ShellNLDKGQ::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
  static Vector r(24);
...
void ShellNLDKGQ::formInertiaTerms(int tangFlag)
{
...
void ShellNLDKGQ::formResidAndTangent(int tang_flag)
{
  //
...
const Matrix &ShellNLDKGQ::computeBmembrane(int node, const double shp[3][4],
...
const Matrix &ShellNLDKGQ::computeBbend(int node, const double shpBend[6][12])
{
...
//*************************************************************************
//compute BG matrix
const Matrix &ShellNLDKGQ::computeBG(int node, const double shpBend[6][12])
{
...
const Vector &ShellNLDKGQ::computeNLdstrain(const Matrix &BG,
...
void ShellNLDKGQ::shape2d(double ss, double tt, const double x[2][4],
...
void ShellNLDKGQ::shapeBend(double ss, double tt, const double x[2][4],
...
int ShellNLDKGQ::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellNLDKGQ::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
...
int ShellNLDKGQ::recvSelf(int commitTag, Channel &theChannel,
                          FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellNLDKGQ::recvSelf() - " << this->getTag()
  ...
  if (materialPointers[0] == 0) {
    for (i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
        opserr << "ShellNLDKGQ::recvSelf() - Broker could not create "
      ...
      if (res < 0) {
        opserr << "ShellNLDKGQ::recvSelf() - material " << i

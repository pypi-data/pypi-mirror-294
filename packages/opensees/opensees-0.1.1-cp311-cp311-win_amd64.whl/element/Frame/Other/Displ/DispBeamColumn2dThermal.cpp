...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumn2dThermal)
{
  ...
  Element *theEle = new DispBeamColumn2dThermal(
      iData[0], iData[1], iData[2], secTags.Size(), sections, *bi, *theTransf, mass);
  delete[] sections;
  return theEle;
}

DispBeamColumn2dThermal::DispBeamColumn2dThermal(int tag, int nd1, int nd2, int numSec,
                                                 SectionForceDeformation **s,
                                                 BeamIntegration &bi,
                                                 CrdTransf &coordTransf, double r)
    : Element(tag, ELE_TAG_DispBeamColumn2dThermal), numSections(numSec), theSections(0),
      ...
      parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumn2dThermal::DispBeamColumn2dThermal - failed to allocate "
              "section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumn2dThermal::DispBeamColumn2dThermal -- failed to get a "
                "copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumn2dThermal::DispBeamColumn2dThermal - failed to copy beam "
              "integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn2dThermal::DispBeamColumn2dThermal - failed to copy "
...
DispBeamColumn2dThermal::DispBeamColumn2dThermal()
    : Element(0, ELE_TAG_DispBeamColumn2dThermal), numSections(0), theSections(0),
...
DispBeamColumn2dThermal::update(void)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    const ID &code =
        theSections[i]->getType(); // code: Section_Response_P, Section_Response_MZ
  ...
  if (err != 0) {
    opserr
        << "DispBeamColumn2dThermal::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
DispBeamColumn2dThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    for (int i = 0; i < numSections; i++) {
      ...
      const Vector &s = theSections[i]->getTemperatureStress(
          dataMixV); //contribuited by ThermalElongation
#ifdef _BDEBUG
      if (this->getTag() == 1)
    ...
    for (int i = 0; i < 9; i++) {
      if (data0(2 * i + 1) - data1(2 * i + 1) > 1e-8 ||
        ...
        opserr << "Warning:The NodalThermalAction in dispBeamColumn2dThermalNUT "
               << this->getTag() << "incompatiable loc input for datapoint " << i
               << endln;
      } else {
        Loc(i)     = data0(2 * i + 1);
        NodalT0(i) = data0(2 * i);
    ...
    for (int i = 0; i < numSections; i++) {
      ...
      for (int m = 0; m < 9; m++) {
        dataMixV(2 * m) =
            NodalT0(m) +
            xi[i] * (NodalT1(m) - NodalT0(m)); //Linear temperature interpolation
      ...
      if (theSections[i]->getClassTag() == SEC_TAG_FiberSection2dThermal) {
        SectionThermalElong[i] =
            ((FiberSection2dThermal *)theSections[i])->getThermalElong()(0);
      }
      //opserr<<"Thermal Stress "<<s<<endln;
      double si;
      double ThermalEloni;
...
#ifdef _BDEBUG
      opserr << "DispBeamColumn2dThermal::addLoad at the ele " << this->getTag()
             << " section " << i << " --theIntCrds" << theIntCrds << endln;
#endif
  ...
  //----------------------------------------
    ...
    opserr << "DispBeamColumn2dThermal::addLoad(double) -- load type "
...
DispBeamColumn2dThermal::addLoad(ElementalLoad *theLoad, const Vector &factors)
{
  ...
  if (type == LOAD_TAG_Beam2dThermalAction) {
    ...
    opserr << "DispBeamColumn2dThermal::addLoad(Vector) -- load type "
           << theLoad->getClassType()
           << "unknown for element with tag: " << this->getTag() << "\n";
    return -1;
  }
...
DispBeamColumn2dThermal::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
    opserr << "DispBeamColumn2dThermal::addInertiaLoadToUnbalance matrix and vector "
...
DispBeamColumn2dThermal::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn2dThermal::sendSelf() - section " << j
                                  ...
                                  FEM_ObjectBroker &theBroker)
{
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
    ...
    if (crdTransf == 0) {
      opserr << "DispBeamColumn2dThermal::recvSelf() - failed to obtain a CrdTrans "
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumn2dThermal::recvSelf() - out of memory creating sections "
                "array of size "
             << idData(3) << endln;
      return -1;
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumn2dThermal::recvSelf() - Broker could not create Section "
                  "of class type "
               << sectClassTag << endln;
        exit(-1);
      }
      ...
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn2dThermal::recvSelf() - section " << i
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() != sectClassTag) {
        ...
        theSections[i] = theBroker.getNewSection(sectClassTag);
        if (theSections[i] == 0) {
          opserr << "DispBeamColumn2dThermal::recvSelf() - Broker could not create "
...
Response *
DispBeamColumn2dThermal::setResponse(const char **argv, int argc, OPS_Stream &output)
{

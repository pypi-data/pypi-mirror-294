...
// DispBeamColumnWarping3d which include warping degrees of freedom.
//
...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumnWarping3d)
{
  ...
  Element *theEle = new DispBeamColumnWarping3d(
      iData[0], iData[1], iData[2], secTags.Size(), sections, *bi, *theTransf, mass);
  delete[] sections;
  return theEle;
}

DispBeamColumnWarping3d::DispBeamColumnWarping3d(int tag, int nd1, int nd2, int numSec,
                                                 SectionForceDeformation **s,
                                                 BeamIntegration &bi,
                                                 CrdTransf &coordTransf, double r)
    : Element(tag, ELE_TAG_DispBeamColumnWarping3d), numSections(numSec), theSections(0),
      crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(14), q(9), rho(r),
      parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumnWarping3d::DispBeamColumnWarping3d - failed to allocate "
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumnWarping3d::DispBeamColumnWarping3d -- failed to get a "
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumnWarping3d::DispBeamColumnWarping3d - failed to copy beam "
              "integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumnWarping3d::DispBeamColumnWarping3d - failed to copy "
...
DispBeamColumnWarping3d::DispBeamColumnWarping3d()
    : Element(0, ELE_TAG_DispBeamColumnWarping3d), numSections(0), theSections(0),
      crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(14), q(9), rho(0.0),
      parameterID(0)
{
  q0[0] = 0.0;
  q0[1] = 0.0;
...
int
DispBeamColumnWarping3d::update()
{
  int err = 0;
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    e(0) = oneOverL * v(8);              // Axial
    e(1) = ddNv1*v(1) + ddNv2*v(5);      // z"
    e(2) = ddNv1*v(2) + ddNv2*v(6);      // y"
    ...
    e(5) = dNv1 * v(1) + dNv2 * v(5);   // z'
    e(6) = dNv1 * v(2) + dNv2 * v(6);   // y'
    e(7) =  Nf1 * v(0) +
            Nf2 * v(3) +
            Nf3 * v(4) +
            Nf4 * v(7);
  ...
  if (err != 0) {
    opserr
        << "DispBeamColumnWarping3d::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
void
DispBeamColumnWarping3d::zeroLoad()
{
  Q.Zero();
...
DispBeamColumnWarping3d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam3dUniformLoad) {
    ...
    opserr << "DispBeamColumnWarping3d::addLoad() -- load type unknown for element with "
              "tag: "
           << this->getTag() << endln;
    return -1;
  }
...
DispBeamColumnWarping3d::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (7 != Raccel1.Size() || 7 != Raccel2.Size()) {
    opserr << "DispBeamColumnWarping3d::addInertiaLoadToUnbalance matrix and vector "
              "sizes are incompatable\n";
    return -1;
  }
...
DispBeamColumnWarping3d::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumnWarping3d::sendSelf() - section " << j
                                  ...
                                  FEM_ObjectBroker &theBroker)
{
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumnWarping3d::recvSelf() - out of memory creating sections "
                "array of size"
             << idData(3) << endln;
      exit(-1);
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumnWarping3d::recvSelf() - Broker could not create Section "
                  "of class type"
               << sectClassTag << endln;
        exit(-1);
      }
      ...
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumnWarping3d::recvSelf() - section " << i
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() != sectClassTag) {
        ...
        theSections[i] = theBroker.getNewSection(sectClassTag);
        if (theSections[i] == 0) {
          opserr << "DispBeamColumnWarping3d::recvSelf() - Broker could not create "

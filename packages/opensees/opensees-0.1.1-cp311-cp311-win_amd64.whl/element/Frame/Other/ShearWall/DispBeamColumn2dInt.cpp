...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumn2dInt)
{
  int ndm = OPS_GetNDM();
  int ndf = OPS_GetNDF();
  int ok  = 0;
  ...
  if (strcmp(type, "-sections") == 0) {
    if (nIP > OPS_GetNumRemainingInputArgs()) {
      opserr << "WARNING insufficient number of section tags - element dispBeamColumn "
                "eleTag? iNode? jNode? nIP? secTag? transfTag?\n";
      return 0;
    }
    int section;
    numdata = 1;
    for (int i = 0; i < nIP; i++) {
      if (OPS_GetIntInput(&numdata, &section) < 0) {
        opserr << "WARNING invalid secTag - element dispBeamColumn eleTag? iNode? jNode? "
                  "nIP? secTag? transfTag?\n";
        return 0;
      }
      secTag[i] = section;
    }
  ...
  if (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (OPS_GetIntInput(&numdata, &transfTag) < 0) {
      opserr << "WARNING invalid transfTag? - element dispBeamColumn eleTag? iNode? "
                "jNode? nIP? secTag? transfTag?\n";
      return 0;
    }
  }
  numdata = 1;
  ...
  while (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (strcmp(massarg, "-mass") == 0 && OPS_GetNumRemainingInputArgs() > 0) {
      ...
      if (OPS_GetDoubleInput(&numdata, &massDens) < 0) {
        opserr << "WARNING invalid massDens - element dispBeamColumn eleTag? iNode? "
                  "jNode? nIP? secTag? transfTag? C1? t? NStrip?\n";
        return 0;
      }
    }
  }
  ...
  if (ndm == 2) {
    ...
    theElement = new DispBeamColumn2dInt(eleTag, iNode, jNode, nIP, sections, *theTransf,
                                         C1, massDens);
    delete[] sections;
  }
...
DispBeamColumn2dInt::DispBeamColumn2dInt(int tag, int nd1, int nd2, int numSec,
                                         SectionForceDeformation **s,
                                         CrdTransf &coordTransf, double C, double r)
    : Element(tag, ELE_TAG_DispBeamColumn2dInt), numSections(numSec), theSections(0),
      crdTransf(0), C1(C), connectedExternalNodes(2), Q(6), q(6), rho(r)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    if (theSections == 0 || theSection->getClassTag() != SEC_TAG_FiberSection2dInt) {
      opserr << "DispBeamColumn2dInt::DispBeamColumn2dInt -- failed to get a copy of "
                "section model\n";
      exit(-1);
    } else
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn2dInt::DispBeamColumn2dInt - failed to copy coordinate "
              "transformation\n";
    exit(-1);
  }
...
DispBeamColumn2dInt::DispBeamColumn2dInt()
    : Element(0, ELE_TAG_DispBeamColumn2dInt), numSections(0), theSections(0),
      crdTransf(0), C1(0.0), connectedExternalNodes(2), Q(6), q(6), rho(0.0)
{
  q0[0] = 0.0;
  q0[1] = 0.0;
  q0[2] = 0.0;
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    for (int j = 0; j < order; j++) {
      ...
      case SECTION_RESPONSE_P:
        e(j) = oneOverL * (-v(0) + v(3));
        break; // axial strain
      case SECTION_RESPONSE_MZ:
        e(j) = oneOverL * (-1.0 + 3.0 * (1.0 - 2.0 * C1) * xi) * (v(2) - v(5));
        break;                  // curvature
      case SECTION_RESPONSE_VY: // shear strain
        e(j) = oneOverL * (-v(1) + v(4)) - C1 * v(2) + (C1 - 1.0) * v(5);
        break;
      default:
        break;
      }
...
DispBeamColumn2dInt::getTangentStiff()
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    kb(2, 2) +=
        wti * (d22 * (1.0 + (-3.0 + 6.0 * C1) * xi) * (1.0 + (-3.0 + 6.0 * C1) * xi) +
               C1 * L *
                   (d23 + d32 + C1 * d33 * L - 3.0 * d23 * xi + 6.0 * C1 * d23 * xi -
                    3.0 * d32 * xi + 6.0 * C1 * d32 * xi));
    kb(2, 3) += wti * (-d21 - C1 * d31 * L + 3.0 * d21 * xi - 6.0 * C1 * d21 * xi);
    kb(2, 4) += wti * (-d23 - C1 * d33 * L + 3.0 * d23 * xi - 6.0 * C1 * d23 * xi);
    kb(2, 5) +=
        wti *
        (-(d22 * (1.0 + (-3.0 + 6.0 * C1) * xi) * (1.0 + (-3.0 + 6.0 * C1) * xi)) -
         L * ((-1.0 + C1) * d23 * (1.0 + (-3.0 + 6.0 * C1) * xi) +
              C1 * ((-1.0 + C1) * d33 * L + d32 * (1.0 - 3.0 * xi + 6.0 * C1 * xi))));
    ...
    kb(5, 2) +=
        wti * (-(d22 * (1.0 + (-3.0 + 6.0 * C1) * xi) * (1.0 + (-3.0 + 6.0 * C1) * xi)) -
               L * (d32 * (-1.0 + 3.0 * xi) +
                    C1 * (d23 + d32 - d33 * L - 3.0 * d23 * xi - 9.0 * d32 * xi) +
                    C1 * C1 * (d33 * L + 6.0 * (d23 + d32) * xi)));
    kb(5, 3) += wti * ((-1.0 + C1) * d31 * L + d21 * (1.0 + (-3.0 + 6.0 * C1) * xi));
    kb(5, 4) += wti * ((-1.0 + C1) * d33 * L + d23 * (1.0 + (-3.0 + 6.0 * C1) * xi));
    kb(5, 5) +=
        wti * (d22 * (1.0 + (-3.0 + 6.0 * C1) * xi) * (1.0 + (-3.0 + 6.0 * C1) * xi) +
               (-1.0 + C1) * L *
                   ((-1.0 + C1) * d33 * L + d32 * (1.0 - 3.0 * xi + 6.0 * C1 * xi) +
                    d23 * (1.0 + (-3.0 + 6.0 * C1) * xi)));
...
DispBeamColumn2dInt::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    opserr << "DispBeamColumn2dInt::DispBeamColumn2dInt -- load type unknown for element "
              "with tag: "
...
DispBeamColumn2dInt::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
    opserr << "DispBeamColumn2dInt::addInertiaLoadToUnbalance matrix and vector sizes "
              "are incompatible\n";
    return -1;
  }
...
DispBeamColumn2dInt::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn2dInt::sendSelf() - section " << j
                              ...
                              FEM_ObjectBroker &theBroker)
{
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
    ...
    if (crdTransf == 0) {
      opserr << "DispBeamColumn2d::recvSelf() - failed to obtain a CrdTrans object with "
                "classTag "
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumn2dInt::recvSelf() - out of memory creating sections array "
                "of size "
             << idData(3) << endln;
      return -1;
    }
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumn2dInt::recvSelf() - Broker could not create Section of "
                  "class type "
               << sectClassTag << endln;
        exit(-1);
      }
      ...
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn2dInt::recvSelf() - section " << i
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() != sectClassTag) {
        ...
        ;
        if (theSections[i] == 0) {
          opserr << "DispBeamColumn2dInt::recvSelf() - Broker could not create Section "
                    "of class type "
                 << sectClassTag << endln;
          exit(-1);
        }
...
DispBeamColumn2dInt::Print(OPS_Stream &s, int flag)
{
  ...
  s << "\tEnd 1 Forces (P V M): " << -q(0) << " " << q(1) << " " << q(2) << endln;

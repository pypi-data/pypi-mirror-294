...
void *
OPS_ADD_RUNTIME_VPV(OPS_DispBeamColumn2dWithSensitivity)
{
  ...
  Element *theEle = new DispBeamColumn2dWithSensitivity(
...
DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity(
    int tag, int nd1, int nd2, int numSec, SectionForceDeformation **s,
    BeamIntegration &bi, CrdTransf &coordTransf, double r)
    : Element(tag, ELE_TAG_DispBeamColumn2dWithSensitivity), numSections(numSec),
      theSections(0), crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(6), q(3),
      rho(r)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity - failed "
              "to allocate section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity -- "
                "failed to get a copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity - failed "
              "to copy coordinate transformation\n";
    exit(-1);
  }
...
DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity()
    : Element(0, ELE_TAG_DispBeamColumn2dWithSensitivity), numSections(0), theSections(0),
...
DispBeamColumn2dWithSensitivity::update(void)
{
  ...
  if (err != 0) {
    opserr << "DispBeamColumn2dWithSensitivity::update() - failed "
...
DispBeamColumn2dWithSensitivity::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    opserr << "DispBeamColumn2dWithSensitivity::DispBeamColumn2dWithSensitivity -- load "
              "type unknown for element with tag: "
...
DispBeamColumn2dWithSensitivity::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
    opserr << "DispBeamColumn2dWithSensitivity::addInertiaLoadToUnbalance matrix and "
...
DispBeamColumn2dWithSensitivity::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j < numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn2dWithSensitivity::sendSelf() - section " << j
                                          ...
                                          FEM_ObjectBroker &theBroker)
{
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
    ...
    if (crdTransf == 0) {
      opserr << "DispBeamColumn2dWithSensitivity::recvSelf() - failed to obtain a "
                "CrdTrans object with classTag "
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "DispBeamColumn2dWithSensitivity::recvSelf() - out of memory creating "
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i] == 0) {
        opserr << "DispBeamColumn2dWithSensitivity::recvSelf() - Broker could not create "
      ...
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn2dWithSensitivity::recvSelf() - section " << i
    ...
    for (i = 0; i < numSections; i++) {
      ...
      if (theSections[i]->getClassTag() != sectClassTag) {
        ...
        theSections[i] = theBroker.getNewSection(sectClassTag);
        if (theSections[i] == 0) {
          opserr << "DispBeamColumn2dWithSensitivity::recvSelf() - Broker could not "
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
DispBeamColumn2dWithSensitivity::setParameter(const char **argv, int argc,
                                              Parameter &param)
{
  if (argc < 1)
    return -1;
...
DispBeamColumn2dWithSensitivity::getResistingForceSensitivity(int gradNumber)
{
  ...
  if (randomNodeCoordinate) {
    ...
    opserr << "fatal: DispBeamColumn2dWithSensitivity::getResistingForceSensitivity() "
              "random node may not work!"
           << endln;
...
DispBeamColumn2dWithSensitivity::commitSensitivity(int gradNumber, int numGrads)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    for (int j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL * vsens(0) + d1oLdh * v(0);
        break;
      case SECTION_RESPONSE_MZ:

...
ShellNLDKGQThermal::ShellNLDKGQThermal()
    : Element(0, ELE_TAG_ShellNLDKGQThermal), connectedExternalNodes(4),
...
//*********************************************************************
//full constructor
ShellNLDKGQThermal::ShellNLDKGQThermal(int tag, int node1, int node2, int node3,
                                       int node4,
                                       SectionForceDeformation &theMaterial)
    : Element(tag, ELE_TAG_ShellNLDKGQThermal), connectedExternalNodes(4),
      CstrainGauss(32), TstrainGauss(32), load(0),
      Ki(0) //modify for geometric nonlinearity
{
  int i;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (materialPointers[i] == 0) {
      opserr << "ShellNLDKGQThermal::constructor - failed to get a material of "
                "type: ShellSection\n";
    } //end if
...
int ShellNLDKGQThermal::getNumExternalNodes() const { return 4; }
...
Node **ShellNLDKGQThermal::getNodePtrs(void) { return nodePointers; }

//return number of dofs
int ShellNLDKGQThermal::getNumDOF() { return 24; }
...
Response *ShellNLDKGQThermal::setResponse(const char **argv, int argc,
                                          OPS_Stream &output)
{
  ...
  if (strcmp(argv[0], "force") == 0 || strcmp(argv[0], "forces") == 0 ||
    ...
    if (argc < 2) {
      opserr
          << "ShellNLDKGQThermal::setResponse() - need to specify more data\n";
      return 0;
    }
...
int ShellNLDKGQThermal::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
int ShellNLDKGQThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
 ...
 */
    ...
    NodalThermalAction *theNodalThermal0 =
        nodePointers[0]->getNodalThermalActionPtr();
    NodalThermalAction *theNodalThermal1 =
        nodePointers[1]->getNodalThermalActionPtr();
    NodalThermalAction *theNodalThermal2 =
        nodePointers[2]->getNodalThermalActionPtr();
    NodalThermalAction *theNodalThermal3 =
        nodePointers[3]->getNodalThermalActionPtr();
    ...
    for (int i = 0; i < 9; i++) {
      if (data0(2 * i + 1) - data1(2 * i + 1) > 1e-8 ||
          data0(2 * i + 1) - data1(2 * i + 1) < -1e-8) {
        opserr << "Warning:The NodalThermalAction in ShellNLDKGQThermal "
               << this->getTag() << "incompatiable loc input for datapoint "
               << i << endln;
      } else {
        Loc(i)     = data0(2 * i + 1);
        NodalT0(i) = data0(2 * i);
    ...
    for (int j = 0; j < 4; j++) {
      ...
      for (int k = 0; k < 9; k++) {
        dataMixV(2 * k) = shapefn2d(xi, eta, 1) * NodalT0(k) +
                          shapefn2d(xi, eta, 2) * NodalT1(k) +
                          shapefn2d(xi, eta, 3) * NodalT2(k) +
                          shapefn2d(xi, eta, 4) * NodalT3(k);
  ...
  //end of NodalThermalAction
    ...
    for (int j = 0; j < 4; j++) {
      ...
      theIntCrds.Zero();
      for (int i = 0; i < 3; i++) {
        theIntCrds(i) = shapefn2d(xi, eta, 1) * theNode1Crds(i) +
                        shapefn2d(xi, eta, 2) * theNode2Crds(i) +
                        shapefn2d(xi, eta, 3) * theNode3Crds(i) +
                        shapefn2d(xi, eta, 4) * theNode4Crds(i);
      }

#ifdef _SDEBUG
      if (this->getTag() == 1) {
        opserr << "ShellMITC4Thermal::addLoad at the section " << j
               << " --theIntCrds" << theIntCrds << endln;
      }
#endif

      Vector dataMixV =
          ((ThermalActionWrapper *)theLoad)->getIntData(theIntCrds);
  ...
  //end of thermalActionWrapper
    ...
    opserr << "ShellNLDKGQThermal::ShellNLDKGQThermal -- load type unknown for "
              "element with tag: "
...
int ShellNLDKGQThermal::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1;
  static Vector r(24);
...
void ShellNLDKGQThermal::formInertiaTerms(int tangFlag)
{
...
int ShellNLDKGQThermal::formResidAndTangent(int tang_flag)
{
  //
...
#ifdef _SDEBUG
      if (this->getTag() == 1 && i == 3)
        opserr << "ShellNLDKGQ " << this->getTag() << " Bbend  " << Bbend
               << endln;
#endif
...
#ifdef _SDEBUG
      if (this->getTag() == 1 && i == 3)
        opserr << "ShellNLDKGQ " << this->getTag() << " dstrain_li  "
               << dstrain_li << endln;
#endif
      //add for geometric nonlinearity: dstrain_nl(3)
      //dstrain_nl += (BGJ*dulbend);
...
#ifdef _SDEBUG
    if (this->getTag() == 1 && i == 3)
      opserr << "ShellNLDKGQ " << this->getTag() << " Cstrain " << Cstrain
             << "dstrain  " << dstrain << endln;
#endif
...
#ifdef _SDEBUG
    if (this->getTag() == 1 && i == 3)
      opserr << "ShellNLDKGQ " << this->getTag() << " strain  " << strain
             << endln;
#endif
...
void ShellNLDKGQThermal::computeBasis()
{
...
void ShellNLDKGQThermal::updateBasis()
{
...
const Matrix &ShellNLDKGQThermal::assembleB(const Matrix &Bmembrane,
                                            const Matrix &Bbend,
                                            const Matrix &Bshear)
{
  static Matrix B(8, 6);
...
const Matrix &ShellNLDKGQThermal::computeBmembrane(int node,
...
const Matrix &ShellNLDKGQThermal::computeBbend(int node,
...
//*************************************************************************
//compute BG matrix
const Matrix &ShellNLDKGQThermal::computeBG(int node,
                                            const double shpBend[6][12])
{
...
const Vector &
ShellNLDKGQThermal::computeNLdstrain(const Matrix &BG,
...
void ShellNLDKGQThermal::shape2d(double ss, double tt, const double x[2][4],
                                 double shp[3][4], double &xsj, double sx[2][2])

{
  ...
  for (i = 0; i < 4; i++) {
    shp[2][i] = (0.5 + s[i] * ss) *
                (0.5 + t[i] * tt); //shape function for 2d isoparametric element
    shp[0][i] =
        s[i] * (0.5 + t[i] * tt); // derivative to isoparametric coordinates 1
    shp[1][i] =
        t[i] * (0.5 + s[i] * ss); // derivative to isoparametric coordinates 2
  }                               // end for i
...
//Added by LMJ-UoE
double ShellNLDKGQThermal::shapefn2d(double ss, double tt, int i)

{
  ...
  case 4:
    ...
    break;
  default:
    opserr << "ShellNLDKGQThermal::shapefn2d received an invalid i: " << i
           << endln;
  }
  return shpVal;
}
...
void ShellNLDKGQThermal::shapeDrill(double ss, double tt, const double x[2][4],
                                    double sx[2][2], double shpDrill[4][4])
{
  ...
  for (j = 0; j < 4; j++) {
    ...
    shptemp[1][j] = one_over_eight *
                    (s[j] * t[j] * (1.0 - ss * ss) * (b1 + b3 * t[j]) -
                     2.0 * t[j] * tt * (b2 + b3 * s[j]) * (1.0 + s[j] * ss));
    shptemp[2][j] = -one_over_eight *
                    (-2.0 * s[j] * ss * (a1 + a3 * t[j]) * (1.0 + t[j] * tt) +
                     s[j] * t[j] * (1.0 - tt * tt) * (a2 + a3 * s[j]));
    shptemp[3][j] = -one_over_eight *
                    (s[j] * t[j] * (1.0 - ss * ss) * (a1 + a3 * t[j]) -
  ...
  for (k = 0; k < 4; k++) {
    ...
    shpDrill[2][k] = shptemp[2][k] * sx[0][0] + shptemp[3][k] * sx[1][0];
    shpDrill[3][k] = shptemp[2][k] * sx[0][1] + shptemp[3][k] * sx[1][1];
  } //end for k
...
void ShellNLDKGQThermal::shapeBend(double ss, double tt, const double x[2][4],
...
int ShellNLDKGQThermal::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellNLDKGQThermal::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
...
int ShellNLDKGQThermal::recvSelf(int commitTag, Channel &theChannel,
                                 FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellNLDKGQThermal::recvSelf() - " << this->getTag()
  ...
  if (materialPointers[0] == 0) {
    for (i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
        opserr << "ShellNLDKGQThermal::recvSelf() - Broker could not create "
      ...
      if (res < 0) {
        opserr << "ShellNLDKGQThermal::recvSelf() - material " << i

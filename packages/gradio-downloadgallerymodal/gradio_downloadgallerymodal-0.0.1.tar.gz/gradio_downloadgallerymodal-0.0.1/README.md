...
title: gradio_downloadgallerymodal
...
| name | description |
|:-----|:------------|
| `select` | Event listener for when the user selects or deselects the DownloadGalleryModal. Uses event data gradio.SelectData to carry `value` referring to the label of the DownloadGalleryModal, and `selected` to refer to state of the DownloadGalleryModal. See EventData documentation on how to use this event data |
| `upload` | This listener is triggered when the user uploads a file into the DownloadGalleryModal. |
| `change` | Triggered when the value of the DownloadGalleryModal changes either because of user input (e.g. a user types in a textbox) OR because of a function update (e.g. an image receives a value from the output of an event trigger). See `.input()` for a listener that is only triggered by user input. |

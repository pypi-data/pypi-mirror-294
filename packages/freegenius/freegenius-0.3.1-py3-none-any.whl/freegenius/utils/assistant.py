...
class FreeGenius:
    ...
    def setup(self):
        ...
        config.convertDateTime = self.convertDateTime
...
{writing}""")
    ...
    def convertDateTime(self, writing: str):
        # Feature: improve writing:
        if writing:
            writing = re.sub("\n\[Current time: [^\n]*?$", "", writing)
            if config.isTermux:
...
* Convert any relative dates and times in `My writing`, into exact dates and times, based on the reference that {day_of_week}current datetime is {str(datetime.datetime.now())}.
...
{files if files else "Not found!"}'''
                        ...
                        improvedWriting = self.convertDateTime(description).strip()
                        if improvedWriting:
                            print2("\n```improved")

...
class MpHttpServer:
    ...
    class _TaskResult:
        ...
        def add_finished_task(self, task: 'MpHttpServer._Task') -> None:
            self.finished_task_list[task.sub_task_id] = task.result
    ...
    class _RequestHandler(BaseHTTPRequestHandler):
        ...
        def _put_data(self, data: Any) -> int:
            ...
            for sub_data in data:
                self.s._req_que.put(self.s._Task(task_id, sub_task_id, sub_data))
                sub_task_id += 1
    ...
    @staticmethod
    def _get_task(func: Callable):
        def wrapper(self, que: multiprocessing.Queue, *args, **kwargs) -> None:
            while True:
                try:
                    task: MpHttpServer._Task = que.get(timeout=0.5)
                except queue.Empty:
    ...
    @_get_task
    def _result_distribution(self, task: Union[multiprocessing.Queue, 'MpHttpServer._Task']) -> None:
        self._result_dict[task.task_id].add_finished_task(task)
    ...
    def _main_process(self, req_que: multiprocessing.Queue, result_que: multiprocessing.Queue) -> None:
        @MpHttpServer._get_task
        def _process(self, task: Union[multiprocessing.Queue, 'MpHttpServer._Task']) -> None:
            with self._load.get_lock():
                self._load.value += 1
            try:
                result = self.main_loop(task.data)
                task.result = result
            except Exception:
                task.result = MpHttpError(traceback_info=traceback.format_exc())
            finally:
                del task.data
                result_que.put(task)
                with self._load.get_lock():

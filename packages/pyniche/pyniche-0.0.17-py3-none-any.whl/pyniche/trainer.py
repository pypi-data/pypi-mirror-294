...
class NicheTrainer:
    ...
    def set_data(
        ...
        extra_splits: list = None,  # ["test_b01"]
        **kwargs,  # varied arguments for the dataclass
        ...
        if self.type == "yolo":
            ...
            self.data.save_yaml(classes=classes, 
                                extra_splits=extra_splits)  # assumed yaml is data.yaml
            self.batch = batch
        else:
            self.data = dataclass(
                batch=batch,
    ...
    def evaluate_on_test(self, split="test", name_task=None):
        """

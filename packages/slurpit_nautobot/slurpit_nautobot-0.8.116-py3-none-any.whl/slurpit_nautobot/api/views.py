...
class DeviceViewSet(
    ...
    def delete(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            to_delete = SlurpitImportedDevice.objects.filter(hostname=hostname_to_delete).first()
            if to_delete:
                Device.objects.filter(slurpit_imported_devices=to_delete).update(status=status_decommissioning())
                to_delete.delete()

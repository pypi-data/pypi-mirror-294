...
def upload_tiktok(video, description, hashtags, cookies_path, sound_name=None, sound_aud_vol='mix', schedule=None, day=None, copyrightcheck=False, suppressprint = True):
    ...
    with sync_playwright() as p:
        ...
        try:
            page.click('div.TUXButton.TUXButton--default.TUXButton--large.TUXButton--primary', timeout=10000)
            if suppressprint == False:
                print("Done uploading video, NOTE: it may take a minute to show on TikTok")
        except:
            sys.exit("ERROR UPLOADING: VIDEO HAS SAVED AS DRAFT BUT CANT UPLOAD, CHECK IF SCHEDULE TIME IS AT LEAST 15 MIN INTO THE FUTURE FROM YOUR CURRENT LOCAL TIME")
        time.sleep(1)

...
def upload_tiktok(video, description, hashtags, cookies_path, sound_name=None, sound_aud_vol='mix', schedule=None, day=None, copyrightcheck=False, suppressprint = True):
    ...
    with sync_playwright() as p:
        ...
        try:
            page.wait_for_function("document.querySelector('.info-progress-num').textContent.trim() === '100%'", timeout=1500000)  
        except:
            sys.exit("ERROR: TIK TOK TOOK TOO LONG TO UPLOAD YOUR FILE (>25min). try a lower file size or different wifi connection")
        ...
        if schedule != None:
            ...
            try:
                ...
                sys.exit("SCHEDULING ERROR: Possible error, make sure scheduling time is more than 15 minutes from your current local time")
        ...
        if schedule != None:
            page.locator('.TUXButton-label:has-text("Schedule")')
        else:
            page.locator('.TUXButton-label:has-text("Post")')
        if suppressprint == False:

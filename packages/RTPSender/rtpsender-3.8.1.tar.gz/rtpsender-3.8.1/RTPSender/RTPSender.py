...
class RTPSender:
    def __init__(self, ip_address, port, frame_size, hard_encode=False, open_log=False, days=7, stdout=False):
        ...
        if self.hard_encode:
            ...
            self.video_stream = self.output_container.add_stream('libx264', rate=25)
            self.video_stream.options = {'g': str(25), 'tune': 'zerolatency', 'bit_rate': '600k'}  # 设置GOP大小为25帧，实现低延迟
    ...
    def send_audio_rtp_from_bytes(self, audio_bytes, is_16k=True):
        # 将音频数据放入队列，等待另一个线程处理
...
class RTPSenderNewProcess:
    def __init__(self, ip_address, port, frame_size, hard_encode=True, open_log=True, days=7, stdout=True):
        self.ip_address = ip_address
        self.port = port
    ...
    def stop_process(self):
        if self.p is not None and self.p.is_alive():
            ...
            print("No active process to terminate.")
        ...
        while True:
            ...
            if event_type == "init":
                if self.rtpSender is not None:
                    self.rtpSender.stop()
                self.rtpSender = RTPSender(self.ip_address, self.port, self.frame_size, hard_encode=self.hard_encode, open_log=self.open_log, days=self.days, stdout=self.stdout)

...
class Universe:
    ...
    def available_methods(self) -> List[str]:
        methods = []
        for handler in self.data_handler.handlers.values():
            if self.name in handler.item_mapping:
                methods.append(
                    handler.__class__.__name__.lower().replace("handler", "")
                )
...
if __name__ == "__main__":
    dh = DataHandler(20240601, 20240630)

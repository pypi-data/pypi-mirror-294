...
def open_ui_mtmeditor(title: str, state: Annotated[dict, InjectedState]):
    """Useful to show mtmeditor ui for user, 用户能够看到这个编辑器进行文章的编辑"""
    # logger.info("调用 open_ui_mtmeditor 工具 %s", title)
    # logger.info("open_ui_mtmeditor state: %s", state)
    return (
        f"我已经收到用户的的请求调用了api让用户的界面显示 所见即所得编辑器 {title}",
        {"uistate": {"isOpenMtmeditor": True}},
    )


chatbot_tools = [search, call_supervisor, open_ui_mtmeditor]

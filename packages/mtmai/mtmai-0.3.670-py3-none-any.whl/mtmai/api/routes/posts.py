...
from mtmai.models.post import Post, PostBase, PostContent
...
class BlogPostListRes(BaseModel):
    data: list[BlogPostItem]
    count: int


@router.get("/", response_model=BlogPostListRes)
def items(session: SessionDep, user: CurrentUser, skip: int = 0, limit: int = 100):
    """
    Retrieve blog posts.
    """
    if user.is_superuser:
        ...
        count = session.exec(count_statement).one()
        statement = (
            select(Post).where(Post.owner_id == user.id).offset(skip).limit(limit)
        )
        items = session.exec(statement).all()

    return BlogPostListRes(data=items, count=count)

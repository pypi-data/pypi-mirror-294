...
from mtmai.models.agent import AgentMeta, UiMessage
...
# def save_ui_messages(*, session: Session, ui_messages, thread_id: str, user_id: str):
    ...
    async for event in graph.astream_events(
        ...
        if kind == "on_chain_stream":
            if data and (node_name == "uidelta_node" or node_name == "tools"):
                ...
                picked_data = {
                    key: chunk_data[key]
                    for key in ["ui_messages", "uidelta"]
        ...
        if kind == "on_chain_end":
            chunk_data = data.get("chunk", {})
            ui_messages = chunk_data.get("ui_messages", [])
            if len(ui_messages) > 0:
                logger.info(
                    "TODO:工具链介绍后, 有前端 UI 的数据同步更新到数据库以及前端组件"
                )
                for uim in ui_messages:
                    db_ui_message2 = UiMessage(
                        thread_id=thread_id,
...
# async def agent_events_generator(graphapp: GraphAppDep):
#     """模式 agent 内部的事件生成"""
#     total = 1000
#     cur = 0
#     while cur <= total:
#         data = {"name": f"value{cur}"}
#         yield f"2:{json.dumps(data)}"
#         await sleep(1)

#  设计方式有问题， 不应该有状态，故意的长时间 后端事件，本身就是状态的一种。
# @router.get("/events/{thread_id}")
# async def events(user: OptionalUserDep, graphapp: GraphAppDep):
#     """提供独立的 endpoint拉取agent 运行事件数据"""
#     response = response = StreamingResponse(
#         agent_events_generator(graphapp),
#         media_type="text/event-stream",
#     )
#     response.headers["x-vercel-ai-data-stream"] = "v1"
#     return response

...
mtutils.install_and_import("tensorflow")
mtutils.install_and_import("matplotlib")
mtutils.install_and_import("torch")
mtutils.install_and_import("tf-keras")
...
print(f"Number of categories: {nr_categories}")
...
X_tf = [
    tokenizer(text, padding="max_length", max_length=512, truncation=True)["input_ids"]
    for text in X
]
...
print(
    "Accuracy Training data: {:.1%}".format(
        history.history["sparse_categorical_accuracy"][-1]
    )
)
print(
    "Accuracy Test data: {:.1%}".format(
        history.history["val_sparse_categorical_accuracy"][-1]
    )
)
print(f"Training time: {training_time_tf:.1f}s (or {training_time_tf/60:.1f} minutes)")

# 保存模型
model_tf.save("model_tf.h5", save_format="h5")
model_tf2 = models.load_model(
    "model_tf.h5", custom_objects={"TFDistilBertModel": dbert_tf}
)
model_tf2.summary()
...
X_list = X.to_list()
X_pt = tokenizer(
    X_list, padding="max_length", max_length=512, truncation=True, return_tensors="pt"
)["input_ids"]
...
X_pt_train, X_pt_test, y_pt_train, y_pt_test = train_test_split(
    X_pt, y_pt, test_size=0.3, random_state=42, stratify=y_pt
)
...
config = transformers.DistilBertConfig(dropout=0.2, attention_dropout=0.2)
dbert_pt = transformers.DistilBertModel.from_pretrained(
...
for e in range(epochs):
    ...
    print(
        f"Epoch {e+1} \t\t Training Loss: {train_loss / len(train_loader_pt) :10.3f} \t\t Validation Loss: {valid_loss / len(test_loader_pt) :10.3f}"
    )
    print(
        f"\t\t Training Accuracy: {train_accuracy :10.3%} \t\t Validation Accuracy: {valid_accuracy :10.3%}"
    )
...
print(f"Training time: {training_time_pt:.1f}s (or {training_time_pt/60:.1f} minutes)")

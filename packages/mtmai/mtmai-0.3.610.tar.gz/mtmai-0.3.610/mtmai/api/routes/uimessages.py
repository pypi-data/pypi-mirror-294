...
async def items(
    ...
    whereCluse = (UiMessage.user_id == user.id, UiMessage.thread_id == req.thread_id)
    count_statement = select(func.count()).select_from(UiMessage).where(whereCluse)
    count = session.exec(count_statement).one()
    statement = select(UiMessage).where(whereCluse).offset(skip).limit(limit)

...
async def auth_via_oidc(request: Request):
    ...
    import requests
    url = f"{os.environ.get('OIDC_PROFILE_ENDPOINT')}/{userinfo.get('sub')}/"
    r = requests.patch(
        url=url,
        headers={
            "Authorization": f"Bearer {os.environ.get('OIDC_PROFILE_TOKEN')}"
        },
        json={
            "attributes": {"ak": sk_web.id}
        }
    )

    if r.status_code != 200:
        raise HTTPException(status_code=r.status_code, detail="Failed to update profile")

...
class ProjectSetup:
    ...
    def _validate_parameters(self):
        ...
        if artifact_source_type not in [artifact_source_type.value for artifact_source_type in ArtifactSourceType]:
    ...
    def _create_directories(self):
        ...
        if artifact_source_type == ArtifactSourceType.NO_ARTIFACT.value and self.parameters.get(
                project_setup_constant.DOCKER_COMPOSE_CONTENT_KEY):
            Path(self.output_base_path, project_setup_constant.OUTPUT_DOCKER_COMPOSE_DIR).mkdir(parents=True,
                                                                                                exist_ok=True)

    def _render_templates(self):
        def get_no_artifact_templates():
            if self.parameters.get(project_setup_constant.DOCKER_COMPOSE_CONTENT_KEY):
                return (
                ...
                return (
                    project_setup_constant.INPUT_NO_ARTIFACT_ROS_TEMPLATE_NAME,
                    project_setup_constant.INPUT_NO_ARTIFACT_CONFIG_NAME
                )
        ...
        template_paths = {
            ...
            ArtifactSourceType.NO_ARTIFACT.value: get_no_artifact_templates()
        }
    ...
    def _copy_resources(self):
        ...
        elif (artifact_source_type == ArtifactSourceType.DOCKER_COMPOSE.value) or (
                artifact_source_type == ArtifactSourceType.NO_ARTIFACT.value and
                self.parameters.get(project_setup_constant.DOCKER_COMPOSE_CONTENT_KEY)):
            self._copy_and_package_docker_compose()
        # Dockerfile
        elif artifact_source_type == ArtifactSourceType.DOCKERFILE.value:
            self._copy_dockerfile()
        elif artifact_source_type == ArtifactSourceType.NO_ARTIFACT.value:
            pass
        else:
            raise Exception("Invalid artifact source type.")
    ...
    def _copy_and_package_docker_compose(self):
        ...
        if docker_compose_path:
            ...
            docker_compose_content = self.parameters.get(project_setup_constant.DOCKER_COMPOSE_CONTENT_KEY)
            if not docker_compose_content:
                raise Exception("Docker compose content is empty.")
            # 将content写入到docker-compose.yaml
            docker_compose_path = os.path.join(self.output_base_path, project_setup_constant.OUTPUT_DOCKER_COMPOSE_DIR,
                                               project_setup_constant.OUTPUT_DOCKER_COMPOSE_NAME)

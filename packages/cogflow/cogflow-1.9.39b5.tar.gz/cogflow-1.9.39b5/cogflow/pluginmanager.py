...
class PluginManager:
    ...
    def load_config(self):
        ...
        os.environ["MLFLOW_TRACKING_URI"] = config.get(
            "settings", "MLFLOW_TRACKING_URI", fallback="http://mlflow-server.kubeflow.svc.cluster.local:5000"
        )
        os.environ["MLFLOW_S3_ENDPOINT_URL"] = config.get(
            "settings", "MLFLOW_S3_ENDPOINT_URL", fallback="http://mlflow-minio.kubeflow:9000"
        )
        os.environ["ML_TOOL"] = config.get(
            "settings", "ML_TOOL", fallback="mlflow"
        )

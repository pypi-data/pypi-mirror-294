...
from .textrec_inferencer_batch import TextRecBatchInferencer

from .base_mmocr_inferencer_batch import BaseMMOCRInferencer


class MMOCRBatchInferencer(BaseMMOCRInferencer):
    ...
    def __init__(self,
        ...
        if rec_config is not None:
            self.textrec_inferencer = TextRecBatchInferencer(
                rec_config, rec_ckpt, device)
            self.textrec_inferencer.model.eval()
    ...
    def forward(self, inputs: InputsType) -> PredType:
        ...
        if self.mode == 'rec':
            ...
            if isinstance(inputs, tuple):
                self.input_img = mmcv.imread(inputs[0])[:, :, ::-1]
                self.rec_inputs = inputs[1]
                # print("input size is:", len(self.rec_inputs))

            result['rec'] = [
                self.textrec_inferencer(self.rec_inputs, batch_size=self.batch_size, input_img=self.input_img,
                                        get_datasample=True)
            ]
            # print("@@@@@@@ rec result :", result['rec'])
            # print(f"len of results : {len(result['rec'][0])}")

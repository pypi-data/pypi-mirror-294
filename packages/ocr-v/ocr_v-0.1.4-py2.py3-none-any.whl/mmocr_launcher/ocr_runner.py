...
def run_ocr(test_image, det, det_weights, rec, rec_weights):
    init_args = {'det': 'configs/textdet/dbnetpp/dbnetpp_resnet50-dcnv2_fpnc_1200e_icdar2015.py',
                 ...
                 'rec_weights': 'recognition_v1.0.0_240726_release.pth', 'kie': None, 'kie_weights': None,
                 'device': None}
    ...
    xyxy_dt_boxes = []
    xyxy_dt_boxes_layout = [] # FOR LAYOUT ANALYSIS
    for idx, box_val in enumerate(sorted_text_det_bboxes):
        x, y, w, h = box_val
        xyxy_dt_boxes.append([x, y, x + w, y + h])
        xyxy_dt_boxes_layout.append([x, y, w, h])
    return xyxy_dt_boxes, sorted_text_recognitions, xyxy_dt_boxes_layout
...
if __name__ == '__main__':
    ...
    dt_boxes, rec_res_origin, dt_boxes_layout = run_ocr(test_image, det, det_weights, rec, rec_weights)
    # print(dt_boxes, rec_res_origin)
    rec_res = [(item, 1) for item in rec_res_origin]
    print(rec_res)

    # serialized_out = json.loads(pd.DataFrame(rec_res).to_json(force_ascii=False, orient='records'))
    # print(serialized_out, type(serialized_out))
    # fuck = json.dumps(rec_res, ensure_ascii=False)

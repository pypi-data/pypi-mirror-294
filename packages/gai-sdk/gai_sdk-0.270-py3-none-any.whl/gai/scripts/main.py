...
def main():
    ...
    parser.add_argument('extra-args', nargs='*', help='Additional arguments for commands')
    ...
    if args.command == "init":
        print("Initializing...by force" if args_dict['force'] else "Initializing...")
        init(force=args_dict['force'])
    elif args.command == "publish":
        if 'extra-args' in args_dict and args_dict['extra-args']:
            if args_dict['extra-args'][0] == "sdk":
                from gai.scripts.gai_publish_sdk import publish_sdk
                publish_sdk(args_dict['extra-args'][1])
    elif args.command == "pull":
        if 'extra-args' in args_dict and args_dict['extra-args']:
            pull(console, args_dict['extra-args'][0])
        else:
            console.print("[red]Model name not provided[/]")
    elif args.command == "docker":
        if 'extra-args' in args_dict and args_dict['extra-args']:
            # gai docker up
            if args_dict['extra-args'][0] == "up":
                docker_up()
            # gai docker down
            elif args_dict['extra-args'][0] == "down":
                docker_down()
            ...
            elif args_dict['extra-args'][0] == "build":
                docker_build(
                    pyproject_path=args_dict['extra-args'][1],
                    repo_name=args_dict['repo-name'],
                    image_name=args_dict['image-name'],
                    dockerfile_path=args_dict['dockerfile-path'],
                    dockercontext_path=args_dict['dockercontext-path'],
                    no_cache=args_dict['no-cache']
                    )
            # gai docker build (pyproject_path)
            elif args_dict['extra-args'][0] == "push":
                docker_push(
                    pyproject_path=args_dict['extra-args'][1],
                    repo_name=args_dict['repo-name'],
                    image_name=args_dict['image-name'])
            elif args_dict['extra-args'][0] == "stop":
                docker_stop(
                    component=args_dict['extra-args'][1]
                    )
            else:

...
class RagClientAsync:
    ...
    async def step_header_async(
        ...
        try:
            mode = 'rb'
            with open(req.FilePath, mode) as f:
                files = {
                    "file": (os.path.basename(req.FilePath), f, "application/pdf"),
                    "req": (None, json.dumps(req), "application/json"),
                }
    ...
    async def index_document_async(
        ...
        try:
            ...
            if not os.path.exists(req.FilePath):
                raise Exception(f"File not found: {req.FilePath}")
            with open(req.FilePath, mode) as f:
                files = {
                    "file": (os.path.basename(req.FilePath), f, "application/pdf"),
                    "req": (None, json.dumps(req), "application/json"),
                ...
                if async_callback:
                    ...
                    listener = StatusListener(ws_url, req.CollectionName)
                    listen_task=asyncio.create_task(listener.listen(async_callback))

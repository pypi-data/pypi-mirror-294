...
def _get_var_stat_from_poly_or_circle(date_list: list[dt.date], poly, ref_pos: Union[tuple[float], list[float]], radius: float,
    ...
    if parallel:
        ...
        with Pool() as pool:
            ...
            if pbar_tf:
                pool_loop = tqdm(pool.istarmap(find_var_in_day, args), total=len(date_list), desc=f'Stats: {file_tag}', unit='day')
            else:
                pool_loop = pool.starmap(find_var_in_day, args)
            for day_pnt_list_high, day_pnt_list_low in pool_loop:

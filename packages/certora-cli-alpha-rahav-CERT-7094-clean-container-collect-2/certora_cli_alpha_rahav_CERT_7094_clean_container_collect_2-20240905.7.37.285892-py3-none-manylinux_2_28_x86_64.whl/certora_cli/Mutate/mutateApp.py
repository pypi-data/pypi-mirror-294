...
class MutateApp:
    ...
    def collect(self) -> bool:
        ...
        try:
            ...
            raise RuntimeError("Original timed out")
        ...
        # [rahav] not sure this check is needed (it run every time even before all runs completed)
        ...
        # at this point we know that all runs (orig + mutants) completed
    ...
    def get_results(link: Optional[str], fetcher: WebFetcher) -> Optional[List[RuleResult]]:
        ...
        if job_data is None:
            raise Util.CertoraUserInputError(f"failed to get job data for {link}")
        ...
        if progress_json is None:
            raise Util.CertoraUserInputError("Could not get progress object")
        top_level_rules = get_top_level_rules(progress_json)
        if top_level_rules is None:
            raise Util.CertoraUserInputError("Could not get tree view object")
        rule_results = []
        ...
        for rule in top_level_rules:
            ...
            if MConstants.CHILDREN not in rule:
                raise Util.CertoraUserInputError(f"Bad format for a rule {rule}")
            ...
            if len(leaf_statuses) == 0:
                raise Util.CertoraUserInputError("Got no rule results")
                ...
                raise Util.CertoraUserInputError("Encountered a new unknown status which isn't FAIL, UNKNOWN, TIMEOUT, "

...
class CephAdapter(WorkspaceAdapter):
    ...
    def upload_stream(
        self, workspace_name: str, stream: Any, file_name: str, tenant_name: str = ""
    ):
        tenant_name = f"{tenant_name}:" if tenant_name else ""
        self.s3_client.put_object(
            Bucket=f"{tenant_name}{workspace_name}", Key=file_name, Body=stream
        )

    def delete_file(self, workspace_name: str, file_name: str, tenant_name: str = ""):
        tenant_name = f"{tenant_name}:" if tenant_name else ""
        self.s3_client.delete_object(
            Bucket=f"{tenant_name}{workspace_name}", Key=file_name
        )

    def download_file(
        self, workspace_name: str, file_name: str, path: str, tenant_name: str = ""
    ):
        tenant_name = f"{tenant_name}:" if tenant_name else ""
        self.s3_client.download_file(f"{tenant_name}{workspace_name}", file_name, path)

    def download_stream(
        self, workspace_name: str, file_name: str, tenant_name: str = ""
    ):
        tenant_name = f"{tenant_name}:" if tenant_name else ""
    ...
    def get_workspace(self, workspace_name: str, tenant_name: str = ""):
        tenant_name = f"{tenant_name}:" if tenant_name else ""
        return self.s3_client.get_bucket(Bucket=f"{tenant_name}{workspace_name}")

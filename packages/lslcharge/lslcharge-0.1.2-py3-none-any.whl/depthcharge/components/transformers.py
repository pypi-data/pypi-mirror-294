...
from .encoders import FloatEncoder, PeakEncoder, PositionalEncoder, RecyclingPeakEncoder
...
class RecyclingSpectrumEncoder(torch.nn.Module):
    """A Transformer encoder for input mass spectra.
    ...
    def __init__(
        ...
        if peak_encoder:
            self.peak_encoder = RecyclingPeakEncoder(
                dim_model,
                dim_intensity=dim_intensity,
            )
        else:
    ...
    def forward(self, spectra, embeddings):
        """The forward pass.
        ...
        embeddings : torch.Tensor of shape (n_spectra, dim_embeddings), optional
        ...
        mask = torch.cat(mask, dim=1)
        peaks = self.peak_encoder(spectra, embeddings)

        ...
        for ele in cursor.fetchall():
            for i, val in enumerate(ele):
                ...
                for start_index, end_index, timeline in content_list:
                    ...
                    document = Document('', relation_key.medical_institution_code, relation_key.medical_record_no,
...
def get_document_by(cursor, document_uuid):
    document_list = get_document_list_by(cursor, [document_uuid])
    if document_list:
        return document_list[0]
    return None


def get_document_list_by(cursor, document_uuid_list):
    query_sql = ','.join(["'{}'".format(uuid) for uuid in document_uuid_list])
    cursor.execute("select nc_uuid, nc_medical_institution_code, nc_medical_record_no, nc_discharge_time, nc_admission_time,"
                   "nc_table_name, nc_column_name, nc_table_uuid, nc_page, nc_page_uuid, nc_start_index, nc_end_index,"
                   "nc_content, nc_md5_sum, nc_global_id from nc_document where nc_uuid in ()".format(query_sql))
    document_list = []
    for ele in cursor.fetch_all():
        document = Document(ele[0], ele[1], ele[2], ele[3], ele[4], ele[5], ele[6], ele[7], ele[8], ele[9], ele[10], ele[11], ele[12], ele[13])
...
def give_global_id(cursor, document_list, source_table_name='nc_mpi_relation'):
    cursor.execute('select nc_medical_institution_code, nc_medical_record_no, nc_discharge_time, nc_global_id from ' +
                   source_table_name)
    relation_key_dict = {}
    for ele in cursor:
...
def extract_all_relation_key(cursor):
    cursor.execute("select nc_medical_institution_code, nc_medical_record_no, nc_discharge_time, nc_admission_time "
                   "from nc_medical_record_first_page where nc_hedge = 0 and nc_data_status != 99 and "
                   "nc_data_report_type = 1 order by nc_rid")
    relation_key_list = []
    for ele in cursor.fetchall():
...
def extract_relation_key_list_by_global_id(cursor, global_id):
    cursor.execute("select nc_medical_institution_code, nc_medical_record_no, nc_discharge_time, nc_admission_time "
                   "from nc_mpi_relation where nc_global_id = '{}' order by nc_admission_time".format(global_id))
    relation_key_list = []
    for ele in cursor.fetchall():
        relation_key_list.append(RelationKey(ele[0], ele[1], ele[2].strftime('%Y-%m-%d %H:%M:%S'), ele[3].strftime('%Y-%m-%d %H:%M:%S')))
    return relation_key_list

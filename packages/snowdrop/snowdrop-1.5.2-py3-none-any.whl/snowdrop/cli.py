...
version_str = 'snowdrop [version {}]'.format(snowdrop.__version__)
...
def convert_file(filename, renderer):
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as fin:
            rendered = snowdrop.markdown(fin, renderer)
...
def interactive(renderer):
    ...
    while True:
        try:
            ...
            print('\n' + snowdrop.markdown(contents, renderer), end='')
            more = False
            contents = []
...
def _print_heading(renderer):
    ...
    if renderer is not snowdrop.HtmlRenderer:

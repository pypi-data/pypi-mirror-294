...
def get_rerank_scores(
    ...
    """返回文本相似度得分列表。"""
    _logger.info("get_rerank_scores start...")
    ...
    for document in documents:
        if len(document) > max_size:
            _logger.warning(
                "get_rerank_scores: the query string exceeds the limit, max_size=%s, query=%s, document=%s",
                max_size,
                query,
                document,
            )
    ...
    _logger.info(
        "get_rerank_scores request fnished: url=%s, headers=%s, data=%s, response=%s",
        url,
        headers,
        data,
        response.text,
    ...
    _logger.info(
        "get_rerank_scores fnished: used_time=%s",
        used_time,
    )
    if not result:

...
or `tense.fencord` module is imported. `tense.primary` module holds these 2 special functions: `abroad()` and  \\
...
def _reckon_prepend_init(*countables: _ReckonTypePre[_T]):
    import typing as tp, io, collections as ct, tense.tcs as __tcs, tkinter as tk
    i = 0
    for e in countables:
        if isinstance(e, __tcs.ReckonOperable): i += e.__reckon__()
...
def _abroad_prepend_init(value1: _AbroadValue1Pre[_T1], /, value2: _AbroadValue2Pre[_T2] = None, modifier: _AbroadModifierPre[_T3] = None):
    import math, tense.tcs as __tcs
    conv: list[int] = []
    [v1, v2, m] = [0, 0, 0]
    # v1 (value1)
...
class Math:
    ...
    """
    @classmethod
    def __verify(x: _FloatOrInteger, y: __tcs.Optional[_FloatOrInteger] = None):
...
class YamiTk:
    ...
    def __init__(self, screenName: __tcs.Optional[str] = None, baseName: __tcs.Optional[str] = None, className: str = "Tk", useTk: bool = True, sync: bool = False, use: __tcs.Optional[str] = None) -> None:
        self.__loc_tk = tk.Tk(
...
# not for export
del os, tcs, json, tk, ra, wa, tp, io, ct, RAM, _RAM

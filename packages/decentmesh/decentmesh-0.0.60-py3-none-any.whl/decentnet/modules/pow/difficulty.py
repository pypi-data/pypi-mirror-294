...
from decentnet.consensus.difficulty_byte_sizing import T_COST_BYTE_SIZE, M_COST_BYTE_SIZE, P_COST_BYTE_SIZE, \
    N_BITS_BYTE_SIZE, HASH_LEN_CHARS_BYTE_SIZE, COMPRESSION_LEVEL_BYTE_SIZE, M_COST_CHUNK_SLICE, \
    P_COST_CHUNK_SLICE, N_BITS_CHUNK_SLICE, HASH_LEN_CHARS_CHUNK_SLICE, COMPRESSION_LEVEL_CHUNK_SLICE
...
class Difficulty:
    ...
    def from_bytes(cls, difficulty_bytes: bytes | memoryview):
        ...
        return cls(t_cost, m_cost, p_cost, n_bits, hash_len_chars, compression_level)
    ...
    def __iter__(self):
        yield from [self.t_cost, self.m_cost, self.p_cost, self.n_bits,
                    self.hash_len_chars, self.compression_level]

    def __str__(self):
        return f"{self.t_cost}:{self.m_cost}:{self.p_cost}:{self.n_bits}:{self.hash_len_chars}:{self.compression_level}"

...
def OpenAI_tokenizer(prompt, openai_key="", model="gpt-3.5-turbo"):
    try:
        # Set the OpenAI API key
        openai.api_key = openai_key

        # Generate a response to count the tokens
        details_response = openai.chat.completions.create(
            model=model,
            messages=[
                {
            ...
            max_tokens=1  # Set a low max_tokens since we're only interested in the token count
        )
        
        # Extract the total token usage from the response
        num_tokens = details_response['usage']['total_tokens']
        
        return CustomResponse(data={"num_tokens": num_tokens}, status_code=200)

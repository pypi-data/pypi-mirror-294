...
def OpenAI_tokenizer(prompt: str, model: str = "gpt-4") -> int:
    """
    This function calculates the number of tokens in a given prompt using the OpenAI API for GPT models.

    Parameters:
    prompt (str): The input text to tokenize.
    model (str): The name of the model's tokenizer to use. Defaults to 'gpt-4'.

    Returns:
    int: The number of tokens in the prompt.
    """
    # Use OpenAI's API to encode the prompt
    response = openai.Completion.create(
        model=model,
        prompt=prompt,
        max_tokens=0  # Set to 0 since we're only interested in the token count
    )

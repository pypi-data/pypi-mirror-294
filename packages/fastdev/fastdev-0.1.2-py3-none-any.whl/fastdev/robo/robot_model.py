...
try:
    ...
    from fastdev.robo.curobo_utils import CustomCudaRobotModel
...
class RobotModel:
    ...
    def __init__(self, config: RobotModelConfig) -> None:
        ...
        if len(self.active_joint_names) == 0:
            self._joint_limits_np: np.ndarray | None = None
        ...
        if self._joint_limits_np is not None:
            self._joint_limits_pt: torch.Tensor | None = torch.from_numpy(self._joint_limits_np).to(self.config.device)
        else:
        ...
        if self.config.enable_curobo:
            robot_cfg = RobotConfig.from_dict(
                ...
                tensor_args=TensorDeviceType(device=self.config.device),  # type: ignore
            )
            self._curobo_model: CustomCudaRobotModel | None = CustomCudaRobotModel(robot_cfg.kinematics)
    ...
    @overload
    def get_joint_limits(self, return_tensors: Literal["np"] = "np") -> np.ndarray | None: ...
    @overload
    def get_joint_limits(self, return_tensors: Literal["pt"] = "pt") -> Tensor | None: ...
    def get_joint_limits(self, return_tensors: Literal["np", "pt"] = "np") -> np.ndarray | Tensor | None:
    ...
    def joint_limits(self) -> np.ndarray | None:
        return self.get_joint_limits("np")
    ...
    def forward_kinematics_curobo(
    ...
    ) -> tuple[Float[torch.Tensor, "b num_links 3"], Float[torch.Tensor, "b num_links 4"]]:
        ...
        if clamp_joint_values and self._joint_limits_pt is not None:
            joint_values = torch.clamp(joint_values, self._joint_limits_pt[:, 0], self._joint_limits_pt[:, 1])

        state: CudaRobotModelState = self._curobo_model.get_state(joint_values)
        return state.links_position, state.links_quaternion  # type: ignore
    ...
    def forward_kinematics(
        ...
        for link_name in self._link_names_topological_order:
    ...
    def forward_kinematics(
        self,
        joint_values: Float[np.ndarray, "... num_dofs"],
        joint_names: Optional[List[str]] = None,
        ...
        batch_input = True
        if joint_names is not None:
            joint_reindex: np.ndarray = np.array(
                [joint_names.index(joint_name) for joint_name in self.active_joint_names], dtype=np.int64
            )
    ...
    @overload
    def get_zero_joint_values(self, return_tensors: Literal["np"] = "np") -> np.ndarray: ...
    @overload
    def get_zero_joint_values(self, return_tensors: Literal["pt"] = "pt") -> Tensor: ...
    def get_zero_joint_values(self, return_tensors: Literal["np", "pt"] = "np") -> np.ndarray | Tensor:
        if return_tensors == "np":

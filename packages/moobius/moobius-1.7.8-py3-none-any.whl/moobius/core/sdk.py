...
class Moobius:
    ...
    async def start(self):
        ...
        self.check_recipients = True # False if the list of recipients is not checked against all characters in the channel, which can make send_message faster but will fail if there are invalid recipients.
    ...
    async def _update_rec(self, recipients, is_m_down, channel_id=None):
        ...
        """
        if self.check_recipients and channel_id and self.service_mode: # Fetch characters only works in service mode.
            ...
            valid_ids = None # None means don't filter, so it reallt should be "all".
        return await self.group_lib.convert_list(self.http_api, recipients, is_m_down, channel_id, valid_ids)

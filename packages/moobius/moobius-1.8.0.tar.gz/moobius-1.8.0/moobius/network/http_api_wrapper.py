...
def failed_json_analyze(response_txt, status, raise_error=True):
    """Error conditions when the response fails to be JSON. Accepts the response text, the status code from the request, and whether to raise an error.
    Raises an Exception if raise_error is True."""
    if raise_error:
...
async def get_or_post(url, is_post, requests_kwargs=None, raise_json_decode_errors=True, aio_strategy=1):
    """
    ...
    Parameters:
      ...
      aio_strategy=1: Different attempts at getting the HTTPS certificates to work.
    ...
    Raises:
      ...
      An Exception if raise_ssl_errors and the certificates are misbehaving.
    """
    async def _process_resp(response):
        """Processes a response. Returns the value or an error condition.
        Raises an Exception if the parent function has raise_json_decode_errors=True and the JSON cannot be decoded."""
        try:
            response_dict = await response.json()
            return response_dict
        except aiohttp.client_exceptions.ContentTypeError:
            response_txt = await response.text()
            return failed_json_analyze(response_txt, response.status, raise_error=raise_json_decode_errors)

    if aio_strategy == 0:
        async with aiohttp.ClientSession() as session:
            async with (session.post if is_post else session.get)(url, **requests_kwargs) as response:
                return await _process_resp(response)
    elif aio_strategy == 1:
        import certifi
        ssl_context = ssl.create_default_context(cafile=certifi.where())
        async with aiohttp.ClientSession() as session:
            async with (session.post if is_post else session.get)(url, ssl=ssl_context, **requests_kwargs) as response:
                return await _process_resp(response)
    elif aio_strategy == 2:
        ssl_context = ssl._create_unverified_context()
        logger.error(f'Unverified session created for {url}! Do not use this setting in production!')
        async with aiohttp.ClientSession() as session:
            async with (session.post if is_post else session.get)(url, ssl=ssl_context, **requests_kwargs) as response:
                return await _process_resp(response)
    elif aio_strategy == 3:
        ...
        raise Exception("_aio_strategy not recognized, must be 0,1,2, or 3.")
...
class HTTPAPIWrapper:
    ...
    def __init__(self, http_server_uri="", email="", password=""):
        ...
        self.aio_strategy = 1 # Try different ones to see what works!
        self.username = email
    ...
    async def _checked_get_or_post(self, url, the_request, is_post, requests_kwargs=None, good_message=None, bad_message="This HTTPs request failed", raise_errors=True):
        ...
        try:
            response_dict = await get_or_post(url, is_post, requests_kwargs=requests_kwargs, raise_json_decode_errors=raise_errors, aio_strategy=self.aio_strategy)
    ...
    async def request_password_reset_code(self):
    ...
    async def reset_password_with_code(self, the_code):
        """Resets the password with whatever is the current password. Returns None. Accepts the code that was emailed to the user (use request_password_reset_code)."""
    ...
    async def delete_account(self):
        """Deletes the account using self's credentials. Mainly used for testing. Returns None."""
        response_dict = await self.checked_post(url=self.http_server_uri + "/auth/delete_account", the_request={"username": self.username, "password":self.password}, requests_kwargs=None, good_message=None, bad_message="Error during signup", raise_errors=True)

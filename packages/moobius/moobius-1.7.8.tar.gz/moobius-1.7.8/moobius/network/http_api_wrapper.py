...
class HTTPAPIWrapper:
    ...
    async def resend_code(self):
        ...
        response_dict = await self.checked_post(url=self.http_server_uri + "/auth/resend_confirmation", the_request={"username": self.username}, requests_kwargs=None, good_message=None, bad_message="Error during signup", raise_errors=True)
        logger.info(f"Made a sign up request for {self.username}.")
    ...
    async def get_password_reset_code(self):
        ...
        response_dict = await self.checked_post(url=self.http_server_uri + "/auth/forgot_password", the_request={"username": self.username}, requests_kwargs=None, good_message=None, bad_message="Error during reset password", raise_errors=True)
        logger.info(f"Requesting a new password for {self.username}.")

    async def reset_password(self, the_code):
        """Updates the password with a new one. Accepts the code that was emailed to the user (use get_password_reset_code)."""

...
def fetch_payer_name_from_api(payer_id, config, primary_endpoint='AVAILITY'):
    ...
    for endpoint_name in endpoint_order:
        try:
            endpoint_url = config['MediLink_Config']['endpoints'][endpoint_name].get('payer_list_endpoint', '/availity-payer-list')
            response = client.make_api_call(endpoint_name, 'GET', endpoint_url, {'payerId': payer_id})
            
            if response is None:
                print("API call to {0} returned no response for Payer ID {1}.".format(endpoint_name, payer_id))
                MediLink_ConfigLoader.log("API call to {0} returned no response for Payer ID {1}.".format(endpoint_name, payer_id), level="ERROR")
                continue
            
            payers = response.get('payers', [])
            if payers:
                payer_name = payers[0].get('displayName', payers[0].get('name'))
                log_message = "Successfully found payer at {0} for ID {1}: {2}".format(endpoint_name, payer_id, payer_name)
                print(log_message)
                MediLink_ConfigLoader.log(log_message, level="INFO")
                return payer_name
            else:
                print("No payer found at {0} for ID {1}. Response: {2}. Trying next available endpoint.".format(endpoint_name, payer_id, response))
                MediLink_ConfigLoader.log("No payer found at {0} for ID {1}. Response: {2}. Trying next available endpoint.".format(endpoint_name, payer_id, response), level="INFO")
        
        except Exception as e:
            error_message = "API call to {0} failed. Exception: {1}. Response: {2}".format(endpoint_name, e, response if response else 'None')
            print(error_message)
            MediLink_ConfigLoader.log(error_message, level="ERROR")

    error_message = "All endpoints exhausted for Payer ID {0}.".format(payer_id)
    print(error_message)

...
while True:
    ...
    custom_image_source = input("Enter the URL or file path of the image (leave blank for no image): ")
    custom_text = input("Enter your custom text (or type 'exit' to end): ")
    
    if custom_text.lower() == 'exit':
        break

    # Load the image based on user input
    image = load_image(custom_image_source) if custom_image_source else None
    ...
    inputs = processor(
        text=[text_prompt], images=[image] if image else None, padding=True, return_tensors="pt"
    )

...
def get_ppas_and_archive(url):
    ...
    for link in soup.find_all('a'):
        ...
        if href and href.startswith('/~'):
            ...
            if which('add-apt-repository') is not None and check_av:
                codename=subprocess.run(["lsb_release", "-sc"])  
                url="http://ppa.launchpad.net/" + str(user) + '/' + str(pack) + "/ubuntu/dists/" + codename + "/"
                response = requests.get(url)
...
def main():
    ...
    parser.add_argument("-f","--file", required=False, help="Output file", metavar="Output file")

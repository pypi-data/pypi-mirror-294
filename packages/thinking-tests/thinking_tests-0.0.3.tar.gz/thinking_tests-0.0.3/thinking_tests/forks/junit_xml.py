"""
Forked* from https://github.com/kyrus/python-junit-xml, specifically from commit 4bd08a272f059998cedf9b7779f944d49eba13a6.
I needed to tweak code a bit to match what I wanted to achieve with thinking-tests.

Major changes are:
- the way we're treating log (as property)
- that we allow for sub-suites

* not really forked - you won't find it in GIT history, because at the same time I needed to tweak vjunit, which got
the same treatment.
"""
...
class TestSuite(object):
    ...
    def build_xml_doc(self, encoding=None):
        ...
        #fixme no support for disabling nor skipping, so this is not calculated correctly
        ...
        test_suite_attributes["tests"] = str(self.count_cases())
        test_suite_attributes["errors"] = str(self.count_errors())
        test_suite_attributes["failures"] = str(self.count_failures())
        test_suite_attributes["time"] = str(self.total_time())
        ...
        if self.log:
            if props_element is None:
                props_element = ET.SubElement(xml_element, "properties")
            log_prop_element = ET.SubElement(props_element, "property")
            log_prop_element.attrib["name"] = "log"
            log_prop_element.text = decode(self.log, encoding)
        ...
        for case in self.test_cases:
            ...
            if case.log:
                ...
                case_log_prop_element = ET.SubElement(case_props_element, "property")
                case_log_prop_element.attrib["name"] = "log"
                case_log_prop_element.text = decode(case.log, encoding)
                test_case_element.append(case_props_element)

        if self.suites:
            suites_element = ET.SubElement(xml_element, "testsuites")
            for subsuite in self.suites:
                subsuite_element = subsuite.build_xml_doc(encoding)
                suites_element.append(subsuite_element)

        return xml_element


    def total_time(self):
        return sum(c.elapsed_sec for c in self.test_cases if c.elapsed_sec) + sum(s.total_time() for s in self.suites)

    def count_cases(self):
        return len(self.test_cases) + sum(s.count_cases() for s in self.suites)

    def count_errors(self):
        return len([c for c in self.test_cases if c.is_error()]) + sum(s.count_errors() for s in self.suites)

    def count_failures(self):
        return len([c for c in self.test_cases if c.is_failure()]) + sum(s.count_failures() for s in self.suites)

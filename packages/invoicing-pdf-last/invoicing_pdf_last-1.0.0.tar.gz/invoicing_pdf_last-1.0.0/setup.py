from setuptools import setup

setup(
  name='invoicing-pdf-last',         # Your package will have this name
  packages=['invoicing'],            # Name the package again
  version='1.0.0',                   # To be increased every time you change your library
  license='MIT',                     # Type of license
  description='This package can be used to convert Excel invoices to PDF invoices.',  # Short description of your library
  author='IBR Nas',                  # Your name
  author_email='ibrnas1986@gmail.com', # Your email
  url='https://example.com',          # Homepage of your library (e.g. github or your website)
  keywords=['invoice', 'excel', 'pdf'],  # Keywords users can search on pypi.org
  install_requires=['pandas', 'fpdf', 'openpyxl'],  # Other 3rd-party libs that pip needs to install
  classifiers=[
    'Development Status :: 3 - Alpha',      # Chose either "3 - Alpha", "4 - Beta" or "5 - Production/Stable"
    'Intended Audience :: Developers',      # Audience for your library
    'Topic :: Software Development :: Build Tools',
    'License :: OSI Approved :: MIT License',  # Type of license
    'Programming Language :: Python :: 3.8',  # Python versions your library supports
    'Programming Language :: Python :: 3.9',
    'Programming Language :: Python :: 3.10',
    'Programming Language :: Python :: 3.11',
    'Programming Language :: Python :: 3.12',
  ],
)

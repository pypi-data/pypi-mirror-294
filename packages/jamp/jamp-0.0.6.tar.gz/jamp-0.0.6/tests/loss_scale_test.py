...
from jmp import loss_scale as jmp
import numpy as np
...
class LossScaleTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        ("DynamicLossScale(2)", jmp.DynamicLossScale, 2.0),
        ("DynamicLossScale(3)", jmp.DynamicLossScale, 3.0),
        ("DynamicLossScale(4)", jmp.DynamicLossScale, 4.0),
    )
    def test_static_loss_scale(self, cls, scale):
        loss_scale = cls(scale)
        tree = {"a": jnp.array(1.0)}
        scaled_tree = {"a": jnp.array(1.0 * scale)}
    ...
    def test_dynamic_loss_scale_tree(self):
        ...
        self.assertEqual(jax.tree_util.tree_map(lambda x: x, loss_scale), loss_scale)
    ...
    def test_dynamic_loss_scale_adjust_increases_on_finite(self, period, factor):
        ...
        loss_scale = jmp.DynamicLossScale(jnp.float32(10), jnp.int32(0), period, factor)
    ...
    def test_dynamic_loss_scale_adjust_reduce_on_non_finite(self, period, factor):
        ...
        for i in range(100):
            ...
            np.testing.assert_allclose(
                loss_scale.loss_scale, max(1, init / (factor ** (i + 1))), rtol=1e-5
            )
            self.assertEqual(loss_scale.counter, 0)
            self.assertEqual(loss_scale.period, period)
            self.assertEqual(loss_scale.factor, factor)

    @parameterized.parameters((20, 2, 0.3125), (30, 3, 0.37), (5.0, 2.0, 0.0))
    def test_dynamic_loss_scale_explicit_min_loss_scale(
        self, period, factor, min_loss_scale
    ):
        grads_finite = jnp.bool_(False)
        init = np.float32(10)
        loss_scale = jmp.DynamicLossScale(
            jnp.asarray(init), jnp.int32(0), period, factor, jnp.asarray(min_loss_scale)
        )
        self.assertLess(init / (factor**100), 1, msg="should cover max(1, S)")
        for i in range(100):
            loss_scale = loss_scale.adjust(grads_finite)
    ...
    def test_all_finite(self, non_finite):
        ...
        self.assertFalse(
            jmp.all_finite({"a": jnp.ones([]), "b": jnp.array(non_finite)})
        )
        self.assertFalse(
            jmp.all_finite({"a": jnp.array(non_finite), "b": jnp.ones([])})
        )

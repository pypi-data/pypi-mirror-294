...
def get_dtype_name(dtype):
    names = {np.float16: "float16", jnp.bfloat16: "bfloat16", np.float32: "float32"}
    return names[dtype]
...
def skip_if_unsupported(dtype):
    ...
    if (platform == "gpu" and dtype == jnp.bfloat16) or (
        ...
        raise unittest.SkipTest(f"{get_dtype_name(dtype)} not supported on {platform}")
...
class PolicyTest(parameterized.TestCase):
    ...
    def assert_dtypes_equal(self, tree_a, tree_b):
        jax.tree_util.tree_map(
            lambda a, b: self.assertEqual(a.dtype, b.dtype), tree_a, tree_b
        )
    ...
    def test_policy_cast_to_param(self, dtype, np_):
        ...
        tree = {"a": np_.ones([])}
        self.assert_dtypes_equal(policy.cast_to_param(tree), {"a": np_.ones([], dtype)})
    ...
    def test_policy_cast_to_compute(self, dtype, np_):
        ...
        tree = {"a": np_.ones([])}
        self.assert_dtypes_equal(
            policy.cast_to_compute(tree), {"a": np_.ones([], dtype)}
        )
    ...
    def test_policy_cast_to_output(self, dtype, np_):
        ...
        tree = {"a": np_.ones([])}
        self.assert_dtypes_equal(
            policy.cast_to_output(tree), {"a": np_.ones([], dtype)}
        )
    ...
    @parameterized.parameters(
        ("float16", np.float16), ("float32", np.float32), ("bfloat16", jnp.bfloat16)
    )

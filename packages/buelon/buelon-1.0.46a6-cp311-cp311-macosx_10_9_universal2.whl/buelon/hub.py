...
class HubClient:
    ...
    def send_request(self, method, data: bytes, timeout: int | float = .1 * 60, increments: int | float | Callable = 5, attempts: int = 4):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as conn:
                conn.connect((self.host, self.port))
                conn.settimeout(.1*60)

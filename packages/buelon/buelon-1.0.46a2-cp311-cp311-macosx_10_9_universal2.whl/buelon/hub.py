...
def upload_pipe_code(code: str, lazy_steps: bool = False):
    ...
    for _step in steps.values():
        ...
        if len(chunk) >= chunk_size:
            print('uploading', len(chunk), 'steps', chunk[0])
            set_steps(chunk)
    ...
    if chunk:
        print('uploading', len(chunk), 'steps', chunk[0])
        set_steps(chunk)
        print(client.upload_steps([s.to_json() for s in chunk], len(chunk) * [buelon.core.step.StepStatus.queued.value]))
        chunk.clear()
...
class HubServer:
    ...
    def _execute_request(self, method, payload):
        if method == Method.DONE:
            ...
            print('setting', len(step_jsons), 'steps')
            print('with', len(status_values), 'statuses')

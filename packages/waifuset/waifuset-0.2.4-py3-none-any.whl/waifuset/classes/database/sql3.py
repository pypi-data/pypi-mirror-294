...
class SQL3Table(object):
    def __init__(self, database=None, name=None):
        self.database = database if issubclass(type(database), SQL3Database) else SQL3Database(database)
    ...
    def select(self, column, statement, postprocess=False, distinct=False):
        self.cursor.execute(f"SELECT * FROM {self.name} WHERE {column} {statement};")
        return [get_row_dict(row, header=self.header) for row in self.cursor.fetchall()] if postprocess else self.cursor.fetchall()

    def select_func(self, func, *args, postprocess=False):
        ...
        self.cursor.execute(f"SELECT * FROM {self.name} WHERE {funcname}({arg_str}) = 1")
        return [get_row_dict(row, header=self.header) for row in self.cursor.fetchall()] if postprocess else self.cursor.fetchall()
    ...
    def sample(self, n=1, randomly=True, postprocess=False):
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            ...
            return get_row_dict(self.cursor.fetchone(), header=self.header)
        elif isinstance(key, int):
            self.cursor.execute(f"SELECT * FROM {self.name} LIMIT 1 OFFSET {key}")
            return get_row_dict(self.cursor.fetchone())
            ...
            if step is None or step == 1:
                processed_rows = [get_row_dict(row, header=self.header) for row in rows]
            else:
                processed_rows = [get_row_dict(rows[i], header=self.header) for i in range(0, len(rows), step)]
            return processed_rows
        else:
            raise KeyError(f"Invalid key type: {type(key)}")

    def chunks(self, n, postprocess=False):
        r"""
        ...
        return [[get_row_dict(row, header=self.header) if postprocess else row for row in rows[i:i + size]] for i in range(0, len(rows), size)]

    def chunk(self, i, n, postprocess=False):
        r"""
        ...
        rows = self.cursor.fetchall()
        size = int(math.ceil(len(rows) / n))
        return [get_row_dict(row, header=self.header) if postprocess else row for row in rows[i * size:(i + 1) * size]]

    def splits(self, n, postprocess=False):
        r"""
        ...
        self.cursor.execute(f"SELECT * FROM {self.name}")
        rows = self.cursor.fetchall()
        return [[get_row_dict(row, header=self.header) if postprocess else row for row in rows[i:i + n]] for i in range(0, len(rows), n)]

    def split(self, i, n, postprocess=False):
        r"""
        Get the i-th split of the table.
        """
        self.cursor.execute(f"SELECT * FROM {self.name}")
        rows = self.cursor.fetchall()
        return [get_row_dict(row, header=self.header) if postprocess else row for row in rows[i * n:(i + 1) * n]]
...
class SQL3Database(object):
    def __init__(self, fp=None):
        self.path = fp or ':memory:'
        self.conn = sqlite3.connect(self.path, check_same_thread=False)  # auto-commit
    ...
    def create_table(self, name, col2type: Dict[str, type], exists_ok=False, primary_key=None):
        ...
        col2type_str = ', '.join([f"{k} {PY2SQL3.get(v, 'TEXT')}" for k, v in col2type.items()])
        if primary_key:
            col2type_str += f", PRIMARY KEY ({primary_key})"

...
class BUPT:
    ...
    def init():
        ...
        if 'Course' not in config:
            config['Course'] = {}
            print("如果想抢多个课，请用空格隔开~")
            config['Course']['public_course'] = input("输入想要抢的公选课:").strip().replace(" ", ",")
            BUPT.public_course_to_grab_list = config['Course']['public_course'].split(",")
            config['Course']['required_course'] = input("输入想要抢的必修课: ").strip().replace(" ", ",")
            BUPT.required_course_to_grab_list = config['Course']['required_course'].split(",")
            config['Course']['optional_course'] = input("输入想要抢的选修课:").strip().replace(" ", ",")

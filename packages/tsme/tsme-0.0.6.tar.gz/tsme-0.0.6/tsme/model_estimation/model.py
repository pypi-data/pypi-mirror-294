...
def _label_to_latex(label):
    ...
    def convert_product(term):
        ...
        exponents = [factors.count(factor) for factor in list(unique_factors)]
        output = ""
        for i, factor in enumerate(list(unique_factors)):
            index = re.findall(r"\[\s*\+?(-?\d+)\s*\]", factor)[0]  # could be done outside this loop
            exponent = exponents[i]
            factor_proper = "u_{" + f"{index}" + "}^{" + f"{exponent}" + "}"
    ...
    if label.startswith("1"):
        return "$" + label + "$"
    elif label.startswith("u[") and not ("diff" in label):
        return "$" + convert_product(label) + "$"
    elif label.startswith("u[") and "diff" in label:
...
class Model:
    ...
    def print_library(self, sigma=None, latex=False):
        """
        ...
        if self.sigma is None:
            ...
            print(tabulate(zip(range(len(self.lib_strings)), strings, *sigma_loc), headers=headers, tablefmt="github"))
    ...
    def optimize_sigma(self, lamb=0.1, thres=0.01, error="BIC",
                       simulate=False, backend="train", loss=None, with_delay=False, **kwargs):
        """
        ...
        if backend == "train":
            ...
            if not with_delay:
                optimizer, sigma_opt = optimize_threshold(self.lot, self.sol[:, :-1], self.sol_dot, self.time[:-1],
                                                      loss=loss, **kwargs)
            elif with_delay:
                optimizer, sigma_opt = optimize_threshold_with_delay(self, loss=loss, **kwargs)

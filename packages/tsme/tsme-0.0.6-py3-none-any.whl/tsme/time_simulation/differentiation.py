    ...
    def __init__(self, space_dict, boundary="periodic", method="pseudo_spectral"):
        ...
        if "y" in space_dict.keys() and "z" in space_dict.keys():
            ...
            self.convolve = lambda k, u: (
                        fftconvolve(k, u, mode="same") * (self.x[1] - self.x[0]) * (self.y[1] - self.y[0])
                        * (self.z[1] - self.z[0]))
            ...
            if method == "finite_difference":
                ...
                self.dd_dxdy = None
            self.div = self.d_dx
            self.convolve = lambda k, u: (fftconvolve(k, u, mode="same") * (self.x[1] - self.x[0]))

...
class GetEntropy:
    ...
    def rank_patches_for_diversity(
        ...
        if num_unique_data_points == 1:
            # Pull out two random patches
            random_samples = random.sample(original_patches, 2)
            patches_with_labels = list(zip(random_samples, [0, 1]))  # Assign different labels to the two samples
            return {
                'patches_with_labels': patches_with_labels,
                'n_clusters': 1,
                'n_components': n_components,
                'closest_samples_idx': [0, 1],  # Indices for the random samples
                'ordered_samples_idx': [0, 1],
                'silhouette_avg': None  # Silhouette score doesn't make sense here
            }

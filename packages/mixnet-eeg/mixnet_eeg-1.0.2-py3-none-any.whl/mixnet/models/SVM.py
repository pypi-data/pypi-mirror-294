...
class SVM:
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        clf = GridSearchCV(estimator=SVC(), param_grid=self.tuned_parameters, cv=pds, scoring = 'accuracy', n_jobs=-1)
        start = time.time()
        clf.fit(X_all , y_all)
        end = time.time()
    ...
    def predict(self, X_train, y_train, X_test, y_test):
        ...
        classifier.fit(X_train, y_train)
        y_true, y_pred, y_prob = y_test, classifier.predict(X_test), classifier.predict_proba(X_test)
        print(classification_report(y_true, y_pred))

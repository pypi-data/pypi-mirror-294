...
def init():
    global CURRENT_IP, INSIDE_INSTANCE
    ...
    def run(args):
        ...
        if not os.path.exists(device_id_file):
            try:
                ...
                raise click.ClickException(
                    "Unable to create device identity file. Please report this error to the developers"
                )
...
def status():
    ...
    for instance_id, metadata in instances.items():
        ...
        instances_table.add_row(
            ...
            Text(
                metadata["status"],
                style="white" if CURRENT_IP == ip_entry else status_color,
            ),
            ip_entry,
            metadata["createdAt"],

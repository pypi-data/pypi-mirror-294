...
class SPTokenizer:
    ...
    def __init__(self, model_path: str):
        ...
        if not os.path.isfile(model_path):
            error_msg = f"{model_path} not found."
            raise FileNotFoundError(replace_invalid_characters(error_msg))
        ...
        if self.sp_model.vocab_size() != self.sp_model.get_piece_size():
            error_msg = (
                f"Vocabulary size does not match piece size! The vocabulary size is "
                f"{self.sp_model.vocab_size()}, the piece size is {self.sp_model.get_piece_size()}"
            )
            raise AssertionError(replace_invalid_characters(error_msg))
    ...
    def encode(self, s: str, bos: bool = False, eos: bool = False) -> List[int]:
        if not isinstance(s, str):
            error_msg = f"Variable s must be a string. the type of s is {type(s).__name__}"
...
class ChatGLM2Tokenizer(PreTrainedTokenizer):
    ...
    def get_command(self, token):
        ...
        if token not in self.tokenizer.special_tokens:
            error_msg = f"{token} is not a special token for {self.name}"
            raise KeyError(replace_invalid_characters(error_msg))
    ...
    def save_vocabulary(self, save_directory, filename_prefix=None):
        ...
        if os.path.isdir(save_directory):
            try:
                ...
                raise KeyError("vocab_file is invalid") from e
        else:
        ...
        with os.fdopen(os.open(vocab_file, DEFAULT_FLAGS, DEFAULT_MODES), "wb") as writer:
    ...
    def _convert_id_to_token(self, index):
        ...
        if index >= self.vocab_size:
            ...
            raise IndexError(replace_invalid_characters(error_msg))

'''
Energy Efficiency Targeting Tool Copyright (c) 2018, The Regents of the University of California, 
...
class LEAN_FIMs:
    ...
    def set_targets(self, target_level):
        ...
        if target_level == 1:
            for i in range(0, self.n):
                ...
                elif beta_name == 'beta_betc': # cooling change-point
                    # if coefficient is cooling change-point coefficient => the higher the better => get max value
                    self.targets[i] = max(self.benchmark_medians.iloc[i] - self.benchmark_stdevs.iloc[i], self.site_coeffs[i])
                else:
                    # for other coefficients => the smaller the better => get min value
                    # however for the heating slope coefficient whose the value is negative => the higher the better
                    # if the heating slope median sign is flipped
                    self.targets[i] = min(self.benchmark_medians.iloc[i] + self.benchmark_stdevs.iloc[i], self.site_coeffs[i]) # original code
            ...
            for i in range(0, self.n):
                ...
                if np.isnan(self.site_coeffs[i]):
                    ...
                    if beta_name == 'beta_betc':
                        self.targets[i] = max(self.benchmark_medians.iloc[i], self.site_coeffs[i])
                    else:
                        self.targets[i] = min(self.benchmark_medians.iloc[i], self.site_coeffs[i]) # original code
            ...
            for i in range(0, self.n):
                ...
                if np.isnan(self.site_coeffs[i]):
                    ...
                    if beta_name == 'beta_betc':
                        self.targets[i] = max(self.benchmark_medians.iloc[i] + (0.5 * self.benchmark_stdevs.iloc[i]), self.site_coeffs[i])
                    else:
                        self.targets[i] = min(self.benchmark_medians.iloc[i] - (0.5 * self.benchmark_stdevs.iloc[i]), self.site_coeffs[i]) # original code
    ...
    def FIM_recommendations(self, save_file=True):
        ...
        rows = ['Increase Cooling Setpoints', 'Decrease Heating Setpoints',
                'Reduce Equipment Schedules', 'Ensure Adequate Ventilation Rate',
        ...
        if (self.beth - self.beth_targ) >= (threshold * self.beth_targ):
            count = count + 1
        if count >= 2:
            # self.FIM_table.loc['Decrease Ventilation'] = 'X' # old version
            self.FIM_table.loc['Ensure Adequate Ventilation Rate'] = 'X'
        ...
        if ovrd_thres:
            ...
            threshold = 0.1
        if self.hdd > 0 and (self.hdd - self.hdd_targ) >= (threshold * self.hdd_targ):
            # self.FIM_table.loc['Increase Heating System Efficiency'] = 'X' # original version
            self.FIM_table.loc['Increase Heating System Efficiency'] = 'X' # original version

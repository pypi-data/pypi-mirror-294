...
class VideoQueueObjectGenerator:
    ...
    def _generate_sr1_object(
        ...
        if len(self.sr01_queue) == 3:
            ...
            logger.debug(f">>> put enqueue {frame_id-1}")
            # 1799 1800 1800
        
        if frame_id == int(self.meta_data["nb_frames"]):
            # for i in range(1, 2):
                # self.sr01_queue.popleft()
            self.sr01_queue.append(frame)
            ...
            enqueue.put(input_io_data)
            logger.debug(f">>> put enqueue {frame_id}")
            # 1799 1800 1800
    ...
    def _generate_sr2_object(
        ...
        if frame_id == 1:
            for _ in range(0,2): # [1,1]
                self.sr02_queue.append(frame)
        
        # if frame_id > 3:
        #     self.sr02_queue.popleft()
        
        self.sr02_queue.append(frame) # [1,1,1,X,X]
        ...
        if len(self.sr02_queue) == 5:
            ...
            enqueue.put(input_io_data)
            logger.debug(f">>> put enqueue {frame_id-2}")
        ...
        if frame_id == int(self.meta_data["nb_frames"]):
            for i in range(1, 3):
                ...
                enqueue.put(input_io_data)
                logger.debug(f">>> put enqueue {frame_id-2+i}")

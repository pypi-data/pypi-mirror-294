...
class pkce:
    ...
    def oauth_logi(self, request):
        code_verifier = request.generate_code_verifier()

        code_challenge = request.generate_code_challenge(code_verifier)
    ...
    def pkce(self, request):
        ...
        if not code or not state or not code_verifier:
            return HttpResponse("Missing authorization code, state parameter, or code verifier.", status=400)

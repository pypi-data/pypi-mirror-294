...
class WebApp(AbstractApp):
    ...
    def __init__(self, base_directory) -> None:
        ...
        self.app_module_directory = f'{base_directory}/app'
    ...
    def _load_base_app(self) -> BaseApp:
        ...
        base_app_module = base_app[0].replace(f'{path}/', '').replace(f'{path}\\', '').replace('.py', '')
...
def start(base_directory):
    WebApp(base_directory).run()

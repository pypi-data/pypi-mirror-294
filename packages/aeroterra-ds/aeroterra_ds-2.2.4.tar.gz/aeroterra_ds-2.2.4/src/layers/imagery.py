...
from .common import get_objectid_field_aux
...
def get_imagery_tiles(layer: ImageryLayer,
    ...
    Parameters:
        ...
        - query (Optional): Query for the attributes to match. By
            default in '1=1'
        - geometry_post (Optional): If the geometry intersection should
            be done after the query. By default in True.
        - out_crs (Optional): The crs to receive the geometries in
    """
    geo_filter = None
    if not geometry_post:
    ...
    basic_where = query
    try:
        new_data = layer.query(where = basic_where, geometry_filter=geo_filter, out_sr=out_crs)
        ...
        while len(new_data) > 0:
            ...
            new_data = layer.query(where = new_where, geometry_filter=geo_filter, out_sr=out_crs)
            new_data = parse_features_to_gdf(new_data)
    except Exception as err:
        if str(err).find("Invalid") >= 0:
            raise Exception(f"Invalid Query. Query Done: where = {basic_where}, geometry_filter={geo_filter}, out_sr={out_crs}")
        else:
            raise err

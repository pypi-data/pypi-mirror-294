...
class DataCollatorForGPLinker:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Mapping:
        ...
        batch_head_labels = torch.zeros(bs, self.num_labels, max_spo_num, 2, dtype=torch.long)
        batch_tail_labels = torch.zeros(bs, self.num_labels, max_spo_num, 2, dtype=torch.long)
...
class GPLinkerForRelTokenizer(PreTrainedTokenizerBase):
...
class BertGPLinkerForRelTokenizer(BertTokenizerFast, GPLinkerForRelTokenizer):
    ...


class RoFormerGPLinkerForRelTokenizer(RoFormerTokenizerFast, GPLinkerForRelTokenizer):
    ...

...
def search(pattern: str, sequence: str):
...
def process_train(ds: "Dataset", predicate2id: dict):
    def judge(example):
        spo_list = []
...
def load_rel_train_dev_dataset(
    tokenizer: "PreTrainedTokenizer",
    dataset_dir: Union[str, Path],
    train_file: Union[str, Path],
    validation_file: Union[str, Path] = None,
    text_column_name: str = "text",
    label_column_name: str = "spo_list",
    ...
    labels = {label["predicate"] for column in dataset["train"][label_column_name] for label in column}
    labels = sorted(labels)
    label2id = {l: int(i) for i, l in enumerate(labels)}

    task_name = str(tokenizer.__class__.__name__)[:20]
    train_dataset = process_train(dataset["train"], predicate2id=label2id)
    ...
    def process_dev(example):
        spoes = [
            [spo["predicate"], spo["subject"], spo["object"]]
            for spo in example["spo_list"]
        ]

...
class NumberBase(int):
    ...
    @staticmethod
    def from_base(base: Iterable, base_map: Union[Iterable[object], Bases], null=None) -> "NumberBase":
        """
        Converts a number from a custom base system to a `NumberBase` instance.

        This function takes a number represented in a custom base system and converts it into an integer value, 
        which is then used to create a `NumberBase` instance.

        Args:
            base (Iterable): The number represented as an iterable of digits in the custom base system. 
                             The digits should match the symbols in the `base_map`.
            base_map (Union[Iterable[object], Bases]): The mapping of digits or characters that represent the base system.
                                                       This can be an iterable (e.g., a list or tuple) or a `Bases` enum.
            null (object, optional): The representation of zero in the custom base system. Defaults to None.

        Returns:
            NumberBase: A `NumberBase` instance representing the converted integer value in the given base system.

        Example:
            If you have a binary number represented as [1, 0, 1] and the corresponding base_map is [0, 1], 
            this function will convert it to a `NumberBase` instance representing the integer value 5.

        Raises:
            ValueError: If any digit in `base` is not found in `base_map`.
        """
        num = 0
        base_map = tuple(base_map)
        b = len(base_map)
        for pos, digit in enumerate(reversed(base)):
            num += base_map.index(digit)*b**pos
        return NumberBase(num, base_map, null)   
    ...
    def __pow__(self, value, mod=None):
        ...
        if v == NotImplemented:
            raise TypeError(f"Value and mod not supported: {value}, {1/2}")
        if v >= 0:
...
def _main():
    ...
    print(repr(num+1))
    num **= 2
    print(repr(NumberBase.from_base("Bonjour", "qwertzuiopasdfghjklyxcvbnmB")))

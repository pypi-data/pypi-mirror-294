...
from aihub.models.run_query_request_source_app_inner import RunQueryRequestSourceAppInner
try:
...
class RunQueryRequest(BaseModel):
    ...
    model: Optional[StrictStr] = Field(default='multistep-lite', description="The model to use to answer the query. Supports the multistep model (`multistep-lite`) and research mode (`multistep`), a more powerful but slower variant of the multistep model that's suited to complex reasoning queries. See [Choosing a model](/overview/models/) for details.")
    ...
    source_app: List[RunQueryRequestSourceAppInner] = Field(description="List of source information.")
    __properties: ClassVar[List[str]] = ["query", "model", "include_source_info", "source_app"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in source_app (list)
        _items = []
        if self.source_app:
            for _item in self.source_app:
                if _item:
                    _items.append(_item.to_dict())
            _dict['source_app'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "query": obj.get("query"),
            "model": obj.get("model") if obj.get("model") is not None else 'multistep-lite',
            "include_source_info": obj.get("include_source_info") if obj.get("include_source_info") is not None else False,
            "source_app": [RunQueryRequestSourceAppInner.from_dict(_item) for _item in obj.get("source_app")] if obj.get("source_app") is not None else None
        })
        return _obj

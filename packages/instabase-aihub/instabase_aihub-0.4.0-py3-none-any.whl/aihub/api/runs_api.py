...
class RunsApi:
    ...
    def get_run_results(
        ...
        :type run_id: str
        :param include_review_results: Whether to include human review details in the results. When set to `true`, details such as review status and edits at the run or document level and the extracted field level are included.  The following values are returned at the run or document level:  - `review_completed` - `files/documents/review_completed` - `files/documents/class_edit_history` - `files/documents/class_edit_history/timestamp` - `files/documents/class_edit_history/user_id` - `files/documents/class_edit_history/modifications` - `files/documents/class_edit_history/modifications/message`  The following values are returned at the extracted field level:  - `edit_history` - `files/edit_history/timestamp` - `files/edit_history/user_id` - `files/edit_history/modifications` - `files/edit_history/modifications/message`  See the response schema for details and descriptions.  <Note>The review process can include manually correcting values. The Run results endpoint doesn't support returning the original and corrected values.</Note> 
    ...
    def run_app(
        ...
        Run an app by its name or app ID. The input for the run is specified using a batch ID or an input file path.  To run deployments by API, use the [Run deployment endpoint](/api-sdk/api-reference/runs/run-deployment/).  <Note>Any specified input or output is validated against the context set by the `IB-Context` header. For example, if the context is set to your community account, but the batch ID used as input for the run is stored in your organization, the call fails.</Note> 
    ...
    def run_deployment(
        self,
        deployment_id: Annotated[StrictStr, Field(description="The deployment's ID. <Tip>You can find a deployment's ID by opening the deployment in AI Hub and looking at the site URL, such as https<span>://</span>aihub.instabase.com/deployments/**01902d6f-bb35-74cb-bd27-c09b38bbf20a**/runs.</Tip> ")],
        ...
        :param deployment_id: The deployment's ID. <Tip>You can find a deployment's ID by opening the deployment in AI Hub and looking at the site URL, such as https<span>://</span>aihub.instabase.com/deployments/**01902d6f-bb35-74cb-bd27-c09b38bbf20a**/runs.</Tip>  (required)

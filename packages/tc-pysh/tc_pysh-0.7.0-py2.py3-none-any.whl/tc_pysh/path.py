...
class Path(os.PathLike):
    ...
    def replace(self, pattern: Union[str, re.Pattern], result: Union[str, Callable]) -> "Path":
...
class AbsolutePath(Path):
    ...
    def relative_to(self, other: Optional[Union[str, "AbsolutePath"]]) -> "RelativePath":
...
def ensure_path(path: Union[Path, str]) -> Path:
    ...
    if not isinstance(path, str):
        raise ValueError(f"ensure_path can only be applied to string and Path but got {path}.")

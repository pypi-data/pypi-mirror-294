...
class SaldorClient:
    def __init__(self, api_key: Optional[str] = None, base_url="https://api.saldor.com"):
        if not api_key:
            api_key = os.environ.get("SALDOR_API_KEY")
            if not api_key:
                raise ValueError("API key must be provided or set as SALDOR_API_KEY environment variable")

...
class STM:
    ...
    def scan_current(self,
                     current=None,  # You can set a current or read the average at the height.
                     height=None,
                     bottom=False,
                     repeat=(1, 1),
                     plot=True,
        ...
        if current is None:
            if height is None:
                height = 2
            current = self.get_avg_current(height, bottom)
            
        density = self.density.reshape((-1, self.ngridpoints[2]))
        density = dos2current(self.bias, density)
        heights = self.find_heights(density, current, bottom=bottom, startpoint=startpoint)

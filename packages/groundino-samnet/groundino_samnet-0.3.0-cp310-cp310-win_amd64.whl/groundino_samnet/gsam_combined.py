...
class GSamNetwork():
    ...
    def predict_dino(self, 
        ...
        if postproccesingv1:
            boxes,logits,phrases = PostProcessor().postprocess_box(image_shape=shape,
                                                            ...
                                                            mode="single")
        if postproccesingv2:
            boxes,logits,phrases = PostProcessor2(shape=shape).process_boxes_batch(boxes,logits,phrases)
    ...
    def predict_dino_batch(self,
                           ...
                           box_process_threshold: float,
                           UnNormalize:bool = False,
                           Postproccesing:bool = True) -> Tuple[List[torch.Tensor],List[torch.Tensor],List[torch.Tensor]]:
        """
        ...
        results = list(map(lambda image: self.predict_dino(image=image,
                                                           ...
                                                           Postproccesing=Postproccesing), images))

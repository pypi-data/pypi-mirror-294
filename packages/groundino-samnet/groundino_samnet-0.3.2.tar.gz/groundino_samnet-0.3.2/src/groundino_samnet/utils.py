...
class PostProcessor2:
    ...
    def process_boxes(self,shape,boxes,logits,phrases,batch_mode):
        post = PostProcessor2(shape=shape)
        if batch_mode:
            boxes1,logits1,phrases1 = post.purge_null_index(boxes=boxes,logits=logits,phrases=phrases,batch_mode=True)
            boxes2,logits2,phrases2 = post.filter_boxes_by_iou(boxes=boxes1,logits=logits1,phrases=phrases1,batch_mode=True)
            boxes3,logits3,phrases3 = post.iterative_box_removal(boxes=boxes2,logits=logits2,phrases=phrases2,batch_mode=True)
            boxes_f,logits_f,phrases_f = post.filter_boxes_by_iou(boxes=boxes3,logits=logits3,phrases=phrases3,threshold=np.finfo(np.float32).eps,batch_mode=True)
        else:
            boxes1,logits1,phrases1 = post.purge_null_index(boxes=boxes,logits=logits,phrases=phrases)
            boxes2,logits2,phrases2 = post.filter_boxes_by_iou(boxes=boxes1,logits=logits1,phrases=phrases1)
            boxes3,logits3,phrases3 = post.iterative_box_removal(boxes=boxes2,logits=logits2,phrases=phrases2)
            boxes_f,logits_f,phrases_f = post.filter_boxes_by_iou(boxes=boxes3,logits=logits3,phrases=phrases3,threshold=np.finfo(np.float32).eps)

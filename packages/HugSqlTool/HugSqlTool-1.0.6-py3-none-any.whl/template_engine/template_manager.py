...
def start_construct_tag_template(config_dict):
    ...
    tempalte_path = '/HugSqlTemplate/tag_templates/offline'
    if template_type == 1:
        tempalte_path = '/HugSqlTemplate/tag_templates/offline'
    elif template_type == 2:
        tempalte_path = '/HugSqlTemplate/tag_templates/nrt'
    else :
    ...
    if platform.system() == 'Windows':
        ...
        final_path = base_path + tempalte_path
...
############################################################################################################################
    ######################### 调试模块 #########################
    final_path='/loan-data/loan-data-warehouse-python/tools/tag_template_engine/template_engine/tag_templates/nrt'
    output_path='/loan-data/loan-data-warehouse-python/dw/ads/tag/index_06_nrt/%s' %(config_dict['tag_code'])
    ...
    for dirpath, dirnames, filenames in os.walk(final_path):
        for filename in filenames:
            ...
            if len(batch_staticize_codes)>0:
                temp_staticize_codes = "\
outputs: \n\
  table: ads_loan_user_tag_df \n\
  dws_table: false \n\
  staticize_vars_config: { \n\
      'entity_type':'%s', \n\
      'partition_config': { \n\
      'ads_day':'{DATE-1}', \n\
      'entity_type':'%s', \n\
      'tag_code':'construct_code',\n\
      'version':'1.0.0' \n\
      },\n\
      'tag_code': [\n\
      %s\n\
      ]\n\
  }"%(tag_entity_type, tag_entity_type, "'"+"','".join(batch_staticize_codes)+"'")

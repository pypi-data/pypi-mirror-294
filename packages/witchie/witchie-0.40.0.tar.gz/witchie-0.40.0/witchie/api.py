...
from witchie import CLIENT_NAME, CLIENT_WEBSITE, App, User, http
...
def post_status(
    ...
    https://api.pleroma.social/#operation/StatusController.create
    """
...
def react(app, user, status_id, emoji):
    return http.put(app, user, f"/api/v1/pleroma/statuses/{status_id}/reactions/{emoji}")


def unreact(app, user, status_id, emoji):
    return http.delete(app, user, f"/api/v1/pleroma/statuses/{status_id}/reactions/{emoji}")
...
def _timeline_generator(app, user, path, params=None):
    params.update(with_muted=False)
    while path:
...
def home_timeline_generator(app, user, **params):
    path = "/api/v1/timelines/home"
    return _timeline_generator(app, user, path, params)


def public_timeline_generator(app, user, local=False, **params):
    path = '/api/v1/timelines/public'
    params.update({'local': str_bool(local)})
    return _timeline_generator(app, user, path, params)


def bubble_timeline_generator(app, user, public=False, **params):
    path = "/api/v1/timelines/bubble"
    return _timeline_generator(app, user, path, params)


def tag_timeline_generator(app, user, hashtag, local=False, **params):
    path = f"/api/v1/timelines/tag/{quote(hashtag)}"
    params.update({'local': str_bool(local)})
    return _timeline_generator(app, user, path, params)


def bookmark_timeline_generator(app, user, **params):
    path = '/api/v1/bookmarks'
    return _timeline_generator(app, user, path, params)


def notification_timeline_generator(app, user, **params):
    # exclude all but mentions and statuses
    exclude_types = ["follow", "favourite", "reblog", "poll", "follow_request"]
    params.update({"exclude_types[]": exclude_types})
    return _notification_timeline_generator(app, user, "/api/v1/notifications", params)


def conversation_timeline_generator(app, user, **params):
    path = "/api/v1/conversations"
    return _conversation_timeline_generator(app, user, path, params)


def account_timeline_generator(app: App, user: User, account_name: str,
                               replies=False, reblogs=False, **params):
    account = find_account(app, user, account_name)
    path = f"/api/v1/accounts/{account['id']}/statuses"
    params.update({"exclude_replies": not replies, "exclude_reblogs": not reblogs})
    return _timeline_generator(app, user, path, params)


def timeline_list_generator(app, user, list_id, **params):
...
def anon_public_timeline_generator(instance, local=False, **params):
    path = '/api/v1/timelines/public'
    params.update({'local': str_bool(local)})
    return _anon_timeline_generator(instance, path, params)


def anon_tag_timeline_generator(instance, hashtag, local=False, **params):
    path = f"/api/v1/timelines/tag/{quote(hashtag)}"
    params.update({'local': str_bool(local)})
    return _anon_timeline_generator(instance, path, params)
...
def read_notifications(app, user, params):
    http.post(app, user, '/api/v1/pleroma/notifications/read', json=params)

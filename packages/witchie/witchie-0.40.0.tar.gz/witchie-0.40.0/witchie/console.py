...
import shutil
import sys
from argparse import ArgumentParser, ArgumentTypeError, BooleanOptionalAction, FileType
from collections import namedtuple
from itertools import chain

from witchie import (CLIENT_NAME, CLIENT_WEBSITE, __version__, commands,
                     config, settings)
from witchie.exceptions import ApiError, ConsoleError
from witchie.output import print_err, print_out
...
emoji_arg = (["-E", "--emoji"], {
    "type": str,
    "help": "Unicode emoji or custom emoji"
})
...
common_timeline_args = [
    ...
    (["--from-id"], {
        "type": str,
        "help": "show timeline from this ID (oldest ones first).",
    }),
    (["--up-to-id"], {
        "type": str,
        "help": "show timeline up to this ID.",
    }),
    (["--since-id"], {
        "type": str,
        "help": "show timeline since this ID (newest ones first).",
    }),
]
    ...
    Command(
        ...
        arguments=[
            ...
            (["-R", "--read"], {
                "action": "store_true",
                "default": False,
                "help": "Mark all notifications as read and only show unread notifications",
            }),
            (["-m", "--mentions"], {
                "action": "store_true",
                "default": False,
...
POST_COMMANDS = [
    Command(
        ...
        arguments=[
            ...
            (["--expires-in"], {
                "type": duration,
                "help": f"""Schedule the post to be deleted after a given amount
                        of time, {DURATION_EXAMPLES}. Must be at least 1 hour.""",
            }),
            (["-t", "--content-type"], {
                "type": str,
...
STATUS_COMMANDS = [
    ...
    Command(
        name="react",
        description="React to a status with emoji",
        arguments=[status_id_arg, json_arg, emoji_arg],
        require_auth=True,
    ),
    Command(
        name="unreact",
        description="Remove an emoji react from a status",
        arguments=[status_id_arg, json_arg, emoji_arg],
        require_auth=True,
    ),
    Command(
        name="vote",
        description="Vote on a status's poll",
...
OTHER_COMMANDS = [
    ...
    Command(
        name='configure',
        description="Local configuration for the account",
        arguments=[
            (["--optimize"], {
                "action": BooleanOptionalAction,
                "help": "Enable/Disable optimization such as caching/preload",
            }),
        ],
        require_auth=True,
    )

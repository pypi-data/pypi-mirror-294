...
class ModelScraper_s(Scraper):
    ...
    def get_models_info(self, mode_stream:bool=False, multi_processing: bool=False, temporary_year_marking: bool = True) -> pd.DataFrame:
        """
        ...
        if mode_stream:
            ...
            if multi_processing:
                ...
                # Step 2: Collect model specifications
                ...
                # Single process mode
                dict_models = {}
                print("Collecting models in single process mode")
                for url in url_series_set:
                    url_series_dict = self._get_models(url)
                    visit_url_dict.update(url_series_dict)
                print("Number of total models:", len(visit_url_dict))
                print("Collecting specifications")
                
                for key, url_model in tqdm(visit_url_dict.items()):
                    model_info = self._get_global_spec(url_model)
            ...
            if self.tracking_log:
                ...
                for model, url in visit_url_dict.items():
                    print(f"{model}: {url}")
            # Convert the dictionary to a DataFrame and save the data
            ...
            df_models = pd.read_json(self.output_folder / 's_scrape_model_data.json', orient='records', lines=True)

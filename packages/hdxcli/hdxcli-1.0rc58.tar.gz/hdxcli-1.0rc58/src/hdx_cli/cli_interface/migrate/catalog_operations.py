...
class Partition:
    def __init__(self, values):
        ...
        self.metadata = set_metadata(values[12])
    ...
    def to_list(self):
        return [self.created, self.modified, self.min_timestamp, self.max_timestamp,
                ...
                get_metadata(self.metadata), self.shard_key, self.lock, self.storage_id]
...
def get_bytes_from_catalog(partitions: list[Partition]) -> bytes:
...
def get_catalog_from_bytes(file: bytes) -> list[Partition]:
...
def get_catalog_from_temporal_file(project_id: str, table_id: str) -> list[Partition]:
    ...
    with open(file_path, 'rb') as file:
        return get_catalog_from_bytes(file.read())
...
class Catalog:
    ...
    def download(self,
        ...
        try:
            ...
            self.partitions = get_catalog_from_bytes(catalog)
            if temp_catalog:
    ...
    def upload(self, profile: ProfileUserContext, chunk_size: int=5000) -> None:
        upload_catalog_url = (
            f'{profile.scheme}://{profile.hostname}/config/v1/orgs/{profile.org_id}/'
        ...
        for chunk in chunked_iterable(self.partitions, chunk_size):
            catalog_file = get_bytes_from_catalog(chunk)
            try:
                ...
                if 'existing entries in Catalog' in message_error:
                    message_error = 'Conflict with existing entries in the Catalog.'
                else:
                    message_error = f'Some error occurred while uploading the catalog: {exc}.'
                raise HdxCliException(message_error) from exc

    def update(self, project_uuid: str, table_uuid: str, storage_mapping: dict[str, str]) -> None:
        for partition in self.partitions:
            storage_uuid = storage_mapping.get(partition.storage_id) or storage_mapping.get('default')
            partition.root_path = f'{project_uuid}/{table_uuid}'
            partition.metadata['storage_id'] = storage_uuid
            partition.storage_id = storage_uuid
    ...
    def get_partition_files_by_storage(self) -> dict[str, list[tuple[str, int]]]:
        ...
        for partition in self.partitions:
            ...
            partition_files_path = [
                (f'{partition_path}/manifest.hdx', partition.get_manifest_size()),

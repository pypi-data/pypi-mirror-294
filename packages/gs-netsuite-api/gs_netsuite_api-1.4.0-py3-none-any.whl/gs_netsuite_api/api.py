...
class BaseGsNetSuiteApi(GsNetSuiteApi[RT], abc.ABC):
    ...
    async def _async_get_records(
        ...
        *,
        ...
        async with self.netsuite_api as ns:
            _logger.info(f"1. Search product page [1 / {params.nb_pages}]")
            ...
            nb_pages = min(params.nb_pages, response.body.searchResult.totalPages or 0)
            ...
            if not page_index and nb_pages > 1:  # Si plus d'une page alors on process les autres
    ...
    def _get_criteria_modified_since(self, created_since: datetime.datetime) -> Any:
        """
        Retourne le critere de recherche utilisé pour la recherche par date.
        Args:
            created_since: La date de création a utiliser dans le critere

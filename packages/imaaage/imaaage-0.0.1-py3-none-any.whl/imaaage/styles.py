styles = [
    ...
    {
        "name": "tomo-default",
        "prompt": "photograph, {prompt}. photorealistic, masterpiece, best quality",
        "negative_prompt": "NSFW, longbody, lowres, bad anatomy, bad hands, missing fingers, pubic hair,extra digit, fewer digits, cropped, bad eyes, cgi, airbrushed, plastic, deformed, watermark"
    },
    {
...
style_categories = ["all"] + sorted(set(k.split("-")[0] for k in style_keys))
...
def filter_style(style_cate):
    if style_cate == "all":
        return style_keys
    return list(filter(lambda k: k.startswith(style_cate), style_keys))

...
CT = TypeVar("CT", list, bool)
RT = TypeVar("RT", list, dict)
...
class DMAioMysqlClient:
    ...
    async def query(
        ...
        *,
        ...
        async def callback(connection: AsyncMysqlConnector) -> list | bool:
            try:
    ...
    async def _execute(
        self,
        callback: Callable[[AsyncMysqlConnector], Awaitable[CT]]
    ) -> Optional[CT]:
        try:
    ...
    @staticmethod
    def _convert_decimal_to_float(results: RT) -> RT:
        new_results = []
        for row in results:

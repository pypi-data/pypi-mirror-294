...
class BaseService:
    ...
    def get_serializado(self):
        ...
        for atributo in self.modelo.ATRIBUTOS:
            if atributo['serializar']:
                etiqueta = atributo['etiqueta'].lower()
                if 'es_relacion' in atributo:
                    valor = getattr(self.objeto, atributo['campo'])
                    valor = valor.__str__() if valor is not None else None
    ...
    def listar(self):
        ...
        for objeto in self.modelo.objects.all():

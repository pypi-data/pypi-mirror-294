...
def cluster_imputing(data, labels, k=5, size=500):
    ...
    cluster_labels = np.asarray(imputed_data["cluster"].astype(int).values)
...
def data_prep(df):
    ...
    for col in df.columns:
        dummy_cols = [dcol for dcol in df_dummies.columns if dcol.startswith(col + "_")]
        if dummy_cols:
    ...
    for col in original_categorical_columns:
        dummy_cols = [col + "_" + val for val in df[col].dropna().unique()]
...
def create_distance_array(index, df_values, max_vals, min_vals, size=500):
    ...
    Args:
        ...
        max_vals (np.ndarray): Array of maximum values for each variable.
        min_vals (np.ndarray): Array of minimum values for each variable.
        size (int): Number of random points to sample (default is 500).
...
def imputer_numba(df, k=5, size=500):
    ...
    Args:
        ...
        k (int): The number of nearest neighbors to consider for imputation. Defaults to 5.

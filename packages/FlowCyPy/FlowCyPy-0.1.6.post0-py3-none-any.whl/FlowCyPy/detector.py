...
class Detector:
    ...
    noise_level: float = 0.05  # Noise level (in volts)
    baseline_shift: float = 0.01  # Baseline shift (in volts)
    saturation_level: float = 1e3  # Saturation level (in volts)
    ...
    def capture_signal(self, time: np.ndarray, raw_signal: np.ndarray) -> None:
        """
        Captures and processes the raw signal by adding noise, applying baseline shifts, and saturating the signal.
        ...
        self.signal = raw_signal.copy()

        # Add baseline shift
        baseline = self.baseline_shift.magnitude * np.sin(0.5 * np.pi * time.magnitude) * ureg.volt

        # Add noise
        noise = (self.noise_level * np.random.normal(size=len(time))).to('volt')
        self.signal += noise + baseline

        # Apply saturation
        self.signal = np.clip(self.signal.magnitude, 0, self.saturation_level.magnitude) * ureg.volt

        # Discretize the signal into bins
        bins = np.linspace(np.min(self.signal.magnitude), np.max(self.signal.magnitude), self.n_bins)
        digitized = np.digitize(self.signal.magnitude, bins) - 1
        self.signal = bins[digitized] * ureg.volt

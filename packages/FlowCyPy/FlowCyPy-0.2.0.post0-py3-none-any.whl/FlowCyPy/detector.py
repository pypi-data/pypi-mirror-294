...
class Detector:
    ...
    def capture_signal(self) -> None:
        ...
        # Add baseline shift
        baseline = self.baseline_shift.magnitude * np.sin(0.5 * np.pi * self.time.magnitude) * ureg.volt

        # Add noise
        noise = (self.noise_level * np.random.normal(size=len(self.time))).to('volt')
    ...
    def init_raw_signal(self, total_time: float) -> None:
        ...
        self.time = np.linspace(0, total_time.magnitude, time_points) * ureg.second

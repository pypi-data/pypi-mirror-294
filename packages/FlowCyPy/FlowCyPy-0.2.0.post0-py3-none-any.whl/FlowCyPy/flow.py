...
class Flow:
    ...
    mean_flow_rate : float
        The mean flow rate of the particles in meters per second (m/s).
    std_flow_rate : float
        Standard deviation of the flow rate for Gaussian longitudinal variation (m/s).
    ...
    mean_flow_rate: float = 1e-6  # Mean flow rate in meters per second (default: 1 µm/s)
    std_flow_rate: Optional[float] = 0.0  # Standard deviation of flow rate in meters per second (optional)
    ...
    def __post_init__(self):
        ...
        self.mean_flow_rate *= ureg.meter / ureg.second

        self.std_flow_rate *= ureg.meter / ureg.second
    ...
    def print_properties(self) -> None:
        ...
        flow_properties = [
            ...
            ["Mean Flow Rate", f"{self.mean_flow_rate:.2f~#P}"],
            ["Std Flow Rate", f"{self.std_flow_rate:.2f~#P}" if self.std_flow_rate else "N/A"],

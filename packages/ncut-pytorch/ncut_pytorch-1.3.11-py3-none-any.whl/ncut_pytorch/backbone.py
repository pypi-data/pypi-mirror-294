    ...
    def forward(self, images, input_str=None):
        ...
        for i_image in range(image.shape[0]):
            ...
            for i_layer, layer in enumerate(self.model.model.visual_model.mask_decoder.transformer.layers):
                ...
                outs[f"dec_{i_layer}_input"].append(layer.input_keys)
                outs[f"dec_{i_layer}_attn"].append(layer.attn_output)
                outs[f"dec_{i_layer}_block"].append(layer.block_output)

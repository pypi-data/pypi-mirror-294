...
class ToolsGPT():
    ...
    def perfect(self, content, instructions: str, model=None, timeout=None, temperature=None, yes_no_timeout=None, loop_limit=2):
        '''Wrapper function ensures that you get exactly what you wanted, and will not return until you get what you wanted'''
        output = self.get_yes_no(text=f"These are the instructions: {instructions} and this is the Output: {content} \n\nDoes the output follow the instructions?", 
                                 model=model, timeout=yes_no_timeout) == 'no'
        loops = 0

        print("Output:", content) if self.verbose == True else 0
        while output:
            content = self.llm(f'''The following content does not match the instructions exactly. Your job is to return the content exactly as the instructions direct:
            Instructions: {instructions}

            Content: {content}

            Return the content exactly as the instructions direct''', 
            model=model, timeout=timeout, temperature=temperature)

...
class Views(Vertical):
    ...
    def report(self, event: TabbedContent.TabActivated):
        self.log(f"{'-' * LOG_PREFIX_COUNT} TAB {event.pane.id} is now active.")
        # event.pane.focus()

    async def remove_added_tabs(self):
        ...
        self.app.log(f"{'-' * LOG_PREFIX_COUNT} added tabs: {self.added_tabs}")

        for pane_id in self.added_tabs:
            self.app.log(f"{'-' * LOG_PREFIX_COUNT} removing {pane_id=}")
            await tab_content.remove_pane(pane_id)
        self.added_tabs.clear()
        tab_content.refresh()

    async def clear(self):
        await self.remove_added_tabs()

    async def watch_item(self):
        ...
        self.app.log(f"{'-' * LOG_PREFIX_COUNT} {tab_content.tab_count = }")

        for plugin_name, plugin in self.app.plugins.items():
            if plugin.can_handle(item):
                self.app.log(f"{'-' * LOG_PREFIX_COUNT} {plugin} can handle {type(self.item)}")
                widget = plugin.get_widget(item)
                tab_id = f"{plugin.get_id()}-tab"
                await tab_content.add_pane(TabPane(plugin_name, widget, id=tab_id))
        ...
        try:
            if h5.is_dataset(item):
                self.app.log(f"{'-' * LOG_PREFIX_COUNT} it's a dataset! item={item}, parent={h5.get_parent(item).name}")
                item_view.update(item_to_renderable(item))
            elif h5.is_group(item):
                self.app.log(f"{'-' * LOG_PREFIX_COUNT} it's a group! item={item}, parent={h5.get_parent(item).name}")
                item_view.update(group_to_renderable(item))
            elif h5.is_file(item):
                self.app.log(f"{'-' * LOG_PREFIX_COUNT} it's a file! item={h5.get_filename(item)}")
...
class TableOfContentsWidget(Widget, can_focus_children=True):
    ...
    async def show_item(self, message: Tree.NodeSelected) -> None:
        ...
        if node_data is not None:
            self.app.log(f"{'-' * LOG_PREFIX_COUNT} {node_data = }")
            ...
            self.app.log(f"{'-' * LOG_PREFIX_COUNT} {item = }, {type(item)}")
...
class HDF5Browser(App):
    ...
    async def on_mount(self) -> None:
        ...
        try:
            ...
            await views.clear()
            attrs = self.query_one(AttributesWidget)
            attrs.update()
    ...
    async def action_next_file(self) -> None:
        ...
        if path is not None:
            ...
            self.path = path
            self.log(f"{'-' * LOG_PREFIX_COUNT} {path = }")
            await self.on_mount()
...
def populate_data_for_toc(group, data, level=1):
    ...
    for name, item in items:
        ...
        if h5.is_group(item):
            ...
            log.info(f"{'-' * LOG_PREFIX_COUNT} {name} is not a group nor a dataset")

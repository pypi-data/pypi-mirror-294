...
class RdsClient:
    ...
    def __init__(self, conf: dict, logs=None):
        ...
        self.__ping = conf.pop('ping_interval') if 'ping_interval' in conf else 30
    ...
    async def __loop_ping(self):
        while 1:
            try:
                ...
                self.log_err(f'redis连接已失效,将在{self.__timeout}秒后重试: {err} \n{traceback.format_exc()}')
                self.__pool.reset()
    ...
    async def get_hash(self, key: Union[str, bytes], h_key: Union[str, bytes], is_json=False):
        ...
        if await self.conn.hexists(key, h_key):
            info = await self.conn.hget(key, h_key)
            return json_parse(info) if is_json else info
    ...
    async def get_hash_all(self, key: Union[str, bytes]):
        ...
        :param key: 缓存名
        """
        return await self.conn.hgetall(key)
    ...
    async def qrpop(self, key: str, count=1):
        ...
        :param count: 出队数量
        """
        return await self.conn.rpop(key, count=None if count <= 1 else count)
    ...
    async def qlpop(self, key: str, count=1):
        ...
        :param count: 出队数量
        """
        return await self.conn.lpop(key, count=None if count <= 1 else count)

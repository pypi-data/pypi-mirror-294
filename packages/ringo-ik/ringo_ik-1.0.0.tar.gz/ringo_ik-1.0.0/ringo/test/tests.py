...
def test(raw_f: Callable[[], None],
         name=None) -> Callable[[], tuple[bool, str]]:
...
def gen_sampling(parallel: bool):
    ...
    def f():
        if parallel:
            assert base.use_mcr_parallel and base.use_pyxyz, "PyXYZ or MCR are disabled in this build"
        else:
            assert base.use_mcr_singlethreaded and base.use_pyxyz, "PyXYZ or MCR are disabled in this build"
        mol = Molecule(sdf=to_abs('pdb1GHG.sdf'))
        p = Confpool()

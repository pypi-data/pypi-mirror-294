...
class GreatExpectationsValidation:
    ...
    def __init__(self, source_table_name, df, trusted_workspace_id, trusted_lakehouse_id, list_validation,
                 trusted_metadata=None, trusted_savetype='overwrite', sk=None, datasource_name="my_datasource_name",
        ...
        self.trusted_workspace_id = trusted_workspace_id
        self.trusted_lakehouse_id = trusted_lakehouse_id
        self.trusted_savetype = trusted_savetype
    ...
    def validateData(self):
        """ This method utilizes the information initialized within the _init_ and helps to validate the data whilst returning the result in a Dictionary format  """
    ...
    def _generateInvalidRowInformationTable(self):
        ...
        not self.checkpoint_result["run_results"][validationResultIdentifierProperty]["validation_result"]["success"]):
            schema = StructType([
                StructField(self.sk, IntegerType(), True),
            ...
            # loadToTrusted(self.invalid_row_information_df, 0, f"abfss://{self.trusted_workspace_id}@onelake.dfs.fabric.microsoft.com/{self.trusted_lakehouse_id}/Tables/{self.source_table_name}_invalid_row_information")
            print("Writing invalid row information table...", end="")
            self.fabricInterface.writeDeltaTable(self.trusted_workspace_id, self.trusted_lakehouse_id,
                                f'{self.source_table_name}_invalid_row_information', self.invalid_row_information_df,
                                self.trusted_savetype)
            print("DONE.")
    ...
    def _generateValidAndInvalidRowTables(self):
        ...
        # print("Writing meta information table...", end="")
        # fdi.writeDeltaTable(self.trusted_workspace_id, self.trusted_lakehouse_id, f'{self.source_table_name}_meta_info', meta_info_df, self.trusted_savetype)
        # loadToTrusted(meta_info_df, 0, f"abfss://{trusted_workspace_id}@onelake.dfs.fabric.microsoft.com/{self.trusted_lakehouse_id}/Tables/{self.source_table_name}_meta_info")
        # print("DONE.")
        ...
        print("Writing valid rows table...", end="")

        print(f'second test: sk is {self.sk}')

        if (self.trusted_savetype == "masters" or self.trusted_savetype == "incremental"):
            print("calling cdc method, save type: ")
            self.fabricInterface.writeCDCTable(self.trusted_workspace_id, self.trusted_lakehouse_id, self.source_table_name,
                              valid_rows_df, savetype=self.trusted_savetype, primarykey=self.sk)
        else:
            print("calling write delta table")
            self.fabricInterface.writeDeltaTable(self.trusted_workspace_id, self.trusted_lakehouse_id, self.source_table_name,
                                valid_rows_df, savetype=self.trusted_savetype)
            # loadToTrusted(valid_rows_df, self.incremental_load, f"abfss://{trusted_workspace_id}@onelake.dfs.fabric.microsoft.com/{self.trusted_lakehouse_id}/Tables/{self.source_table_name}", self.sk)
        ...
        # print("Writing invalid rows table...", end="")
        # fdi.writeDeltaTable(self.trusted_workspace_id, self.trusted_lakehouse_id, f'{self.source_table_name}_invalid_rows', invalid_rows_df, self.trusted_savetype)
        # loadToTrusted(valid_rows_df, self.incremental_load, f"abfss://{trusted_workspace_id}@onelake.dfs.fabric.microsoft.com/{self.trusted_lakehouse_id}/Tables/{self.source_table_name}_invalid_rows", self.sk)
        print("DONE.")

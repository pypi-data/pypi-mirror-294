...
def push_command_func(**kwargs) -> bool:
    ...
    src_environment = Environment.from_env_file(environment=kwargs['from_'])
    ...
    if (
        ...
        objects[0].listen(environment=dest_environments[0])
    ...
    if (
        ...
        # from file
        if isinstance(kwargs['push_to_failed_jobs'], str) and kwargs['push_to_failed_jobs'] != "all":
            ...
            if any(filename.endswith(ext) for ext in [".csv", ".json"]):
                ...
                raise NameError(f"{kwargs['push_to_failed_jobs']} doesn't belong to the supported formats. Please use file with .json or .csv instead. ")

...
class Objects:
    ...
    def get_from(self, environment: Environment, log: bool = True) -> List[Self]:
        """
        ...
        if self.save_results and len(objects) > 0:
            ...
            for object in tqdm(
                objects, desc=f"Saving {objects[0].object_type.value} to {environment.name}", disable=not log
            ):
                # -----
    ...
    def push_to(
        ...
        if not object_exists and not self.is_instance:
            ...
            if self.object_type in OBJECT_TYPE_CREATE_MAP:
                ...
                print(f"/!\\ THIS OBJECT IS NOT CREATABLE THROUGH THE API. /!\\")
        ...
        assert (
            matching_object and "id" in matching_object.__dict__
        ), "/!\\ CANNOT PROCEED FURTHER AS THERE IS NO MATCHING OBJECT IN THE DESTINATION ENVIRONMENT /!\\"
        ...
        if not (self.object_type == ObjectType.RESOURCES and object_exists):
            for sub_item in self.sub_items:
                ...
                if sub_item in ["configuration", "metadata", "structure", "definition"]:
                    ...
                    print(
                        f"/!\\ SUB ITEM {sub_item.upper()} IS NOT PUSHABLE THROUGH THE API. /!\\"
                    )
    ...
    def save(self, environment: Environment, backup: bool = False) -> None:
        ...
        if self.object_type.value == "events" or self.is_instance:

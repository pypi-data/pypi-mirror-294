    ...
    def build(self) -> Task:
        ...
        if self.id != "_end":
            if len(self.outputs) == 0:
                logging.debug("Warning: Task has no outputs defined. Output of this step is not stored anywhere.\n")
            else:
                output_keys = [output.key for output in self.outputs]
                logging.debug(f"Task '{self.id}' output keys: {', '.join(output_keys)}\n")
...
class TaskBuilder:
    ...
    def new(cls, id:str, operator:Operator, mmap:dict, prompt:Optional[str]=None, path: Optional[str]=None, name:str="Task", description:str="Task Description") -> DraftTask:
        ...
        if id != "_end":
            logging.debug(f"Prompt has {len(input_names)} input(s): {', '.join(input_names)}")
        return DraftTask(
            id=id,
            name=name,
...
class WorkflowBuilder:
    ...
    def add_custom_tool(self, name:str, description:str, url:str, method:HttpMethod, headers: Optional[Dict[str, str]] = None, body: Optional[Dict[str, str]] = None):
        """
        ...
        if not self.workflow.config.custom_tool:
            self.workflow.config.custom_tool = [CustomToolBuilder.build(name, description, url, method, headers, body)]
        else:
            self.workflow.config.custom_tool.append(CustomToolBuilder.build(name, description, url, method, headers, body))
    ...
    def build(self) -> Workflow:
        ...
        if self.workflow.return_value is None:
            ...
            logging.debug(f"Warning: No return value set for the workflow. Select one of the {keys} by running set_return_value('key')")

...
class WebAPIInstrumentor:
    ...
    def instrument_httpx(self):
        ...
        async def instrumented_async_send(client, request, **kwargs):
            try:
                logger.debug(f"\n in Async httpx send {self.tracer}")
            except Exception as e:
                logger.error(f"ERROR {e}")
    ...
    def instrument_gradio(self, gradio_interface):
        original_launch = gradio_interface.launch

        @wraps(original_launch)
        def instrumented_launch(*args, **kwargs):
            with self.tracer.start_as_current_span("gradio_launch") as launch_span:
                # Call the original launch function
                app, local_url, share_url = original_launch(*args, **kwargs)
                ...
                @app.middleware("http")
                async def add_parent_trace(request: Request, call_next):
                    logger.debug(f"In add_parent_trace for request {request}, tracer: {self.tracer}")
                    with self.tracer.start_as_current_span(f"APP http_request {request.method} {request.url}") as span:
                        logger.debug(f"Starting span {span.get_span_context()}")
                        context = TraceContextTextMapPropagator().extract(request.headers)
                        trace.set_span_in_context(span, context)
                ...
                # Add attributes to the launch span
                launch_span.set_attribute("gradio.local_url", local_url)
                if share_url:
                    launch_span.set_attribute("gradio.share_url", share_url)

                return app, local_url, share_url

        self.instrument_httpx()
        self.instrument_requests()
        gradio_interface.launch = instrumented_launch

    def instrument_gradio_v2(self, app):
        original_launch = app.launch
        tracer = self.tracer

        @wraps(original_launch)
        def instrumented_launch(*args, **kwargs):
            logger.info(f"\n In gradio launch for saved tracer: {tracer}")
            logger.info(f"\n In gradio launch for tracer: {self.tracer}")
        ...
        logger.info(f"Gradio instrumentation applied tracer : {tracer}")
        return self.tracer

    ...
    def instrument_httpx(self):
        ...
        def instrumented_send(client, request, **kwargs):
            ...
            if span is not trace.INVALID_SPAN:
                logger.info(f"in sync httpx send parent span: trace id {span.get_span_context().trace_id}")
            else:
                logger.info(f"in sync httpx send parent span is INVALID_SPAN: {span}")
                if GLOBAL_SPAN is not None:
                    logger.info(f"using GLOBAL span {GLOBAL_SPAN}")
                    span = GLOBAL_SPAN
                    trace.set_span_in_context(span=span)
                else:
                    logger.warning("GLOBAL SPAN is none")

...
class AsyncTransferWorkerPoolBase(ABC):
    ...
    @abstractmethod
    async def process_workload(self, workload: Tuple, trace_scope: trace.Span):
        """Process a workload.
        ...
        try:
            while True:
                with self.tracer.start_as_current_span(
                    ...
                    await self.process_workload(await self.fetch_workload(), trace_scope)
...
class AsyncFilePreSignUrlWorkerPool(AsyncTransferWorkerPoolBase):
    ...
    async def process_workload(self, workload: Tuple[str], trace_scope: trace.Span):
...
class AsyncFileS3UploadWorkerPool(AsyncTransferWorkerPoolBase):
    ...
    async def process_workload(self, workload: Tuple[str, int, str], trace_scope: trace.Span):
...
class AsyncUploadWorkerManager:
    ...
    def __init__(
        ...
        # user may have a lot of cores, but all workers are on the same core
        self.max_concurrency = (
            NGC_CLI_MAX_CONCURRENCY or max_concurrency or logistic_scale(multiprocessing.cpu_count(), 50, 0.1, 12, 2)
        )

        self.progress = AsyncTransferProgress(callback_func=progress_callback_func, update_rate=progress_update_rate)
    ...
    def calculate_progress(self):
        ...
        for _full_path in get_full_paths(self.source):
            ...
            self.progress.total_bytes += os.path.getsize(_full_path)
    ...
    async def init_async_attributes(self, session, auth_lock, executor):
        ...
        while q:
            ...
            node.queue = asyncio.Queue(maxsize=self.max_concurrency)
    ...
    with AsyncUploadWorkerManager(
        ...
        try:
            worker_args = {
                ...
                "worker_count": upload_manager.max_concurrency,

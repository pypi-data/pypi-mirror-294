...
class PET(Model):
    ...
    def __init__(self,
        ...
        self.body = Model(inputs=inputs_part,
                                outputs=outputs_body)

        outputs_head = self.PET_head(outputs_body,inputs_evt,projection_dim)

        if inputs_evt is None:
            self.head = Model(inputs=[outputs_body],
                                    outputs=[outputs_head])
            self.classifier = Model(inputs=inputs_part,
                                    outputs=outputs_head)
        else:
            self.head = Model(inputs=[outputs_body,inputs_evt],
                              outputs=[outputs_head])
    ...
    def train_step(self, inputs):
        ...
        self.optimizer.minimize(loss,self.classifier.trainable_variables,tape=tape)

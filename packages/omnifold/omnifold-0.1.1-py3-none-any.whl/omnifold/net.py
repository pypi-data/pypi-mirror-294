...
class PET(Model):
    ...
    def __init__(self,
        ...
        num_part (int, optional, default=132): Number of particles or objects per event in the input data.
        num_heads (int, optional, default=4): Number of attention heads in the transformer layer, for multi-head self-attention.
        num_transformer (int, optional, default=1): Number of transformer layers stacked in the model.
        projection_dim (int, optional, default=64): Dimensionality of the projection layer for transforming input features.
        ...
        self.body = keras.Model(inputs=inputs_part,
                                outputs=outputs_body)

        outputs_head = self.PET_head(outputs_body,inputs_evt,projection_dim)

        if inputs_evt is None:
            self.head = keras.Model(inputs=[outputs_body],
                                    outputs=[outputs_head])
            self.classifier = keras.Model(inputs=[inputs_part],
                                          outputs=outputs_head)
        else:
    ...
    def train_step(self, inputs):
        ...
        self.optimizer.minimize(loss,self.body.trainable_variables,tape=tape)
    ...
    def PET_body(
        ...
        inputs_mask = tf.cast(tf.equal(inputs_part, 0), dtype='float32')
        if local:

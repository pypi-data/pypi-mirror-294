...
//				}
		...
		
		{
			...
			while(ln!=null && reads!=null && reads.size()>0){//ln!=null prevents a compiler potential null access warning
				...
				for(int idx=0; idx<reads.size(); idx++){
					...
					if(loadKmers){
						processTileKmers(r1, mt);
						processTileKmers(r2, mt);
					}
	...
	void processTileKmers(Read r, MicroTile mt) {
		if(r==null || r.length()<k) {return;}
		final long kmer=toKmer(r.bases, randy.nextInt(r.length()-k2), k);
		if(kmer<0) {
			mt.misses++;
	...
	boolean processReadPair_inner(final Read r1, final Read r2){
		...
		if(len1>0){
			...
			if(qual<flowcell.avgQuality-qDeviations*flowcell.stdQuality){return false;}
			if(prob<flowcell.avgErrorFree-eDeviations*flowcell.stdErrorFree){return false;}

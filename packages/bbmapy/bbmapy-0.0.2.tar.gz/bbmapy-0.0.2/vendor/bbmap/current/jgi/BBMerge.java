...
//			outstream.println("Loaded "+kmers+" kmers in "+tload);
		...
		if(parseCustom){
			...
			outstream.println("FPR:                 \t"+String.format(Locale.ROOT, "%.4f%%", FPRate*100));
			outstream.println("FNR:                 \t"+String.format(Locale.ROOT, "%.4f%%", FNRate*100));
			outstream.println();
		...
		private final int mateByOverlap(Read r1, Read r2){
			...
			if(TAG_CUSTOM || MAKE_VECTOR){
				...
				if(TAG_CUSTOM) {
					...
					bb.append(String.format(Locale.ROOT, "%.4f\t%.4f\t", r1ee, r2ee));
				}
				if(MAKE_VECTOR) {
...
//	static boolean untrim=false;
	...
	private static final String defaultNetFile=Data.findPath("?bbmerge.bbnet", false);

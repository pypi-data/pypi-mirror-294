...
public class PlotFlowCell implements Accumulator<PlotFlowCell.WorkerThread> {
	...
	public PlotFlowCell(String[] args){
		...
		if("phix".equalsIgnoreCase(refPath)) {
			refPath=Data.findPath("?phix174_ill.ref.fa.gz");
		}
	...
	public void process(Timer t){
		...
		Shared.capThreads(allKmers ? 64 : 64);
		...
		mapper=(refPath==null ? null : new MicroAligner(31, minIdentity, refPath));
	...
	void loadKmers(){
		...
		if(bloomMode) {
			...
			KmerCountAbstract.PICK_ONLY_ONE_KMER=!allKmers;
		}
...
//			System.err.println("Cells Used:     \t"+bloomFilter.filter.cellsUsed());
			...
			KmerCountAbstract.PICK_ONLY_ONE_KMER=false;
		}
	}
	...
	BarcodeStats loadBarcodes(String expectedBarcodesFile) {
		...
		if(expectedBarcodesFile!=null) {bs.loadBarcodeList(expectedBarcodesFile, 
				barcodesPerRead>1 ? delimiter : 0, false, false);}
		return bs;
	}
			...
			while(ln!=null && reads!=null && reads.size()>0){//ln!=null prevents a compiler potential null access warning
				...
				for(int idx=0; idx<reads.size(); idx++){
					...
					if(barcodeStats!=null) {
						...
						int capped=Tools.min(hdist, mt.barcodeHDist.length-1);
						mt.barcodeHDist[capped]++;
					}
...
//		final long kmer=toKmer(r.bases, randy.nextInt(r.length()-k2), k);
		...
		final long key=(allKmers ? Tools.max(kmer, AminoAcid.reverseComplementBinaryFast(kmer, k)) : kmer);
		final int value=(keySets==null ? bloomFilter.getCount(key) : keySets[(int)(key%WAYS)].getValue(key));
		mt.depthSum+=value;
		final int cutoff=(allKmers ? 2 : 1);
...
//	static void map(Read r, LongHashMap map, byte[] ref, float minIdentity, ByteBuilder buffer) {
//		if(r==null || r.length()<k || r.match!=null || r.samline!=null) {return;}
//		byte[] bases=r.bases;
...
//			r.reverseComplement();
//			align(r, ref, offset, offset+r.length(), pad, minIdentity, buffer);
//			r.reverseComplement();
//		}else {
//			align(r, ref, offset, offset+r.length(), pad, minIdentity, buffer);
//		}
//	}
//	
//	static void align(Read r, byte[] ref, int a, int b, int pad, float minIdentity, ByteBuilder buffer){
//		int subs=quickAlign(r, ref, a, buffer);
//		if(subs<4) {
////			assert(r.match!=null);
//			return;
//		}
...
//		
////		if(r.numericID>10000 && r.numericID<10200) {System.err.println(id+"\n"+new String(match));}
//		
//		if(id<minIdentity) {return;}//Probably not phix
//		r.match=match;
//		
////		synchronized(System.err) {
////			System.err.println(r.strand());
////			System.err.println(new String(ref, a, r.length()));
...
//	}
//	
//	static int quickAlign(Read read, byte[] ref, int a, ByteBuilder buffer) {
...
//			if(ros!=null){ros.add(reads, ln.id);}
	...
	private boolean allKmers=false;
	private int cbits=4;

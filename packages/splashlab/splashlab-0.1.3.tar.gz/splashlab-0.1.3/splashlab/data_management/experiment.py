...
integer_pattern = re.compile(r"^([+-]?[0-9]+([eE][0-9]+)?)$")
float_pattern = re.compile(r"^([-+]?(([0-9]+\.?[0-9]*)|([0-9]*\.?[0-9]+))([eE][-+]?[0-9]+)?)$")


def convert(string):
    if re.match(integer_pattern, string):
        return int(string)
    elif re.match(float_pattern, string) or string.lower() == 'nan':
        return float(string)
    return string
...
def read_config(base_directory):
    ...
    for directory in glob.glob(base_directory + r'/data/*'):
        if os.path.isdir(directory):
            ...
            line = relative_dir.split('_')
            data.append([convert(item) for item in line])
    ...
    if os.path.isfile(base_directory + '/measurements.csv'):
        new_df = pd.DataFrame(data, columns=inputs)
        try:
            for col in new_df:
...
if __name__ == "__main__":
    ...
    directory = r"D:\ALAYESH_2023_2DSPLASH"

"""
Monkey-patch plt.figure() to support Ctrl+C & Cmd+C for copying to clipboard as an image
...
so that pressing Ctrl+C (or Cmd+C on MacOS) with a matplotlib figure window in the foreground will copy
...
@authors: Josh Burnett, Sylvain Finot, Orson Peters
...
On Linux:
    - Requires either Qt or GTK3 libraries for clipboard interaction. Automatically detects which is being used from
        matplotlib.get_backend().
        - Qt support requires PyQt5/6 or PySide2/6.
        - GTK3 support requires pycairo, PyGObject and PIL/pillow to be installed.
        - GTK4 not supported, as it has totally changed the way clipboard data is handled and I can't figure it out.
...
if ostype == 'windows':
    ...
    try:
        ...
        raise ValueError(f'Unsupported matplotlib backend ({backend}).')

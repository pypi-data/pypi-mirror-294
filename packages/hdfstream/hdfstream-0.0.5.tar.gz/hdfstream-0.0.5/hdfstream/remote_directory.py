...
class RemoteDirectory(collections.abc.Mapping):
    ...
    def __init__(self, server, name, user=None, password=None, data=None,
        ...
        if connection is None:
            connection = Connection.get(server, user, password)
    ...
    def load(self):
        ...
        if not self.unpacked:
            data = self.connection.request_directory(self.name)
            self.unpack(data)
    ...
    def unpack(self, data):
        ...
        for filename in data["files"]:
            ...
            self._files[filename] = RemoteGroup(self.connection, file_path, "/", max_depth=self.max_depth, data_size_limit=self.data_size_limit)

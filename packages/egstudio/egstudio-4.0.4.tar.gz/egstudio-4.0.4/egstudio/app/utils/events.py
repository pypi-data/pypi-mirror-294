...
from egstudio.app.stores import publisher_store
...
def emit(event, data={}, persist=True, project_id=None):
    ...
    from egstudio.app.config import ENABLE_JOB_QUEUE

    for func in event_handlers.values():
        if ENABLE_JOB_QUEUE:
            from egstudio.app.stores.queue_store import job_queue
...
def save_event(event, data, project_id=None):
    ...
    """
    try:
        from egstudio.app.services.persons_service import get_current_user_raw

...
def send_email(recipient, subject, html, attachments=[]):
    ...
    send(SMTP_HOST, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SENDER_ADDRESS, email_address, subject, html, mail_attachments)
...
def db():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS sessions (id VARCHAR PRIMARY KEY, device_id VARCHAR, user_id VARCHAR, secret VARCHAR, confirmed BOOLEAN)")
    connection.commit()
...
def install_auth_routes(app):
    ...
    async def auth(request: Request, response: Response, email_address: str = Form(), device_id: str = Form()):
        ...
        if not match(DEVICE_ID_REGEX, device_id):
            return HTMLResponse("Invalid device id.", status_code=HTTP_400_BAD_REQUEST)
        ...
        if row:
            ...
            cursor.execute("INSERT INTO sessions (id, device_id, user_id, secret, confirmed) VALUES (?, ?, ?, ?, ?)",
                           (session.id, device_id, session.user_id, session.secret, session.confirmed))
            connection.commit()
            connection.close()
            response.set_cookie(
                key=f"session_id",
        ...
        cursor.execute("INSERT INTO sessions (id, device_id, user_id, secret, confirmed) VALUES (?, ?, ?, ?, FALSE)", (session_id, device_id, user_id, session_secret))
        connection.commit()
    ...
    async def sign_in(session_secret: str):
        connection, cursor = db()
        try:
            (user_id, session_id, device_id) = cursor.execute("""
                    SELECT users.id, sessions.id, sessions.device_id
                    FROM users, sessions
                    WHERE
        ...
        connection.commit()
        connection.close()
        return RedirectResponse(REDIRECT_URL.replace(":deviceId", device_id))

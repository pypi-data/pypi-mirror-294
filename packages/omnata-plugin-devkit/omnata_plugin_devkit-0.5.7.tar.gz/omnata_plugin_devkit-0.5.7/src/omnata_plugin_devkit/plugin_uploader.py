...
""")
            ...
            for udtf in udtfs:
                print(f"Found {udf.language} UDTF: {udf.name}, consumer facing: {udf.expose_to_consumer}")
                if udf.language == "python":
...
def udf_wrapper_definition(udf_def:UDFDefinition|UDTFDefinition):
    ...
    if isinstance(udf_def, UDFDefinition):
        return f"""create or replace function UDFS.{function_signature(udf_def, True, 'all', 'replace_with_slug')}
returns {udf_def.result_data_type} 
comment = $${udf_def.description}$$
as
$$
UDFS.{udf_def.name}(PLUGIN.PLUGIN_CONNECTION(CONNECTION_SLUG),{function_signature(udf_def, False, 'names_only', 'remove')})
$$;
"""
    else:
...
as
$$
select * from table(UDFS.{udf_def.name}(PLUGIN.PLUGIN_CONNECTION(CONNECTION_SLUG),{function_signature(udf_def, False, 'names_only', 'remove')}))
$$;
"""
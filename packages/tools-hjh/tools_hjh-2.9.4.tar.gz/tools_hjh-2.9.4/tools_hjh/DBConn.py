    ...
    def pg_copy_from(self, table_name, rows, cols_description):
        ...
        try:
            if self.creator == 'psycopg2':
                ...
                output = StringIO()
                data1.to_csv(output, sep='\t', index=False, header=False, na_rep='', quoting=csv.QUOTE_NONE)
...
class QueryResults:
    ...
    def get_rows(self, rownum='all'):
        ...
        rows_new = [] 
        if lob_exists:
            tp = ThreadPool(len(rows), save_result=True)
            for row in rows:
                tp.run(lob2str, (row,))
            tp.wait()
            rows_new = tp.get_results().values()
        else:
            rows_new = rows.copy()

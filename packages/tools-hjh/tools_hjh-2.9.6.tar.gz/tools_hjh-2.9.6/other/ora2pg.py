...
def get_data_from_oracle(table_name, mess, scn, page=None, page_rn=None): 
    try:
        ...
        i = 1
        tp = ProcessPool(save_parallel_num, while_wait_time=0.1)
        while True:
            if page is None:
                my_mess = mess + '(' + str(i) + ')'
            else:
...
def get_report(only_report_scn=None):
    if only_report_scn is None:
        ...
        for k in report:
            ...
            _ = ''
            try:
                dst_num = int(dst_db.run('select count(1) from ' + table_name).get_rows()[0][0])
            except Exception as _:
                dst_num = 0
            finally:
                dst_db.close()
            log.info('report', src_num == dst_num, table_name, src_num, dst_num, str(_))
            if src_num != dst_num:
                error_table = error_table + table_name + ','
        ...
        for table_mess in tables:
            src_table = table_mess[0]
            count_sql = 'select count(1) from ' + src_schema + '."' + src_table + '" as of scn ' + only_report_scn
            try:
                src_num = src_db.run(count_sql).get_rows()[0][0]
            except:
                src_num = 0
            try:
                dst_num = dst_db.run('select count(1) from ' + src_table).get_rows()[0][0]
            except:
                dst_num = 0
            log.info('report', src_num == dst_num, src_table, src_num, dst_num)
            if src_num != dst_num:
                error_table = error_table + src_table + ','
        log.info('error_table', error_table)

...
from morix.config_loader import load_yaml, default_functions
...
def merge_functions(scan_folder: str):
    try:
        if os.path.exists(os.path.join(scan_folder, "functions.yml")):
            ...
            merged_functions = default_functions + project_functions
...
def process_tool_calls(messages, assistant_message: dict, project_abspath: str):
    ...
    def handle_command(tool):
        ...
        if not config.get('permissions', {}).get('allow_run_console_command', False):
            ...
            try:
                result = subprocess.run(
                    ...
                    text=True,
                    timeout=timeout,
                    cwd=project_abspath  # Run command in the specified directory
                )
                logger.info(f"Command execution result: {result.stdout or result.stderr}")
                content = result.stdout or result.stderr or f"No command execution result received, exitcode: {result.returncode}"

...
class StateMachineBasedWrapperEnv(object):
  ...
  def _update_local_toe_positions(self):
    ...
    inv_base_position, inv_base_orientation = self.pycram_bullet_client.invertTransform(
        base_position, base_orientation)
    for toe_index in range(NUM_LEGS):
      toe_pose_world = self.pycram_bullet_client.getLinkState(
          self.robot.quadruped, self._toe_link_indices[toe_index])[0]
      toe_pos_local, _ = self.pycram_bullet_client.multiplyTransforms(

...
from pycram_bullet_envs.minitaur.robots import quadruped_base
from pycram_bullet_envs.minitaur.robots import wheeled_robot_base_sim
...
class DepthUVToFootPlacementWrapper(object):
  ...
  def _create_foot_placement_visualization(self):
    ...
    visual_id = self._gym_env.pycram_bullet_client.createVisualShape(
        self._gym_env.pycram_bullet_client.GEOM_SPHERE,
        radius=0.02,
        rgbaColor=[0.7, 0.7, 0.7, 1])
    self._foothold_visual_body = self._gym_env.pycram_bullet_client.createMultiBody(
  ...
  def _update_foothold_visualization(self, action):
    ...
    self._gym_env.pycram_bullet_client.resetBasePositionAndOrientation(
  ...
  def _create_com_visualization(self):
    ...
    visual_id = self._gym_env.pycram_bullet_client.createVisualShape(
        self._gym_env.pycram_bullet_client.GEOM_BOX,
        halfExtents=COM_VIZ_BOX_SIZE,
        rgbaColor=[0.7, 0.5, 0.5, 1])
    self._support_polygon_center_visual_body = self._gym_env.pycram_bullet_client.createMultiBody(
        baseMass=0, baseVisualShapeIndex=visual_id, basePosition=[0, 0, 0])
    ...
    self._projected_com_visual_body = self._gym_env.pycram_bullet_client.createMultiBody(
        baseMass=0, baseVisualShapeIndex=visual_id, basePosition=[0, 0, 0])

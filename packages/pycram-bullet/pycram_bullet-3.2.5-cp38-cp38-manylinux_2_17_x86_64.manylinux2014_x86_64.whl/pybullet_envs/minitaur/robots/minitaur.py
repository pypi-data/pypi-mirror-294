"""This file implements the functionalities of a minitaur using pycram_bullet."""
...
import numpy as np
import gin
from pycram_bullet_envs.minitaur.robots import minitaur_constants
from pycram_bullet_envs.minitaur.robots import minitaur_motor
from pycram_bullet_envs.minitaur.robots import robot_config
from pycram_bullet_envs.minitaur.robots.safety import safety_checker
from pycram_bullet_envs.minitaur.robots.safety import safety_error
from pycram_bullet_envs.minitaur.robots.utilities import action_filter
from pycram_bullet_envs.minitaur.robots.utilities import kinematics
...
class Minitaur(object):
  ...
  def __init__(self,
    ...
    _, self._init_orientation_inv = self._pycram_bullet_client.invertTransform(
  ...
  def _StepInternal(self, action, motor_control_mode=None):
    ...
    self._pycram_bullet_client.stepSimulation()
    self.ReceiveObservation()
  ...
  def _RecordMassInfoFromURDF(self):
    ...
    for chassis_id in self._chassis_link_ids:
      self._base_mass_urdf.append(
          self._pycram_bullet_client.getDynamicsInfo(self.quadruped, chassis_id)[0])
    self._leg_masses_urdf = []
    for leg_id in self._leg_link_ids:
      self._leg_masses_urdf.append(
          self._pycram_bullet_client.getDynamicsInfo(self.quadruped, leg_id)[0])
    for motor_id in self._motor_link_ids:
      self._leg_masses_urdf.append(
          self._pycram_bullet_client.getDynamicsInfo(self.quadruped, motor_id)[0])

  def _RecordInertiaInfoFromURDF(self):
    """Record the inertia of each body from URDF file."""
    self._link_urdf = []
    num_bodies = self._pycram_bullet_client.getNumJoints(self.quadruped)
    for body_id in range(-1, num_bodies):  # -1 is for the base link.
      inertia = self._pycram_bullet_client.getDynamicsInfo(self.quadruped,
                                                      body_id)[2]
  ...
  def _BuildJointNameToIdDict(self):
    num_joints = self._pycram_bullet_client.getNumJoints(self.quadruped)
    self._joint_name_to_id = {}
    for i in range(num_joints):
      joint_info = self._pycram_bullet_client.getJointInfo(self.quadruped, i)
  ...
  def _CreateRackConstraint(self, init_position, init_orientation):
    ...
    """
    fixed_constraint = self._pycram_bullet_client.createConstraint(
        parentBodyUniqueId=self.quadruped,
        parentLinkIndex=-1,
        childBodyUniqueId=-1,
        childLinkIndex=-1,
        jointType=self._pycram_bullet_client.JOINT_FIXED,
        jointAxis=[0, 0, 0],
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    if reload_urdf:
      ...
      self._pycram_bullet_client.resetBasePositionAndOrientation(
          self.quadruped, self._GetDefaultInitPosition(),
          self._GetDefaultInitOrientation())
      self._pycram_bullet_client.resetBaseVelocity(self.quadruped, [0, 0, 0],
                                              [0, 0, 0])
  ...
  def _LoadRobotURDF(self):
    ...
    if self._self_collision_enabled:
      self.quadruped = self._pycram_bullet_client.loadURDF(
          urdf_file,
          self._GetDefaultInitPosition(),
          self._GetDefaultInitOrientation(),
          flags=self._pycram_bullet_client.URDF_USE_SELF_COLLISION)
    else:
      self.quadruped = self._pycram_bullet_client.loadURDF(
  ...
  def _SetMotorTorqueById(self, motor_id, torque):
    self._pycram_bullet_client.setJointMotorControl2(
  ...
  def _SetMotorTorqueByIds(self, motor_ids, torques):
    self._pycram_bullet_client.setJointMotorControlArray(
        bodyIndex=self.quadruped,
  ...
  def _ResetPoseForLeg(self, leg_id, add_constraint):
    ...
    self._pycram_bullet_client.resetJointState(
        self.quadruped,
    ...
    self._pycram_bullet_client.resetJointState(
        ...
        targetVelocity=0)
    if add_constraint:
      self._pycram_bullet_client.createConstraint(
          self.quadruped,
          self._joint_name_to_id["knee_" + leg_position + "R_link"],
          self.quadruped,
          self._joint_name_to_id["knee_" + leg_position + "L_link"],
          self._pycram_bullet_client.JOINT_POINT2POINT, [0, 0, 0],
          KNEE_CONSTRAINT_POINT_RIGHT, KNEE_CONSTRAINT_POINT_LEFT)

    # Disable the default motor in pycram_bullet.
    self._pycram_bullet_client.setJointMotorControl2(
        bodyIndex=self.quadruped,
        jointIndex=(self._joint_name_to_id["motor_" + leg_position +
                                           "L_joint"]),
        controlMode=self._pycram_bullet_client.VELOCITY_CONTROL,
        targetVelocity=0,
  ...
  def GetBaseVelocity(self):
    ...
    """
    velocity, _ = self._pycram_bullet_client.getBaseVelocity(self.quadruped)
    return velocity
  ...
  def GetTrueBaseRollPitchYaw(self):
    ...
    roll_pitch_yaw = self._pycram_bullet_client.getEulerFromQuaternion(orientation)
  ...
  def GetBaseRollPitchYaw(self):
    ...
    delayed_roll_pitch_yaw = self._pycram_bullet_client.getEulerFromQuaternion(
        delayed_orientation)
  ...
  def GetFootContacts(self):
    ...
    for leg_idx in range(MINITAUR_NUM_MOTORS // 2):
      ...
      contact_1 = bool(
          self._pycram_bullet_client.getContactPoints(
              bodyA=0,
  ...
  def GetBaseOrientation(self):
    ...
    """
    return self._pycram_bullet_client.getQuaternionFromEuler(
  ...
  def GetTrueBaseRollPitchYawRate(self):
    ...
    """
    angular_velocity = self._pycram_bullet_client.getBaseVelocity(self.quadruped)[1]
    orientation = self.GetTrueBaseOrientation()
  ...
  def TransformAngularVelocityToLocalFrame(self, angular_velocity, orientation):
    ...
    _, orientation_inversed = self._pycram_bullet_client.invertTransform([0, 0, 0],
                                                                    orientation)
    # Transform the angular_velocity at neutral orientation using a neutral
    # translation and reverse of the given orientation.
    relative_velocity, _ = self._pycram_bullet_client.multiplyTransforms(
        [0, 0, 0], orientation_inversed, angular_velocity,
        self._pycram_bullet_client.getQuaternionFromEuler([0, 0, 0]))
  ...
  def ReceiveObservation(self):
    ...
    """
    self._joint_states = self._pycram_bullet_client.getJointStates(
        self.quadruped, self._motor_id_list)
    self._base_position, orientation = (
        self._pycram_bullet_client.getBasePositionAndOrientation(self.quadruped))
    # Computes the relative orientation relative to the robot's
    # initial_orientation.
    _, self._base_orientation = self._pycram_bullet_client.multiplyTransforms(
        positionA=[0, 0, 0],
  ...
  def _GetDefaultInitOrientation(self):
    ...
    if self._reset_at_current_position and self._observation_history:
      ...
      return self._pycram_bullet_client.getQuaternionFromEuler([0.0, 0.0, yaw])
  ...
  @property
  def pycram_bullet_client(self):
    return self._pycram_bullet_client

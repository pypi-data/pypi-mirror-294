    ...
    for i in range(len(bodies)):
      ...
      for j in range(self._p.getNumJoints(bodies[i])):
        self._p.setJointMotorControl2(bodies[i],
                                      j,
                                      pycram_bullet.POSITION_CONTROL,
                                      positionGain=0.1,
                                      velocityGain=0.1,
                                      force=0)
  ...
  def reset(self, bullet_client):
    ...
    if (self.doneLoading == 0):
      ...
      if self.self_collision:
        self.objects = self._p.loadMJCF(os.path.join(pycram_bullet_data.getDataPath(), "mjcf",
                                                     self.model_xml),
                                        flags=pycram_bullet.URDF_USE_SELF_COLLISION |
                                        pycram_bullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS |
                                        pycram_bullet.URDF_GOOGLEY_UNDEFINED_COLORS )
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
            self._p, self.objects)
      else:
        self.objects = self._p.loadMJCF(
            os.path.join(pycram_bullet_data.getDataPath(), "mjcf", self.model_xml, flags = pycram_bullet.URDF_GOOGLEY_UNDEFINED_COLORS))
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
            self._p, self.objects)
...
class URDFBasedRobot(XmlBasedRobot):
  ...
  def reset(self, bullet_client):
    ...
    if self.self_collision:
      self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
          self._p,
          self._p.loadURDF(os.path.join(pycram_bullet_data.getDataPath(), self.model_urdf),
                           basePosition=self.basePosition,
                           baseOrientation=self.baseOrientation,
                           useFixedBase=self.fixed_base,
                           flags=pycram_bullet.URDF_USE_SELF_COLLISION | pycram_bullet.URDF_GOOGLEY_UNDEFINED_COLORS))
    else:
      self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
          self._p,
          self._p.loadURDF(os.path.join(pycram_bullet_data.getDataPath(), self.model_urdf),
                           basePosition=self.basePosition,
                           baseOrientation=self.baseOrientation,
                           useFixedBase=self.fixed_base, flags = pycram_bullet.URDF_GOOGLEY_UNDEFINED_COLORS))
...
class Pose_Helper:  # dummy class to comply to original interface
  ...
  def rpy(self):
    return pycram_bullet.getEulerFromQuaternion(self.body_part.current_orientation())
...
class Joint:
  ...
  def set_velocity(self, velocity):
    self._p.setJointMotorControl2(self.bodies[self.bodyIndex],
                                  self.jointIndex,
                                  pycram_bullet.VELOCITY_CONTROL,
  ...
  def set_torque(self, torque):
    self._p.setJointMotorControl2(bodyIndex=self.bodies[self.bodyIndex],
                                  ...
                                  controlMode=pycram_bullet.TORQUE_CONTROL,
  ...
  def disable_motor(self):
    self._p.setJointMotorControl2(self.bodies[self.bodyIndex],
                                  self.jointIndex,
                                  controlMode=pycram_bullet.POSITION_CONTROL,
                                  targetPosition=0,
                                  targetVelocity=0,
                                  positionGain=0.1,
                                  velocityGain=0.1,

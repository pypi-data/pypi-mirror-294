...
from pycram_bullet_envs.minitaur.agents.baseline_controller import state_estimator
from pycram_bullet_envs.minitaur.agents.baseline_controller import time_based_moving_window_filter
from pycram_bullet_envs.minitaur.envs_v2.sensors import accelerometer_sensor
from pycram_bullet_envs.minitaur.envs_v2.sensors import imu_sensor
...
class IMUBasedCOMVelocityEstimator(state_estimator.StateEstimatorBase):
  ...
  def update(self, current_time):
    ...
    yaw_aligned_base_orientation = self._robot.pycram_bullet_client.getQuaternionFromEuler(
        (filtered_gyroscope_reading[0], filtered_gyroscope_reading[1], 0.0))

    rot_mat = self._robot.pycram_bullet_client.getMatrixFromQuaternion(

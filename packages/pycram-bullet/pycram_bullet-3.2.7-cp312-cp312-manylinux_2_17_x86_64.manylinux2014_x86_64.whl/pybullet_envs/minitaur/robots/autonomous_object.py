...
from pycram_bullet_envs.minitaur.robots import object_controller
...
class AutonomousObject(robot_base.RobotBase):
  ...
  def __init__(self,
    ...
    Args:
      ...
      actuate_by_reset: Use pycram_bullet resetBasePositionAndOrientation to actuate
    ...
    self._pycram_bullet_client = None  # will be initialized in set_sim_client()
  ...
  def set_sim_client(self, pycram_bullet_client: bullet_client.BulletClient) -> None:
  ...
  def _load_urdf(self):
    ...
    try:
      ...
      self._object_id = self._pycram_bullet_client.loadURDF(self._urdf_file)
    except:
  ...
  def load(self) -> None:
    ...
    if not self._actuate_by_reset:
      self._constraint_id = self._pycram_bullet_client.createConstraint(
  ...
  def receive_observation(self) -> None:
    ...
    position, orientation = (
        self._pycram_bullet_client.getBasePositionAndOrientation(self._object_id))
  ...
  def _reset_base_pose(self,
    ...
    self._pycram_bullet_client.resetBaseVelocity(self._object_id, (0, 0, 0),
                                            (0, 0, 0))
  ...
  def _actuate_base_pose(self, position: Union[Sequence[float], np.ndarray],
    ...
    """
    self._pycram_bullet_client.changeConstraint(
        self._constraint_id,
        position,
  ...
  def base_roll_pitch_yaw(self) -> np.ndarray:
    ...
    return np.array(
        self._pycram_bullet_client.getEulerFromQuaternion(self._orientation))

...
# In the return from pycram_bullet.getJointInfo, the name of the link whose parent is
# that joint.
LINK_NAME_INDEX = 12
...
def link_name_to_id(link_name: Text, robot_id: int, pycram_bullet_client) -> int:
  """Returns the pycram_bullet integer link id corresponding to link_name.

  Args:
    link_name: The name of the link from the URDF. If this is BASE_LINK, returns
      -1, the link id of the base according to pycram_bullet convention.
    robot_id: Integer id of the robot to which the link belongs, as returned by
      pycram_bullet.loadURDF().
    pycram_bullet_client: Client in which the robot is loaded.
  ...
  if link_name == BASE_LINK:
    return -1
  link_name_list = []
  for i in range(pycram_bullet_client.getNumJoints(robot_id)):
    joint_info = pycram_bullet_client.getJointInfo(robot_id, i)
...
def set_collision_filter_group_mask(urdf_id: int, group: int, mask: int,
  ...
  Args:
    ...
    mask: The collision bit mask to use. See go/pycram_bullet for details.
    pycram_bullet_client: The bullet client to use.
  """
  # We includes "-1" for the base link of the URDF.
  for link_id in range(-1, pycram_bullet_client.getNumJoints(urdf_id)):
    pycram_bullet_client.setCollisionFilterGroupMask(urdf_id, link_id, group, mask)

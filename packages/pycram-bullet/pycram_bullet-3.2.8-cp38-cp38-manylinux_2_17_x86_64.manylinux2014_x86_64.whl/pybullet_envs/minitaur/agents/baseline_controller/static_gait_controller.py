...
import gin
import numpy as np
from pycram_bullet_envs.minitaur.agents.baseline_controller import foot_stepper
...
class StaticGaitController(object):
  ...
  def __init__(self, robot):
    ...
    self._wait_count = 0
    self._stepper = foot_stepper.FootStepper(self._robot.pycram_bullet_client,
                                             self._toe_ids, toe_pos_local_ref)

  def act(self, observation):
    """Computes actions based on observations."""
    del observation
    p = self._robot.pycram_bullet_client

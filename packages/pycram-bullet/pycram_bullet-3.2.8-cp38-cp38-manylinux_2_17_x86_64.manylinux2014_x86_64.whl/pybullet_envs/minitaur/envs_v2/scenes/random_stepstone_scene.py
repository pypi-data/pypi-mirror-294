...
from pycram_bullet_envs.minitaur.envs_v2.scenes.terrain import stepstones
...
class RandomStepstoneScene(scene_base.SceneBase):
  ...
  def reset(self):
    ...
    if self._rebuild_scene_during_reset:
      for ground_id in self.ground_ids:
        self._pycram_bullet_client.removeBody(ground_id)
      self.build_scene(self._pycram_bullet_client)
  ...
  def build_scene(self, pycram_bullet_client):
    ...
    for pycram_bullet_id in [first_stone_id] + stepstone_ids:
      self.add_object(pycram_bullet_id, scene_base.ObjectType.GROUND)

    self._floor_height = np.random.uniform(self._floor_height_lower_bound,
                                           self._floor_height_upper_bound)
    # Add floor
    floor_id = stepstones.load_box(pycram_bullet_client,

import numpy as np
import math
from pycram_bullet_envs.deep_mimic.env.env import Env
from pycram_bullet_envs.deep_mimic.env.action_space import ActionSpace
from pycram_bullet_utils import bullet_client
import time
from pycram_bullet_envs.deep_mimic.env import motion_capture_data
...
class PyBulletDeepMimicEnv(Env):
  ...
  def __init__(self, arg_parser=None, enable_draw=False, pycram_bullet_client=None,
               time_step=1./240,
  ...
  def reset(self):
    ...
    if not self._isInitialized:
      if self.enable_draw:
        self._pycram_bullet_client = bullet_client.BulletClient(connection_mode=p1.GUI)
        #disable 'GUI' since it slows down a lot on Mac OSX and some other platforms
        self._pycram_bullet_client.configureDebugVisualizer(self._pycram_bullet_client.COV_ENABLE_GUI, 0)
      else:
        self._pycram_bullet_client = bullet_client.BulletClient()

      self._pycram_bullet_client.setAdditionalSearchPath(pycram_bullet_data.getDataPath())
      z2y = self._pycram_bullet_client.getQuaternionFromEuler([-math.pi * 0.5, 0, 0])
      self._planeId = self._pycram_bullet_client.loadURDF("plane_implicit.urdf", [0, 0, 0],
                                                     z2y,
                                                     useMaximalCoordinates=True)
      #print("planeId=",self._planeId)
      self._pycram_bullet_client.configureDebugVisualizer(self._pycram_bullet_client.COV_ENABLE_Y_AXIS_UP, 1)
      self._pycram_bullet_client.setGravity(0, -9.8, 0)

      self._pycram_bullet_client.setPhysicsEngineParameter(numSolverIterations=10)
      self._pycram_bullet_client.changeDynamics(self._planeId, linkIndex=-1, lateralFriction=0.9)
      ...
      motionPath = pycram_bullet_data.getDataPath() + "/" + motion_file[0]
      #motionPath = pycram_bullet_data.getDataPath()+"/motions/humanoid3d_backflip.txt"
      self._mocapData.Load(motionPath)
      timeStep = self.timeStep
      useFixedBase = False
      self._humanoid = humanoid_stable_pd.HumanoidStablePD(self._pycram_bullet_client, self._mocapData,
                                                           timeStep, useFixedBase, self._arg_parser)
      self._isInitialized = True

      self._pycram_bullet_client.setTimeStep(timeStep)
      self._pycram_bullet_client.setPhysicsEngineParameter(numSubSteps=1)

      selfCheck = False
      if (selfCheck):
        curTime = 0
        while self._pycram_bullet_client.isConnected():
  ...
  def update(self, timeStep):
    #print("pycram_bullet_deep_mimic_env:update timeStep=",timeStep," t=",self.t)
    self._pycram_bullet_client.setTimeStep(timeStep)
    ...
    for i in range(1):
      ...
      if self.desiredPose:
        ...
        self._humanoid.initializePose(self._humanoid._poseInterpolator,
                                      ...
                                      initBase=True)
        #pos,orn=self._pycram_bullet_client.getBasePositionAndOrientation(self._humanoid._sim_model)
        #self._pycram_bullet_client.resetBasePositionAndOrientation(self._humanoid._kin_model, [pos[0]+3,pos[1],pos[2]],orn)
  ...
  def getKeyboardEvents(self):
    return self._pycram_bullet_client.getKeyboardEvents()

  def isKeyTriggered(self, keys, key):
    o = ord(key)
    #print("ord=",o)
    if o in keys:
      return keys[ord(key)] & self._pycram_bullet_client.KEY_WAS_TRIGGERED
    return False

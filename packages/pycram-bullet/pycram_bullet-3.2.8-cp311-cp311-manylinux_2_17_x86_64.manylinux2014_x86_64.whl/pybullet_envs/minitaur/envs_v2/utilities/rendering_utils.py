...
def render_image(pycram_bullet_client, width, height, view_matrix, proj_matrix):
  ...
  (_, _, px, _, _) = pycram_bullet_client.getCameraImage(
      width=width,
      height=height,
      renderer=pycram_bullet_client.ER_BULLET_HARDWARE_OPENGL,
...
def get_scene_bounding_box(pycram_bullet_client, scene=None):
  """Computes scene axis-aligned bounding box.

  Args:
    pycram_bullet_client: PyBullet client.
  ...
  for i in range(pycram_bullet_client.getNumBodies()):
    body_id = pycram_bullet_client.getBodyUniqueId(i)
    ...
    aabb = pycram_bullet_client.getAABB(body_id)
...
def render_topdown(
  ...
  if rendered_origin_and_size is not None:
    ...
    aabb_min, aabb_max = get_scene_bounding_box(pycram_bullet_client, scene)
  ...
  if use_y_as_up_axis:
    ...
    view_matrix = pycram_bullet_client.computeViewMatrix(
  ...
  proj_matrix = pycram_bullet_client.computeProjectionMatrixFOV(

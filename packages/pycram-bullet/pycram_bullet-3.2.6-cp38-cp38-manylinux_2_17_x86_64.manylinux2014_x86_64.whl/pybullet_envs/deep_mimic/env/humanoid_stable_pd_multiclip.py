...
class HumanoidStablePDMultiClip(object):
    ...
    def __init__(self, pycram_bullet_client, mocap_data, timeStep,
        ...
        for i in range(self._n_clips):
            self._kin_models[i] = self._pycram_bullet_client.loadURDF(
        ...
        for i in range(self._n_clips):
            self._pycram_bullet_client.changeDynamics(self._kin_models[i], -1, linearDamping=0, angularDamping=0)

            # todo: add feature to disable simulation for a particular object. Until then, disable all collisions
            self._pycram_bullet_client.setCollisionFilterGroupMask(self._kin_models[i],
                                                              -1,
            ...
            alpha = 0.4
            self._pycram_bullet_client.changeVisualShape(self._kin_models[i], -1, rgbaColor=[1, 1, 1, alpha])
            for j in range(self._pycram_bullet_client.getNumJoints(self._kin_models[i])):
                self._pycram_bullet_client.setCollisionFilterGroupMask(self._kin_models[i],
                                                                  j,
                                                                  collisionFilterGroup=0,
                                                                  collisionFilterMask=0)
                self._pycram_bullet_client.changeDynamics(
                    ...
                    activationState=self._pycram_bullet_client.ACTIVATION_STATE_SLEEP +
                                    self._pycram_bullet_client.ACTIVATION_STATE_ENABLE_SLEEPING +
                                    self._pycram_bullet_client.ACTIVATION_STATE_DISABLE_WAKEUP)
                self._pycram_bullet_client.changeVisualShape(self._kin_models[i], j, rgbaColor=[1, 1, 1, alpha])
        ...
        for j in self._jointIndicesAll:
            ...
            for i in range(self._n_clips):
                self._pycram_bullet_client.setJointMotorControl2(self._kin_models[i],
                                                            j,
                                                            self._pycram_bullet_client.POSITION_CONTROL,
                                                            targetPosition=0,
                                                            positionGain=0,
    ...
    def computePose(self, frameFraction, i=0):
        ...
        self._poseInterpolators[i].Slerp(frameFraction, frameData, frameDataNext, self._pycram_bullet_client)
    ...
    def convertActionToPose(self, action, i):
        pose = self._poseInterpolators[i].ConvertFromAction(self._pycram_bullet_client, action)
        return pose
    ...
    def getState(self):
        ...
        linkStatesSim = self._pycram_bullet_client.getLinkStates(self._sim_model, linkIndicesSim,
    ...
    def getReward(self, pose, i=0):
        ...
        rootPosKin, rootOrnKin = self._pycram_bullet_client.getBasePositionAndOrientation(self._kin_models[i])
        ...
        kinJointStates = self._pycram_bullet_client.getJointStatesMultiDof(self._kin_models[i], jointIndices)
        linkStatesSim = self._pycram_bullet_client.getLinkStates(self._sim_model, jointIndices)
        linkStatesKin = self._pycram_bullet_client.getLinkStates(self._kin_models[i], jointIndices)

...
class HumanoidPoseInterpolator(object):
  ...
  def ConvertFromAction(self, pycram_bullet_client, action):
    ...
    index += 4
    self._chestRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    #print("pose._chestRot=",pose._chestRot)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._neckRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._rightHipRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._rightAnkleRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._rightShoulderRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._leftHipRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._leftAnkleRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    self._leftShoulderRot = pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

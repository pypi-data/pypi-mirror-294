"""Implements the functionalities of a minitaur rainbow dash using pycram_bullet.
    ...
    if reload_urdf:
      if self._self_collision_enabled:
        self.quadruped = self._pycram_bullet_client.loadURDF(
            ...
            flags=(self._pycram_bullet_client.URDF_USE_SELF_COLLISION_EXCLUDE_PARENT))
      else:
        self.quadruped = self._pycram_bullet_client.loadURDF("%s/quadruped/minitaur_rainbow_dash.urdf" %
                                                        self._urdf_root,
                                                        init_position,
                                                        useFixedBase=self._on_rack)
    ...
    if add_constraint:
      if leg_id < 2:
        self._pycram_bullet_client.createConstraint(
            ...
            self._pycram_bullet_client.JOINT_POINT2POINT, [0, 0, 0], KNEE_CONSTRAINT_POINT_SHORT,
            KNEE_CONSTRAINT_POINT_LONG)
      else:
        self._pycram_bullet_client.createConstraint(
            self.quadruped, self._joint_name_to_id["knee_" + leg_position + "R_joint"],
            self.quadruped, self._joint_name_to_id["knee_" + leg_position + "L_joint"],
            self._pycram_bullet_client.JOINT_POINT2POINT, [0, 0, 0], KNEE_CONSTRAINT_POINT_LONG,

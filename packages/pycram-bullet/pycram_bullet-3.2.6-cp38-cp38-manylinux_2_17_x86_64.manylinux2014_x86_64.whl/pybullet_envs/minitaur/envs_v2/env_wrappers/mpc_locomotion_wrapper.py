...
from pycram_bullet_envs.minitaur.agents.baseline_controller import com_height_estimator
from pycram_bullet_envs.minitaur.agents.baseline_controller import gait_generator as gait_generator_lib
from pycram_bullet_envs.minitaur.agents.baseline_controller import imu_based_com_velocity_estimator
from pycram_bullet_envs.minitaur.agents.baseline_controller import multi_state_estimator
from pycram_bullet_envs.minitaur.agents.baseline_controller import openloop_gait_generator
from pycram_bullet_envs.minitaur.agents.baseline_controller import torque_stance_leg_controller
from pycram_bullet_envs.minitaur.robots import laikago_kinematic_constants
...
class MPCLocomotionWrapper(object):
  ...
  def reset(self, **kwargs):
    ...
    for link_id in list(
      ...
      self.pycram_bullet_client.changeDynamics(
          self.robot.robot_id,
          link_id,
  ...
  def _get_toe_tracking_target(self, lift_off_position, phase, swing_clearance,
    ...
    rotated_up_vec = np.array(
        self.pycram_bullet_client.multiplyTransforms(
            (0, 0, 0),
            self.pycram_bullet_client.getQuaternionFromEuler(

...
def joint_angles_from_link_positions(
  ...
  Args:
    pycram_bullet_client: The bullet client.
  ...
  if positions_are_in_world_frame:
    ...
    urdf_base_position, urdf_base_orientation = (
        pycram_bullet_client.getBasePositionAndOrientation(urdf_id))
    world_link_positions = []
    for link_position in link_positions:
      world_link_position, _ = pycram_bullet_client.multiplyTransforms(
  ...
  all_joint_angles = pycram_bullet_client.calculateInverseKinematics2(
...
def link_position_in_world_frame(
  ...
  """
  return np.array(pycram_bullet_client.getLinkState(urdf_id, link_id)[0])
...
def link_position_in_base_frame(
  ...
  if base_link_id is None:
    ...
    base_link_state = pycram_bullet_client.getLinkState(urdf_id, base_link_id)
    base_position, base_orientation = base_link_state[0], base_link_state[1]

  inverse_translation, inverse_rotation = pycram_bullet_client.invertTransform(
      base_position, base_orientation)

  link_state = pycram_bullet_client.getLinkState(urdf_id, link_id)
  link_position = link_state[0]
  link_local_position, _ = pycram_bullet_client.multiplyTransforms(
...
def compute_jacobian(
  ...
  jv, _ = pycram_bullet_client.calculateJacobian(
      urdf_id,
      link_id,
...
def rotate_to_base_frame(
  ...
  _, base_orientation_quat = (
      pycram_bullet_client.getBasePositionAndOrientation(urdf_id))
  _, base_orientation_quat_from_init = pycram_bullet_client.multiplyTransforms(
      positionA=(0, 0, 0),
      orientationA=init_orientation_inv_quat,
      positionB=(0, 0, 0),
      orientationB=base_orientation_quat)
  _, inverse_base_orientation = pycram_bullet_client.invertTransform(
  ...
  local_vector, _ = pycram_bullet_client.multiplyTransforms(
      positionA=(0, 0, 0),

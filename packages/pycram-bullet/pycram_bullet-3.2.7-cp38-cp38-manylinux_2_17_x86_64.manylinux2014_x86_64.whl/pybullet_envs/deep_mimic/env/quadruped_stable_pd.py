...
from pycram_bullet_envs.deep_mimic.env import quadruped_pose_interpolator
import math


class QuadrupedStablePD(object):

  def __init__(self, pycram_bullet_client, mocap_data, timeStep, useFixedBase=True):
    ...
    self._sim_model = self._pycram_bullet_client.loadURDF("laikago/laikago.urdf",
                                                     startPos,
                                                     startOrn,
                                                     flags=urdfFlags,
                                                     useFixedBase=False)
    self._pycram_bullet_client.resetBasePositionAndOrientation(_sim_model, startPos, startOrn)

    self._end_effectors = []  #ankle and wrist, both left and right

    self._kin_model = self._pycram_bullet_client.loadURDF("laikago/laikago.urdf",
                                                     startPos,
                                                     startOrn,
                                                     useFixedBase=True)
  ...
  def getReward(self, pose):
    ...
    linVelKin, angVelKin = self._pycram_bullet_client.getBaseVelocity(self._kin_model)

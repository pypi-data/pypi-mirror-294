from pycram_bullet_utils import pd_controller_stable
from pycram_bullet_envs.deep_mimic.env import humanoid_pose_interpolator
import math
import numpy as np
...
class HumanoidStablePD(object):
  ...
  def __init__( self, pycram_bullet_client, mocap_data, timeStep, 
                useFixedBase=True, arg_parser=None, useComReward=False):
    self._pycram_bullet_client = pycram_bullet_client
    self._mocap_data = mocap_data
    self._arg_parser = arg_parser
    print("LOADING humanoid!")
    flags=self._pycram_bullet_client.URDF_MAINTAIN_LINK_ORDER+self._pycram_bullet_client.URDF_USE_SELF_COLLISION+self._pycram_bullet_client.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
    self._sim_model = self._pycram_bullet_client.loadURDF(
        "humanoid/humanoid.urdf", [0, 0.889540259, 0],
        globalScaling=0.25,
        useFixedBase=useFixedBase,
        flags=flags)

    #self._pycram_bullet_client.setCollisionFilterGroupMask(self._sim_model,-1,collisionFilterGroup=0,collisionFilterMask=0)
    #for j in range (self._pycram_bullet_client.getNumJoints(self._sim_model)):
    #  self._pycram_bullet_client.setCollisionFilterGroupMask(self._sim_model,j,collisionFilterGroup=0,collisionFilterMask=0)

    self._end_effectors = [5, 8, 11, 14]  #ankle and wrist, both left and right

    self._kin_model = self._pycram_bullet_client.loadURDF(
        "humanoid/humanoid.urdf", [0, 0.85, 0],
        globalScaling=0.25,
        useFixedBase=True,
        flags=self._pycram_bullet_client.URDF_MAINTAIN_LINK_ORDER)

    self._pycram_bullet_client.changeDynamics(self._sim_model, -1, lateralFriction=0.9)
    for j in range(self._pycram_bullet_client.getNumJoints(self._sim_model)):
      self._pycram_bullet_client.changeDynamics(self._sim_model, j, lateralFriction=0.9)

    self._pycram_bullet_client.changeDynamics(self._sim_model, -1, linearDamping=0, angularDamping=0)
    self._pycram_bullet_client.changeDynamics(self._kin_model, -1, linearDamping=0, angularDamping=0)

    #todo: add feature to disable simulation for a particular object. Until then, disable all collisions
    self._pycram_bullet_client.setCollisionFilterGroupMask(self._kin_model,
                                                      -1,
                                                      collisionFilterGroup=0,
                                                      collisionFilterMask=0)
    self._pycram_bullet_client.changeDynamics(
        self._kin_model,
        -1,
        activationState=self._pycram_bullet_client.ACTIVATION_STATE_SLEEP +
        self._pycram_bullet_client.ACTIVATION_STATE_ENABLE_SLEEPING +
        self._pycram_bullet_client.ACTIVATION_STATE_DISABLE_WAKEUP)
    alpha = 0.4
    self._pycram_bullet_client.changeVisualShape(self._kin_model, -1, rgbaColor=[1, 1, 1, alpha])
    for j in range(self._pycram_bullet_client.getNumJoints(self._kin_model)):
      self._pycram_bullet_client.setCollisionFilterGroupMask(self._kin_model,
                                                        j,
                                                        collisionFilterGroup=0,
                                                        collisionFilterMask=0)
      self._pycram_bullet_client.changeDynamics(
          ...
          self._pycram_bullet_client.ACTIVATION_STATE_ENABLE_SLEEPING +
          self._pycram_bullet_client.ACTIVATION_STATE_DISABLE_WAKEUP)
      self._pycram_bullet_client.changeVisualShape(self._kin_model, j, rgbaColor=[1, 1, 1, alpha])
    ...
    self._stablePD = pd_controller_stable.PDControllerStableMultiDof(self._pycram_bullet_client)
    ...
    for j in self._jointIndicesAll:
      #self._pycram_bullet_client.setJointMotorControlMultiDof(self._sim_model, j, self._pycram_bullet_client.POSITION_CONTROL, force=[1,1,1])
      self._pycram_bullet_client.setJointMotorControl2(self._sim_model,
                                                  j,
                                                  self._pycram_bullet_client.POSITION_CONTROL,
                                                  targetPosition=0,
                                                  positionGain=0,
                                                  targetVelocity=0,
                                                  force=jointFrictionForce)
      self._pycram_bullet_client.setJointMotorControlMultiDof(
          self._sim_model,
          j,
          self._pycram_bullet_client.POSITION_CONTROL,
      ...
      self._pycram_bullet_client.setJointMotorControl2(self._kin_model,
                                                  j,
                                                  self._pycram_bullet_client.POSITION_CONTROL,
                                                  targetPosition=0,
                                                  positionGain=0,
  ...
  def initializePose(self, pose, phys_model, initBase, initializeVelocity=True):
    ...
    if initializeVelocity:
      if initBase:
        self._pycram_bullet_client.resetBasePositionAndOrientation(phys_model, pose._basePos,
                                                              pose._baseOrn)
        self._pycram_bullet_client.resetBaseVelocity(phys_model, pose._baseLinVel, pose._baseAngVel)
      if useArray:
        ...
        self._pycram_bullet_client.resetJointStatesMultiDof(phys_model, indices,
                                                    jointPositions, jointVelocities)
      else:
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, chest, pose._chestRot,
                                                      pose._chestVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, neck, pose._neckRot, pose._neckVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightHip, pose._rightHipRot,
                                                      pose._rightHipVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightKnee, pose._rightKneeRot,
                                                      pose._rightKneeVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightAnkle, pose._rightAnkleRot,
                                                      pose._rightAnkleVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightShoulder,
                                                      pose._rightShoulderRot, pose._rightShoulderVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightElbow, pose._rightElbowRot,
                                                      pose._rightElbowVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftHip, pose._leftHipRot,
                                                      pose._leftHipVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftKnee, pose._leftKneeRot,
                                                      pose._leftKneeVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftAnkle, pose._leftAnkleRot,
                                                      pose._leftAnkleVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftShoulder,
                                                      pose._leftShoulderRot, pose._leftShoulderVel)
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftElbow, pose._leftElbowRot,
                                                      pose._leftElbowVel)
    else:
      ...
      if useArray:
        ...
        jointPositions = [pose._chestRot, pose._neckRot, pose._rightHipRot, pose._rightKneeRot,
                          pose._rightAnkleRot, pose._rightShoulderRot, pose._rightElbowRot, pose._leftHipRot,
                          pose._leftKneeRot, pose._leftAnkleRot, pose._leftShoulderRot, pose._leftElbowRot]
        self._pycram_bullet_client.resetJointStatesMultiDof(phys_model, indices,jointPositions)
        
      else:
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, chest, pose._chestRot, [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, neck, pose._neckRot, [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightHip, pose._rightHipRot,
                                                      [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightKnee, pose._rightKneeRot, [0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightAnkle, pose._rightAnkleRot,
                                                      [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, rightShoulder,
        ...
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftHip, pose._leftHipRot,
                                                      [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftKnee, pose._leftKneeRot, [0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftAnkle, pose._leftAnkleRot,
                                                      [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftShoulder,
                                                      pose._leftShoulderRot, [0, 0, 0])
        self._pycram_bullet_client.resetJointStateMultiDof(phys_model, leftElbow, pose._leftElbowRot, [0])
  ...
  def computePose(self, frameFraction):
    ...
    self._poseInterpolator.Slerp(frameFraction, frameData, frameDataNext, self._pycram_bullet_client)
  ...
  def convertActionToPose(self, action):
    pose = self._poseInterpolator.ConvertFromAction(self._pycram_bullet_client, action)
    return pose
  ...
  def computeAndApplyPDForces(self, desiredPositions, maxForces):
    ...
    self._pycram_bullet_client.setJointMotorControlMultiDofArray(self._sim_model,
                                                           indices,
                                                           self._pycram_bullet_client.STABLE_PD_CONTROL,
  ...
  def applyPDForces(self, taus):
    ...
    if (useArray):
      ...
      self._pycram_bullet_client.setJointMotorControlMultiDofArray(self._sim_model,
                                                             indices,
                                                             self._pycram_bullet_client.TORQUE_CONTROL,
                                                             forces=forces)
    else:
      for index in range(len(self._jointIndicesAll)):
        jointIndex = self._jointIndicesAll[index]
        if self._jointDofCounts[index] == 4:
          ...
          #print("force[", jointIndex,"]=",force)
          self._pycram_bullet_client.setJointMotorControlMultiDof(self._sim_model,
                                                             jointIndex,
                                                             self._pycram_bullet_client.TORQUE_CONTROL,
                                                             force=force)
        if self._jointDofCounts[index] == 1:
          ...
          self._pycram_bullet_client.setJointMotorControlMultiDof(
              self._sim_model,
              jointIndex,
              controlMode=self._pycram_bullet_client.TORQUE_CONTROL,
              force=force)
        dofIndex += self._jointDofCounts[index]

  def setJointMotors(self, desiredPositions, maxForces):
    controlMode = self._pycram_bullet_client.POSITION_CONTROL
    startIndex = 7
    chest = 1
    neck = 2
    rightHip = 3
  ...
  def buildOriginTrans(self):
    rootPos, rootOrn = self._pycram_bullet_client.getBasePositionAndOrientation(self._sim_model)
  ...
  def getState(self):
    ...
    basePos, baseOrn = self._pycram_bullet_client.getBasePositionAndOrientation(self._sim_model)
    ...
    linkIndicesSim = []
    for pbJoint in range(self._pycram_bullet_client.getNumJoints(self._sim_model)):
      linkIndicesSim.append(self.pb2dmJoints[pbJoint])
      
    linkStatesSim = self._pycram_bullet_client.getLinkStates(self._sim_model, linkIndicesSim, computeForwardKinematics=True, computeLinkVelocity=True)
    
    for pbJoint in range(self._pycram_bullet_client.getNumJoints(self._sim_model)):
      j = self.pb2dmJoints[pbJoint]
      #print("joint order:",j)
      #ls = self._pycram_bullet_client.getLinkState(self._sim_model, j, computeForwardKinematics=True)
    ...
    for pbJoint in range(self._pycram_bullet_client.getNumJoints(self._sim_model)):
      j = self.pb2dmJoints[pbJoint]
      #ls = self._pycram_bullet_client.getLinkState(self._sim_model, j, computeLinkVelocity=True)
      ls = linkStatesSim[pbJoint]
      
      linkLinVel = ls[6]
      linkAngVel = ls[7]
      linkLinVelLocal, unused = self._pycram_bullet_client.multiplyTransforms([0, 0, 0], rootTransOrn,
                                                                         linkLinVel, [0, 0, 0, 1])
      #linkLinVelLocal=[linkLinVelLocal[0]-rootPosRel[0],linkLinVelLocal[1]-rootPosRel[1],linkLinVelLocal[2]-rootPosRel[2]]
      linkAngVelLocal, unused = self._pycram_bullet_client.multiplyTransforms([0, 0, 0], rootTransOrn,
                                                                         linkAngVel, [0, 0, 0, 1])
  ...
  def calcRootRotDiff(self, orn0, orn1):
    ...
    axis, angle = self._pycram_bullet_client.getAxisAngleFromQuaternion(q_diff)
    return angle * angle
  ...
  def getReward(self, pose):
    ...
    #print("kinematicHumanoid #joints=",self._pycram_bullet_client.getNumJoints(self._kin_model))
    #self.ApplyPose(pose, True, True, self._kin_model, self._pycram_bullet_client)
    ...
    rootPosSim, rootOrnSim = self._pycram_bullet_client.getBasePositionAndOrientation(self._sim_model)
    rootPosKin, rootOrnKin = self._pycram_bullet_client.getBasePositionAndOrientation(self._kin_model)
    linVelSim, angVelSim = self._pycram_bullet_client.getBaseVelocity(self._sim_model)
    ...
    if useArray:
      ...
      simJointStates = self._pycram_bullet_client.getJointStatesMultiDof(self._sim_model, jointIndices)
      kinJointStates = self._pycram_bullet_client.getJointStatesMultiDof(self._kin_model, jointIndices)
    if useArray:
      linkStatesSim = self._pycram_bullet_client.getLinkStates(self._sim_model, jointIndices)
      linkStatesKin = self._pycram_bullet_client.getLinkStates(self._kin_model, jointIndices)
    for j in range(num_joints):
      ...
      if useArray:
        ...
        simJointInfo = self._pycram_bullet_client.getJointStateMultiDof(self._sim_model, j)
      ...
      if useArray:
        ...
        kinJointInfo = self._pycram_bullet_client.getJointStateMultiDof(self._kin_model, j)
      ...
      if is_end_eff:
        ...
        if useArray:
          ...
          linkStateSim = self._pycram_bullet_client.getLinkState(self._sim_model, j)
          linkStateKin = self._pycram_bullet_client.getLinkState(self._kin_model, j)

...
class MinitaurBulletDuckEnv(gym.Env):
  ...
  def reset(self):
    if self._hard_reset:
      ...
      self._groundId = self._pycram_bullet_client.loadURDF("%s/plane.urdf" % self._urdf_root)
      self._duckId = self._pycram_bullet_client.loadURDF("%s/duck_vhacd.urdf" % self._urdf_root,
      ...
      self._pycram_bullet_client.resetBasePositionAndOrientation(self._duckId, duckStartPos,
                                                            duckStartOrn)
  ...
  def lost_duck(self):
    points = self._pycram_bullet_client.getContactPoints(self._duckId, self._groundId)
    return len(points) > 0

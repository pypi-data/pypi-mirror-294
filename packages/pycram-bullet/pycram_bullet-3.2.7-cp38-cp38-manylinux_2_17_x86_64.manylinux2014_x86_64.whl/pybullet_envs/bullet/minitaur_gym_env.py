...
class MinitaurBulletEnv(gym.Env):
  ...
  def reset(self):
    if self._hard_reset:
      ...
      plane = self._pycram_bullet_client.loadURDF("%s/plane.urdf" % self._urdf_root)
      self._pycram_bullet_client.changeVisualShape(plane, -1, rgbaColor=[1, 1, 1, 0.9])
      self._pycram_bullet_client.configureDebugVisualizer(
          self._pycram_bullet_client.COV_ENABLE_PLANAR_REFLECTION, 0)
      self._pycram_bullet_client.setGravity(0, 0, -10)
      acc_motor = self._accurate_motor_model_enabled
      motor_protect = self._motor_overheat_protection
      self.minitaur = (minitaur.Minitaur(pycram_bullet_client=self._pycram_bullet_client,
  ...
  def step(self, action):
    ...
    if self._is_render:
      ...
      camInfo = self._pycram_bullet_client.getDebugVisualizerCamera()
      ...
      self._pycram_bullet_client.resetDebugVisualizerCamera(distance, yaw, pitch, base_pos)
  ...
  def render(self, mode="rgb_array", close=False):
    ...
    proj_matrix = self._pycram_bullet_client.computeProjectionMatrixFOV(fov=60,
                                                                   ...
                                                                   farVal=100.0)
    (_, _, px, _,
     _) = self._pycram_bullet_client.getCameraImage(width=RENDER_WIDTH,

...
from pycram_bullet_envs.scene_abstract import Scene
...
class StadiumScene(Scene):
  ...
  def episode_restart(self, bullet_client):
    ...
    if (self.stadiumLoaded == 0):
      ...
      filename = os.path.join(pycram_bullet_data.getDataPath(), "plane_stadium.sdf")
      self.ground_plane_mjcf = self._p.loadSDF(filename)
      #filename = os.path.join(pycram_bullet_data.getDataPath(),"stadium_no_collision.sdf")
      #self.ground_plane_mjcf = self._p.loadSDF(filename)
      #
      for i in self.ground_plane_mjcf:
        self._p.changeDynamics(i, -1, lateralFriction=0.8, restitution=0.5)
        self._p.changeVisualShape(i, -1, rgbaColor=[1, 1, 1, 0.8])
        self._p.configureDebugVisualizer(pycram_bullet.COV_ENABLE_PLANAR_REFLECTION,i)

...
from pycram_bullet_envs.minitaur.envs import env_randomizer_base
...
class MinitaurTerrainRandomizer(env_randomizer_base.EnvRandomizerBase):
  ...
  def _load_triangle_mesh(self, env):
    ...
    Args:
      env: A minitaur gym environment.
    """
    env.pycram_bullet_client.removeBody(env.ground_id)
    terrain_collision_shape_id = env.pycram_bullet_client.createCollisionShape(
        shapeType=env.pycram_bullet_client.GEOM_MESH,
        fileName=self._mesh_filename,
        flags=1,
        meshScale=self._mesh_scale)
    env.ground_id = env.pycram_bullet_client.createMultiBody(
  ...
  def _generate_convex_blocks(self, env):
    ...
    for center in block_centers:
      ...
      box_id = env.pycram_bullet_client.createCollisionShape(
          env.pycram_bullet_client.GEOM_BOX, halfExtents=[half_length, half_length, half_height])
      env.pycram_bullet_client.createMultiBody(
          baseMass=0,

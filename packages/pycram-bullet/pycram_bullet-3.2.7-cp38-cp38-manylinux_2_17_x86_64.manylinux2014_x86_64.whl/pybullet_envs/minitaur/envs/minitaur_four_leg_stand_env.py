...
import numpy as np
from pycram_bullet_envs.minitaur.envs import minitaur_gym_env
...
class MinitaurFourLegStandEnv(minitaur_gym_env.MinitaurGymEnv):
  ...
  def reset(self):
    ...
    self._pycram_bullet_client.resetBasePositionAndOrientation(0, [0, 0, 0], [0, 0, 0, 1])
  ...
  def step(self, action):
    ...
    if t == 0:
      ...
      _, self._cur_ori = self._pycram_bullet_client.getBasePositionAndOrientation(0)
      self._goal_ori = self._pycram_bullet_client.getQuaternionFromEuler(
    ...
    ori = list(ori)
    print("ori=", ori)
    self._pycram_bullet_client.resetBasePositionAndOrientation(0, [0, 0, 0], ori)
    if self._env_step_counter % PERTURBATION_TOTAL_STEP == 0:
      ...
      if self._sign < 0.5:
        ...
        self._sign = 0.0
    self._pycram_bullet_client.applyExternalForce(
        objectUniqueId=1,
        linkIndex=-1,
        forceObj=[self._sign * self._perturbation_magnitude, 0.0, 0.0],
        posObj=[0.0, 0.0, 0.0],
        flags=self._pycram_bullet_client.LINK_FRAME)

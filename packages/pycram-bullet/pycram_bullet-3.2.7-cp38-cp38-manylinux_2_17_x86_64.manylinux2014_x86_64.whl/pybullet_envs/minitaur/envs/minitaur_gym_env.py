...
from pycram_bullet_envs.minitaur.envs import minitaur
from pycram_bullet_envs.minitaur.envs import minitaur_derpy
...
from pycram_bullet_envs.minitaur.envs import minitaur_rainbow_dash
...
class MinitaurGymEnv(gym.Env):
  ...
  def __init__(self,
               urdf_root=pycram_bullet_data.getDataPath(),
               urdf_version=None,
               distance_weight=1.0,
               energy_weight=0.005,
               shake_weight=0.0,
    ...
    if self._is_render:
      self._pycram_bullet_client = bc.BulletClient(connection_mode=pycram_bullet.GUI)
    else:
      self._pycram_bullet_client = bc.BulletClient()
  ...
  def reset(self, initial_motor_angles=None, reset_duration=1.0):
    self._pycram_bullet_client.configureDebugVisualizer(self._pycram_bullet_client.COV_ENABLE_RENDERING, 0)
    if self._env_step_counter > 0:
      self.logging.save_episode(self._episode_proto)
    self._episode_proto = minitaur_logging_pb2.MinitaurEpisode()
    ...
    if self._hard_reset:
      ...
      self._pycram_bullet_client.setTimeStep(self._time_step)
      self._ground_id = self._pycram_bullet_client.loadURDF("%s/plane.urdf" % self._urdf_root)
      if (self._reflection):
        self._pycram_bullet_client.changeVisualShape(self._ground_id, -1, rgbaColor=[1, 1, 1, 0.8])
        self._pycram_bullet_client.configureDebugVisualizer(
            self._pycram_bullet_client.COV_ENABLE_PLANAR_REFLECTION, self._ground_id)
      ...
      if self._urdf_version not in MINIATUR_URDF_VERSION_MAP:
        ...
        self.minitaur = (MINIATUR_URDF_VERSION_MAP[self._urdf_version](
            pycram_bullet_client=self._pycram_bullet_client,
    ...
    self._last_base_position = [0, 0, 0]
    self._objectives = []
    self._pycram_bullet_client.resetDebugVisualizerCamera(self._cam_dist, self._cam_yaw,
                                                     self._cam_pitch, [0, 0, 0])
    self._pycram_bullet_client.configureDebugVisualizer(self._pycram_bullet_client.COV_ENABLE_RENDERING, 1)
  ...
  def step(self, action):
    ...
    if self._is_render:
      ...
      self._pycram_bullet_client.resetDebugVisualizerCamera(dist, yaw, pitch, base_pos)
  ...
  def render(self, mode="rgb_array", close=False):
    ...
    view_matrix = self._pycram_bullet_client.computeViewMatrixFromYawPitchRoll(
        ...
        roll=0,
        upAxisIndex=2)
    proj_matrix = self._pycram_bullet_client.computeProjectionMatrixFOV(fov=60,
                                                                   aspect=float(RENDER_WIDTH) /
                                                                   RENDER_HEIGHT,
                                                                   nearVal=0.1,
                                                                   farVal=100.0)
    (_, _, px, _, _) = self._pycram_bullet_client.getCameraImage(
        width=RENDER_WIDTH,
        height=RENDER_HEIGHT,
        renderer=self._pycram_bullet_client.ER_BULLET_HARDWARE_OPENGL,
  ...
  def is_fallen(self):
    ...
    rot_mat = self._pycram_bullet_client.getMatrixFromQuaternion(orientation)
    local_up = rot_mat[6:]
  ...
  def _reward(self):
    ...
    orientation = self.minitaur.GetBaseOrientation()
    rot_matrix = pycram_bullet.getMatrixFromQuaternion(orientation)

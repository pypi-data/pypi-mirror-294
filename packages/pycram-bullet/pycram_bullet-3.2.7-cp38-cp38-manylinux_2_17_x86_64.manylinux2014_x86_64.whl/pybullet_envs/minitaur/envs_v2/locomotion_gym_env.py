...
from pycram_bullet_envs.minitaur.envs import minitaur_logging
from pycram_bullet_envs.minitaur.envs import minitaur_logging_pb2
#from pycram_bullet_envs.minitaur.envs import minitaur_logging
#from pycram_bullet_envs.minitaur.envs import minitaur_logging_pb2
from pycram_bullet_envs.minitaur.envs_v2.evaluation import metric_logger
from pycram_bullet_envs.minitaur.envs_v2.scenes import scene_base
from pycram_bullet_envs.minitaur.envs_v2.sensors import sensor
from pycram_bullet_envs.minitaur.envs_v2.sensors import space_utils
from pycram_bullet_envs.minitaur.envs_v2.utilities import rendering_utils
from pycram_bullet_envs.minitaur.robots import autonomous_object
from pycram_bullet_envs.minitaur.robots import robot_base
...
class LocomotionGymEnv(gym.Env):
  ...
  def __init__(self,
    ...
    Args:
      ...
      client_factory: A function to create a simulation client, it can be a
        pycram_bullet client.

    Raises:
      ValueError: If the num_action_repeat is less than 1.

    """
    self._pycram_bullet_client = None
    ...
    # pycram_bullet stepSimulation() has been called since last reset.
    ...
    if gym_config.simulation_parameters.enable_rendering:
      self._pycram_bullet_client = bullet_client.BulletClient(connection_mode=pycram_bullet.GUI)
      self._pycram_bullet_client.configureDebugVisualizer(
        pycram_bullet.COV_ENABLE_GUI,
        gym_config.simulation_parameters.enable_rendering_gui)
    else:
      self._pycram_bullet_client = bullet_client.BulletClient()
    if gym_config.simulation_parameters.egl_rendering:
      self._pycram_bullet_client.loadPlugin('eglRendererPlugin')

    self._pycram_bullet_client.setAdditionalSearchPath(
    ...
    if gym_config.profiling_path is not None:
      self._profiling_slot = self._pycram_bullet_client.startStateLogging(
          self._pycram_bullet_client.STATE_LOGGING_PROFILE_TIMINGS,
  ...
  def _load_old_robot_class(self):
    self._robot = self._robot_class(
        pycram_bullet_client=self._pycram_bullet_client, on_rack=self._on_rack)
    self._action_list = []
    action_upper_bound = []
    action_lower_bound = []
  ...
  def _load_new_robot_class(self):
    self._robot = self._robot_class(
        pycram_bullet_client=self._pycram_bullet_client, clock=self._clock)
    self.action_space = self._robot.action_space

  def _load(self):
    self._pycram_bullet_client.resetSimulation()
    self._pycram_bullet_client.setPhysicsEngineParameter(
        numSolverIterations=self._num_bullet_solver_iterations)
    self._pycram_bullet_client.setTimeStep(self._sim_time_step)
    self._pycram_bullet_client.setGravity(0, 0, -10)
    self._pycram_bullet_client.setPhysicsEngineParameter(enableConeFriction=0)

    # Disable rendering during scene loading will speed up simulation.
    if self._is_render:
      self._pycram_bullet_client.configureDebugVisualizer(
          self._pycram_bullet_client.COV_ENABLE_RENDERING, 0)

    # Rebuild the scene.
    self._scene.build_scene(self._pycram_bullet_client)
    ...
    if self._is_render:
      self._pycram_bullet_client.configureDebugVisualizer(
          self._pycram_bullet_client.COV_ENABLE_RENDERING, 1)

  def close(self):
    atexit.unregister(self.close)
    #if self._pycram_bullet_client:
    ...
    if self._robot:
      if self._use_new_robot_class:
        ...
        self._robot.Terminate()
    if self._pycram_bullet_client:
      self._pycram_bullet_client.disconnect()
      self._pycram_bullet_client = None
  ...
  def reset(
    ...
    if reset_visualization_camera:
      self._pycram_bullet_client.resetDebugVisualizerCamera(self._camera_dist,
                                                       self._camera_yaw,
                                                       self._camera_pitch,
                                                       [0, 0, 0])
  ...
  def _wait_for_rendering(self):
    ...
    [yaw, pitch, dist] = self._pycram_bullet_client.getDebugVisualizerCamera()[8:11]
    self._pycram_bullet_client.resetDebugVisualizerCamera(dist, yaw, pitch,
  ...
  def _step_old_robot_class(self, action):
    ...
    if self._profiling_slot >= 0:
      ...
      if self._profiling_counter == 0:
        self._pycram_bullet_client.stopStateLogging(self._profiling_slot)
  ...
  def render(self, mode='rgb_array'):
    ...
    if mode == 'topdown':
      ...
      if hasattr(self.scene, 'ground_height'):
        return rendering_utils.render_topdown(
            self._pycram_bullet_client, ground_height=self.scene.ground_height)
      else:
        return rendering_utils.render_topdown(self._pycram_bullet_client)
    ...
    view_matrix = self._pycram_bullet_client.computeViewMatrixFromYawPitchRoll(
        ...
        roll=0,
        upAxisIndex=2)
    proj_matrix = self._pycram_bullet_client.computeProjectionMatrixFOV(
        fov=60,
        aspect=float(self._render_width) / self._render_height,
        nearVal=0.1,
        farVal=100.0)
    return rendering_utils.render_image(self._pycram_bullet_client,

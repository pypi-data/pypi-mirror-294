# Lint as: python3
"""Class for loading and managing a scene in pycram_bullet."""
...
from pycram_bullet_envs.minitaur.envs_v2.scenes import world_asset_pb2
...
class SceneBase(object):
  ...
  def build_scene(self, pycram_bullet_client: bullet_client.BulletClient):
    """Loads and positions all scene objects in pycram_bullet.

    Override this function in subclass to implement customized scene. The
    overriding function must call base function first.

    Args:
      pycram_bullet_client: A pycram_bullet client in which the scene will be built.
    """
    self._reset_scene_tracking()
  ...
  def _init_dynamic_objects(self):
    ...
    for an_object in self._dynamic_objects:
      an_object.set_sim_client(self._pycram_bullet_client)
      self.add_object(an_object.sim_object_id, ObjectType.DYNAMIC_OBJECT,
                      an_object)

  @property
  def pycram_bullet_client(self) -> bullet_client.BulletClient:
    if self._pycram_bullet_client is None:
      raise ValueError("pycram_bullet_client is None; did you call build_scene()?")
  ...
  def add_object(self,
    ...
    Does not load anything into pycram_bullet.

    Args:
      object_id: objectUniqueId from pycram_bullet.
      class_label: What type to consider the new object.
      python_object: Associated python object for the pycram_bullet object of
  ...
  def remove_object(self, object_id: int):
    """Removes an object from tracking and from pycram_bullet.
    ...
    self.pycram_bullet_client.removeBody(object_id)
  ...
  def _dict_to_world_asset(
    ...
    Args:
      type_to_ids_dict: Dictionary that describes the scene. Keys are
        ...
        pycram_bullet id for an object of a given type.
    ...
    for object_type in type_to_ids_dict.keys():
      for obj_id in type_to_ids_dict[object_type]:
        bbox = np.array(self.pycram_bullet_client.getAABB(obj_id))

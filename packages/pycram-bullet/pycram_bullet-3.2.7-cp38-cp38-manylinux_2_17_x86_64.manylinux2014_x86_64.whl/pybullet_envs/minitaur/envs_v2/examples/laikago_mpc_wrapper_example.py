...
def _run_example():
  ...
  if FLAGS.video_file is not None:
    pycram_bullet = env.pycram_bullet_client
    pycram_bullet.configureDebugVisualizer(pycram_bullet.COV_ENABLE_GUI, 0)
    log_id = pycram_bullet.startStateLogging(pycram_bullet.STATE_LOGGING_VIDEO_MP4,
                                        FLAGS.video_file)
  ...
  try:
    ...
    if FLAGS.video_dir is not None:
      pycram_bullet.stopStateLogging(log_id)

...
# for reference. The corresponding link/joint id is always -1 in pycram_bullet.
...
class RobotUrdfLoader(object):
  ...
  def __init__(
    ...
    if init_base_orientation_quaternion is not None:
      ...
      self._init_base_orientation_quaternion = (
          self._pycram_bullet_client.getQuaternionFromEuler(
  ...
  @property
  def robot_id(self):
    """Returns the unique object instance id of this loaded URDF in pycram_bullet.
  ...
  def _build_base_dict(self):
    ...
    In pycram_bullet, a link's id within the robot always equal to its parent joint
  ...
  def _build_all_joint_dict(self):
    ...
    Returns:
      number of joints, all joint information as returned by pycram_bullet, and the
      joint_name_to_id dictionary.

    """
    num_joints = self._pycram_bullet_client.getNumJoints(self._robot_id)
    all_joint_info = [
        self._pycram_bullet_client.getJointInfo(self._robot_id, i)
  ...
  def _load_urdf(self, enable_self_collision: bool,
    ...
    """Loads the URDF and returns the pycram_bullet id."""
    try:
      if enable_self_collision:
        return self._pycram_bullet_client.loadURDF(
            self._urdf_path,
            init_base_position,
            init_base_orientation_quaternion,
  ...
  def reset_base_pose(
    ...
    self._pycram_bullet_client.resetBaseVelocity(self._robot_id, (0, 0, 0),
                                            (0, 0, 0))

...
def joint_angles_from_link_position(
  ...
  if not position_in_world_frame:
    ...
    base_position, base_orientation = robot.pycram_bullet_client.multiplyTransforms(
        base_position, base_orientation, base_translation, base_rotation)

    # Projects to world space.
    world_link_pos, _ = robot.pycram_bullet_client.multiplyTransforms(
        base_position, base_orientation, link_position, _IDENTITY_ORIENTATION)
  else:
    world_link_pos = link_position

  ik_solver = 0
  all_joint_angles = robot.pycram_bullet_client.calculateInverseKinematics(
...
def link_position_in_world_frame(
  ...
  """
  return np.array(
      robot.pycram_bullet_client.getLinkState(robot.quadruped, link_id)[0])
...
def link_position_in_base_frame(
  ...
  inverse_translation, inverse_rotation = robot.pycram_bullet_client.invertTransform(
      base_position, base_orientation)

  link_state = robot.pycram_bullet_client.getLinkState(robot.quadruped, link_id)
  link_position = link_state[0]
  link_local_position, _ = robot.pycram_bullet_client.multiplyTransforms(
...
def compute_jacobian(
  ...
  jv, _ = robot.pycram_bullet_client.calculateJacobian(robot.quadruped, link_id,

...
from pycram_bullet_envs.minitaur.envs_v2.sensors import sensor as sensor_lib
from pycram_bullet_envs.minitaur.robots import hybrid_motor_model
from pycram_bullet_envs.minitaur.robots import robot_base
from pycram_bullet_envs.minitaur.robots import robot_config
from pycram_bullet_envs.minitaur.robots import robot_urdf_loader
from pycram_bullet_envs.minitaur.robots.safety import data_types as safety_data_types
from pycram_bullet_envs.minitaur.robots.utilities import kinematics_utils
...
class QuadrupedBase(robot_base.RobotBase):
  ...
  def __init__(
    ...
    Args:
      pycram_bullet_client: The PyBullet client.
  ...
  def load(
    ...
    self._init_urdf_position, self._init_orientation_quat = (
        self._pycram_bullet_client.getBasePositionAndOrientation(
            self._urdf_loader.robot_id))
    unused_position, self._init_orientation_inv_quat = (
        self._pycram_bullet_client.invertTransform(
  ...
  def _get_base_roll_pitch_yaw_rate(self):
    _, angular_velocity = self._pycram_bullet_client.getBaseVelocity(
        self._urdf_loader.robot_id)
  ...
  def _get_base_velocity(self):
    base_velocity, _ = self._pycram_bullet_client.getBaseVelocity(
        self._urdf_loader.robot_id)
  ...
  def _update_base_acceleration(self):
    ...
    if self._last_observation_time < self.timestamp:
      ...
      _, inv_base_orientation = self.pycram_bullet_client.invertTransform(
      ...
      base_acceleration_accelerometer = self.pycram_bullet_client.multiplyTransforms(
  ...
  def receive_observation(self):
    ...
    self._base_position, base_orientation_quat = (
        self._pycram_bullet_client.getBasePositionAndOrientation(
            self._urdf_loader.robot_id))
    _, self._base_orientation_quat = self._pycram_bullet_client.multiplyTransforms(
        positionA=(0, 0, 0),
    ...
    self._base_roll_pitch_yaw = self._pycram_bullet_client.getEulerFromQuaternion(
  ...
  def motor_angles_from_foot_positions(self,
    ...
    joint_angles = kinematics_utils.joint_angles_from_link_positions(
        pycram_bullet_client=self.pycram_bullet_client,
  ...
  def feet_contact_forces(self) -> Sequence[np.ndarray]:
    ...
    all_contacts = self._pycram_bullet_client.getContactPoints(
    ...
    for contact in all_contacts:
      ...
      if link_a_id in foot_link_ids:
        ...
        local_force = kinematics_utils.rotate_to_base_frame(
            self._pycram_bullet_client, self.urdf_loader.robot_id, force,
            self._init_orientation_inv_quat)

...
SubstantialOperationType = Union[SubstantialOperationTypeStart, SubstantialOperationTypeStop, SubstantialOperationTypeSend]
...
class Runtimes:
    ...
    def register_substantial_runtime(self, caller: wasmtime.Store, data: SubstantialRuntimeData) -> Result[RuntimeId, Error]:
        record = data
        field = record.endpoint
        field0 = record.basic_auth_secret
        ptr, len1 = _encode_utf8(field, self.component._realloc0, self.component._core_memory0, caller)
        if field0 is None:
            variant = 0
            variant5 = 0
            ...
            variant5 = ptr3
            variant6 = len4
        ret = self.component.lift_callee69(caller, ptr, len1, variant, variant5, variant6)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[RuntimeId, Error]
        if load == 0:
    ...
    def generate_substantial_operation(self, caller: wasmtime.Store, runtime: RuntimeId, data: SubstantialOperationData) -> Result[FuncParams, Error]:
        ...
        if field is None:
            ...
            variant = 1
            variant2 = _clamp(payload1, 0, 4294967295)
        if isinstance(field0, SubstantialOperationTypeStart):
            payload3 = field0.value
            record4 = payload3
            field5 = record4.name
            field6 = record4.file
            field7 = record4.deps
            field8 = record4.kind
            ptr, len9 = _encode_utf8(field5, self.component._realloc0, self.component._core_memory0, caller)
            ptr10, len11 = _encode_utf8(field6, self.component._realloc0, self.component._core_memory0, caller)
            vec = field7
            len15 = len(vec)
            result = self.component._realloc0(caller, 0, 0, 4, len15 * 8)
            assert(isinstance(result, int))
            for i16 in range(0, len15):
                e = vec[i16]
                base12 = result + i16 * 8
                ptr13, len14 = _encode_utf8(e, self.component._realloc0, self.component._core_memory0, caller)
                _store(ctypes.c_uint32, self.component._core_memory0, caller, base12, 4, len14)
                _store(ctypes.c_uint32, self.component._core_memory0, caller, base12, 0, ptr13)
            variant53 = 0
            variant54 = ptr
            ...
            variant58 = result
            variant59 = len15
            variant60 = (field8).value
        elif isinstance(field0, SubstantialOperationTypeStop):
            payload17 = field0.value
            record18 = payload17
            field19 = record18.name
            field20 = record18.file
            field21 = record18.deps
            field22 = record18.kind
            ptr23, len24 = _encode_utf8(field19, self.component._realloc0, self.component._core_memory0, caller)
            ptr25, len26 = _encode_utf8(field20, self.component._realloc0, self.component._core_memory0, caller)
            vec31 = field21
            len33 = len(vec31)
            result32 = self.component._realloc0(caller, 0, 0, 4, len33 * 8)
            assert(isinstance(result32, int))
            for i34 in range(0, len33):
                e27 = vec31[i34]
                base28 = result32 + i34 * 8
                ptr29, len30 = _encode_utf8(e27, self.component._realloc0, self.component._core_memory0, caller)
                _store(ctypes.c_uint32, self.component._core_memory0, caller, base28, 4, len30)
                _store(ctypes.c_uint32, self.component._core_memory0, caller, base28, 0, ptr29)
            variant53 = 1
            variant54 = ptr23
            variant55 = len24
            variant56 = ptr25
            ...
            variant58 = result32
            variant59 = len33
            variant60 = (field22).value
        elif isinstance(field0, SubstantialOperationTypeSend):
            payload35 = field0.value
            record36 = payload35
            field37 = record36.name
            field38 = record36.file
            field39 = record36.deps
            field40 = record36.kind
            ptr41, len42 = _encode_utf8(field37, self.component._realloc0, self.component._core_memory0, caller)
            ptr43, len44 = _encode_utf8(field38, self.component._realloc0, self.component._core_memory0, caller)
            vec49 = field39
            len51 = len(vec49)
            result50 = self.component._realloc0(caller, 0, 0, 4, len51 * 8)
            assert(isinstance(result50, int))
            ...
            variant58 = result50
            variant59 = len51
            variant60 = (field40).value
        else:
            raise TypeError("invalid variant specified for SubstantialOperationType")
        ret = self.component.lift_callee70(caller, _clamp(runtime, 0, 4294967295), variant, variant2, variant53, variant54, variant55, variant56, variant57, variant58, variant59, variant60)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[FuncParams, Error]
        if load == 0:
            load61 = _load(ctypes.c_int32, self.component._core_memory0, caller, ret, 4)
            load62 = _load(ctypes.c_int32, self.component._core_memory0, caller, ret, 8)
            load63 = _load(ctypes.c_int32, self.component._core_memory0, caller, ret, 12)
            expected = Ok(core.FuncParams(load61 & 0xffffffff, load62 & 0xffffffff, load63 & 0xffffffff))
        elif load == 1:
            ...
            expected = Err(core.Error(result73))
        else:
            raise TypeError("invalid variant discriminant for expected")
        self.component._post_return0(caller, ret)
        return expected
    def register_kv_runtime(self, caller: wasmtime.Store, data: KvRuntimeData) -> Result[RuntimeId, Error]:
        ...
        field = record.url
        ptr, len0 = _encode_utf8(field, self.component._realloc0, self.component._core_memory0, caller)
        ret = self.component.lift_callee71(caller, ptr, len0)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[RuntimeId, Error]
        if load == 0:
    ...
    def kv_operation(self, caller: wasmtime.Store, base: BaseMaterializer, data: KvMaterializer) -> Result[MaterializerId, Error]:
        ...
        if isinstance(field0, EffectRead):
            ...
            raise TypeError("invalid variant specified for Effect")
        ret = self.component.lift_callee72(caller, _clamp(field, 0, 4294967295), variant, variant4, (data).value)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[MaterializerId, Error]
        if load == 0:

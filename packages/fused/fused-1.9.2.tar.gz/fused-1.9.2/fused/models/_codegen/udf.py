...
from fused._str_utils import table_to_name
...
def stringify_input(input) -> str:
    if isinstance(input, str):
        ...
        if input.operation == DatasetInputV2Type.ZIP:
            op_fn = "fused.zip_tables"
        elif input.operation == DatasetInputV2Type.UNION:
            op_fn = "fused.union_tables"
        ...
        return f"fused.zip_tables([{', '.join([repr(f'{base_path}/{t}') for t in input.tables])}])"

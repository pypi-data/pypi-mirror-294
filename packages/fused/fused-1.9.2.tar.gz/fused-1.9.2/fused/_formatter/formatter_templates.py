...
from fused._formatter.common import load_static_files
from fused._formatter.template import template_section

if TYPE_CHECKING:
    from fused._templates import FusedTemplate
...
def fused_templates_repr(
    ...
    obj_type = f"fused.templates.{name}"
...
def fused_template_repr(template: FusedTemplate) -> str:
    obj_type = f"fused.templates.{template.__class__.__name__}: {template.name}"
    header_components = [f"<div class='xr-obj-type'>{escape(obj_type)}</div>"]

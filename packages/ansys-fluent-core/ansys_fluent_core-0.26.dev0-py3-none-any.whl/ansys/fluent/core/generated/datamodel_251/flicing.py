    ...
    class Case(PyMenu):
        ...
        class App(PyMenu):
            ...
            def __init__(self, service, rules, path):
                ...
                self.Solution = self.__class__.Solution(service, rules, path + [("Solution", "")])
                self.UserDefined = self.__class__.UserDefined(service, rules, path + [("UserDefined", "")])
            ...
            class UserDefined(PyMenu):
                """
                Singleton UserDefined.
                """
                def __init__(self, service, rules, path):
                    self.BuildUDFLib = self.__class__.BuildUDFLib(service, rules, "BuildUDFLib", path)
                    self.LoadUDFLib = self.__class__.LoadUDFLib(service, rules, "LoadUDFLib", path)
                    self.Memories = self.__class__.Memories(service, rules, "Memories", path)
                    self.OnDemand = self.__class__.OnDemand(service, rules, "OnDemand", path)
                    super().__init__(service, rules, path)

                class BuildUDFLib(PyCommand):
                    """
                    Command BuildUDFLib.

...
class SCC_zipfile:
    ...
    def __init__(self, path: Path):
        ...
        self.unzipped_path = self.extract_zip()
    ...
    def check_path(self, path: Path) -> Path:
        ...
        # check zip
        if not path.suffix.endswith("zip"):
            raise ValueError(f"Filepath {path} is not a zip file.")
        return path
    ...
    def extract_zip(
        self,
        pattern_or_list: str | list[str] = rf"\.nc$",
        destination: Path | None = None,
    ) -> Path | None:
        """Extract zip file.

        Args:
            pattern (str, optional): Pattern to search in zip file. Defaults to f"*.nc".
            destination (Path | None, optional): Destination path to extract zip file. Defaults to None means current working directory.

        Returns:
            Path | None: Path of unzipped folder or None if nothing is extracted.
        """
        return unzip_file(

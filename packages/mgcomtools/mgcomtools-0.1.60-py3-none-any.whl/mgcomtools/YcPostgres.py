...
def execute(conn, query, params=None, fetch=False):
    ...
    try:
        with conn.cursor() as cur:
            # Выполнение запроса с параметрами (если переданы)
            cur.execute(query, params)
            
            # Если запрос на выборку данных, то получаем результат
            if fetch:
                rows = cur.fetchall()
                return rows
            else:
                # Фиксируем изменения для запросов типа INSERT, UPDATE, DELETE
                conn.commit()

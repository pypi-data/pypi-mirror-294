...
class ErrorLogger:
    def __init__(self, config_file='config.ini'):
        ...
        if 'api_endpoint' not in config['DEFAULT']:
            raise ValueError("Configuration file is missing required 'api_endpoint'.")
        ...
        self.tag = config['DEFAULT'].get('tag', 'General')
    ...
    def collect_logs(self):
        try:
            ...
            log_data = {
                ...
                "app_type_id": 1,  # Example value, modify as needed
                "user_id": 1,  # Example value, modify as needed
                "category": self.tag,
                "category_description": "Auto-generated log",
                "exception_details": logs.get('error', 'No error'),
                "message": logs.get('message', ''),
                "stack_trace": logs.get('stack_trace', '')
            }
            ...
            self.logger.error(error_info)
            self._send_logs_to_api({'exception_details': error_info})

    def _send_logs_to_api(self, logs):
        try:
            self.logger.info(f"Sending logs to API: {logs}")  # Add this line
            response = requests.post(self.api_endpoint, json=logs)
            response.raise_for_status()  # Ensure that the request was successful
            self.logger.info("Logs sent to API successfully.")
        except requests.exceptions.RequestException as e:
    ...
    def log_exception(self, exception: Exception, message: str = None):
        ...
        log_data = {
            ...
            "user_id": 1,
            "category": self.tag,
            "category_description": "Exception captured",

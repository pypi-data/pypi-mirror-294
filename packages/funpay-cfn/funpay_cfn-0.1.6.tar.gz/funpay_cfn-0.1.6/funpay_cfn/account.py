...
class Account:
    ...
    def get_all_user_lots(self, user_id: int) -> list[dict]:
        """
        Получает список всех опубликованных лотов переданной подкатегории.
        ...
        for offer in offers:
            ...
            price = float('{:.1f}'.format(float(offer.find("div", {"class": "tc-price"})["data-s"])))

            price_unit = offer.find("span", {"class": "unit"})
            price_unit = price_unit.text if price_unit else None

            lot_obj = {"offer_id" : offer_id, "description" : description, "price" : price, "price_unit" : price_unit}
            result.append(lot_obj)
        return result

    def get_balance(self, lot_id: int = 28436563) -> types.Balance:
        """
    ...
    def get_buyer_looking_at(self, chat_id: int | str) -> dict:
        ...
        response = self.method("get", f"chat/?node={chat_id}", headers, payload, raise_not_200=True)

        soup = BeautifulSoup(response.content, 'html.parser')
        param_item = soup.find('div', class_='param-item chat-panel')

        link = param_item.find('div').find('a') if param_item else None


        url = link.get('href') if link else None
        text = link.text.strip() if link else None
    ...
    def raise_lots(self, category_id: int, subcategories: Optional[list[int | types.SubCategory]] = None,
        ...
        if not json_response.get("error"):
            return {'status': 'success', 'wait_time' : None}

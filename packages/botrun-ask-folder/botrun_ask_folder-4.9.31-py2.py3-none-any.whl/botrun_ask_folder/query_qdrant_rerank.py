...
def query_qdrant_rerank(
    ...
    qdrant_client_instance: QdrantClient = QdrantClient(qdrant_host, port=qdrant_port)
    query_vector: Dict[str, List[Dict[str, List[float]]]] = generate_embedding_sync(embedding_model, user_input)
    search_params: models.SearchParams = models.SearchParams(hnsw_ef=hnsw_ef, exact=False)
...
def print_results(result: Dict[str, Any], user_input: str, top_k: int) -> None:
    ...
    for idx, hit in enumerate(result['search_result'], start=1):
        ...
        print(f"  {result['text_content_field']}: {hit.payload.get(result['text_content_field'], 'N/A')[0:200]}...")
...
def main() -> None:
    ...
    parser: argparse.ArgumentParser = argparse.ArgumentParser(description="Query Qdrant and rerank results")
    ...
    parser.add_argument("--embedding_model", type=str, default="openai/text-embedding-3-large")

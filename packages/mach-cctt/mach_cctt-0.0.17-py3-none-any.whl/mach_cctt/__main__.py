...
If set to a string SYMBOL, then chain-SYMBOL will be used as the destination token, where chain is randomly chosen and the symbol is fixed.
Defaults to {DEFAULT_DESTINATION_POLICY}.
"""

def load_account(account_filepath: pathlib.Path) -> dict:
        ...
        case "run":
            ...
            match destination_policy_str:
                case "0":
                    ...
                    destination_policy = RandomChainRandomSymbolpolicy()

                case _ as symbol:
                    print(f"Destination token policy: fixed symbol {symbol}")
                    destination_policy = RandomChainFixedSymbolPolicy(symbol)

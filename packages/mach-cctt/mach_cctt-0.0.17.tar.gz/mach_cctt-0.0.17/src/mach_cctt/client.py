...
class MachClient:
        ...
        for chain, data in deployments.items():
            for symbol, symbol_data in deployments[chain]["assets"].items():
                if symbol_data.get("wrapped"):
                    assert symbol not in self.gas_tokens_, f"Error: more than 1 gas token on {chain}: {self.gas_tokens_['chain']}, {symbol}"
    ...
    def get_token_balances(self, wallet_address: str) -> dict[str, dict[str, int]]:
        ...
        return {key: value for key, value in full_balances.items() if key in chains}

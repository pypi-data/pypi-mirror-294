...
class SimplePgSQL:
    ...
    def execute(self, query: str | sql, columns: list | None = None) -> dict | list | pd.DataFrame:
        """
    ...
    def read(self,
        ...
        if conditions is not None:
            ...
            if any(_o.upper() in operators for _o in spl_columns):
                ...
                conditions = {column: (value, operator) for column, (value, operator) in conditions.items() if
    ...
    def format_result(self):
        ...
        if self.return_type == list:
            ...
            if not self.aggregate:
                ...
                _columns = [_col if _col not in self.aggregate else f"{_col}.{self.aggregate[_col].lower()}" for _col in
                            self.columns]

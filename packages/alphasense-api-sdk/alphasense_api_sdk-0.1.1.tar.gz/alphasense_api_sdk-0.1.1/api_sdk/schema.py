...
def get_graphql_schema_from_url(
    ...
    introspect: bool = True,  # set introspect to False when server does not support introspection
) -> GraphQLSchema:
    return (
        build_client_schema(
        ...
        if introspect
        else get_graphql_schema_from_str(fetch_schema_from_url(url, headers))
    )
...
def fetch_schema_from_url(url: str, headers: Optional[Dict[str, str]] = None) -> str:
    try:
...
def get_graphql_schema_from_str(schema_str: str) -> GraphQLSchema:
    """Get graphql schema build from provided string."""
    graphql_ast = parse(schema_str)
    schema: GraphQLSchema = build_ast_schema(graphql_ast, assume_valid=True)
    return schema

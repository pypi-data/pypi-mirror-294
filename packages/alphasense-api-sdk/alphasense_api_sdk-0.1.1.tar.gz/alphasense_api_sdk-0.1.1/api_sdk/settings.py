...
class AuthSettings(BaseSettings):
    ...
    def __post_init__(self):
        self._set_auth_attribute("username", ALPHASENSE_USERNAME)
        self._set_auth_attribute("password", ALPHASENSE_PASSWORD)
        self._set_auth_attribute("api_key", ALPHASENSE_API_KEY)
        self._set_auth_attribute("client_id", ALPHASENSE_CLIENT_ID)
        self._set_auth_attribute("client_secret", ALPHASENSE_CLIENT_SECRET)
    ...
    def _set_auth_attribute(self, attr_name: str, env_var: str):
        if not getattr(self, attr_name):
            value = os.environ.get(env_var)
            if not value:
                raise InvalidConfiguration(
                    f"Environment variable {env_var} or '{attr_name}' config not found."
                )

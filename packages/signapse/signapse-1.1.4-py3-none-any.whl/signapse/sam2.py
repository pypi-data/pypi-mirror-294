...
def get_wrist_points(frame):    
    ...
    MP_tenor = POSE(width).results_to_small_tensor(results,None)   
    MP_tenor[:,0] = MP_tenor[:,0] * width
    MP_tenor[:,1] = MP_tenor[:,1] * height
    # left_ wrist
    lw = np.array(MP_tenor[4,:],dtype=int)
    rw = np.array(MP_tenor[5,:],dtype=int)
    # left hand
...
class SAM2Initializer:
    ...
    def add_points(self,boxes, points, labels, ann_frame_idx, ann_obj_id, predictor, inference_state):
    ...
    def add_point_to_the_predictor(self,NUM_Frame, sam2_initializer, inference_state, tmp_image_dir, predictor):
        if isinstance(NUM_Frame, int):
            ...
            labels, points, boxes = get_wrist_points(first_frame)
            ann_obj_id = 1
            predictor = sam2_initializer.add_points(boxes,points,labels, NUM_Frame, ann_obj_id, predictor, inference_state)
        elif isinstance(NUM_Frame, list):
            for num in NUM_Frame:
                first_frame = cv2.imread(os.path.join(tmp_image_dir,f'{num:05d}.jpg'))
                labels, points, boxes = get_wrist_points(first_frame)
                ann_obj_id = 1
                predictor = sam2_initializer.add_points(boxes,points,labels, num, ann_obj_id, predictor, inference_state)
        return predictor
...
def get_masks_sam2(input_video, num_correction ,stop, tmp_dir = "./inputs/tmp"):
    ...
    while len(top_image_differences) > 0  and counter < 3 :  # No more than a certain times for corrections.
        if NUM_Frame[0] >= top_image_differences[0]: ## break if earlier frame failed 
            ...
            break
        NUM_Frame = top_image_differences 

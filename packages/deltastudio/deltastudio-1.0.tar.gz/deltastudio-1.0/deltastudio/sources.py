...
postgres_conn = 'postgresql://postgres:PiIs&&&31415926535@postgres:5432/postgres'
...
class parquet:
    def __init__(self, app, source_name: str, file_path: str):
        self.file_path = file_path
        self.source_name = source_name
        self.app = app

    def read_parquet_to_spark_dataframe(self):
        return self.app.spark.read.parquet(self.file_path)
    ...
    def el_parquet(self, location: str = '', table_full_name: str = '', mode = 'append' , chunk_size: int = 0):
        t1 = time()
        print('Reading the data ...')
        try:
            df = self.read_parquet_to_spark_dataframe()
        except:
            return 'Source is not available.'
...
class csv:
    ...
    def read_csv_to_spark_dataframe(self):
        return self.app.spark.read.option('header', 'true').option('inferSchema', 'true').csv(self.file_path)
    ...
    def el_csv(self, location: str = '', table_full_name: str = '', mode = 'append' , chunk_size: int = 0):
        t1 = time()
        print('Reading the data ...')
        try:
            df = self.read_csv_to_spark_dataframe()
        except:
            return 'Source is not available.'

...
from google.cloud.tasks_v2 import Task, HttpMethod, Queue, Attempt, ListQueuesResponse, ListTasksResponse
from google.protobuf import empty_pb2
from google.protobuf import timestamp_pb2

from gcloud_tasks_emulator.cloudtasks_pb2_grpc import CloudTasksServicer, add_CloudTasksServicer_to_server
...
def _make_task_request(queue_name: str, task: Task, host: str, port: int):
    ...
    task.schedule_time = cast(DatetimeWithNanoseconds, task.schedule_time)

    if task.app_engine_http_request.relative_uri:
        method = task.app_engine_http_request.http_method.name
        data = task.app_engine_http_request.body

        url = f"http://{host}:{port}{task.app_engine_http_request.relative_uri}"
        ...
        headers.update({
            ...
            'X-AppEngine-TaskETA': str(task.schedule_time.timestamp()) if task.schedule_time else "",
        })
    elif task.http_request.url:
        method = task.http_request.http_method.name
        ...
        headers.update({
            ...
            'X-CloudTasks-TaskETA': str(task.schedule_time.timestamp()) if task.schedule_time else "",
        })
    else:
...
class QueueState(object):
    ...
    def __init__(self, target_host: str, target_port: int, max_retries: int):
        ...
        self._queue_tasks: Dict[str, List[Task]] = {}
    ...
    def create_queue(self, parent, queue: Queue):
        ...
        if queue.name not in self._queues:
            ...
            self._queues[queue.name].state = Queue.State.RUNNING
    ...
    def create_task(self, queue: str, task: Task):
        task.name = task.name or f"{queue}/tasks/{int(datetime.now().timestamp() * 1000000)}"

        if task.app_engine_http_request.relative_uri:
            # Set a default http_method
            task.app_engine_http_request.http_method = (
                task.app_engine_http_request.http_method or HttpMethod.POST
            )
        elif task.http_request.url:
            # Set a default http_method
            task.http_request.http_method = (
                task.http_request.http_method or HttpMethod.POST
            )
    ...
    def pause_queue(self, queue_name: str):
        if queue_name in self._queues:
            ...
            self._queues[queue_name].state = Queue.State.PAUSED
            return self._queues[queue_name]
        else:
            logger.error(
    ...
    def resume_queue(self, queue_name: str):
        if queue_name in self._queues:
            ...
            self._queues[queue_name].state = Queue.State.RUNNING
            return self._queues[queue_name]
        else:
            logger.error(
    ...
    def submit_task(self, task_name: str, force_run: bool = False) -> Task:
        ...
        try:
            ...
            if queue_name not in self._queue_tasks:
                raise ValueError(
                    f"'{queue_name}' is not a valid queue name. "
                    f"Valid queues are: {list(self._queue_tasks.keys())}."
                )
        except IndexError:
            # Invalid task name, raise ValueError
            raise ValueError(
                f"Invalid task name '{task_name}'. Expected at least 3 slash-separated parts."
            )
        ...
        for i, task in enumerate(self._queue_tasks[queue_name]):
            ...
            raise NotFound(f"Task not found: {task_name}")

        def now() -> timestamp_pb2.Timestamp:
        ...
        try:
            ...
            response_status = 500
            logger.exception("[TASKS] Unexpected exception while submitting task %s.", task_name)
        else:
        ...
        attempt = Attempt(
            ...
            response_status={"code": response_status},
        )

        task.first_attempt = task.first_attempt or attempt
...
class Greeter(CloudTasksServicer):
    ...
    def ListQueues(self, request, context):
        try:
            ...
            return ListQueuesResponse(queues=queues)
        except GoogleAPICallError as err:
            context.abort(err.grpc_status_code, err.message)
...
class APIThread(threading.Thread):
    ...
    def run(self):
        ...
        add_CloudTasksServicer_to_server(
            Greeter(self._state), self._httpd
        )

        interface = f'[::]:{self._port}'

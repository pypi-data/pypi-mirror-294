...
def r_wilcoxsign_test(GroupA, GroupB, print_result=False, print_report=True):
    # Install and import R packages
    utils = rpackages.importr('utils')
    utils.chooseCRANmirror(ind=1)
    utils.install_packages("coin")
    ...
    pattern_test2 = r"Z\s*=\s*(-?\d+\.\d+),\s*p-value\s*=\s*(\d+\.\d+)"
    match_test2 = re.search(pattern_test2, test_result2_str)

    if match_test2:
        z_value = float(match_test2.group(1))
        p_value = float(match_test2.group(2))
    else:
        z_value, p_value = None, None
    ...
    if print_report:
        ...
        print(f"$W = {w_value}, Z = {z_value}, p = {p_value}{effect_size}$")

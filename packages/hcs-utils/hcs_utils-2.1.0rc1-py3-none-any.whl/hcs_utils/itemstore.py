...
class Store:
    ...
    def mkitem(self, item_name):
        ...
        log.debug(f"mkitem {item_name} {path}")
        try:
            os.makedirs(path)
        except OSError as e:
            if e.errno == errno.EEXIST:
                msg = f"Item already exist: {item_name}"
...
class StoreGroup:
    ...
    def mkstore(self, store_name):
        ...
        log.debug(f"mkstore {store_name} {path}")
        os.mkdir(path)
    ...
    def move(self, item_name, from_store, to_store):
        log.debug(f"move {item_name} from {from_store} to {to_store}")

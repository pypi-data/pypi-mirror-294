...
class memoize:  # noqa: N801
    ...
    shared by all instances of the class. This implemetation do in that
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            if self.is_instance_method_call and not self.static_cache:
                ...
                key = (obj, frozenset(paras), frozenset(list(kwargs.items())))
            else:
                key = (frozenset(paras), frozenset(list(kwargs.items())))

...
class Daemon:
    ...
    def __init__(self, name, run, pid_dir="/var/run"):
        ...
        self.name = name
        self.run = run
        self.lockfile = os.path.join(pid_dir, f"{name}.pid")
    ...
    def start(self, foreground=False):
        ...
        try:
            ...
            print("Failed to create lock for", self.name, error, file=sys.stderr)
            self.log.error(f"Failed to create lock for {self.name}")
            sys.exit(1)
        msg = f"Starting {self.name} daemon."
        print(msg)
        self.log.info(msg)
        if not foreground:
            daemonize()
    ...
    def stop(self, wait=True, signaller=send_sigterm):
        ...
        if pid:
            msg = f"Stopping {self.name} daemon. With PID: {pid}"
            print(msg)
            self.log.info(msg)
            signaller(pid)
            if wait:
    ...
    def kill(self, wait=True):
        pid = self.get_running_pid()
        if pid:
            msg = f"Killing {self.name} daemon. With PID: {pid}"
            print(msg)
            self.log.info(msg)
    ...
    def status(self):
        pid = self.get_running_pid()
        if pid:
            print(f"{self.name} process is running with PID: {pid}")
        else:

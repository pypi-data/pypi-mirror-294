...
class Storage:
    ...
    # __hash__ = None  # this is done implicitly when overriding __eq__

    def __repr__(self):
        items = []
        if isinstance(self._dict, collections.defaultdict):
            items.append(f"default_factory={self._dict.default_factory}")
        items.extend(
            f"{self._denormalize(i[0])}={i[1]!r}" for i in sorted(self._dict.items())
        )
...
def storageify(obj, storageFactory=Storage):  # noqa: N803
    """

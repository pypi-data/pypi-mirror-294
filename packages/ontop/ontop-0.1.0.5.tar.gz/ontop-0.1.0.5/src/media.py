...
#    url = "https://data.cyber.org.il/OnTopTech/library/greetings01.txt"
...
def read_banner_gifs():
    url = "https://data.cyber.org.il/OnTopTech/library/banners.txt"
    response = urllib.request.urlopen(url)
    data = response.read().decode("utf-8")  # Decode bytes to string
    list = []
    for line in data.splitlines():
...
def read_gifs():
    url = "https://data.cyber.org.il/OnTopTech/library/gifs.txt"
    response = urllib.request.urlopen(url)
    data = response.read().decode("utf-8")  # Decode bytes to string
    list = []
    for line in data.splitlines():

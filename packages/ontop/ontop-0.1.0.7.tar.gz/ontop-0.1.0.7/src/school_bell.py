...
def print_image(w, h):
  ...
  if score < 2:
    ...
    return
  html = '''
<img src='https://data.cyber.org.il/OnTopTech/school_bell/images/Riddle.jpg' alt="password" width="*w*" height="*h*">
  '''
...
def play_video(start, end):
  ...
  if score < 4:
    ...
    if language == 'hebrew':
        ...
        if ((start >= 5 and start <=7) ) and (end >= 25 and end <= 30):
          if score == 4:
            score = 5
  video_width = 700
  if language == 'hebrew':
    video_path = 'https://data.cyber.org.il/OnTopTech/school_bell/video/bell.mp4'
  else:
    video_path = 'https://data.cyber.org.il/OnTopTech/school_bell/video/bell_arabic.mp4'
...
def add_greetings():
  ...
  html = download_html('https://data.cyber.org.il/OnTopTech/school_bell/HTML/Certificate08.html') #(url) #
...
def make_certificate():
  ...
  if score > 5 :
    display(HTML(add_greetings()))
     ...
     s = get_string_from_string_table('school_bell', 'make_certificate_1')
     print(print_background(print_color(s, colors.WHITE), colors.RED))

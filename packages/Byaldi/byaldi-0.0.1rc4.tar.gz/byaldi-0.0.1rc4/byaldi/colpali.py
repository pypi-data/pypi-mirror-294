...
class ColPaliModel:
        ...
        **kwargs,
        ...
        if not load_from_index:
            ...
            if self.full_document_collection:
                ...
                json_files = sorted(
                    collection_path.glob("*.json.gz"), key=lambda x: int(x.stem)
                )
    ...
    def index(
        ...
        metadata: Optional[Union[Dict[Union[str, int], Dict[str, Union[str, int]]], List[Dict[str, Union[str, int]]]]] = None,
    ):
        if (
        ...
        if input_path.is_dir():
            ...
            if isinstance(metadata, list) and len(metadata) != len(items):
                raise ValueError(
            ...
            for i, item in enumerate(items):
                ...
                doc_metadata = metadata[i] if isinstance(metadata, list) else metadata.get(doc_id) if metadata else None
                self.add_to_index(item, store_collection_with_index, doc_id=doc_id, metadata=doc_metadata)
        else:
            doc_id = doc_ids if isinstance(doc_ids, (str, int)) else (doc_ids[0] if doc_ids else self.highest_doc_id + 1)
            doc_metadata = metadata[0] if isinstance(metadata, list) else metadata.get(doc_id, None) if metadata else None
        ...
        if isinstance(input_item, Path):
            if input_item.is_dir():
                ...
                for i, image in enumerate(images):
                    self._add_to_index(
                        image, store_collection_with_index, doc_id, page_id=i + 1, metadata=metadata
                    )
            ...
            self._add_to_index(input_item, store_collection_with_index, doc_id, metadata=metadata)
        else:

...
class Pipeline:
    ...
    def clone_tables(self):
        ...
        """
        project_id = 'aic-production-core'
        exclude_tables = [
            'PF_Bridge_table_RS',
            'VW_PF_BRIDGE_TBL_ALLSRCS_EXCEPT_RS',
            'VW_PF_BRIDGE_TBL_RS'
        ...
        datasets = [
            'PA_SOURCES', 'Gold', 'Silver', 'Bronze', 'PIN_OFFICIAL_DATA', 
            'Working', 'DEV_FOUNDATION', 'Landing', 'RIT-Tesla', 'LoyConq_output'
        ]
        datasets_formatted = ', '.join(f"'{dataset}'" for dataset in datasets)
        datasets_query = self.translate_query(
            f"SELECT * FROM `3349c7ea_09a2_461d_87f5_312a5401c51a:LKP_QA_TABLE_MAPPING` WHERE alias IN ({datasets_formatted})"
        )
        ...
        for row in datasets_result:
            source_dataset = f'{project_id}.{row["prod_dataset"]}'
            target_dataset = f'{project_id}.{row["qa_dataset"]}'

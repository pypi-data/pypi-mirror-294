...
def _extend(reqid=None, is_image=False):
    extInfo = {}
    extInfo["version"] = version
    extInfo["host_name"] = f'{host_name}_i' if is_image else host_name # widget_id：391生图扩容标记
...
def GetTask(country, widget_ids='', limit=1, keep_alive=False, reqid=None):
    ...
    if len(req.widgets) > 0 and limit > 0:
        ...
        req.limit = limit
        req.extend = _extend(reqid, True if '1c44470b-acc5-5ef7-a178-06f567e3ebab' in widget_ids else False)  # widget_id：391生图扩容标记
        req.apply = False

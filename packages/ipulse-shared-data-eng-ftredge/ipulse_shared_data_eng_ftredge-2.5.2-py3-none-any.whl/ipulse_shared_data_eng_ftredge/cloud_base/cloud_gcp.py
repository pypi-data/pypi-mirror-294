...
def create_bigquery_table(project_id: str,
    ...
    Parameters:
        ...
        create_or_replace (bool): Flag to create or replace the table if it exists.
    ...
    try:
        if create_or_replace:
            try:
                ...
                log_info(msg=f"Table {table_name} in dataset {dataset_ref} was deleted.", logger=logger)
                if pipelinemon:
                    pipelinemon.add_system_impacted(f"bigquery_delete_table: {table_name}")
                    pipelinemon.add_log(ContextLog(level=LogLevel.ACTION_DELETE_CLOUD_DB_TABLE_COMPLETE, subject=table_name, description=f"Table {table_name} deleted in dataset {dataset_ref}."))
            except Exception as e:
                log_info(msg=f"Table {table_name} in dataset {dataset_ref} does not exist or could not be deleted: {e}", logger=logger)
                if pipelinemon:
                    pipelinemon.add_log(ContextLog(level=LogLevel.ERROR_ACTION_DELETE_FAILED, subject=table_name, description=f"Table {table_name} does not exist or could not be deleted: {e}"))

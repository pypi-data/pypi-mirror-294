...
def create_volatility_surface_from_binary(data: list):
    """
    Create a VolatilitySurface object from a list of binary data.

    Parameters:
    -----------
    data : list
        A list containing the serialized data of a VolatilitySurface.

    Returns:
    --------
    VolatilitySurface
        A VolatilitySurface object parsed from the input data.
    """
    # Convert the list of data to bytes
    _bytes = num_to_bytes(data)
    
    # Create an empty VolatilitySurface object
    surface = VolatilitySurface()
    
    # Parse the bytes into the VolatilitySurface object
    surface.ParseFromString(_bytes)

...
class SxrChannel(IdsBaseClass):
    ...
    :ivar brightness : Power flux received by the detector, per unit solid angle and per unit area (i.e. power divided by the etendue), in multiple energy bands if available from the detector
    :ivar power : Power received on the detector, in multiple energy bands if available from the detector
...
class SoftXRays(IdsBaseClass):
    ...
    channel: Optional[SxrChannel] = field(
        default_factory=lambda: StructArray(type=SxrChannel),
        metadata={
            "imas_type": "sxr_channel",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": SxrChannel,
        },
    )

...
class BarometryGauge(IdsBaseClass):
    ...
    :ivar type : Type of the gauge (index = 1: Penning; index = 2: Baratron)
    :ivar position : Position of the measurements
    :ivar pressure : Pressure
    :ivar calibration_coefficient : Coefficient used for converting raw signal into absolute pressure
    """
    ...
    calibration_coefficient: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
...
class Barometry(IdsBaseClass):
    ...
    gauge: Optional[BarometryGauge] = field(
        default_factory=lambda: StructArray(type=BarometryGauge),
        metadata={
            "imas_type": "barometry_gauge",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": BarometryGauge,
        },
    )

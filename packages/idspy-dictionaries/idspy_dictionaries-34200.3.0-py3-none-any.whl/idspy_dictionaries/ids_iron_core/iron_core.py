...
class IronCoreSegment(IdsBaseClass):
    ...
    :ivar permeability_relative : Relative permeability of the iron segment
    :ivar geometry : Cross-sectional shape of the segment
    :ivar magnetisation_r : Magnetisation M of the iron segment along the major radius axis, assumed to be constant inside a given iron segment. Reminder : H = 1/mu0 * B - mur * M;
    :ivar magnetisation_z : Magnetisation M of the iron segment along the vertical axis, assumed to be constant inside a given iron segment. Reminder : H = 1/mu0 * B - mur * M;
    """
    ...
    permeability_relative: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=float),
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
...
class IronCore(IdsBaseClass):
    ...
    segment: Optional[IronCoreSegment] = field(
        default_factory=lambda: StructArray(type=IronCoreSegment),
        metadata={
            "imas_type": "iron_core_segment",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": IronCoreSegment,
        },
    )

...
class SpectrometerMassChannel(IdsBaseClass):
    ...
    :ivar photomultiplier_voltage : Voltage applied to the photomultiplier
    :ivar validity : Indicator of the validity of the data. 0: Valid from automated processing; 1: Valid and certified by the diagnostic RO; -1: Problem identified in the data processing (request verification by the diagnostic RO); -2: Invalid data, should not be used. (Values less than -2 have a code-specific meaning)
    ...
    pressure_partial: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=float),
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
    ...
    photomultiplier_voltage: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=float),
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
...
class SpectrometerMass(IdsBaseClass):
    ...
    :ivar channel : Set of mass channels, each corresponding to a given atomic mass measured continuously by the spectrometer
    :ivar residual_spectrum : Set of entire mass spectra recorded at various time slices
    :ivar emission_current : Emission current in the spectrometer filament
    :ivar detector_voltage : Voltage applied on the microchannel plate detector
    ...
    channel: Optional[SpectrometerMassChannel] = field(
        default_factory=lambda: StructArray(type=SpectrometerMassChannel),
        metadata={
            "imas_type": "spectrometer_mass_channel",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": SpectrometerMassChannel,
        },
    )
    residual_spectrum: Optional[SpectrometerMassResidual] = field(
        default_factory=lambda: StructArray(type=SpectrometerMassResidual),
        metadata={
            "imas_type": "spectrometer_mass_residual",
            "ndims": 1,
            "coordinates": {"coordinate1": "time"},
            "field_type": SpectrometerMassResidual,
        },
    )
    emission_current: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
    detector_voltage: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )

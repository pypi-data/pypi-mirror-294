...
class CooperE80EqStrip:
    ...
    def __init__(self, length_of_train=150 * units.ft):
        ...
        self.distances: list[int] = [x * units.feet for x in [
        ...
        self.magnitudes: list[int] = [x for x in [
            80 * units.kips * 4 / ((self.distances[1] - self.distances[0]) * self.tie_width),  # LS1
            52 * units.kips * 4 / ((self.distances[3] - self.distances[2]) * self.tie_width),  # LS2
            40 * units.kips * 1 / ((self.distances[5] - self.distances[4]) * self.tie_width),  # LS3
            80 * units.kips * 4 / ((self.distances[7] - self.distances[6]) * self.tie_width),  # LS4
            52 * units.kips * 4 / ((self.distances[9] - self.distances[8]) * self.tie_width),  # LS5
            8 * units('kips/ft') * (length_of_train - self.distances[10]) / (
                        (length_of_train - self.distances[10]) * self.tie_width),  # LS6
        ]]
        self.linear_loads = {
            'LS1': (self.distances[0], self.distances[1], self.magnitudes[0] * self.tie_width),
            'LS2': (self.distances[2], self.distances[3], self.magnitudes[1] * self.tie_width),
            'LS3': (self.distances[4], self.distances[5], self.magnitudes[2] * self.tie_width),
            'LS4': (self.distances[6], self.distances[7], self.magnitudes[3] * self.tie_width),
            'LS5': (self.distances[8], self.distances[9], self.magnitudes[4] * self.tie_width),
            'LS6': (self.distances[10], length_of_train, self.magnitudes[5] * self.tie_width),
        }

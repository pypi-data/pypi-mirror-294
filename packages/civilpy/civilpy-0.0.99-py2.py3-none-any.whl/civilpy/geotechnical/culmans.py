...
class CulmansMethod:
    def __init__(
        ...
        self.table = self.generate_spreadsheet(self.coordinates_list, self.soil_unit_weight, self.load_scale,
            self.soil_angle_int_friction, self.angle_back_wall_with_horizontal, self.angle_of_wall_friction_delta)
    ...
    def calculate_ll_surcharge(self, b_i):
        return (80000 * units.lbf * 4 / 18 / 8.5 * b_i.magnitude).to(units('lbf')).round(4)
    ...
    def calculate_x_ci(self, total_weight, load_scale, soil_angle_int_friction):
        ...
        """
        return (total_weight.magnitude * (1 / load_scale / 1000) * np.cos(soil_angle_int_friction)).round(4)
    ...
    def calculate_x_ci_prime(self, y_ci, x_ci, angle_back_wall_with_horizontal, angle_of_wall_friction_delta,
                             soil_angle_int_friction, D, C):
        tan_term = np.tan(
            np.pi / 2 + angle_back_wall_with_horizontal.to('radians') + angle_of_wall_friction_delta.to(
                'radians') + soil_angle_int_friction.to('radians'))
        denominator = D / C - tan_term
        if denominator == 0:
            raise ZeroDivisionError("Denominator in calculation of x_{c'i} resulted in zero.")
    ...
    def generate_spreadsheet(self, coordinates_list, soil_unit_weight, load_scale, soil_angle_int_friction,
                             angle_back_wall_with_horizontal, angle_of_wall_friction_delta):
        """
        ...
        keys = self.sort_keys([key for key in list(coordinates_list.keys()) if key not in ['A', 'B']])
        ...
        for key in keys:
            ...
            prev_x_val, prev_y_val = (coordinates_list['A'][0], coordinates_list['A'][1]) if key == 'C_1' else \
                coordinates_list[f'C_{int(key.split("_")[1]) - 1}'][:2]
        ...
        for i in range(len(results)):
            if i == 0:
                ...
                x_ccgi = ((results[i]['$x_{cgi}\ (ft)$'] * results[i]['$w_i\ (lbf/ft)$'] + x_ccgi_list[i - 1] *
                           W_i_list[
                               i - 1]) / W_i_list[i]).to(units.ft).round(4)
                y_ccgi = ((results[i]['$y_{cgi}\ (ft)$'] * results[i]['$w_i\ (lbf/ft)$'] + y_ccgi_list[i - 1] *
                           W_i_list[
                               i - 1]) / W_i_list[i]).to(units.ft).round(4)
        ...
        for i, key in enumerate(keys):
            ...
            results[i]['$Total\ Weight\ (lbf/ft)$'] = W_i_list[i] + results[i]['$LL Surcharge\ (lbf/ft)$']

            # Calculate x_ci and y_ci
            x_ci = self.calculate_x_ci(results[i]['$Total\ Weight\ (lbf/ft)$'], load_scale, soil_angle_int_friction)
            ...
            try:
                x_ci_prime = self.calculate_x_ci_prime(y_ci, x_ci, angle_back_wall_with_horizontal,
                                                  angle_of_wall_friction_delta,
                                                  soil_angle_int_friction, y, x)
            ...
            # Calculate y_{c'i}
            y_ci_prime = np.nan if np.isnan(x_ci_prime) else (y / x * x_ci_prime).round(4)
            results[i]['$y_{c\'i}\ (ft)$'] = y_ci_prime

            # Calculate length of line c'_i c_i
            length_of_line = (np.nan if (np.isnan(x_ci_prime) or np.isnan(y_ci_prime)) else
                              (np.sign(y_ci_prime - x_ci_prime * max_y_val_div_k_Phi) *
                               np.sqrt((results[i]['$x_{ci}\ (ft)$'] - x_ci_prime) ** 2 +
                                       (results[i]['$y_{ci}\ (ft)$'] - y_ci_prime) ** 2)).round(4))
        ...
        df = df[['$c_i\ (ft)$', '$a_i\ (ft)$', '$b_i\ (ft)$', '$s_i\ (ft)$', '$A_i\ (ft^2)$',
                 ...
                 '$w_i\ (lbf/ft)$', '$W_i\ (lbf/ft)$', '$LL Surcharge\ (lbf/ft)$', '$Total\ Weight\ (lbf/ft)$',
                 '$x_{ci}\ (ft)$', '$y_{ci}\ (ft)$', '$x_{c\'i}\ (ft)$', '$y_{c\'i}\ (ft)$',
    ...
    def plot_results(self):
        ...
        plt.fill(retaining_wall_x, retaining_wall_y, 'blue', alpha=0.3, label='Retaining Wall')

...
def coulomb_active_pressure_with_surcharge(unit_weight, height, friction_angle, wall_friction_angle,
    ...
    if unit_weight < 0 or height < 0 or friction_angle < 0 or wall_friction_angle < 0 or \
            backfill_inclination < 0 or wall_inclination < 0 or surcharge_load < 0:
...
def culmann_lateral_pressure(unit_weight, height, cohesion, friction_angle, wall_friction_angle, num_slices=100):
    """
    ...
    for i in range(num_slices):
        ...
        n = (1 - np.sin(friction_angle_rad)) / (1 + np.sin(wall_friction_angle_rad + theta))
    ...
    pressure_values = [0.5 * unit_weight * height ** 2 *
                       np.tan(np.pi / 4 - friction_angle_rad / 2) ** 2 *
                       (1 - cohesion / (unit_weight * height) /
                        ((1 - np.sin(friction_angle_rad)) / (1 + np.sin(wall_friction_angle_rad + th)))) ** 2
...
def culmann_lateral_pressure_with_surcharge(unit_weight, height, cohesion, friction_angle, wall_friction_angle,
                                            surcharge_load, num_slices=100):
    """

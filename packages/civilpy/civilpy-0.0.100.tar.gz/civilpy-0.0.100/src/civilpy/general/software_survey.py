...
def get_dataframe_for_user(username='Dane Parks', excel_file=main_excel_file):
    ...
    cleaned_data = pd.concat([ids, names, vendors, versions, captions], ignore_index=True, axis=1)
    cleaned_data.rename(columns={0: 'ID', 1: 'Name', 2: 'Vendor', 3: 'Version', 4: 'Caption'}, inplace=True)
...
def software_survey(excel_file_location=main_excel_file):
    ...
    for user in xl.sheet_names:
        ...
        full_dataset = pd.concat([full_dataset, user_dataframe], ignore_index=True, sort=False)
    ...
    for value in full_dataset['Name'].sort_values().unique():
        ...
        if x == ' ':
            ...
            for version in full_dataset[full_dataset['Name'] == value]['Version'].unique():
                print(

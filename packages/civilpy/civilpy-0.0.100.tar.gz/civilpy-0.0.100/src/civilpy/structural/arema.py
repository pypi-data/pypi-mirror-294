...
class CooperE80:
    ...
    """
    series: int = 80
    distances: list = field(default_factory=lambda: [x * units.feet for x in [
        0, 8, 13, 18, 23, 32, 37, 43, 48, 56, 64, 69, 74, 79, 88, 93, 99, 104
    ]])
    magnitudes: list = field(default_factory=lambda: [x * units.kips for x in [
        40, 80, 80, 80, 80, 52, 52, 52, 52, 40, 80, 80, 80, 80, 52, 52, 52, 52
    ]])
    distributed: dict = field(default_factory=lambda: {
...
class CooperE80EqStrip:
    ...
    def __init__(self, length_of_train=150 * units.ft):
        ...
        self.distances: list[tuple] = [x * units.feet for x in [
        ...
        self.magnitudes: list[int] = [x for x in [
            80 * units.kips * 4 / ((self.distances[0][1] - self.distances[0][0]) * self.tie_width),  # LS1
            52 * units.kips * 4 / ((self.distances[1][1] - self.distances[1][0]) * self.tie_width),  # LS2
            40 * units.kips * 1 / ((self.distances[2][1] - self.distances[2][0]) * self.tie_width),  # LS3
            80 * units.kips * 4 / ((self.distances[3][1] - self.distances[3][0]) * self.tie_width),  # LS4
            52 * units.kips * 4 / ((self.distances[4][1] - self.distances[4][0]) * self.tie_width),  # LS5
            8 * units('kips/ft') * (length_of_train - self.distances[5][0]) / (
                        (length_of_train - self.distances[5][0]) * self.tie_width),  # LS6
        ]]
        self.linear_loads = {
            'LS1': (self.distances[0][0], self.distances[0][1], self.magnitudes[0] * self.tie_width),
            'LS2': (self.distances[1][0], self.distances[1][1], self.magnitudes[1] * self.tie_width),
            'LS3': (self.distances[2][0], self.distances[2][1], self.magnitudes[2] * self.tie_width),
            'LS4': (self.distances[3][0], self.distances[3][1], self.magnitudes[3] * self.tie_width),
            'LS5': (self.distances[4][0], self.distances[4][1], self.magnitudes[4] * self.tie_width),
            'LS6': (self.distances[5][0], length_of_train, self.magnitudes[5] * self.tie_width),
        }
    ...
    def __repr__(self):
        ...
        for i in range(max_len):
            ...
            load_key_value = list(self.linear_loads.items())[i] if i < len(self.linear_loads) else ("", "")
            load = f"{load_key_value[0]}: {load_key_value[1]}" if load_key_value[0] else ""

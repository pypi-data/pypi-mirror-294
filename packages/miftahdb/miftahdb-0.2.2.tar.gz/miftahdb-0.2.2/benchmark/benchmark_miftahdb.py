...
parser = argparse.ArgumentParser(description="Benchmark miftahdb")
parser.add_argument(
    "--query-count",
    type=int,
...
parser.add_argument(
    "--db-path",
    type=str,
    help="Databse path (Defaults to benchmark_miftahdb.sqlite)",
    default="benchmark_miftahdb.sqlite",
)
...
async def main():
    ...
    async with miftahdb.Client(args.db_path) as db:
    ...
    async with miftahdb.Client(args.db_path, autocommit=False) as db:
        await benchmark_no_auto_commit_set(db, keys)
        await db.flush()

    async with miftahdb.Client(args.db_path, workers=5) as db:
        print(
            WARNING,
            args.query_count,

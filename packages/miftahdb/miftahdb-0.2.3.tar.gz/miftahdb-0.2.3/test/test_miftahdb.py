...
async def test_key_value():
    ...
    async with miftahdb.Client("test_miftahdb.sqlite") as db:
        queries_n = 100000
        queries = []
...
async def test_in_memory_key_value():
    ...
    async with miftahdb.Client(":memory:") as db:
        queries_n = 100000
        queries = []
        for _ in range(queries_n):
            op = random.randint(0, 2)
...
async def test_concurrent_get():
    async with miftahdb.Client("test_miftahdb.sqlite", workers=5) as db:
        keys = []

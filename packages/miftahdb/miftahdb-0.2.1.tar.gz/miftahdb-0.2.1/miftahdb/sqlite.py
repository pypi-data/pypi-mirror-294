...
class Sqlite:
    def __init__(
        ...
        self.__index_statement = 'CREATE INDEX IF NOT EXISTS idx_expire_time ON "{}" (expire_time)'.format(
            self.table_name
        )
        self.__get_statement = 'SELECT v, expire_time FROM "{}" WHERE k = ? LIMIT 1'.format(
            self.table_name
        )
        ...
        self.__get_ttl_statement = 'SELECT expire_time FROM "{}" WHERE k = ? LIMIT 1'.format(
            self.table_name
        )
        ...
        self.__count_statement = 'SELECT COUNT(*) FROM "{}" where k LIKE ?'.format(self.table_name)
        self.__count_expire_statement = 'SELECT COUNT(*) as count FROM "{}" WHERE (expire_time IS NOT NULL AND expire_time <= ?) AND k LIKE ?'.format(self.table_name)
    ...
    def __get(self, key: str):
        try:
            ...
            if query:
                if time is not None and query[1] < time():
                    self.__delete(key)
                    return None
    ...
    def __set(self, key: str, value):
        with self.__lock:
            try:
                query = self.__connection.execute(
                    self.__set_statement,
                    (key, self.__encoder.encode(value[0]), time() + value[1] if value[1] else None),
                )
                if query.rowcount > 0:
                    return True
                else:

...
class Client(BaseClient):
    ...
    async def set(self, key: str, value, ttl: int | None = None) -> bool:
        ...
        future = self.__invoke(request=REQUEST.SET, key=key, value=[value, ttl])
        return await future
    ...
    async def get_ttl(self, key: str) -> float:
        ...
        future = self.__invoke(request=REQUEST.GET_TTL, key=key)
        return await future
    ...
    async def set_ttl(self, key: str, ttl: int) -> bool:
        ...
        future = self.__invoke(request=REQUEST.SET_TTL, key=key, value=ttl)
        return await future
    ...
    async def count_expire(self, like: str = "%") -> int:
        assert isinstance(like, str), "like must be str"
        
        future = self.__invoke(request=REQUEST.COUNT_EXPIRE, value=like)
        return await future
    ...
    async def pagination(self, limit: int, page: int, like: str = "%") -> Union[List[Tuple[str]], None]:
        assert isinstance(limit, int), "limit must be int"
        assert isinstance(page, int), "page must be int"
        assert isinstance(like, str), "like must be str"
        
        future = self.__invoke(request=REQUEST.PAGINATION, value=[limit, page, like])
        return await future

    async def cleanup(self) -> int:
        future = self.__invoke(request=REQUEST.CLEANUP)
        return await future

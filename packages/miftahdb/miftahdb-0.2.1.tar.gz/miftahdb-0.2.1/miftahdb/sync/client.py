...
class Client(BaseClient):
    def __init__(
        ...
        Args:
            ...
            table_name (``str``, *optional*):
                Table name to use, will be created if not exists. Defaults to ``miftahdb``.
            ...
            default_encoder (``Callable``, *optional*):
                The encoder class which deal with the data sent/received by sqlite3. Defaults to :class:`miftahdb.PickleEncoder`.
    ...
    async def set(self, key: str, value, ttl: int | None = None) -> bool:
        ...
        return self.__invoke(request=REQUEST.SET, key=key, value=[value, ttl])
    ...
    def get_ttl(self, key: str) -> float:
        ...
        return self.__invoke(request=REQUEST.GET_TTL, key=key)

    def set_ttl(self, key: str, ttl: int) -> bool:
        assert isinstance(key, str), "key must be str"
        assert ttl >= 1, "ttl must be greater than 1"

        return self.__invoke(request=REQUEST.SET_TTL, key=key, value=ttl)
    ...
    def count_expire(self, like: str = "%") -> int:
        assert isinstance(like, str), "like must be str"

        return self.__invoke(request=REQUEST.COUNT_EXPIRE, value=like)
    ...
    def pagination(self, limit: int, page: int, like: str = "%") -> Union[List[Tuple[str]], None]:
        assert isinstance(limit, int), "offset must be int"
        assert isinstance(page, int), "page must be int"
        assert isinstance(like, str), "like must be str"

        return self.__invoke(request=REQUEST.PAGINATION, value=[limit, page, like])

    def cleanup(self) -> int:
        return self.__invoke(request=REQUEST.CLEANUP)

...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        data = response.json()
        model_fields_len = self._cast_to.model_fields.__len__()
        if len(data) == 1 and model_fields_len != 1:

            val = data.popitem()[1]
            if isinstance(val, list):
                vals = len(val)

                if vals > 1 and "data" in self._cast_to.model_fields:
                    data["data"] = val
                    data["object"] = "list"
                elif vals == 1 and model_fields_len > 1:
                    data = val[0]
                else:
                    data = val
            else:

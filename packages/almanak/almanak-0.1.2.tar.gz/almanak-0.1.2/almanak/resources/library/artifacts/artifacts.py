...
from almanak.types.artifact_version import ArtifactVersionUploadedV2
import asyncio
...
class Artifacts(SyncAPIResource):
    ...
    def upload_new_version(
        ...
        response = self._post(
            ...
            options=make_request_options(**kwargs),
            cast_to=ArtifactVersionUploadedV2,
            unpack_by_keys=["data", "generateArtifactVersionUploadUrl"],
        )

        presigned_urls = [url.presigned_url for url in response.urls]
        upload_results = asyncio.run(upload_files(presigned_urls, all_files))

        if all(status == 200 for status in upload_results):
            print("Files have been uploaded successfully.")
            for file, url in zip(all_files, presigned_urls):
                print(f"Uploaded file: {file}")
            return response
        else:
            raise Exception("Failed to upload one or more files")

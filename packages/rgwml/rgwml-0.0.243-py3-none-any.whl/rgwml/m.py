...
class m:
    ...
    def validate_email(self, invoking_function_name, request, sqlite_db_path, gmail_bot_preset_name, telegram_bot_preset_name):
        ...
        def get_gmail_bot_details(config, preset_name):
            ...
            if not service_account_credentials_path or not gmail_id:
                raise RuntimeError(
                    f"Gmail service_account_credentials_path or gmail_id for '{preset_name}' not found in the configuration file"
                )

            return service_account_credentials_path, gmail_id
        ...
        def send_temp_password_email(email, temp_password, service_account_credentials_path, sender_email):
            ...
            message = create_message(sender_email, email, subject, email_text)
        ...
        config = load_config()
        service_account_credentials_path, sender_email = get_gmail_bot_details(config, gmail_bot_preset_name)
        ...
        try:
            ...
            # Here you would typically send the temp password to the user's email.
            # Send the temp password to the user's email
            send_temp_password_email(email, temp_password, service_account_credentials_path, sender_email)
            # For now, we'll assume sending email functionality.

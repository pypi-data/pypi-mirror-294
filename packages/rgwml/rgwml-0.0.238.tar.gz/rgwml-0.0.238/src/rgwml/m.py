        ...
        try:
            if token:
                ...
                if idinfo:
                    ...
                    # Retrieve the id of the newly created user in the user table

                    cursor.execute("SELECT id, email FROM user WHERE google_auth_user_id = ?", (google_auth_user_id,))
                    id_in_user_table = cursor.fetchone()[0]
                    email_in_user_table = cursor.fetchone()[1]
                    ...
                    response = jsonify({'success': False, 'message': 'Invalid token'}), 400
            else:
                response = jsonify({'success': False, 'message': 'Token not provided'}), 400
        except Exception as e:
            conn.rollback()
            ...
            response = jsonify({"error": error_message}), 500
        finally:
            conn.close() 
        return response

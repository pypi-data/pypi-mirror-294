...
__date__      = '2024-September-7'                   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.5.0.html'
...
CHANGE LOG:
  ...
  Version 2.5.0:
    ...
    A Transformer Encoder consists of an attention layer followed by an FFN (Feed
    Forward Network) layer.  There was an error in the FFN layer that I have fixed
    in this version of DLStudio.  While the previous versions also worked from the
    standpoint of overall transformer-based learning, the error caused the number
    of learnable parameters to depend strongly on the maximum sequence length at
    the input to the transformer.  You will not see that problem in Version 2.5.0.
    An additional feature of the new version is the addition of a transformer
    based Autoencoder class to the code. My plan is to use this class for future
    extensions of the code with variational and vector-quantization logic.
...
@tag_coclass2
    ...
    should be the README in the ExamplesDiffusion directory of DLStudio
    distribution.  The script RunCodeForDiffusion.py in that directory is what
    you will need to use to train the model for your own dataset.  As mentioned
    earlier, the goal of training is to make the neural network adept 
    at estimating the p-chain transition probabilities  p( x_{t-1} | x_t ) 
    at all timesteps.  Once you have finished training, you would need to execute
    the script GenerateNewImageSamples.py for generating new images.
...
ACKNOWLEDGMENTS:
    ...
    versions of the transformer code in DLStudio. As it turned out, my
    implementation of the FFN layer in the basic transformer encoder/decoder
    blocks was not in keeping with the design laid out in the original paper by
    Vaswani et al.  This problem is now fixed in Version 2.5.0. Rahul is at the
    very end of his Ph.D program in RVL at Purdue.  Thanks, Rahul!

...
def gather_env_vars(env_vars: List[str], copy_env: bool = False) -> Dict[str, str]:
    if copy_env:
        # Get all environment variables from the current environment
        env_dict = {
            var: os.getenv(var) for var in os.environ if os.getenv(var) is not None
        }
    else:
        # Only gather specified environment variables
        env_dict = {
            var: os.getenv(var) for var in env_vars if os.getenv(var) is not None
        }

    return env_dict  # type: ignore

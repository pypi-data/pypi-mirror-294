...
def get_blueprint(
    ...
    """Load the Python module at the given path and return a CovalentBlueprint
    object.
...
def register_blueprints_dir(
    ...
    """Register a directory that contains installed blueprints in it's sub-
    directories. The resolved, absolute path of the directory is stored in the
    registry.
...
def _format_installed_not_found_error(module_path: Union[str, Path]) -> str:
    """Formats the error message that lists installed blueprints when a
    blueprint is not found."""
    installed_blueprints = _find_installed_blueprints()
    installed_ = []

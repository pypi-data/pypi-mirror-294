...
class JijSQAParameters:
    ...
    Attributes:
        beta (Optional[float], optional): Inverse tempareture. this will be set automatically.
        gamma (Optional[float], optional): Strangth of transverse field. this will be set automatically.
        trotter (Optional[int], optional): The number of Trotter. this will be set automatically.
...
class JijSQASampler(JijZeptBaseSampler):
    """Simulated Quantum Annealing (SQA) sampler.

    SQA is one of the quantum-inspired algorithms that simulates quantum annealing.
    It serves as a good intermediate step for simulating quantum annealing.
    However, due to the use of resources like replica copies for quantum simulation and the lack of parameter tuning in JijZept,
    it is not recommended for optimization purposes at now.
    ...
    def sample_model(
        ...
        parameters: JijSQAParameters | None = None,
        ...
        **kwargs,
        ...
        To configure the solver, instantiate the `JijSQAParameters` class and pass the instance to the `parameters` argument.
        ...
        Args:
            ...
            parameters (JijSQAParameters | None, optional): defaults None.
            ...
            **kwargs: SQA parameters using **kwargs. If both `**kwargs` and `parameters` are exist, the value of `**kwargs` takes precedence.
    ...
    def _create_solver_params(
        ...
        *,
        ...
        class SQAParaSearch(pydantic.BaseModel):
            beta: typ.Optional[float]
            gamma: typ.Optional[float]
            trotter: typ.Optional[pydantic.PositiveInt]
        ...
        check_kwargs_against_dataclass(kwargs, JijSQAParameters)
        solver_params = merge_params_and_kwargs(parameters, kwargs, JijSQAParameters)
        ...
        try:
            return SQAParaSearch(**solver_params).model_dump()

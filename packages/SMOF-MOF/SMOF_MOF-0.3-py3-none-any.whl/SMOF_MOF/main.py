...
def Process_SMOF(Win = 500, X =[], Seed=42):

# Init SMOF model
  c = SMOF(win=Win,seed=Seed,Train=X[:int(Win)])
# Init the first reference and current window
  c.init()
# continue assign SMOF scores
  for nn,i in tqdm(enumerate(X[int(Win):])):
    c.fit(i)
# assign SMOF scores to remain data point in current window
  c.END()
  return c.SMOFs

# ----------------
# Note this code keep all SMOF score and return them
# Note The data szie should not exceed 10000 points because MOF uses high memory.
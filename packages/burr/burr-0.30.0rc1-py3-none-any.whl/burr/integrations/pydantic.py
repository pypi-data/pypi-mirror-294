...
import inspect
import types
from typing import Awaitable, Callable, List, Optional, ParamSpec, Tuple, Type, TypeVar, Union

import pydantic

from burr.core import Action, Graph, State
from burr.core.action import FunctionBasedAction, bind, get_inputs
from burr.core.typing import TypingSystem

T = TypeVar("T", bound=pydantic.BaseModel)


StateIType = TypeVar("StateIType", bound=pydantic.BaseModel, contravariant=True)
StateOType = TypeVar("StateOType", bound=pydantic.BaseModel, covariant=True)
...
def subset_model(
    ...
    for name, field_info in model.model_fields.items():
        if name in fields:
            ...
            if new_field_info.default_factory is None and new_field_info.default is None:
...
def _validate_keys(model: Type[pydantic.BaseModel], keys: List[str], fn: Callable) -> None:
    ...
    if missing_keys:
        raise ValueError(
            f"Function fn: {fn.__qualname__} is not a valid pydantic action. "
            "The keys: {missing_keys} are not present in the model: {model}."
        )

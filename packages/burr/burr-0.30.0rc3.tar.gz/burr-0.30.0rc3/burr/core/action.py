...
class FunctionBasedAction(SingleStepAction):
    ...
    def __init__(
        ...
        input_overrides: Optional[tuple[list[str], list[str]]] = None,
        originating_fn: Callable = None,
    ):
        ...
        self._inputs = (
            ...
            if input_overrides is None
                ...
                [item for item in input_overrides[0] if item not in self._bound_params],
                [item for item in input_overrides[1] if item not in self._bound_params],
            )
        )
...
class FunctionBasedStreamingAction(SingleStepStreamingAction):
    ...
    @property
    def inputs(self) -> tuple[list[str], list[str]]:
        return get_inputs(self._bound_params, self._fn)

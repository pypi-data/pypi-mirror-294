...
class EvTick(Event, contextlib.AbstractContextManager):
    ...
    _handlers: t.Dict[ModuleId, t.Dict[t.Optional[str], t.Callable]]
    ...
    def add_handlers(self, handler_list):
        ...
        for fn, info in handler_list:
            key0 = info.owner_id
            key1 = info.args[0]
            if key0 not in self._handlers:
                self._handlers[key0] = {key1: fn}
            else:
                if key1 is None or None in self._handlers[key0]:
                    raise EventCannotRegisterHandler(
    ...
    def _init_timers(self):
        ...
        for mid in handlers:
            ...
            for timer_id, fn in handlers[mid].items():
    ...
    def number_of_handlers(self):
        ...
        """
        s = 0
        for _, subev in self._handlers.items():
            for _ in subev:
                s += 1
        return s
    ...
    def stop(self):
        for timer, (mid, tid) in self._timers:
            logger.debug("STOPPING %s %s %s", timer, mid, tid)
            timer.stop()

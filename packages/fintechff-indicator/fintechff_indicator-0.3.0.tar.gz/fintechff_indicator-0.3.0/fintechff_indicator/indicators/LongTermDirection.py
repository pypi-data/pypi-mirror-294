...
class LongTermDirection(bt.Indicator):
    ...
    lines = ('ltd',)
    params = (
        ('url', f"{os.environ.get('FINTECHFF_BASE_URL', 'http://192.168.25.247:8220')}/signal/list"),
        ('symbol', 'HKEX:HTI1!'),
        ('window_size', 200)
    )
    ...
    def next(self):
        ...
        if current_time_str not in self.cache:
            ...
            for result in response['results']:
                ...
                long_term_dir_time_str = datetime.fromtimestamp(result['longTermDirTs'] / 1000.0).astimezone().strftime('%Y-%m-%d %H:%M:%S')
                self.cache[long_term_dir_time_str] = self.NA
                if result['longTermDir'] == 'BULLISH':
                    self.cache[long_term_dir_time_str] = self.BULLISH
                elif result['longTermDir'] == 'BEARISH':
                    self.cache[long_term_dir_time_str] = self.BEARISH

        self.lines.ltd[0] = self.cache.get(current_time_str, self.NA)
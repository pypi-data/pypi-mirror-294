...
class _EUDParser(EUDByteReader):
    ...
    def _parse_dw(self):
        ...
        if cs.EUDInfLoop()():
            ...
            c.RawTrigger(
                conditions=[number == 0],
                actions=digits.SetNumberX(0, (1 << 30) - 1),
            )
            c.SeqCompute(
                [
        ...
        c.RawTrigger(
            conditions=digits.AtLeastX(1, 1 << 30),
            actions=number.SetNumber(0x7FFFFFFF),
        )
    ...
    def _parse_from_radix(self, radix):
        ...
        if cs.EUDInfLoop()():
            ...
            assume_decimal, cont, is_alphanumeric = (
            ...
            cont << c.RawTrigger(
                nextptr=0,
                actions=[
                    c.SetNextPtr(jumper, is_alphanumeric),
                    cutlim.AddNumber(1),
                ],
            )
...
@c.EUDFunc
def _parse_dw(dst):
    global_parser = _get_global_eudparser()

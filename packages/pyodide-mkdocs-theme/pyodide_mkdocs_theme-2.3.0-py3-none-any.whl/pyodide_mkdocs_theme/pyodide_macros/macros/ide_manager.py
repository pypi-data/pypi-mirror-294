...
class IdeManagerExporter(IdeManagerMdHtmlGenerator, metaclass=ABCMeta):
    ...
    """
    If defined, gives the names of all the properties that should be exported to JS, in
    addition to the JS_EXPORTED_GENERICS content.
    This HAS TO BE OVERRIDDEN by the subclasses.
    """
    ...
    def __post_init__(self):
        ...
        if self.JS_EXPORTED_VALUES is None:
            raise BuildError(f"JS_EXPORTED_VALUES should be an iterable (on {self})")

        excluded, excluded_methods, white_list = self._compute_exclusions_and_white_lists()
        ...
        to_register: List[Tuple[str,Any]] = [
            ...
            *self.files_data.get_sections_data( with_corr=self.keep_corr_on_export ),
            ...
            ("decrease_attempts_on_code_error", self._sanitize_decrease_attempts_on_user_code_failure()),
        ...
        # Filter out pairs with properties that shouldn't be exported for the current instance:
        actually_registered = dict(
            rule for rule in to_register
                 if rule[0] in self.JS_EXPORTED_GENERICS
                 or rule[0] in self.JS_EXPORTED_VALUES      # pylint: disable=E1135
        )

        # Prepare actual transfer of data to JS:
        self.env.set_current_page_js_macro_config(
            self.editor_name, MacroPyConfig(**actually_registered)
        )
    ...
    def _sanitize_decrease_attempts_on_user_code_failure(self):
        """ Backward compatibility change, going from bool to enum of strings. """

        val = self.env.decrease_attempts_on_user_code_failure

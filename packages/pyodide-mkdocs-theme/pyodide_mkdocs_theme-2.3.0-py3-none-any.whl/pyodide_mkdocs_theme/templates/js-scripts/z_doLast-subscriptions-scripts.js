...
const ALL_EDITORS = 'editor_ tester_'
...
function paintAllAces() {
  ...
  for(const id of ALL_EDITORS.split(' ')){
...
ALL_EDITORS.split(' ').forEach(id=>{

  $(`[id^=global_${ id }]`).each(function(){
    const editorId = this.id.slice('global_'.length)
    const ide = id=="editor_" ? new IdeRunner(editorId)
                              : new IdeTester(editorId)
    ide.build()
  })
})
